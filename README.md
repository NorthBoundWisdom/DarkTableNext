...
仍用于维持可运行基线和核对行为；目标前端是 Qt 6/QML，并通过 GeoControls 复用
通用控件。仓库提供一个显式启用的欢迎页→主界面 QML 壳，它尚未连接图像核心，
因此不构成完整的产品 UI 或迁移完成声明。
...
当前已提供一个独立、显式启用的 Qt/QML 壳：`DarkTableNext/` 中的欢迎页会展示启动
组件状态，随后进入尚未连接图像核心的主工作区。它通过 `GeoControls` 和
`GeoControls.AppShell` 组成，用于后续接入组件加载器、领域控制器和照片视图模型。
...
Qt/QML 壳不会改变默认 GTK 构建。安装 Qt 6.8+ 后，显式指定 Qt 前缀并启用它：

```sh
cmake --preset mac_clang_debug \
  -DDARKTABLENEXT_BUILD_QML_SHELL=ON \
  -DDARKTABLENEXT_QT_ROOT=/path/to/Qt/6.8+/macos
cmake --build --preset mac_clang_debug
```

该目标会从 FreeCM 物化的 GeoControls 源码根构建；首次使用前仍需完成上面的
`--init` 与 `--update`。在 FreeCM 的 **Run** 菜单中选择 **Run Qt/QML Shell**
（`runqt`）可构建对应目标并启动该壳。
