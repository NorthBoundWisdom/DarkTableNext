...
- 若旧应用临时复用 Ravo，依赖只能从 `src` 指向 Ravo 的稳定 facade，且要满足 `MIGRATION.md` 的
  删除收益和退役条件。
...
- Ravo 通过差分和新单元测试后，才切换消费者并删除 `src` 对应所有权。删除必须覆盖 CMake、资源、
  配置、文档与测试，并以全仓搜索证明没有可达消费者。
- 旧 OpenCL 仍受根级退役门槛约束；Ravo CPU 能力完成不等于可以提前删除旧应用的 GPU 路径。
