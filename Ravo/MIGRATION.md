...
Ravo 最终取代 `src/`，而不是永远与它并存。迁移必须让新所有权持续进入 `Ravo/`，同时在验收后删除
`src/` 中对应的旧实现。单纯复制代码会增加总代码量，不算迁移。
...
- 差分 runner 将旧 CLI 与 `ravo` 当作两个独立进程执行；
- Ravo 通过 FreeCM 直接消费已固定的第三方依赖；
- 必要时由旧 `src` 消费者通过窄 adapter 调用稳定的 Ravo facade，前提是同一变更或已记录后续会
  删除更多旧实现。
...
依赖方向只能保持为独立并行，或在必要过渡期形成 `src` → Ravo；永远不得形成 Ravo → `src`。
...
2. **冻结行为**：运行旧 CPU 路径，保存真实 RAW、XMP/参数、像素/元数据输出、容差、错误与性能基线。
...
6. **切换所有权**：让新 CLI/服务成为正式消费者；若收益明确，可让旧侧通过窄 adapter 临时消费。
7. **删除旧实现**：清理 `src`、CMake、注册表、资源、配置、文档和重复测试；全仓搜索剩余消费者。
...
- GPU 只在 CPU 参考路径完成后进入 Ravo；旧 OpenCL 按根级门槛退役。
- 最后切换发行物并删除旧应用入口，而不是在早期让两个应用共享可写 catalog。

## 初始迁移 ledger

| Capability | 旧 owner | Ravo owner | 状态 | 删除门槛 |
| --- | --- | --- | --- | --- |
| 基础错误/任务/取消 | `src/common`, `src/control` | `foundation` | 未开始 | 新契约测试；无旧类型泄漏 |
| Recipe 与 operation schema | IOP params/introspection/XMP | `recipe` | 未开始 | 版本化格式与 legacy adapter |
| 输入检查与 RAW 解码 | common/imageio/LibRaw 路径 | `engine` + codec adapter | 未开始 | inspect 与真实 RAW fixture |
| CPU pixelpipe/ROI/分块 | `src/develop` | `engine` | 未开始 | 垂直切片和资源门槛 |
| 色彩管理 | common/develop/iop | `engine` | 未开始 | CPU 金样与 ICC/metadata 测试 |
| Mask/blend | `src/develop` | `recipe` + `engine` | 未开始 | schema、重放和像素差分 |
| 保留 operation | `src/iop` | `engine` | 未开始 | 每项 operation 验收清单 |
| 本地输出 | `src/imageio` | engine + codec adapter | 未开始 | 四类输出与失败原子性 |
| CLI | `src/cli` | `cli` | 未开始 | 独立安装、contract 与回归入口 |
