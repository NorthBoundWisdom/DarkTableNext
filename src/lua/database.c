...
static int database_len(lua_State *L)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), "SELECT COUNT(*) FROM main.images ",
                                -1, &stmt, NULL);
...
static int database_get_image(lua_State *L)
{
    ...
    sqlite3_stmt *stmt = NULL;
    char query[1024];
    snprintf(query, sizeof(query), "SELECT id FROM main.images WHERE id = %d LIMIT 1", img_id);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), query, -1, &stmt, NULL);
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        dt_imgid_t imgid = sqlite3_column_int(stmt, 0);
...
static void on_film_imported(gpointer instance, uint32_t id, gpointer user_data)
{
    dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "const char*", "post-import-film", LUA_ASYNC_TYPENAME, "dt_lua_film_t",
                            GINT_TO_POINTER(id), LUA_ASYNC_DONE);
}
