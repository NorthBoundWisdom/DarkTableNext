...
static int _tensor_tostring(lua_State *L)
{
    ...
    for (int i = 0; i < t->ndim; i++)
        pos += snprintf(buf + pos, sizeof(buf) - pos, "%s%" PRId64, i ? "x" : "", t->shape[i]);
...
static int _tensor_get(lua_State *L)
{
    ...
    if (nidx != t->ndim)
        return luaL_error(L, "expected %d indices, got %d", t->ndim, nidx);
    ...
    for (int i = 0; i < t->ndim; i++)
    {
        ...
        if (idx < 0 || idx >= t->shape[i])
            return luaL_error(L, "index %d out of range [0, %d)", idx, (int)t->shape[i]);
...
static int _tensor_crop(lua_State *L)
{
    ...
    if (y < 0 || x < 0 || h <= 0 || w <= 0 || y + h > H || x + w > W)
        return luaL_error(L, "crop region (%d,%d,%d,%d) out of bounds (%d,%d)", y, x, h, w, H, W);
    ...
    dt_lua_ai_tensor_t *out = lua_newuserdata(L, sizeof(dt_lua_ai_tensor_t));
    ...
    for (size_t s = 0; s < leading; s++)
    {
        ...
        for (int row = 0; row < h; row++)
            memcpy(dst + (size_t)row * w, src + (size_t)(y + row) * W + x,
...
{
    dt_lua_ai_tensor_t *dst = luaL_checkudata(L, 1, "dt_lua_ai_tensor_t");
    dt_lua_ai_tensor_t *src = luaL_checkudata(L, 2, "dt_lua_ai_tensor_t");
    if (!dst->data || !src->data)
        return luaL_error(L, "tensor has been freed");
    if (dst->ndim != src->ndim)
        return luaL_error(L, "dimension mismatch: dst has %d dims, src has %d", dst->ndim,
                          src->ndim);
    if (dst->ndim < 2)
    ...
    for (int i = 0; i < dst->ndim - 2; i++)
    {
        if (dst->shape[i] != src->shape[i])
            return luaL_error(L, "leading dimension %d mismatch: dst=%d, src=%d", i,
                              (int)dst->shape[i], (int)src->shape[i]);
    }
    ...
    if (y < 0 || x < 0 || y + src_h > dst_H || x + src_w > dst_W)
        return luaL_error(L, "paste region (%d,%d,%d,%d) out of bounds (%d,%d)", y, x, src_h, src_w,
                          dst_H, dst_W);
    ...
    for (size_t s = 0; s < leading; s++)
    {
        ...
        for (int row = 0; row < src_h; row++)
            memcpy(d + (size_t)(y + row) * dst_W + x, sr + (size_t)row * src_w,
...
{
    dt_lua_ai_tensor_t *a = luaL_checkudata(L, 1, "dt_lua_ai_tensor_t");
    dt_lua_ai_tensor_t *b = luaL_checkudata(L, 2, "dt_lua_ai_tensor_t");
    if (!a->data || !b->data)
        return luaL_error(L, "tensor has been freed");
    if (a->size != b->size)
        return luaL_error(L, "dot product requires same size: %zu vs %zu", a->size, b->size);
...
static int _tensor_scale_add_planes(lua_State *L)
{
    ...
    size_t plane = 1;
    for (int d = 2; d < t->ndim; d++)
        plane *= (size_t)t->shape[d];
...
static int _tensor_mean(lua_State *L)
{
    ...
    if (!t->data)
        return luaL_error(L, "tensor has been freed");
    if (t->size == 0)
        return luaL_error(L, "mean of empty tensor");
    double s = 0.0;
    for (size_t i = 0; i < t->size; i++)
        s += t->data[i];
...
static int _tensor_bayer_pack(lua_State *L)
{
    ...
    out->data = g_try_malloc(total * sizeof(float));
    if (!out->data)
        return luaL_error(L, "failed to allocate packed tensor");
...
static int _tensor_bayer_unpack(lua_State *L)
{
    ...
    out->data = g_try_malloc(total * sizeof(float));
    if (!out->data)
        return luaL_error(L, "failed to allocate unpacked tensor");
...
static int _collect_tensors(lua_State *L, const int tbl_idx, dt_ai_tensor_t **out,
                            const char *label)
{
    ...
    *out = g_new0(dt_ai_tensor_t, n);
    for (int i = 0; i < n; i++)
    {
        ...
        if (!t || !t->data)
        {
            g_free(*out);
            *out = NULL;
            return luaL_error(L, "%s[%d] is not a valid tensor", label, i + 1);
        }
        (*out)[i].data = t->data;
...
static int _context_run(lua_State *L)
{
    ...
    const gboolean two_table = (lua_gettop(L) == 3 && lua_istable(L, 2) && lua_istable(L, 3));
    ...
    if (n_in < 1)
        return luaL_error(L, "run() requires input tensors or {inputs},{outputs} tables");
    ...
    int64_t (*out_shapes)[MAX_TENSOR_DIMS] = g_malloc0(n_out * MAX_TENSOR_DIMS * sizeof(int64_t));
    int *out_ndims = g_new0(int, n_out);

    for (int i = 0; i < n_out; i++)
    {
        out_ndims[i] = dt_ai_get_output_shape(ctx, i, out_shapes[i], MAX_TENSOR_DIMS);
        if (out_ndims[i] <= 0)
        {
            for (int j = 0; j < i; j++)
                g_free(out_bufs[j]);
        ...
        if (!dynamic)
        {
            ...
            if (!out_bufs[i])
            {
                ...
                g_free(outputs);
                g_free(inputs);
                return luaL_error(L, "failed to allocate output %d buffer", i);
            }
        }
    ...
    for (int i = 0; i < n_out; i++)
    {
        dt_lua_ai_tensor_t *lt = lua_newuserdata(L, sizeof(dt_lua_ai_tensor_t));
        ...
        memcpy(lt->shape, outputs[i].shape, outputs[i].ndim * sizeof(int64_t));
        lt->size = 1;
...
static int _ai_load_model(lua_State *L)
{
    ...
    dt_ai_context_t *ctx = dt_ai_load_model(env, model_id, model_file, provider);
    if (!ctx)
        return luaL_error(L, "failed to load model '%s'%s%s", model_id, model_file ? " file " : "",
...
static int _ai_create_tensor(lua_State *L)
{
    ...
    if (ndim <= 0 || ndim > MAX_TENSOR_DIMS)
        return luaL_error(L, "shape must have 1-%d dimensions, got %d", MAX_TENSOR_DIMS, ndim);
    ...
    dt_lua_ai_tensor_t *t = lua_newuserdata(L, sizeof(dt_lua_ai_tensor_t));
    memset(t, 0, sizeof(*t));
...
static int _load_image_from_file(lua_State *L)
{
    ...
    if (req_w > 0 && req_h > 0)
    {
        GdkPixbuf *scaled = gdk_pixbuf_scale_simple(pb, req_w, req_h, GDK_INTERP_BILINEAR);
        g_object_unref(pb);
        if (!scaled)
    ...
    for (int c = 0; c < C; c++)
        for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++)
                t->data[c * h * w + y * w + x] = pixels[y * stride + x * ch + c] / 255.0f;
...
static int _lua_capture_write(dt_imageio_module_data_t *data, const char *filename, const void *in,
...
static int _load_image_from_imgid(lua_State *L)
{
    ...
    dt_imageio_module_format_t fmt = {.mime = _lua_capture_mime,
    ...
    dt_imageio_export_with_flags(imgid, "unused", &fmt, (dt_imageio_module_data_t *)&cap,
                                 TRUE,  // ignore_exif
                                 ...
                                 FALSE, // copy_metadata
                                 FALSE, // export_masks
                                 dst_type, NULL, DT_INTENT_PERCEPTUAL, NULL, NULL, 1, 1, NULL, -1);
    ...
    for (int c = 0; c < C; c++)
        for (int y = 0; y < h; y++)
            for (int x = 0; x < w; x++)
                t->data[c * h * w + y * w + x] = cap.pixels[((size_t)y * w + x) * 4 + c];
...
static int _ai_load_image(lua_State *L)
{
    ...
    return luaL_error(L, "load_image expects a file path (string) or image object");
}
...
static int _ai_load_raw(lua_State *L)
{
    ...
    if (!buf.buf || buf.width <= 0 || buf.height <= 0)
    {
        ...
        return luaL_error(L, "cannot load raw data for image %d", imgid);
    }
    ...
    memcpy(black_separate, img->raw_black_level_separate, sizeof(black_separate));
    ...
    memcpy(color_matrix, img->adobe_XYZ_to_CAM, sizeof(color_matrix));
...
static int _read_exif_for_imgid(dt_imgid_t imgid, int width, int height, uint8_t **out_blob)
{
    *out_blob = NULL;
    ...
    return dt_exif_read_blob(out_blob, srcpath, imgid, FALSE, width, height, TRUE);
}
...
static int _ai_save_dng(lua_State *L)
{
    ...
    if (!t || !t->data)
        return luaL_error(L, "tensor has been freed");
    if (t->ndim != 4 || t->shape[0] != 1 || t->shape[1] != 1)
        return luaL_error(L, "save_dng requires [1,1,H,W] CFA tensor");
    ...
    if (!cfa)
        return luaL_error(L, "failed to allocate CFA buffer");
    for (size_t i = 0; i < total; i++)
    {
        const float v = t->data[i];
        cfa[i] = v <= 0.0f ? 0 : v >= 65535.0f ? 65535 : (uint16_t)(v + 0.5f);
    }
    ...
    const int rc = dt_imageio_dng_write_cfa_bayer(path, cfa, W, H, img, exif_blob, exif_len,
                                                  NULL /* preview */);
    dt_image_cache_read_release(img);
    g_free(cfa);
    g_free(exif_blob);
    ...
    if (rc)
        return luaL_error(L, "failed to write DNG '%s'", path);
    return 0;
}
...
static int _ai_save_dng_linear(lua_State *L)
{
    ...
    if (t->ndim != 4 || t->shape[0] != 1 || t->shape[1] != 3)
        return luaL_error(L, "save_dng_linear requires [1,3,H,W] RGB tensor");
    ...
    if (!rgb)
        return luaL_error(L, "failed to allocate RGB buffer");
    ...
    const int rc =
        dt_imageio_dng_write_linear(path, rgb, W, H, img, exif_blob, exif_len, NULL /* preview */);
    dt_image_cache_read_release(img);
    g_free(rgb);
    g_free(exif_blob);

    if (rc)
        return luaL_error(L, "failed to write linear DNG '%s'", path);
    return 0;
}
...
static int _tensor_save_tiff(lua_State *L)
{
    ...
    if (t->ndim != 4 || t->shape[0] != 1)
        return luaL_error(L, "save_tiff requires NCHW tensor with batch=1");
    if (bpp != 16 && bpp != 32)
    ...
    if (C != 3 && C != 1)
        return luaL_error(L, "save_tiff requires 1 or 3 channels, got %d", C);
    ...
    TIFFSetField(tif, TIFFTAG_SAMPLEFORMAT, bpp == 32 ? SAMPLEFORMAT_IEEEFP : SAMPLEFORMAT_UINT);
    TIFFSetField(tif, TIFFTAG_PHOTOMETRIC, C == 1 ? PHOTOMETRIC_MINISBLACK : PHOTOMETRIC_RGB);
    ...
    if (lua_gettop(L) >= 4 && dt_lua_isa(L, 4, dt_lua_image_t))
    {
        ...
        const dt_colorspaces_color_profile_t *cp = dt_colorspaces_get_work_profile(imgid);
        if (cp && cp->profile)
        {
            uint32_t icc_len = 0;
    ...
    else
    {
        ...
        uint16_t *row = g_try_malloc((size_t)W * C * sizeof(uint16_t));
        if (!row)
        {
            TIFFClose(tif);
            g_unlink(path);
...
static int _tensor_save(lua_State *L)
{
    ...
    if (!t || !t->data)
        return luaL_error(L, "tensor has been freed");
    if (t->ndim != 4 || t->shape[0] != 1)
        return luaL_error(L, "save requires NCHW tensor with batch=1");
    ...
    if (C != 3 && C != 1)
        return luaL_error(L, "save requires 1 or 3 channels, got %d", C);
    ...
    for (int y = 0; y < H; y++)
        for (int x = 0; x < W; x++)
            for (int c = 0; c < out_ch; c++)
            {
                ...
                v = CLAMP(v, 0.0f, 1.0f);
                pixels[(y * W + x) * out_ch + c] = (guchar)(v * 255.0f + 0.5f);
            }

    GdkPixbuf *pb = gdk_pixbuf_new_from_data(pixels, GDK_COLORSPACE_RGB, FALSE, 8, W, H, W * out_ch,
                                             NULL, NULL);
    if (!pb)
    {
        g_free(pixels);
    ...
    if (ext)
    {
        if (!g_ascii_strcasecmp(ext, ".jpg") || !g_ascii_strcasecmp(ext, ".jpeg"))
            fmt = "jpeg";
        else if (!g_ascii_strcasecmp(ext, ".tiff") || !g_ascii_strcasecmp(ext, ".tif"))
            fmt = "tiff";
    }
