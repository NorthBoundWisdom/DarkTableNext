...
static int style_table_index(lua_State *L)
{
    ...
    char query[1024];
    snprintf(query, sizeof(query), "SELECT name FROM data.styles ORDER BY name LIMIT 1 OFFSET %d",
             index - 1);
...
{
    sqlite3_stmt *stmt = NULL;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), "SELECT COUNT(*) FROM data.styles",
                                -1, &stmt, NULL);
    if (sqlite3_step(stmt) == SQLITE_ROW)
