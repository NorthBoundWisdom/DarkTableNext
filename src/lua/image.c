...
void dt_lua_image_push(lua_State *L, const dt_imgid_t imgid)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT id FROM main.images WHERE id = ?1", -1, &stmt, NULL);
...
static int generate_cache(lua_State *L)
{
    ...
    for (int k = max; k >= min && k >= 0; k--)
    {
        ...
        snprintf(filename, sizeof(filename), "%s.d/%d/%d.jpg", darktable.mipmap_cache->cachedir, k,
                 imgid);
...
static int rating_member(lua_State *L)
{
    if (lua_gettop(L) != 3)
    {
        ...
        if (score > 6)
            score = 5;
        if (score == DT_VIEW_REJECT)
            score = -1;
        // check the reject flag just to be sure
        if (my_image->flags & DT_IMAGE_REJECTED)
            score = -1;
...
int dt_lua_init_image(lua_State *L)
{
    ...
    luaA_struct_member_name(L, dt_image_t, geoloc.longitude, protected_double,
                            longitude); // set to NAN if value is not set
    luaA_struct_member_name(L, dt_image_t, geoloc.latitude, protected_double,
                            latitude); // set to NAN if value is not set
    luaA_struct_member_name(L, dt_image_t, geoloc.elevation, protected_double,
                            elevation); // set to NAN if value is not set
