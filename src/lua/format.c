...
static int write_image(lua_State *L)
{
    ...
    luaL_argcheck(L, dt_lua_isa(L, 1, dt_imageio_module_format_t), -1,
                  "dt_imageio_module_format_t expected");
    ...
    gboolean result = dt_imageio_export(imgid, filename, format, fdata, high_quality, upscale,
                                        is_scaling, scale_factor, FALSE, export_masks, icc_type,
...
void dt_lua_register_format_type(lua_State *L, dt_imageio_module_format_t *module,
                                 luaA_Type type_id)
{
