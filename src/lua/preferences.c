...
static void update_widget_string(struct pref_element *cur_elt, GtkWidget *dialog,
                                 GtkWidget *labelev);
...
static void update_widget_float(struct pref_element *cur_elt, GtkWidget *dialog,
                                GtkWidget *labelev);
...
static gboolean _is_global_script(const char *script)
{
    for (int i = 0; _global_scripts[i]; i++)
        if (strcmp(_global_scripts[i], script) == 0)
            return TRUE;
    return FALSE;
}
...
static void _combo_changed(GtkComboBox *combo, GtkWidget *script_stack)
{
    ...
    if (section_label)
        gtk_label_set_text(GTK_LABEL(section_label), _get_script_display_name(id));
}
...
static void response_callback_filechooser(GtkDialog *dialog, gint response_id,
...
static void response_callback_bool(GtkDialog *dialog, gint response_id, pref_element *cur_elt)
{
    if (response_id == GTK_RESPONSE_DELETE_EVENT)
    {
        char pref_name[1024];
        get_pref_name(pref_name, sizeof(pref_name), cur_elt->script, cur_elt->name);
        dt_conf_set_bool(pref_name,
                         gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(cur_elt->widget)));
    }
}
...
static gboolean reset_widget_string(GtkWidget *label, GdkEventButton *event, pref_element *cur_elt)
{
    if (event->type == GDK_2BUTTON_PRESS)
    {
        gtk_entry_set_text(GTK_ENTRY(cur_elt->widget),
...
static gboolean reset_widget_int(GtkWidget *label, GdkEventButton *event, pref_element *cur_elt)
{
    if (event->type == GDK_2BUTTON_PRESS)
    {
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(cur_elt->widget),
...
static void update_widget_enum(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    char pref_name[1024];
    get_pref_name(pref_name, sizeof(pref_name), cur_elt->script, cur_elt->name);
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_enum),
                     cur_elt);
    ...
    {
        char *active_entry =
            gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(cur_elt->widget));
        if (!active_entry)
        {
        ...
        {
            gtk_combo_box_set_active(GTK_COMBO_BOX(cur_elt->widget),
                                     gtk_combo_box_get_active(GTK_COMBO_BOX(cur_elt->widget)) + 1);
            g_free(active_entry);
        }
    } while (true);
}
...
static void update_widget_dir(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    ...
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_dir),
                     cur_elt);
    g_signal_connect(G_OBJECT(dialog), "response", G_CALLBACK(response_callback_filechooser),
                     cur_elt);
}
...
static void update_widget_file(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    ...
    const char *str = dt_conf_get_string_const(pref_name);
    gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(cur_elt->widget), str);
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_file),
                     cur_elt);
    g_signal_connect(G_OBJECT(dialog), "response", G_CALLBACK(response_callback_filechooser),
                     cur_elt);
}
...
static void update_widget_string(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    char pref_name[1024];
    get_pref_name(pref_name, sizeof(pref_name), cur_elt->script, cur_elt->name);
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_string),
                     cur_elt);
...
static void update_widget_bool(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    char pref_name[1024];
    get_pref_name(pref_name, sizeof(pref_name), cur_elt->script, cur_elt->name);
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(click_widget_bool),
                     cur_elt);
...
static void update_widget_int(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    ...
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_int),
                     cur_elt);
...
static void update_widget_float(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    ...
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_float),
                     cur_elt);
...
static void update_widget_lua(pref_element *cur_elt, GtkWidget *dialog, GtkWidget *labelev)
{
    ...
    lua_call(L, 2, 0);
    dt_lua_unlock();
    g_signal_connect(G_OBJECT(labelev), "button-press-event", G_CALLBACK(reset_widget_lua),
                     cur_elt);
    ...
    *tmp = calloc(1, sizeof(pref_element));
    ...
    case pref_enum:
    {
        ...
        while (!lua_isnoneornil(L, cur_param))
        {
            ...
            gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(built_elt->widget),
                                           luaL_checkstring(L, cur_param));
            cur_param++;
            value++;
        }
    ...
    case pref_dir:
        ...
        built_elt->widget = gtk_file_chooser_button_new(_("select directory"),
                                                        GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
        gtk_file_chooser_button_set_width_chars(GTK_FILE_CHOOSER_BUTTON(built_elt->widget), 20);
        g_object_ref_sink(G_OBJECT(built_elt->widget));
        built_elt->tooltip_reset = g_strdup_printf(_("double-click to reset to `%s'"),
                                                   built_elt->type_data.dir_data.default_value);
    ...
    case pref_file:
        ...
        built_elt->widget =
            gtk_file_chooser_button_new(_("select file"), GTK_FILE_CHOOSER_ACTION_OPEN);
        gtk_file_chooser_button_set_width_chars(GTK_FILE_CHOOSER_BUTTON(built_elt->widget), 20);
        built_elt->tooltip_reset = g_strdup_printf(_("double-click to reset to `%s'"),
                                                   built_elt->type_data.file_data.default_value);
        g_object_ref_sink(G_OBJECT(built_elt->widget));
    ...
    case pref_bool:
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(built_elt->widget),
                                     dt_conf_get_bool(pref_name));
        g_object_ref_sink(G_OBJECT(built_elt->widget));
        built_elt->tooltip_reset =
            g_strdup_printf(_("double-click to reset to `%s'"),
...
{
    if (!pref_list)
        return; // no option registered => don't create the tab
    ...
    {
        if (!_is_global_script(cur_elt->script))
            continue;
