...
static int film_len(lua_State *L)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT COUNT(*) FROM main.images WHERE film_id = ?1  ", -1, &stmt,
                                NULL);
    DT_DEBUG_SQLITE3_BIND_INT(stmt, 1, film_id);
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
...
static int film_getnum(lua_State *L)
{
    ...
    snprintf(query, sizeof(query),
             "SELECT id FROM main.images WHERE film_id = ?1 ORDER BY id LIMIT 1 OFFSET %d",
             index - 1);
...
static int films_len(lua_State *L)
{
    sqlite3_stmt *stmt = NULL;
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT COUNT(*) FROM main.film_rolls ", -1, &stmt, NULL);
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
        lua_pushinteger(L, sqlite3_column_int(stmt, 0));
    }
...
static int films_index(lua_State *L)
{
    ...
    sqlite3_stmt *stmt = NULL;
    char query[1024];
    snprintf(query, sizeof(query), "SELECT id FROM main.film_rolls ORDER BY id LIMIT 1 OFFSET %d",
             index - 1);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), query, -1, &stmt, NULL);
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
