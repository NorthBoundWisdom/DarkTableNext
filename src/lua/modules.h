...
#define dt_lua_register_module_member_indirect(L, storage, struct_type, struct_member, child_type, \
                                               child_member, member_type)                          \
    luaA_struct_member_type(                                                                       \
        L, storage->parameter_lua_type, #child_member, luaA_type(L, member_type),                  \
...
void dt_lua_module_entry_new(lua_State *L, int index, const char *module_type_name,
                             const char *entry_name);
/// create a new entry into the module, a singleton is created for you that contains entry
void dt_lua_module_entry_new_singleton(lua_State *L, const char *module_type_name,
                                       const char *entry_name, void *entry);
...
luaA_Type dt_lua_module_entry_get_type(lua_State *L, const char *module_type_name,
                                       const char *entry_name);
...
void dt_lua_register_module_presets_type(lua_State *L, const char *module_type_name,
                                         const char *entry_name, luaA_Type preset_type);
luaA_Type dt_lua_module_get_preset_type(lua_State *L, const char *module_type_name,
                                        const char *entry_name);
void dt_lua_register_current_preset(lua_State *L, const char *module_type_name,
                                    const char *entry_name, lua_CFunction pusher,
