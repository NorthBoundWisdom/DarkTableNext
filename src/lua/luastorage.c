...
static int store_wrapper(struct dt_imageio_module_storage_t *self,
                         ...
                         const gboolean upscale, const gboolean is_scaling,
                         ...
                         dt_iop_color_intent_t icc_intent, dt_export_metadata_t *metadata)
{
    ...
    if (dt_imageio_export(imgid, complete_name, format, fdata, high_quality, upscale, is_scaling,
                          scale_factor, TRUE, export_masks, icc_type, icc_filename, icc_intent,
                          self, self_data, num, total, metadata) != 0)
    {
        dt_print(DT_DEBUG_ALWAYS, "[lua] %s: could not export to file `%s'!", self->name(self),
                 complete_name);
...
static int initialize_store_wrapper(struct dt_imageio_module_storage_t *self,
                                    ...
                                    dt_imageio_module_data_t **fdata, GList **images,
...
static int set_params_wrapper(struct dt_imageio_module_storage_t *self, const void *params,
                              const int size)
{
    return 0;
}
...
static int destroy_storage(lua_State *L)
{
    ...
    if (storage->widget)
        g_object_unref(storage->widget);
    if (storage->module)
        g_module_close(storage->module);
    free(storage);
    return 0;
}
