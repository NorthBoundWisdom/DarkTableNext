...
static int tag_index(lua_State *L)
{
    ...
    snprintf(query, sizeof(query),
             "SELECT imgid FROM main.tagged_images WHERE tagid=?1 ORDER BY imgid LIMIT 1 OFFSET %d",
             index - 1);
...
static int tag_lib_length(lua_State *L)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), "SELECT COUNT(*) FROM data.tags", -1,
                                &stmt, NULL);
    rv = sqlite3_step(stmt);
    if (rv != SQLITE_ROW)
    {
...
static int tag_lib_index(lua_State *L)
{
    ...
    sqlite3_stmt *stmt = NULL;
    char query[1024];
    snprintf(query, sizeof(query), "SELECT id FROM data.tags ORDER BY id LIMIT 1 OFFSET %d",
             index - 1);
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), query, -1, &stmt, NULL);
    if (sqlite3_step(stmt) == SQLITE_ROW)
    {
...
int dt_lua_tag_get_attached(lua_State *L)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT tagid FROM main.tagged_images WHERE imgid=?1", -1, &stmt,
                                NULL);
