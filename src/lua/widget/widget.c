...
dt_lua_widget_type_t widget_type = {.name = "widget",
                                    .gui_init = NULL,
                                    .gui_cleanup = NULL,
...
static int widget_gc(lua_State *L)
{
    ...
    if (!lwidget)
        return 0; // object has been destroyed
    if (gtk_widget_get_parent(lwidget->widget))
    {
        luaL_error(
            L, "Destroying a widget which is still parented, this should never happen (%s at %p)\n",
...
static int get_widget_params(lua_State *L)
{
    ...
    {
        g_signal_connect_data(widget->widget, lua_tostring(L, -2),
                              G_CALLBACK(lua_touserdata(L, -1)), widget, NULL, 0);
        lua_pop(L, 1);
    }
    lua_pop(L, 1);
...
luaA_Type dt_lua_init_widget_type_type(lua_State *L, dt_lua_widget_type_t *widget_type,
                                       const char *lua_type, GType gtk_type)
{
    luaA_Type type_id =
        dt_lua_init_gpointer_type_type(L, luaA_type_add(L, lua_type, sizeof(gpointer)));
...
void dt_lua_widget_register_gtk_callback_type(lua_State *L, luaA_Type type_id,
                                              const char *signal_name, const char *lua_name,
                                              GCallback callback)
{
