...
static struct dt_lua_expand_enabled_info expand_store = {.used = FALSE};

static struct dt_lua_fill_enabled_info fill_store = {.used = FALSE};

static struct dt_lua_padding_size_info padding_store = {.used = FALSE};
...
void _get_packing_info(lua_box box, gboolean *expand, gboolean *fill, guint *padding)
{
    ...
    {
        gtk_box_query_child_packing(GTK_BOX(box->widget), GTK_WIDGET(l->data), expand, fill,
                                    padding, NULL);
        break;
    }
...
void _set_packing_info(lua_box box, gboolean expand, gboolean fill, guint padding)
{
    ...
    for (const GList *l = children; l; l = g_list_next(l))
    {
        gtk_box_set_child_packing(GTK_BOX(box->widget), GTK_WIDGET(l->data), expand, fill, padding,
                                  GTK_PACK_START);
    }
    g_list_free(children);
}
...
static int orientation_member(lua_State *L)
{
    ...
    if (lua_gettop(L) > 2)
    {
        ...
        if (gtk_orientable_get_orientation(GTK_ORIENTABLE(box->widget)) ==
            GTK_ORIENTATION_HORIZONTAL)
        {
            GList *children = gtk_container_get_children(GTK_CONTAINER(box->widget));
            for (const GList *l = children; l; l = g_list_next(l))
            {
                gtk_box_set_child_packing(GTK_BOX(box->widget), GTK_WIDGET(l->data), TRUE, TRUE, 0,
                                          GTK_PACK_START);
            }
            g_list_free(children);
