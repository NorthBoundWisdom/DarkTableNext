...
static dt_lua_widget_type_t check_button_type = {.name = "check_button",
                                                 .gui_init = NULL,
                                                 .gui_cleanup = NULL,
...
static int label_member(lua_State *L)
{
    ...
    if (lua_gettop(L) > 2)
    {
        ...
        gtk_label_set_ellipsize(GTK_LABEL(gtk_bin_get_child(GTK_BIN(check_button->widget))),
                                PANGO_ELLIPSIZE_END);
        return 0;
    }
...
int dt_lua_init_widget_check_button(lua_State *L)
{
    ...
    dt_lua_widget_register_gtk_callback(L, lua_check_button, "clicked", "clicked_callback",
