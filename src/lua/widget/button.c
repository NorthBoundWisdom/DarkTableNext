...
static struct dt_lua_ellipsize_mode_info ellipsize_store = {.used = FALSE};

static struct dt_lua_halign_info halign_store = {.used = FALSE};

static struct dt_lua_position_type_info position_type_store = {.used = FALSE};
...
static void clicked_callback(GtkButton *widget, gpointer user_data)
{
    dt_lua_async_call_alien(dt_lua_widget_trigger_callback, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "lua_widget", user_data, LUA_ASYNC_TYPENAME, "const char*", "clicked",
                            LUA_ASYNC_DONE);
}
...
static int ellipsize_member(lua_State *L)
{
    ...
    if (lua_gettop(L) > 2)
    {
        ...
        if (gtk_button_get_label(GTK_BUTTON(button->widget)))
            gtk_label_set_ellipsize(GTK_LABEL(gtk_bin_get_child(GTK_BIN(button->widget))),
                                    ellipsize);
        else
        {
...
static int halign_member(lua_State *L)
{
    ...
    if (lua_gettop(L) > 2)
    {
        ...
        // check for label before trying to ellipsize it
        if (gtk_button_get_label(GTK_BUTTON(button->widget)))
            gtk_widget_set_halign(GTK_WIDGET(GTK_LABEL(gtk_bin_get_child(GTK_BIN(button->widget)))),
                                  halign);
        else
        {
    ...
    halign =
        gtk_widget_get_halign(GTK_WIDGET(GTK_LABEL(gtk_bin_get_child(GTK_BIN(button->widget)))));
...
static int tostring_member(lua_State *L)
{
    ...
    gchar *res =
        g_strdup_printf("%s (\"%s\")", G_OBJECT_TYPE_NAME(widget->widget), text ? text : "");
...
int dt_lua_init_widget_button(lua_State *L)
{
    ...
    dt_lua_widget_register_gtk_callback(L, lua_button, "clicked", "clicked_callback",
                                        G_CALLBACK(clicked_callback));
