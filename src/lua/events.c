...
void dt_lua_event_add(lua_State *L, const char *evt_name)
{
    ...
    else
    {
        dt_print(DT_DEBUG_LUA,
                 "LUA ERROR :%s: function argument not found for on_event for event %s",
    ...
    {
        dt_print(DT_DEBUG_LUA,
                 "LUA ERROR : %s: function argument not found for on_destroy for event %s",
                 __FUNCTION__, evt_name);
        return;
    }
    ...
    {
        dt_print(DT_DEBUG_LUA,
                 "LUA ERROR : %s: function argument not found for on_register for event %s",
                 __FUNCTION__, evt_name);
        return;
    }
...
int dt_lua_event_multiinstance_destroy(lua_State *L)
{
    ...
    if (luaL_len(L, 1) != luaL_len(L, 2))
        luaL_error(
            L,
            "index table and data table sizes differ after event removal.  %s events are corrupted.\n",
            luaL_checkstring(L, 4));
...
static void shortcut_callback(dt_action_t *action)
{
    dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "const char*", "shortcut", LUA_ASYNC_TYPENAME_WITH_FREE, "char*",
                            strdup(action->label), g_cclosure_new(G_CALLBACK(&free), NULL, NULL),
                            LUA_ASYNC_DONE);
}
