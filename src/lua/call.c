...
void dt_lua_async_call_internal(const char *function, int line, lua_State *L, int nargs,
                                int nresults, dt_lua_finish_callback cb, void *data)
{
#ifdef _DEBUG
    dt_print(DT_DEBUG_LUA, "LUA DEBUG : %s called from %s %d, nargs : %d", __FUNCTION__, function,
             line, nargs);
#endif
...
void dt_lua_async_call_alien_internal(const char *call_function, int line, lua_CFunction pusher,
                                      int nresults, dt_lua_finish_callback cb, void *cb_data,
...
void dt_lua_async_call_string_internal(const char *function, int line, const char *lua_string,
                                       int nresults, dt_lua_finish_callback cb, void *cb_data)
{
#ifdef _DEBUG
    dt_print(DT_DEBUG_LUA, "LUA DEBUG : %s called from %s %d, string %s", __FUNCTION__, function,
             line, lua_string);
#endif
...
gboolean dt_lua_gtk_wrap_callback(gpointer data)
{
    ...
    communication->retval =
        lua_pcall(communication->L, lua_gettop(communication->L) - 1, LUA_MULTRET, 0);
...
static int gtk_wrap(lua_State *L)
{
    ...
    else
    {
        ...
        g_main_context_invoke_full(NULL, G_PRIORITY_HIGH_IDLE, dt_lua_gtk_wrap_callback,
                                   &communication, NULL);
        g_mutex_lock(&communication.end_mutex);
