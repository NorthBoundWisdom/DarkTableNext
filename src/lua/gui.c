...
static int _mimic_cb(lua_State *L)
{
    ...
    for (int i = 0; i < darktable.control->widget_definitions->len; i++)
    {
        ...
        if (!strcmp(def->name, ac_type))
            break;
    }

    lua_getglobal(L, "script_manager_running_script");
    dt_action_define(&darktable.control->actions_lua, lua_tolstring(L, -1, NULL), ac_name, NULL,
                     def);
...
static int _panel_hide_all_cb(lua_State *L)
{
    for (int k = 0; k < DT_UI_PANEL_SIZE; k++)
        dt_ui_panel_show(darktable.gui->ui, k, FALSE, TRUE);
    // code goes here
    return 0;
}
...
{
    for (int k = 0; k < DT_UI_PANEL_SIZE; k++)
        dt_ui_panel_show(darktable.gui->ui, k, TRUE, TRUE);
    return 0;
}
...
{
    dt_lua_async_call_alien(_job_canceled, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "dt_lua_backgroundjob_t", progress, LUA_ASYNC_DONE);
}
...
static int _lua_job_progress(lua_State *L)
{
    ...
    if (!iter)
        luaL_error(L, "Accessing an invalid job");
    if (lua_isnone(L, 3))
    {
...
static void _on_mouse_over_image_changed(gpointer instance, gpointer user_data)
{
    ...
    if (dt_is_valid_imgid(imgid))
    {
        dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                                "char*", "mouse-over-image-changed", LUA_ASYNC_TYPENAME,
                                "dt_lua_image_t", imgid, LUA_ASYNC_DONE);
    }
    else
    {
        dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                                "char*", "mouse-over-image-changed", LUA_ASYNC_DONE);
    }
}
...
int dt_lua_init_gui(lua_State *L)
{
    if (darktable.gui != NULL)
    {
        ...
        DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_MOUSE_OVER_IMAGE_CHANGE, _on_mouse_over_image_changed,
                                  NULL);
    }
    return 0;
}
