...
#ifdef HAVE_AI
void dtgtk_cairo_paint_masks_object(cairo_t *cr, const gint x, const gint y, const gint w,
                                    const gint h, gint flags, void *data)
{
    PREAMBLE(1.15, 1, 0, 0)
    ...
    // top → right
    cairo_curve_to(cr, cx + r * 0.25, cy - r * pinch, cx + r * pinch, cy - r * 0.25, cx + r * tip,
                   cy);

    // right → bottom
    cairo_curve_to(cr, cx + r * pinch, cy + r * 0.25, cx + r * 0.25, cy + r * pinch, cx,
                   cy + r * tip);
