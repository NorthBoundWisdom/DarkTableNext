            ...
            if (g_file_test(config_path, G_FILE_TEST_EXISTS))
            {
                ...
                if (json_parser_load_from_file(parser, config_path, &error))
                {
                    ...
                    if (id && name)
                    {
                        ...
                        {
                            dt_print(DT_DEBUG_AI, "[darktable_ai] skipping duplicate model ID: %s",
                                     id);
                        }
                        else
                        {
                            ...
                            info->num_inputs =
                                json_object_has_member(obj, "num_inputs") ?
                            ...
                            if (json_object_has_member(obj, "cpu_only"))
                            {
                                ...
                                if (co_node && (JSON_NODE_HOLDS_ARRAY(co_node) ||
                            ...
                            {
                                JsonNode *cf_node = json_object_get_member(obj, "coreml_format");
                                if (cf_node && (JSON_NODE_HOLDS_VALUE(cf_node) ||
...
const dt_ai_model_info_t *dt_ai_get_model_info_by_index(dt_ai_environment_t *env, int index)
{
    if (!env)
        return NULL;
    g_mutex_lock(&env->lock);
...
const dt_ai_model_info_t *dt_ai_get_model_info_by_id(dt_ai_environment_t *env, const char *id)
{
    if (!env || !id)
        return NULL;
    g_mutex_lock(&env->lock);
...
static void _free_model_info(gpointer data)
{
    g_free(data);
}
...
extern dt_ai_context_t *dt_ai_onnx_load_ext(const char *model_dir, const char *model_file,
                                            dt_ai_provider_t provider, dt_ai_opt_level_t opt_level,
                                            const dt_ai_dim_override_t *dim_overrides,
                                            int n_overrides, uint32_t ep_flags);

static gboolean _provider_cpu_only(const dt_ai_model_info_t *info, const char *model_file,
                                   dt_ai_provider_t configured);

static dt_ai_provider_t _resolve_provider(dt_ai_provider_t configured, const char *model_id,
                                          const dt_ai_model_info_t *info, const char *model_file,
                                          uint32_t *inout_ep_flags);

// model loading with backend dispatch

dt_ai_context_t *dt_ai_load_model(dt_ai_environment_t *env, const char *model_id,
                                  const char *model_file, const dt_ai_provider_t provider)
{
    return dt_ai_load_model_ext(env, model_id, model_file, provider, DT_AI_OPT_ALL, NULL, 0);
}

dt_ai_context_t *dt_ai_load_model_ext(dt_ai_environment_t *env, const char *model_id,
                                      const char *model_file, const dt_ai_provider_t provider,
                                      ...
                                      const int n_overrides)
{
    ...
    uint32_t ep_flags = 0;
    const dt_ai_model_info_t *model_info = dt_ai_get_model_info_by_id(env, model_id);
    const dt_ai_provider_t resolved =
        _resolve_provider(provider, model_id, model_info, model_file, &ep_flags);

    g_mutex_lock(&env->lock);
    const char *model_dir_orig = (const char *)g_hash_table_lookup(env->model_paths, model_id);
    ...
    {
        ctx = dt_ai_onnx_load_ext(model_dir, model_file, resolved, opt_level, dim_overrides,
                                  n_overrides, ep_flags);
    }
    else
    {
        dt_print(DT_DEBUG_AI, "[darktable_ai] unknown backend '%s' for model '%s'", backend_copy,
                 model_id);
    }
...
static JsonParser *_attribute_node(const dt_ai_model_info_t *info, const char *key,
                                   JsonNode **out_node)
{
    *out_node = NULL;
    if (!info || !info->attributes || !key)
        return NULL;
    JsonParser *parser = json_parser_new();
    ...
    for (int i = 0; i < n; i++)
    {
        if (!json_object_has_member(obj, segments[i]))
            goto out;
...
gboolean dt_ai_model_attribute_bool(const dt_ai_model_info_t *info, const char *key)
{
    JsonNode *v = NULL;
...
int dt_ai_model_attribute_int(const dt_ai_model_info_t *info, const char *key, int default_value)
{
    JsonNode *v = NULL;
    JsonParser *p = _attribute_node(info, key, &v);
...
double dt_ai_model_attribute_double(const dt_ai_model_info_t *info, const char *key,
...
char *dt_ai_model_attribute_string(const dt_ai_model_info_t *info, const char *key)
{
    JsonNode *v = NULL;
    JsonParser *p = _attribute_node(info, key, &v);
    char *result = NULL;
...
int *dt_ai_model_attribute_int_array(const dt_ai_model_info_t *info, const char *key,
                                     int *out_count)
{
    if (out_count)
        *out_count = 0;
                                   ...
                                   dt_ai_provider_t concrete)
{
    ...
    if (json_parser_load_from_data(parser, info->cpu_only, -1, &err))
    {
        ...
        else if (root && JSON_NODE_HOLDS_OBJECT(root) && model_file)
        {
            ...
            stem = dot ? g_strndup(model_file, dot - model_file) : g_strdup(model_file);
            JsonObject *obj = json_node_get_object(root);
...
static gboolean _coreml_use_mlprogram(const dt_ai_model_info_t *info, const char *model_file)
{
    if (!info || !info->coreml_format)
        return FALSE;
...
static const dt_ai_provider_t *_auto_candidates(int *n_out)
{
    ...
    static const dt_ai_provider_t list[] = {DT_AI_PROVIDER_CUDA, DT_AI_PROVIDER_MIGRAPHX,
...
{
    for (int i = 0; i < DT_AI_PROVIDER_COUNT; i++)
        if (dt_ai_providers[i].value == p)
            return dt_ai_providers[i].config_string;
    return "?";
}
                                          ...
                                          uint32_t *inout_ep_flags)
{
    ...
    if (configured == DT_AI_PROVIDER_CPU)
    ...
    if (configured == DT_AI_PROVIDER_AUTO)
    {
        ...
        for (int i = 0; i < n && !resolved; i++)
        {
            ...
            if (!dt_ai_probe_provider(c))
                continue;
            const gboolean banned = _provider_cpu_only(info, model_file, c);
            if (!banned)
            {
                dt_print(DT_DEBUG_AI, "[darktable_ai] provider auto -> %s for %s",
            ...
            {
                dt_print(DT_DEBUG_AI, "[darktable_ai] auto: skipping %s for %s — model prefers CPU",
                         _provider_config_name(c), id);
            }
        }
        if (!resolved)
    ...
    else if (_provider_cpu_only(info, model_file, configured))
    {
        ...
        {
            dt_print(DT_DEBUG_AI, "[darktable_ai] %s prefers CPU on %s — switching to CPU provider",
                     id, _provider_config_name(configured));
        }
    }
    else
    ...
    if (result == DT_AI_PROVIDER_COREML && _coreml_use_mlprogram(info, model_file))
...
gboolean dt_ai_backend_cache_dir(dt_ai_provider_t provider, const char *fingerprint,
                                 const char *model_id, char *out, size_t size)
{
    if (!out || size == 0)
        return FALSE;
    const char *ep = _ep_name(provider);
    if (!ep || !fingerprint || !model_id || !model_id[0])
        return FALSE;

    char cachedir[PATH_MAX] = {0};
    dt_loc_get_user_cache_dir(cachedir, sizeof(cachedir));

    gchar *subdir = g_strdup_printf("ai_v%d_%s_%s", DT_AI_CACHE_SCHEMA, ep, fingerprint);
    ...
    {
        dt_print(DT_DEBUG_AI, "[ai_cache] failed to create cache directory: %s", full);
        g_free(full);
        return FALSE;
    }
