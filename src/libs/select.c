...
static void _image_selection_changed_callback(gpointer instance, dt_lib_module_t *self)
{
    ...
    dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "char*", "selection-changed", LUA_ASYNC_DONE);
#endif
}
...
{
    dt_lua_async_call_alien(lua_button_clicked_cb, 0, NULL, NULL, LUA_ASYNC_TYPENAME, "void*",
                            user_data, LUA_ASYNC_DONE);
}
...
static int lua_register_selection(lua_State *L)
{
    ...
    data->self = self;
    gulong s = g_signal_connect_data(G_OBJECT(button), "clicked", G_CALLBACK(lua_button_clicked),
                                     data, NULL, 0);
