...
static void _lib_location_entry_activated(GtkButton *button, dt_lib_module_t *self);

static gboolean _lib_location_result_item_activated(GtkButton *button, GdkEventButton *ev,
                                                    gpointer user_data);
                                                 ...
                                                 _lib_location_result_t *place)
{
    ...
    if (param)
    {
        ...
        g_signal_connect(G_OBJECT(eb), "button-press-event",
                         G_CALLBACK(_lib_location_result_item_activated), (gpointer)param);
    }
    return eb;
}

static size_t _lib_location_curl_write_data(void *buffer, const size_t size, const size_t nmemb,
                                            void *userp)
{
...
static GMarkupParser _lib_location_parser = {_lib_location_parser_start_element, NULL, NULL, NULL,
                                             NULL};
...
{
    if (lib->marker_type == MAP_DISPLAY_NONE)
        return;
...
static void _show_location(dt_lib_location_t *lib, _lib_location_result_t *p)
{
    ...
    {
        dt_view_map_center_on_bbox(darktable.view_manager, p->bbox.lon1, p->bbox.lat1, p->bbox.lon2,
                                   p->bbox.lat2);
    }
...
static void _lib_location_search_finish(dt_lib_module_t *self)
{
    ...
    for (const GList *item = lib->places; item; item = g_list_next(item))
    {
        ...
        gtk_box_pack_start(GTK_BOX(lib->result), _lib_location_place_widget_new(lib, place), TRUE,
                           TRUE, 0);
...
static gboolean _lib_location_search(dt_lib_module_t *self)
{
    ...
    if (!g_strcmp0(
            search_url,
            "https://nominatim.openstreetmap.org/search/%s?format=xml&limit=%d&polygon_text=1"))
    {
...
gboolean _lib_location_result_item_activated(GtkButton *button, GdkEventButton *ev,
                                             gpointer user_data)
{
...
void _lib_location_entry_activated(GtkButton *button, dt_lib_module_t *self)
{
    dt_lib_location_t *lib = self->data;
    ...
    g_idle_add_full(G_PRIORITY_DEFAULT_IDLE, (GSourceFunc)_lib_location_search, self,
                    (GDestroyNotify)_lib_location_search_finish);
}
                                               ...
                                               GError **e)
{
    ...
    if (*aname)
    {
        while (*aname)
        {
            ...
            else if (show_outline && strcmp(*aname, "geotext") == 0 &&
                     place->marker_type == MAP_DISPLAY_NONE)
            {
                ...
                )
                {
                    ...
                    while (1)
                    {
                        ...
                        if (*endptr ==
                            ')') // TODO: support holes in POLYGON and several forms in MULTIPOLYGON?
                        {
                    ...
                    {
                        place->marker_type = g_str_has_prefix(*avalue, "LINESTRING") ?
                                                 MAP_DISPLAY_TRACK :
                                                 MAP_DISPLAY_POLYGON;
                    }
                }
                else
                {
                    ...
                    dt_print(DT_DEBUG_ALWAYS, "unsupported outline: %s%s", s,
                             strlen(s) == strlen(*avalue) ? "" : " ...");
                    g_free(s);
                }
            }
...
int set_params(dt_lib_module_t *self, const void *params, int size)
{
    ...
    if (size_points % 2 * sizeof(float) != 0)
        return 1;
    ...
    for (const float *points = (float *)((uint8_t *)params + size_fixed + size_name);
         (uint8_t *)points < (uint8_t *)params + size; points += 2)
    {
        dt_geo_map_display_point_t *p = malloc(sizeof(dt_geo_map_display_point_t));
        if (p)
        {
