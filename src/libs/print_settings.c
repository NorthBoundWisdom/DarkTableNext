...
static void _precision_by_unit(const _unit_t unit, int *n_digits, float *incr, char **format)
{
...
static float _to_mm(dt_lib_print_settings_t *ps, const double value)
{
...
static float _mm_to_hscreen(dt_lib_print_settings_t *ps, const float value, const gboolean offset)
{
    ...
    return (offset ? ps->imgs.screen.page.x : 0) + (ps->imgs.screen.page.width * value / width);
}
...
static float _mm_to_vscreen(dt_lib_print_settings_t *ps, const float value, const gboolean offset)
{
    ...
    return (offset ? ps->imgs.screen.page.y : 0) + (ps->imgs.screen.page.height * value / height);
}
...
static float _hscreen_to_mm(dt_lib_print_settings_t *ps, const float value, const gboolean offset)
{
    ...
    return width * (value - (offset ? ps->imgs.screen.page.x : 0.0f)) / ps->imgs.screen.page.width;
}
...
static inline float _percent_unit_of(dt_lib_print_settings_t *ps, const float ref,
                                     const float value)
{
...
static int write_image(dt_imageio_module_data_t *data, const char *filename, const void *in,
                       ...
                       void *exif, int exif_len, const dt_imgid_t imgid, const int num,
                       const int total, dt_dev_pixelpipe_t *pipe, const gboolean export_masks)
{
...
static int _export_image(dt_job_t *job, dt_image_box *img)
{
    ...
    if (params->style)
        g_strlcpy(dat.head.style, params->style, sizeof(dat.head.style));
    ...
    dt_imageio_export_with_flags(
        img->imgid, "unused", &buf, (dt_imageio_module_data_t *)&dat, TRUE, FALSE, high_quality,
        upscale, is_scaling, scale_factor, FALSE, NULL, FALSE, export_masks, params->buf_icc_type,
    ...
    const dt_colorspaces_color_profile_t *buf_profile = dt_colorspaces_get_output_profile(
        img->imgid, params->buf_icc_type, params->buf_icc_profile);
    if (*params->p_icc_profile)
    {
        const dt_colorspaces_color_profile_t *pprof = dt_colorspaces_get_profile(
            params->p_icc_type, params->p_icc_profile, DT_PROFILE_DIRECTION_OUT);
        if (!pprof)
        {
            dt_control_log(_("cannot open printer profile `%s'"), params->p_icc_profile);
            dt_print(DT_DEBUG_ALWAYS, "cannot open printer profile `%s'", params->p_icc_profile);
        ...
        else
        {
            if (!buf_profile || !buf_profile->profile)
            {
                ...
                dt_print(DT_DEBUG_ALWAYS, "error getting output profile for image %d", img->imgid);
                dt_control_queue_redraw();
                return 1;
            }
            if (dt_apply_printer_profile((void **)&(params->buf), dat.head.width, dat.head.height,
                                         dat.bpp, buf_profile->profile, pprof->profile,
                                         params->p_icc_intent, params->black_point_compensation))
            {
...
static void _create_pdf(dt_job_t *job, dt_images_box imgs, const float width, const float height)
{
    ...
    for (int k = 0; k < imgs.count; k++)
    {
        ...
        {
            pdf_image[count] = dt_pdf_add_image(pdf, (uint8_t *)box->buf, box->exp_width,
                                                box->exp_height, 8, icc_id, 0.0);
...
static int _export_and_setup_pos(dt_job_t *job, dt_image_box *img, const int32_t idx)
{
    ...
    dt_print(DT_DEBUG_PRINT, "[print] max image size %d x %d (at resolution %d)", img->max_width,
             img->max_height, params->prt.printer.resolution);

    if (_export_image(job, img))
        return 1;

    dt_printing_setup_image(&params->imgs, idx, img->imgid, img->exp_width, img->exp_height,
                            img->alignment);
...
static void _print_job_cleanup(void *p)
{
    ...
    if (params->pdf_filename[0])
        g_unlink(params->pdf_filename);
...
static void _print_button_clicked(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    gchar *message =
        g_strdup_printf(_("processing `%s' for `%s'"), params->job_title, params->prt.printer.name);
...
static void _update_slider(dt_lib_print_settings_t *ps)
{
    ...
    if (ps->selected != -1 && dt_is_valid_imgid(ps->imgs.box[ps->selected].imgid) && ps->width &&
        ps->height && ps->info)
    {
        ...
        const double scale = (iwidth <= awidth) ? awidth / iwidth : aheight / iheight;

        value = g_strdup_printf(_("%3.2f (dpi:%d)"), scale,
                                scale <= 1.0 ? (int)ps->prt.printer.resolution :
...
static void _alignment_callback(GtkWidget *tb, dt_lib_module_t *self)
{
    DT_GUARD_GUI_UPDATE();
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        if (GTK_WIDGET(ps->dtba[i]) == tb)
        {
            ...
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ps->dtba[i]), FALSE);
    ...
    if (ps->last_selected != -1)
    {
        dt_printing_setup_image(&ps->imgs, ps->last_selected, ps->imgs.box[ps->last_selected].imgid,
                                100, 100, index);
    }
...
static void _unit_changed(GtkWidget *combo, dt_lib_module_t *self)
{
    ...
    if (unit < 0)
        return; // shouldn't happen, but it could be -1 if nothing is selected
    ps->unit = unit;
...
static void _update_style_label(dt_lib_print_settings_t *ps, const char *name)
{
    ...
    char *localized_style =
        is_style_set ? dt_util_localize_segmented_name(name, TRUE) : g_strdup(_("none"));
...
static void _style_mode_changed(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_print_settings_t *ps = self->data;
...
static void _load_image_full_page(dt_lib_print_settings_t *ps, const dt_imgid_t imgid)
{
    _set_orientation(ps, imgid);

    dt_printing_setup_box(&ps->imgs, 0, ps->imgs.screen.page.x, ps->imgs.screen.page.y,
...
static void _print_settings_update_callback(gpointer instance, const dt_imgid_t imgid,
...
static void _print_settings_activate_callback(gpointer instance, const dt_imgid_t imgid,
                                              dt_lib_module_t *self)
{
    dt_lib_print_settings_t *ps = self->data;
                ...
                struct dt_view_t *new_view)
{
    ...
    DT_CONTROL_SIGNAL_HANDLE(DT_SIGNAL_VIEWMANAGER_THUMBTABLE_ACTIVATE,
                             _print_settings_activate_callback);
...
int mouse_moved(struct dt_lib_module_t *self, const double x, const double y, const double pressure,
                const int which)
{
    dt_lib_print_settings_t *ps = self->data;
                   ...
                   const double pressure, const int which, const int type, const uint32_t state)
{
    ...
    if (ps->creation)
    {
        ...
        _snap_to_grid(ps, &ps->x1, &ps->y1);
             ...
             (which == GDK_BUTTON_MIDDLE ||
              (which == GDK_BUTTON_PRIMARY && dt_modifier_is(state, GDK_CONTROL_MASK))))
    {
...
void _cairo_rectangle(cairo_t *cr, const int sel_controls, const int x1, const int y1, const int x2,
                      const int y2)
{
    ...
    {
        cairo_rectangle(cr, x2 - DT_PIXEL_APPLY_DPI(15), y1, DT_PIXEL_APPLY_DPI(15),
                        DT_PIXEL_APPLY_DPI(15));
        cairo_stroke(cr);
    }

    if (sel_controls == BOX_BOTTOM_LEFT)
    {
        cairo_rectangle(cr, x1, y2 - DT_PIXEL_APPLY_DPI(15), DT_PIXEL_APPLY_DPI(15),
                        DT_PIXEL_APPLY_DPI(15));
        cairo_stroke(cr);
    }
...
void gui_post_expose(struct dt_lib_module_t *self, cairo_t *cr, const int32_t width,
                     const int32_t height, const int32_t pointerx, const int32_t pointery)
{
        ...
        (int)_mm_to_hscreen(ps, step, FALSE) > DT_PIXEL_APPLY_DPI(5))
    {
        ...
        while (grid_pos < ps->imgs.screen.page.x + ps->imgs.screen.page.width)
        {
            ...
            cairo_set_line_width(cr, ((n % 5) == 0) ? DT_PIXEL_APPLY_DPI(1.0) :
    ...
    for (int k = 0; k < ps->imgs.count; k++)
    {
        ...
        if (dt_is_valid_imgid(img->imgid))
        {
            ...
            else
            {
                ...
                const double alpha =
                    (ps->dragging || (ps->selected != -1 && ps->selected != k)) ? 0.25 : 1.0;
        ...
        if (k == ps->selected || !dt_is_valid_imgid(img->imgid))
        {
            ...
            _cairo_rectangle(cr, (k == ps->selected) ? ps->sel_controls : 0, img->screen.x,
                             img->screen.y, img->screen.x + img->screen.width,
        ...
        else if (k == ps->last_selected)
        {
            ...
            cairo_rectangle(cr, img->screen.x, img->screen.y, img->screen.width,
                            img->screen.height);
            cairo_stroke(cr);
        }
    ...
    if (ps->dragging || ps->selected != -1)
    {
            ...
            x2 <= (ps->imgs.screen.page.x + ps->imgs.screen.page.width))
        {
            ...
            cairo_line_to(cr, ps->imgs.screen.page.x + ps->imgs.screen.page.width,
                          yp + text_h * 0.5);
            cairo_stroke_preserve(cr);
            cairo_set_source_rgba(cr, .5, .5, .5, .9);
            cairo_set_dash(cr, &dash, 1, dash);
            ...
            y2 <= (ps->imgs.screen.page.y + ps->imgs.screen.page.height))
        {
            ...
            cairo_line_to(cr, xp + text_h * 0.5,
        ...
        cairo_set_source_rgba(cr, .5, .5, .5, .9);
        dt_gui_draw_rounded_rectangle(cr, ext.width + 2 * margin, text_h + 2 * margin, xp - margin,
                                      yp - margin);
        cairo_set_source_rgb(cr, .8, .8, .8);
        cairo_move_to(cr, xp, yp);
        pango_cairo_show_layout(cr, layout);
        ...
        yp = (y1 + y2) * .5;
        cairo_set_source_rgba(cr, .5, .5, .5, .9);
        dt_gui_draw_rounded_rectangle(cr, text_h + 2 * margin, ext.width + 2 * margin, xp - margin,
                                      yp - margin - 0.5 * ext.width);
        cairo_set_source_rgb(cr, .8, .8, .8);
...
static void _width_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    dt_printing_setup_box(&ps->imgs, ps->last_selected, box->screen.x, box->screen.y,
...
static void _x_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    dt_printing_setup_box(&ps->imgs, ps->last_selected, _mm_to_hscreen(ps, nv_mm, TRUE),
                          box->screen.y, box->screen.width, box->screen.height);
...
static void _y_changed(GtkWidget *widget, gpointer user_data)
{
    ...
    dt_printing_setup_box(&ps->imgs, ps->last_selected, box->screen.x,
                          _mm_to_vscreen(ps, nv_mm, TRUE), box->screen.width, box->screen.height);
...
void gui_init(dt_lib_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(d->pprofile), "value-changed", G_CALLBACK(_printer_profile_changed),
                     (gpointer)self);

    //  Add printer intent combo

    d->v_pintent = dt_conf_get_int("plugins/print/printer/iccintent");
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->pintent, self, N_("printer"), N_("intent"), NULL, d->v_pintent,
                                 _printer_intent_callback, self, N_("perceptual"),
    ...
    gtk_box_pack_start(GTK_BOX(self->widget), GTK_WIDGET(d->black_point_compensation), TRUE, FALSE,
                       0);
    g_signal_connect(d->black_point_compensation, "toggled", G_CALLBACK(_printer_bpc_callback),
                     (gpointer)self);
    ...
    GtkWidget *ucomb =
        dt_bauhaus_combobox_new_full(DT_ACTION(self), NULL, NULL, _("measurement units"), d->unit,
    ...
    gtk_widget_set_tooltip_text(hboxinfo, _("image scale factor from native printer DPI:\n"
    ...
    g_signal_connect(G_OBJECT(d->b_top), "value-changed", G_CALLBACK(_top_border_callback), self);
    g_signal_connect(G_OBJECT(d->b_bottom), "value-changed", G_CALLBACK(_bottom_border_callback),
                     self);
    g_signal_connect(G_OBJECT(d->b_left), "value-changed", G_CALLBACK(_left_border_callback), self);
    g_signal_connect(G_OBJECT(d->b_right), "value-changed", G_CALLBACK(_right_border_callback),
                     self);
    g_signal_connect(G_OBJECT(d->lock_button), "toggled", G_CALLBACK(_lock_callback), self);
    ...
    // grid & snap grid
    {
        ...
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(d->grid_size),
                                  dt_conf_get_float(PRINT_CONFIG_PREFIX "grid_size") *
                                      units[d->unit]);
        ...
        g_signal_connect(G_OBJECT(d->grid_size), "value-changed", G_CALLBACK(_grid_size_changed),
                         self);
    ...
    GtkWidget *bclear = dt_action_button_new(self, N_("clear layout"), _page_clear_area_clicked,
                                             self, _("remove all image areas from the page"), 0, 0);
    ...
    g_signal_connect(G_OBJECT(d->b_width), "value-changed", G_CALLBACK(_width_changed),
                     (gpointer)d);
    g_signal_connect(G_OBJECT(d->b_height), "value-changed", G_CALLBACK(_height_changed),
                     (gpointer)d);
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->intent, self, NULL, N_("intent"), NULL,
                                 ...
                                 _intent_callback, self, N_("image settings"), N_("perceptual"),
                                 N_("relative colorimetric"), NC_("rendering intent", "saturation"),
                                 N_("absolute colorimetric"));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        ...
        d->v_style_append ? 1 : 0, _style_mode_changed, self, N_("replace history"),
        N_("append history"));
    ...
    GtkWidget *button =
        dt_action_button_new(self, N_("print"), _print_button_clicked, self,
                    ...
                    size_t *new_size)
{
    if (old_version == 1)
    {
        ...
        //   <rest>
        memcpy((uint8_t *)new_params + pos, buf,
...
int set_params(dt_lib_module_t *self, const void *params, const int size)
{
    ...
    if (size != printer_len + paper_len + media_len + profile_len + pprofile_len + style_len +
                    8 * sizeof(int32_t) + 4 * sizeof(double) + sizeof(int32_t) +
    ...
    for (GList *iter = ps->profiles; iter; iter = g_list_next(iter))
    {
        ...
        if (f_profile_type == p->type &&
            (f_profile_type != DT_COLORSPACE_FILE || !g_strcmp0(f_profile, p->filename)))
        {
    ...
    {
        dt_lib_export_profile_t *p = iter->data;
        if (f_pprofile_type == p->type &&
            (f_pprofile_type != DT_COLORSPACE_FILE || !g_strcmp0(f_pprofile, p->filename)))
        {
...
void *get_params(dt_lib_module_t *self, int *size)
{
    ...
    dt_colorspaces_color_profile_type_t profile_type = DT_COLORSPACE_NONE,
                                        pprofile_type = DT_COLORSPACE_NONE;
    ...
    *size = printer_len + paper_len + media_len + profile_len + pprofile_len + style_len +
            8 * sizeof(int32_t) + 4 * sizeof(double) + sizeof(int32_t) +
            (ps->imgs.count * 4 * sizeof(float));
...
void gui_cleanup(dt_lib_module_t *self)
{
    ...
    g_signal_handlers_disconnect_by_func(G_OBJECT(ps->b_top), G_CALLBACK(_top_border_callback),
                                         self);
    g_signal_handlers_disconnect_by_func(G_OBJECT(ps->b_bottom),
                                         G_CALLBACK(_bottom_border_callback), self);
    g_signal_handlers_disconnect_by_func(G_OBJECT(ps->b_left), G_CALLBACK(_left_border_callback),
                                         self);
    g_signal_handlers_disconnect_by_func(G_OBJECT(ps->b_right), G_CALLBACK(_right_border_callback),
                                         self);
