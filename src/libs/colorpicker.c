...
static gboolean _sample_draw_callback(GtkWidget *widget, cairo_t *cr,
...
static void _update_sample_label(dt_lib_module_t *self, dt_colorpicker_sample_t *sample)
{
    ...
    case DT_LIB_COLORPICKER_MODEL_RGB:
        snprintf(text, sizeof(text), "%6d %6d %6d", sample->label_rgb[0], sample->label_rgb[1],
                 sample->label_rgb[2]);
        break;
    case DT_LIB_COLORPICKER_MODEL_RGB_PERCENT:
        snprintf(text, sizeof(text), "%6.1f%% %6.1f%% %6.1f%%",
                 (sample->scope[statistic][0] * 100.f), (sample->scope[statistic][1] * 100.f),
                 (sample->scope[statistic][2] * 100.f));
        break;
    case DT_LIB_COLORPICKER_MODEL_LAB:
        snprintf(text, sizeof(text), "%6.02f %6.02f %6.02f",
                 CLAMP(sample->lab[statistic][0], .0f, 100.0f), sample->lab[statistic][1],
                 sample->lab[statistic][2]);
        break;

    case DT_LIB_COLORPICKER_MODEL_LCH:
        dt_Lab_2_LCH(sample->lab[statistic], alt);
        snprintf(text, sizeof(text), "%6.02f %6.02f %6.02f", CLAMP(alt[0], .0f, 100.0f), alt[1],
                 alt[2] * 360.f);
        break;

    case DT_LIB_COLORPICKER_MODEL_HSL:
        dt_RGB_2_HSL(sample->scope[statistic], alt);
        snprintf(text, sizeof(text), "%6.02f %6.02f %6.02f", alt[0] * 360.f, alt[1] * 100.f,
                 alt[2] * 100.f);
        break;
    ...
    case DT_LIB_COLORPICKER_MODEL_HEX:
        ...
        snprintf(text, sizeof(text), "0x%02X%02X%02X", CLAMP(sample->label_rgb[0], 0, 255),
                 CLAMP(sample->label_rgb[1], 0, 255), CLAMP(sample->label_rgb[2], 0, 255));
        break;
...
static void _update_size(dt_lib_module_t *self, const dt_lib_colorpicker_size_t size)
{
    dt_lib_colorpicker_t *data = self->data;
...
static void _set_sample_box_area(dt_lib_module_t *self, const dt_pickerbox_t box)
{
    dt_lib_colorpicker_t *data = self->data;
...
static void _set_sample_point(dt_lib_module_t *self, const dt_pickerpoint_t pos)
{
    dt_lib_colorpicker_t *data = self->data;
...
static gboolean _sample_tooltip_callback(GtkWidget *widget, const gint x, const gint y,
                                         ...
                                         const dt_colorpicker_sample_t *sample)
{
    ...
    sample_parts[3] = g_strdup_printf(
        "%22s(0x%02X%02X%02X)\n<big><b>%14s</b></big>", " ", CLAMP(sample->label_rgb[0], 0, 255),
        CLAMP(sample->label_rgb[1], 0, 255), CLAMP(sample->label_rgb[2], 0, 255), _("RGB"));
    sample_parts[7] = g_strdup_printf("\n<big><b>%14s</b></big>", _("Lab"));

    for (int i = 0; i < DT_PICK_N; i++)
    {
        sample_parts[i] =
            g_strdup_printf("<span background='#%02X%02X%02X'>%32s</span>",
        ...
        sample_parts[i + 4] = g_strdup_printf(
            "<span foreground='#FF7F7F'>%6d</span>  "
            "<span foreground='#7FFF7F'>%6d</span>  "
            "<span foreground='#7F7FFF'>%6d</span>  %s",
            (int)roundf(sample->scope[i][0] * 255.f), (int)roundf(sample->scope[i][1] * 255.f),
            (int)roundf(sample->scope[i][2] * 255.f), _(dt_lib_colorpicker_statistic_names[i]));

        sample_parts[i + 8] =
            g_strdup_printf("%6.02f  %6.02f  %6.02f  %s", sample->lab[i][0], sample->lab[i][1],
                            sample->lab[i][2], _(dt_lib_colorpicker_statistic_names[i]));
    }
...
static void _label_size_allocate_callback(GtkWidget *widget, GdkRectangle *allocation,
                                          gpointer user_data)
{
    gint label_width;
...
static gboolean _sample_enter_callback(GtkWidget *widget, GdkEvent *event,
                                       dt_colorpicker_sample_t *sample)
{
...
static void _add_sample(GtkButton *widget, dt_lib_module_t *self)
{
    ...
    gtk_container_add(
        GTK_CONTAINER(sample->container),
        dt_gui_hbox(color_patch_wrapper, dt_gui_expand(sample->output_label), delete_button));
...
void gui_init(dt_lib_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(color_patch), "draw", G_CALLBACK(_sample_draw_callback),
                     &data->primary_sample);
    g_signal_connect(G_OBJECT(color_patch), "button-press-event", G_CALLBACK(_large_patch_toggle),
                     data);
    ...
    data->statistic_selector = dt_bauhaus_combobox_new_full(
        DT_ACTION(self), NULL, N_("statistic"), _("select which statistic to show"),
        data->statistic, (GtkCallback)_statistic_changed, self, dt_lib_colorpicker_statistic_names);
    ...
    data->color_mode_selector = dt_bauhaus_combobox_new_full(
        DT_ACTION(self), NULL, N_("color mode"), _("select which color mode to use"), data->model,
    ...
    gtk_widget_set_tooltip_text(
        data->picker_button, _("turn on color picker\nctrl+click or right-click to select an area\n"
                               "ctrl+click on canvas to switch between point/area"));
    gtk_widget_set_name(GTK_WIDGET(data->picker_button), "color-picker-button");
    g_signal_connect(G_OBJECT(data->picker_button), "toggled", G_CALLBACK(_picker_button_toggled),
                     data);
    dt_action_define(DT_ACTION(self), NULL, N_("pick color"), data->picker_button,
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("add sample"), data->add_sample_button,
                     &dt_action_def_button);

    gtk_container_add(
        GTK_CONTAINER(sample_row_events),
        dt_gui_hbox(sample_patch_wrapper, dt_gui_expand(label), data->add_sample_button));
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("display samples"), data->display_samples_check_box,
                     &dt_action_def_toggle);
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("restrict scope"), restrict_button,
                     &dt_action_def_toggle);
    gtk_label_set_ellipsize(GTK_LABEL(gtk_bin_get_child(GTK_BIN(restrict_button))),
                            PANGO_ELLIPSIZE_MIDDLE);
    ...
    g_signal_connect(G_OBJECT(restrict_button), "toggled", G_CALLBACK(_restrict_histogram_changed),
                     NULL);

    // Setting up the GUI
    self->widget = dt_gui_vbox(
        color_patch_wrapper, picker_row, sample_row_events,
        dt_ui_section_label_new(C_("section", "live samples")),
        dt_ui_resize_wrap(data->samples_container, 1, "plugins/darkroom/colorpicker/windowheight"),
        data->display_samples_check_box, restrict_button);
