...
typedef struct dt_lib_geotagging_t
{
    ...
    struct
    {
        ...
        GtkWidget *gpx_section, *preview_button, *apply_gpx_button, *select_button, *nb_imgs_label;
...
{
    if (!g_ascii_strcasecmp(filter_info->mime_type, "application/gpx+xml"))
        return TRUE;
    ...
    if (!g_ascii_strcasecmp(cc, ".gpx"))
        return TRUE;

    return FALSE;
}

static GtkWidget *_set_up_label(const char *name, const int align, GtkWidget *grid, const int col,
                                const int line, const int ellipsize)
{
...
static GDateTime *_localtime_text_to_utc_timeval(const char *date_time, GTimeZone *tz_camera,
                                                 GTimeZone *tz_utc, GTimeSpan offset)
{
                                   ...
                                   dt_lib_module_t *self)
{
    ...
    for (GList *i = d->imgs; i; i = g_list_next(i))
    {
        ...
        {
            GDateTime *dt =
                _localtime_text_to_utc_timeval(im->dt, d->tz_camera, darktable.utc_tz, d->offset);
...
static void _refresh_images_displayed_on_track(const int segid, const gboolean active,
                                               dt_lib_module_t *self)
{
    ...
    for (GList *i = d->imgs; active && i; i = g_list_next(i))
    {
        ...
        {
            count++;
            dt_sel_img_t *next = i->next ? (dt_sel_img_t *)i->next->data : NULL;
            if (!im->image && (!next || !((next->gl.latitude == im->gl.latitude) &&
...
static void _update_nb_images(dt_lib_module_t *self)
{
    ...
    for (int segid = 0; valid && segid < d->map.nb_tracks; segid++)
    {
        gboolean active;
        int nb;
        gtk_tree_model_get(model, &iter, DT_GEO_TRACKS_ACTIVE, &active, DT_GEO_TRACKS_IMAGES, &nb,
                           -1);
        if (active)
            nb_imgs += nb;
...
static void _update_buttons(dt_lib_module_t *self)
{
    ...
    gtk_label_set_text(GTK_LABEL(label),
                       d->offset ? _("apply offset and geo-location") : _("apply geo-location"));
    ...
    gtk_widget_set_sensitive(d->map.select_button, d->map.nb_imgs && d->map.nb_imgs != d->nb_imgs);
}
...
static gboolean _refresh_display_track(const gboolean active, const int segid,
                                       dt_lib_module_t *self)
{
    dt_lib_geotagging_t *d = self->data;
    gboolean grow = FALSE;
    if (active)
    {
        ...
        if (!d->map.tracks->td[segid].track)
            d->map.tracks->td[segid].track =
                dt_view_map_add_marker(darktable.view_manager, MAP_DISPLAY_TRACK, pts);
        osm_gps_map_track_set_color((OsmGpsMapTrack *)d->map.tracks->td[segid].track,
                                    &color[segid % 6]);
...
static gboolean _click_for_entire_track(GtkEntry *spin, GdkEventButton *event,
                                        dt_lib_module_t *self)
{
...
static void _track_seg_toggled(GtkCellRendererToggle *cell_renderer, gchar *path_str,
                               dt_lib_module_t *self)
{
    dt_lib_geotagging_t *d = self->data;
    GtkTreeModel *model = gtk_tree_view_get_model(GTK_TREE_VIEW(d->map.gpx_view));
    ...
    gtk_tree_model_get(model, &iter, DT_GEO_TRACKS_ACTIVE, &active, DT_GEO_TRACKS_SEGID, &segid,
                       -1);
...
static void _refresh_track_list(dt_lib_module_t *self)
{
    ...
    for (GList *ts = trkseg; ts && valid; ts = g_list_next(ts))
    {
        ...
        gtk_list_store_set(GTK_LIST_STORE(model), &iter, DT_GEO_TRACKS_DATETIME, dts,
                           DT_GEO_TRACKS_POINTS, t->nb_trkpt, DT_GEO_TRACKS_IMAGES, nb_imgs,
                           DT_GEO_TRACKS_TOOLTIP, tooltip, -1);
        g_free(dts);
        g_free(tooltip);
        valid = gtk_tree_model_iter_next(model, &iter);
    }
...
static void _show_gpx_tracks(dt_lib_module_t *self)
{
    ...
    for (GList *ts = trkseg; ts; ts = g_list_next(ts))
    {
        ...
        gtk_list_store_insert_with_values(
            GTK_LIST_STORE(model), NULL, -1, DT_GEO_TRACKS_ACTIVE, active, DT_GEO_TRACKS_DATETIME,
            dts, DT_GEO_TRACKS_POINTS, t->nb_trkpt, DT_GEO_TRACKS_IMAGES, nb_imgs,
            DT_GEO_TRACKS_SEGID, segid, DT_GEO_TRACKS_TOOLTIP, tooltip, -1);
        segid++;
        g_free(dts);
        g_free(tooltip);
    }
...
static void _preview_gpx_file(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(_("GPX file track segments"), GTK_WINDOW(win),
                                                    GTK_DIALOG_DESTROY_WITH_PARENT, _("_done"),
...
static void _choose_gpx_callback(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    GtkWidget *filechooser = gtk_file_chooser_dialog_new(
        _("open GPX file"), GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_OPEN, _("preview"),
        GTK_RESPONSE_ACCEPT, _("_cancel"), GTK_RESPONSE_CANCEL, _("_open"), GTK_RESPONSE_OK,
        (char *)NULL);
#ifdef GDK_WINDOWING_QUARTZ
    ...
    {
        dt_conf_set_folder_from_file_chooser("ui_last/gpx_last_directory",
...
static int _sort_timezones(gconstpointer a, gconstpointer b)
{
    ...
    else if (utc_neg_a && utc_pos_b)
        return -1;
    else if (utc_pos_a && utc_neg_b)
        return 1;
#endif
...
#undef MAX_LINE_LENGTH
    ...
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, keypath, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
    {
        ...
        if (RegQueryInfoKey(hKey, NULL, NULL, NULL, &n_subkeys, &max_subkey_len, NULL, NULL, NULL,
                            NULL, NULL, NULL) == ERROR_SUCCESS)
        {
            ...
            for (DWORD i = 1; i < n_subkeys; i++)
            {
                ...
                if (RegEnumKeyExW(hKey, i, subkeyname, &subkeyname_length, NULL, NULL, NULL,
                                  NULL) == ERROR_SUCCESS)
                {
                    DWORD buffer_size;
                    ...
                    if (RegGetValueW(HKEY_LOCAL_MACHINE, subkeypath, L"Display", RRF_RT_ANY, NULL,
                                     NULL, &buffer_size) == ERROR_SUCCESS)
                    {
                        wchar_t *display_name = malloc(buffer_size);
                        if (RegGetValueW(HKEY_LOCAL_MACHINE, subkeypath, L"Display", RRF_RT_ANY,
                                         NULL, display_name, &buffer_size) == ERROR_SUCCESS)
                        {
...
static void _display_datetime(dt_lib_datetime_t *dtw, GDateTime *datetime, const gboolean lock,
                              dt_lib_module_t *self)
{
    dt_lib_geotagging_t *d = self->data;
...
static void _datetime_scroll_over(GtkEventControllerScroll *controller, double dx, double dy,
                                  dt_lib_module_t *self)
{
    dt_lib_geotagging_t *d = self->data;
    if (!d->editing)
...
static GtkWidget *_gui_init_datetime(gchar *text, dt_lib_datetime_t *dt, const int type,
                                     dt_lib_module_t *self, GtkSizeGroup *group, GtkWidget *button,
                                     gchar *tip)
{
    ...
    for (int i = 0; i < DT_GEOTAG_PARTS_NB; i++)
    {
        ...
        if (i >= 2 || type != 2)
        {
            ...
            if (type == 0)
            {
                dt_action_define(DT_ACTION(self), NULL, i <= 2 ? N_("date") : N_("time"),
                                 dt->widget[i], &dt_action_def_entry);
    ...
    gtk_container_foreach(GTK_CONTAINER(flow2), (GtkCallback)gtk_widget_set_can_focus,
...
GtkTreeViewColumn *_new_tree_text_column(const char *name, const gboolean expand,
                                         const float xalign, const int m_col, const int ellipsize)
{
...
static void _show_milliseconds(dt_lib_geotagging_t *d)
{
    ...
    gtk_widget_set_visible(g_object_get_data(G_OBJECT(d->dt.widget[5]), "msec_label"),
                           milliseconds);
    gtk_widget_set_visible(g_object_get_data(G_OBJECT(d->dt0.widget[5]), "msec_label"),
                           milliseconds);
    gtk_widget_set_visible(g_object_get_data(G_OBJECT(d->of.widget[5]), "msec_label"),
                           milliseconds);
}
...
void gui_init(dt_lib_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        d->lock_offset, _("lock date/time offset value to apply it onto another selection"));
    gtk_widget_set_halign(d->lock_offset, GTK_ALIGN_START);
    g_signal_connect(G_OBJECT(d->lock_offset), "clicked", G_CALLBACK(_toggle_lock_button_callback),
                     (gpointer)self);
    ...
    d->apply_datetime =
        dt_action_button_new(self, N_("apply date/time"), _apply_datetime_callback, self,
    ...
    gtk_widget_set_tooltip_markup(label,
                                  _(dt_confgen_get_tooltip("plugins/lighttable/geotagging/tz")));

    gtk_grid_attach(grid, label, 0, line, 2, 1);

    d->timezone = dt_ui_entry_new(0);
    gtk_widget_set_tooltip_text(
        d->timezone, _("start typing to show a list of permitted values and select your timezone.\n"
    ...
    {
        tz_tuple_t *tz_tuple = (tz_tuple_t *)iter->data;
        gtk_list_store_insert_with_values(model, NULL, -1, 0, tz_tuple->display, 1, tz_tuple->name,
                                          -1);
    ...
    g_signal_connect(G_OBJECT(d->timezone), "key-press-event", G_CALLBACK(_timezone_key_pressed),
                     self);
    g_signal_connect(G_OBJECT(d->timezone), "focus-out-event", G_CALLBACK(_timezone_focus_out),
                     self);

    // gpx
    d->gpx_button =
        dt_action_button_new(self, N_("apply GPX track file..."), _choose_gpx_callback, self,
    ...
    g_signal_connect(G_OBJECT(d->map.gpx_button), "clicked", G_CALLBACK(_choose_gpx_callback),
                     self);
    ...
    model = gtk_list_store_new(DT_GEO_TRACKS_NUM_COLS, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_INT,
    ...
    GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(
        "", renderer, "active", DT_GEO_TRACKS_ACTIVE, NULL);
    ...
    column = _new_tree_text_column(_("start time"), TRUE, 0.0, DT_GEO_TRACKS_DATETIME,
                                   PANGO_ELLIPSIZE_START);
    gtk_tree_view_append_column(GTK_TREE_VIEW(d->map.gpx_view), column);
    column =
        _new_tree_text_column(_("points"), FALSE, 1.0, DT_GEO_TRACKS_POINTS, PANGO_ELLIPSIZE_NONE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(d->map.gpx_view), column);
    column =
        _new_tree_text_column(_("images"), FALSE, 1.0, DT_GEO_TRACKS_IMAGES, PANGO_ELLIPSIZE_NONE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(d->map.gpx_view), column);

    g_object_set(G_OBJECT(d->map.gpx_view), "has-tooltip", TRUE, NULL);
    g_signal_connect(G_OBJECT(d->map.gpx_view), "query-tooltip", G_CALLBACK(_row_tooltip_setup),
                     self);
    g_signal_connect(G_OBJECT(d->map.gpx_view), "button-press-event",
                     G_CALLBACK(_click_for_entire_track), self);

    // avoid ugly console pixman messages due to headers
    gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(d->map.gpx_view), FALSE);
    GtkWidget *w = dt_ui_resize_wrap(GTK_WIDGET(d->map.gpx_view), 100,
    ...
    GtkWidget *gpx_view_button =
        dt_action_button_new(self, N_("view entire track"), _refresh_display_all_tracks, self,
    ...
    gtk_label_set_ellipsize(GTK_LABEL(gtk_bin_get_child(GTK_BIN(d->map.preview_button))),
                            PANGO_ELLIPSIZE_END);
    ...
    g_signal_connect(GTK_TOGGLE_BUTTON(d->map.preview_button), "toggled",
                     G_CALLBACK(_images_preview_toggled), self);
    ...
    d->map.apply_gpx_button =
        dt_action_button_new(self, N_("apply geo-location"), _apply_gpx, self,
    ...
    for (int i = 0; i < DT_GEOTAG_PARTS_NB; i++)
    {
        ...
        g_signal_connect(d->dt.widget[i], "key-press-event", G_CALLBACK(_datetime_key_pressed),
                         self);
        dt_gui_connect_scroll(d->dt.widget[i],
                              GTK_EVENT_CONTROLLER_SCROLL_VERTICAL |
                                  GTK_EVENT_CONTROLLER_SCROLL_DISCRETE,
                              _datetime_scroll_over, self);
    }
