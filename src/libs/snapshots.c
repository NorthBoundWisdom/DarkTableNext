...
static int name_member(lua_State *L)
{
    ...
    GtkLabel *num =
        (GtkLabel *)_lib_snapshot_button_get_item(d->snapshot[index].button, _SNAPSHOT_BUTTON_NUM);
    GtkLabel *name =
        (GtkLabel *)_lib_snapshot_button_get_item(d->snapshot[index].button, _SNAPSHOT_BUTTON_NAME);
    ...
    char *value = g_strdup_printf("%s (%s)", gtk_label_get_text(name), n);
