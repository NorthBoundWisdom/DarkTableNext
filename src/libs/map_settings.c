...
void gui_init(dt_lib_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(d->map_source_dropdown,
                                _("select the source of the map. some entries might not work"));
    ...
    for (int i = 1; i < OSM_GPS_MAP_SOURCE_LAST; i++)
    {
        if (osm_gps_map_source_is_valid(i))
        {
            ...
            dt_bauhaus_combobox_add_full(d->map_source_dropdown, name,
                                         DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT, GINT_TO_POINTER(i), NULL,
                                         TRUE);
    ...
    g_signal_connect(G_OBJECT(d->map_source_dropdown), "value-changed",
    ...
    d->show_osd_checkbutton =
        dt_gui_preferences_bool(grid, "plugins/map/show_map_osd", 0, line++, FALSE);
    g_signal_connect(G_OBJECT(d->show_osd_checkbutton), "toggled", G_CALLBACK(_show_osd_toggled),
                     NULL);
    d->filtered_images_checkbutton =
        dt_gui_preferences_bool(grid, "plugins/map/filter_images_drawn", 0, line++, FALSE);
    g_signal_connect(G_OBJECT(d->filtered_images_checkbutton), "toggled",
                     G_CALLBACK(_parameter_changed), NULL);
    ...
    g_signal_connect(G_OBJECT(d->max_images_entry), "value-changed", G_CALLBACK(_parameter_changed),
                     self);
    d->epsilon_factor = dt_gui_preferences_int(grid, "plugins/map/epsilon_factor", 0, line++);
    g_signal_connect(G_OBJECT(d->epsilon_factor), "value-changed", G_CALLBACK(_parameter_changed),
                     self);
    d->min_images = dt_gui_preferences_int(grid, "plugins/map/min_images_per_group", 0, line++);
    g_signal_connect(G_OBJECT(d->min_images), "value-changed", G_CALLBACK(_parameter_changed),
                     self);
    ...
    dt_action_t *ac = dt_bauhaus_widget_set_label(
        d->images_thumb, NULL, dt_confgen_get_label("plugins/map/images_thumbnail"));
    dt_shortcut_register(ac, 0, 0, GDK_KEY_s, GDK_SHIFT_MASK);
    g_signal_connect(G_OBJECT(d->images_thumb), "value-changed", G_CALLBACK(_parameter_changed),
                     self);
