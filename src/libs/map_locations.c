...
const DTGTKCairoPaintIconFunc location_shapes[] = {
    dtgtk_cairo_paint_masks_circle,   // MAP_LOCATION_SHAPE_ELLIPSE
...
static void _locations_tree_update(dt_lib_module_t *self, const guint locid)
{
            ...
            if (tokens)
            {
                ...
                for (char **token = &tokens[common_length]; *token; token++)
                {
                    ...
                    gtk_tree_store_insert_with_values(
                        GTK_TREE_STORE(model), &iter, common_length > 0 ? &parent : NULL, -1,
                        DT_MAP_LOCATION_COL_TAG, *token, DT_MAP_LOCATION_COL_ID,
                        (token == &tokens[tokens_length - 1]) ?
                            ((dt_map_location_t *)stag->data)->id :
                            0,
                        DT_MAP_LOCATION_COL_PATH, pth2, DT_MAP_LOCATION_COL_COUNT,
                        (token == &tokens[tokens_length - 1]) ?
                            ((dt_map_location_t *)stag->data)->count :
                            0,
                        -1);
                    common_length++;
                    parent = iter;
    ...
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(model), DT_MAP_POSITION_SORT_NAME_ID,
                                         GTK_SORT_ASCENDING);
    if (locid)
    {
...
static void _display_buttons(dt_lib_module_t *self)
{
    ...
    {
        gtk_label_set_text(GTK_LABEL(gtk_bin_get_child(GTK_BIN(d->new_button))),
                           _("new sub-location"));
    }
    else
    {
...
static void _tree_name_show(GtkTreeViewColumn *col, GtkCellRenderer *renderer, GtkTreeModel *model,
                            GtkTreeIter *iter, gpointer data)
{
    ...
    guint count;
    gchar *coltext;
    gtk_tree_model_get(model, iter, DT_MAP_LOCATION_COL_ID, &locid, DT_MAP_LOCATION_COL_TAG, &name,
                       DT_MAP_LOCATION_COL_COUNT, &count, DT_MAP_LOCATION_COL_PATH, &path, -1);
    if (count < 1)
    {
...
static void _new_button_clicked(GtkButton *button, dt_lib_module_t *self)
{
    ...
    gtk_tree_store_insert_with_values(GTK_TREE_STORE(model), &iter, path ? &parent : NULL, -1,
                                      ...
                                      DT_MAP_LOCATION_COL_ID, -1, DT_MAP_LOCATION_COL_PATH,
                                      new_name, DT_MAP_LOCATION_COL_COUNT, 0, -1);
    g_free(new_name);
    g_free(name);
    g_free(path);
                                          ...
                                          dt_loc_op_t *to)
{
    ...
    if (g_str_has_prefix(tagname, oldtagname))
    {
        if (strlen(tagname) == strlen(oldtagname))
        {
            ...
            gtk_tree_store_set(GTK_TREE_STORE(model), iter, DT_MAP_LOCATION_COL_PATH, newtagname,
        ...
        else if (strlen(tagname) > strlen(oldtagname) && tagname[strlen(oldtagname)] == '|')
        {
            ...
            gtk_tree_store_set(GTK_TREE_STORE(model), iter, DT_MAP_LOCATION_COL_PATH, newpath, -1);
            g_free(newpath);
        }
    }
    g_free(tagname);
...
static void _view_map_geotag_changed(gpointer instance, GList *imgs, const int newlocid,
    ...
    else
    {
        ...
        if (gtk_tree_model_get_iter_first(model, &iter))
        {
            for (GList *loc = locs; loc; loc = g_list_next(loc))
            {
                ...
                {
                    gtk_tree_store_set(GTK_TREE_STORE(model), &iter2, DT_MAP_LOCATION_COL_COUNT,
                                       ((dt_map_location_t *)loc->data)->count, -1);
                }
            }
        }
...
static void _name_editing_done(GtkCellEditable *editable, dt_lib_module_t *self)
{
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter))
    {
        ...
        char *leave = NULL;
        guint locid;
        gtk_tree_model_get(model, &iter, DT_MAP_LOCATION_COL_ID, &locid, DT_MAP_LOCATION_COL_PATH,
                           &path, DT_MAP_LOCATION_COL_TAG, &leave, -1);
        if (reset && locid)
            ...
            if (!new_exists)
            {
                ...
                if (locid == -1)
                {
                    ...
                    if (locid != -1)
                    {
                        ...
                        {
                            gtk_tree_store_set(GTK_TREE_STORE(model), &iter, DT_MAP_LOCATION_COL_ID,
                                               locid, DT_MAP_LOCATION_COL_PATH, new_path,
                                               DT_MAP_LOCATION_COL_TAG, name,
                                               DT_MAP_LOCATION_COL_COUNT, count, -1);
                        }
                    }
                    else
                        canceled = TRUE;
                ...
                else
                {
                    ...
                    for (GList *tag = children; tag; tag = g_list_next(tag))
                    {
                        ...
                        const char *new_part =
                            &((dt_map_location_t *)tag->data)->tag[path_len + (reset ? 1 : 0)];
                    ...
                    else
                    {
                        ...
                        gtk_tree_sortable_get_sort_column_id(GTK_TREE_SORTABLE(model), &sort_column,
                                                             &sort_order);
                        gtk_tree_sortable_set_sort_column_id(
                            GTK_TREE_SORTABLE(model), GTK_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID,
                            GTK_SORT_ASCENDING);
                        gtk_tree_model_foreach(
                            model, (GtkTreeModelForeachFunc)_update_tag_name_per_name, &to);
                        gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(model), sort_column,
                                                             sort_order);
                    }
...
static void _name_start_editing(GtkCellRenderer *renderer, GtkCellEditable *editable, char *path,
                                dt_lib_module_t *self)
{
    dt_lib_map_locations_t *d = self->data;
...
static gint _sort_position_names_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                                      dt_lib_module_t *self)
{
    char *tag_a = NULL;
    char *tag_b = NULL;
...
static gboolean _set_location_collection(dt_lib_module_t *self)
{
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter))
    {
        ...
        char *collection =
            g_strdup_printf("1:0:%d:%s|%s$", DT_COLLECTION_PROP_GEOTAGGING, _("tagged"), name);
...
static gboolean _click_on_view(GtkWidget *view, GdkEventButton *event, dt_lib_module_t *self)
{
    ...
    {
        dt_control_log(
            _("terminate edit (press enter or escape) before selecting another location"));
        return TRUE;
    }
...
void gui_init(dt_lib_module_t *self)
{
    ...
    gtk_box_pack_start(GTK_BOX(self->widget),
                       dt_ui_resize_wrap(d->view, 100, "plugins/map/heightlocationwindow"), TRUE,
                       TRUE, 0);
    ...
    d->shape_button_handler = g_signal_connect_data(
        G_OBJECT(d->shape_button), "clicked", G_CALLBACK(_shape_button_clicked), self, NULL, 0);
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(d->shape_button),
    ...
    d->new_button =
        dt_action_button_new(self, N_("new location"), _new_button_clicked, self,
    ...
    gtk_label_set_ellipsize(GTK_LABEL(gtk_bin_get_child(GTK_BIN(d->show_all_button))),
                            PANGO_ELLIPSIZE_END);
    ...
    g_signal_connect(G_OBJECT(d->show_all_button), "clicked", G_CALLBACK(_show_all_button_clicked),
                     self);
