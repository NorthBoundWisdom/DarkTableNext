                    ...
                    size_t *new_size)
{
    if (old_version == 1)
    {
        ...
        const int table[DT_COLLECTION_PROP_LAST] = {
            DT_COLLECTION_PROP_FILMROLL, DT_COLLECTION_PROP_FOLDERS, DT_COLLECTION_PROP_CAMERA,
            DT_COLLECTION_PROP_TAG, DT_COLLECTION_PROP_DAY, DT_COLLECTION_PROP_TIME,
            DT_COLLECTION_PROP_HISTORY, DT_COLLECTION_PROP_COLORLABEL,

            // spaces for the metadata, see metadata.h
            DT_COLLECTION_PROP_COLORLABEL + 1, DT_COLLECTION_PROP_COLORLABEL + 2,
            DT_COLLECTION_PROP_COLORLABEL + 3, DT_COLLECTION_PROP_COLORLABEL + 4,
            DT_COLLECTION_PROP_COLORLABEL + 5,

            DT_COLLECTION_PROP_LENS, DT_COLLECTION_PROP_FOCAL_LENGTH, DT_COLLECTION_PROP_ISO,
            DT_COLLECTION_PROP_APERTURE, DT_COLLECTION_PROP_EXPOSURE,
            DT_COLLECTION_PROP_ASPECT_RATIO, DT_COLLECTION_PROP_FILENAME,
            DT_COLLECTION_PROP_GEOTAGGING, DT_COLLECTION_PROP_GROUPING,
            DT_COLLECTION_PROP_LOCAL_COPY, DT_COLLECTION_PROP_MODULE, DT_COLLECTION_PROP_ORDER};
    ...
    else if (old_version == 2)
    {
        ...
        const int table[DT_COLLECTION_PROP_LAST] = {
            DT_COLLECTION_PROP_FILMROLL, DT_COLLECTION_PROP_FOLDERS, DT_COLLECTION_PROP_FILENAME,
            DT_COLLECTION_PROP_CAMERA, DT_COLLECTION_PROP_LENS, DT_COLLECTION_PROP_APERTURE,
            DT_COLLECTION_PROP_EXPOSURE, DT_COLLECTION_PROP_FOCAL_LENGTH, DT_COLLECTION_PROP_ISO,
            DT_COLLECTION_PROP_DAY, DT_COLLECTION_PROP_TIME, DT_COLLECTION_PROP_GEOTAGGING,
            DT_COLLECTION_PROP_ASPECT_RATIO, DT_COLLECTION_PROP_TAG, DT_COLLECTION_PROP_COLORLABEL,
            ...
            DT_COLLECTION_PROP_GROUPING, DT_COLLECTION_PROP_LOCAL_COPY, DT_COLLECTION_PROP_HISTORY,
            DT_COLLECTION_PROP_MODULE, DT_COLLECTION_PROP_ORDER};
...
static void view_popup_menu_onSearchFilmroll(GtkWidget *menuitem, gpointer userdata)
{
    ...
    GtkFileChooserNative *filechooser = gtk_file_chooser_native_new(
        _("update path to files"), GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER,
        _("_open"), _("_cancel"));
                                     ...
                                     dt_lib_collect_t *d)
{
    ...
    const gboolean get_path = gtk_tree_view_get_path_at_pos(
    ...
    if (get_path && dt_modifier_is(event->state, GDK_SHIFT_MASK) &&
        ...
        (d->view_rule == DT_COLLECTION_PROP_DAY || d->view_rule == DT_COLLECTION_PROP_MONTH ||
         _is_time_property(d->view_rule) || d->view_rule == DT_COLLECTION_PROP_APERTURE ||
    ...
    if ((!d->singleclick && event->type == GDK_2BUTTON_PRESS &&
         event->button == GDK_BUTTON_PRIMARY) ||
        (d->singleclick && event->type == GDK_BUTTON_PRESS &&
         event->button == GDK_BUTTON_PRIMARY) ||
        (!d->singleclick && event->type == GDK_BUTTON_PRESS &&
         event->button == GDK_BUTTON_PRIMARY &&
         (dt_modifier_is(event->state, GDK_SHIFT_MASK) ||
          dt_modifier_is(event->state, GDK_CONTROL_MASK))) ||
        (d->view_rule == DT_COLLECTION_PROP_MONTH && event->type == GDK_BUTTON_PRESS &&
         event->button == GDK_BUTTON_PRIMARY))
    {
        row_activated_with_event(GTK_TREE_VIEW(treeview), path, NULL, event, d);
...
static void _scroll_to_position(GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *column,
...
static gboolean list_select(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                            gpointer data)
{
...
static gboolean range_select(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                             gpointer data)
{
...
static gboolean _datetime_range_select(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                       gpointer data)
{
    dt_lib_collect_rule_t *dr = (dt_lib_collect_rule_t *)data;
    gchar *str;
    ...
    g_free(str);
    GTimeSpan target =
        dr->datetime_range.path1 == NULL ? dr->datetime_range.nb1 : dr->datetime_range.nb2;
...
static gboolean tree_expand(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                            gpointer data)
{
    dt_lib_collect_rule_t *dr = (dt_lib_collect_rule_t *)data;
    gchar *str = NULL;
...
static gboolean list_match_string(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                  gpointer data)
{
    dt_lib_collect_rule_t *dr = (dt_lib_collect_rule_t *)data;
    gchar *str = NULL;
    ...
    gtk_tree_model_get(model, iter, DT_LIB_COLLECT_COL_PATH, &str, DT_LIB_COLLECT_COL_VISIBLE,
                       &was_visible, -1);
    ...
    else if (property == DT_COLLECTION_PROP_FILENAME && strchr(needle, ',') != NULL)
    {
        ...
        for (const GList *l = list; l; l = g_list_next(l))
        {
            ...
            if ((visible = (g_strrstr(haystack, name + (name[0] == '%')) != NULL)))
                break;
        }
...
static gboolean tree_match_string(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                  gpointer data)
{
    dt_lib_collect_rule_t *dr = (dt_lib_collect_rule_t *)data;
    gchar *str = NULL;
...
static gboolean tree_reveal_func(GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter,
                                 gpointer data)
{
    gboolean state;
...
static void _lib_folders_update_collection(const gchar *filmroll)
{
    ...
    if (cquery && cquery[0] != '\0')
    {
        gchar *complete_query =
            g_strdup_printf("DELETE FROM main.selected_images WHERE imgid NOT IN (%s)", cquery);
    ...
    if (!darktable.collection->clone)
    {
        ...
        DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_COLLECTION_CHANGED, DT_COLLECTION_CHANGE_NEW_QUERY,
        ...
        while (gtk_tree_model_iter_n_children(model, level > 0 ? &iter : NULL) > 0)
        {
            if (level > 0)
            {
                ...
                if (id != -1)
                {
                    ...
                    if (!gtk_tree_model_iter_parent(model, &child, &iter))
                        level = 0;
                    iter = child;
                    break;
                }
...
void tree_count_show(GtkTreeViewColumn *col, GtkCellRenderer *renderer, GtkTreeModel *model,
                     GtkTreeIter *iter, gpointer data)
{
    gchar *name;
    guint count;

    gtk_tree_model_get(model, iter, DT_LIB_COLLECT_COL_TEXT, &name, DT_LIB_COLLECT_COL_COUNT,
                       &count, -1);
    if (!count)
    {
...
{
    GtkTreePath *p1 = gtk_tree_model_filter_convert_child_path_to_path(
        GTK_TREE_MODEL_FILTER(d->treefilter), path1);
    if (!p1)
        return;

    GtkTreePath *p2 = path2 ? gtk_tree_model_filter_convert_child_path_to_path(
                                  GTK_TREE_MODEL_FILTER(d->treefilter), path2) :
                              NULL;
    GtkTreePath *p3 = NULL;
    GtkTreeIter iter;
...
static void _tree_view(dt_lib_collect_rule_t *dr)
{
    ...
    if (d->view_rule != property)
    {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            else if (property == DT_COLLECTION_PROP_MONTH)
            {
                ...
                if (month_num >= 1 && month_num <= 12)
                    name =
                        g_strdup_printf("%02d - %s", month_num, _(dt_month_names[month_num - 1]));
                else
            ...
            tuple->count = count;
            tuple->status =
                property == DT_COLLECTION_PROP_FOLDERS ? sqlite3_column_int(stmt, 3) : -1;
        ...
        for (GList *names = sorted_names; names; names = g_list_next(names))
        {
            ...
            {
                char *next_name =
                    g_strdup(names->next ? ((name_key_tuple_t *)names->next->data)->name : "");
                ...
                if (g_strcmp0(next_name, name) && g_strcmp0(name, _("not tagged")))
                {
                    ...
                    if (!uncategorized.stamp)
                    {
                        gtk_tree_store_insert_with_values(
                            ...
                            DT_LIB_COLLECT_COL_TEXT, _(UNCATEGORIZED_TAG), DT_LIB_COLLECT_COL_PATH,
                    ...
                    gtk_tree_store_insert_with_values(
                        GTK_TREE_STORE(model), &temp, &uncategorized, 0, DT_LIB_COLLECT_COL_TEXT,
                        name, DT_LIB_COLLECT_COL_PATH, name, DT_LIB_COLLECT_COL_VISIBLE, TRUE,
            ...
            if (!uncategorized_found)
            {
                ...
                if (tokens != NULL)
                {
                    ...
                    for (char **token = &tokens[common_length]; *token; token++)
                    {
                        ...
                        if (_is_time_property(property) && !*(token + 1))
                            pth[10] = ' ';
                        ...
                        gtk_tree_store_insert_with_values(
                            ...
                            (*(token + 1) ? 0 : count), DT_LIB_COLLECT_COL_INDEX, index,
                            ...
                            !*(token + 1))
                        {
                            ...
                            {
                                gtk_tree_model_get(model, &parent2, DT_LIB_COLLECT_COL_COUNT,
                                                   &parentcount, -1);
                                gtk_tree_store_set(GTK_TREE_STORE(model), &parent2,
                                                   DT_LIB_COLLECT_COL_COUNT, count + parentcount,
                                                   -1);
                                child = parent2;
                            }
                        }
    ...
    if (_is_time_property(property) || property == DT_COLLECTION_PROP_DAY)
    {
        ...
        dt_collection_split_operator_datetime(gtk_entry_get_text(GTK_ENTRY(dr->text)), &number1,
                                              &number2, &dr->datetime_range.operator);
...
static void _list_view(dt_lib_collect_rule_t *dr)
{
    ...
    if (d->view_rule != property)
    {
        ...
        case DT_COLLECTION_PROP_ORDER: // modules order
        {
            ...
            {
                dt_util_str_cat(&orders, "WHEN mo.version = %d THEN '%s' ", i,
        ...
        default:
          ...
          snprintf(query, sizeof(query),
            ...
            // filmroll
            {
                ...
                {
                    order_by =
                        g_strdup(sort_descending ? "film_rolls_id DESC" : "film_rolls_id ASC");
                }
                else
                {
                    order_by =
                        g_strdup(sort_descending ? "lower(folder) DESC" : "lower(folder) ASC");
                }
        ...
        if (strlen(query) > 0)
        {
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW)
            {
                ...
                gtk_list_store_insert_with_values(
                    GTK_LIST_STORE(model), NULL, -1, DT_LIB_COLLECT_COL_TEXT, folder,
                    ...
                    DT_LIB_COLLECT_COL_COUNT, count, DT_LIB_COLLECT_COL_UNREACHABLE, status, -1);
            ...
            if (property == DT_COLLECTION_PROP_FILMROLL)
            {
                ...
                for (GList *l = rows; l; l = l->next)
                {
                    ...
                    gtk_list_store_insert_with_values(
                        GTK_LIST_STORE(model), NULL, -1, DT_LIB_COLLECT_COL_TEXT, r->folder,
                        DT_LIB_COLLECT_COL_ID, r->id, DT_LIB_COLLECT_COL_TOOLTIP, escaped_text,
                        DT_LIB_COLLECT_COL_PATH, r->value, DT_LIB_COLLECT_COL_VISIBLE, TRUE,
                        DT_LIB_COLLECT_COL_COUNT, r->count, DT_LIB_COLLECT_COL_UNREACHABLE,
                        r->status, -1);
        ...
        property == DT_COLLECTION_PROP_ASPECT_RATIO || property == DT_COLLECTION_PROP_RATING)
    {
        ...
        g_regex_match_full(regex, gtk_entry_get_text(GTK_ENTRY(dr->text)), -1, 0, 0, &match_info,
                           NULL);
        ...
        if (match_count == 3)
        {
            ...
            {
                gtk_tree_selection_select_range(gtk_tree_view_get_selection(d->view), range->path1,
                                                range->path2);
            }
            g_free(range->start);
            g_free(range->stop);
                ...
                struct dt_view_t *new_view)
{
    ...
    const gboolean hide =
        dt_lib_is_visible(darktable.view_manager->proxy.module_recentcollect.module);
...
static void _lib_collect_gui_update(dt_lib_module_t *self)
{
    ...
    for (int i = 0; i <= active; i++)
    {
        ...
        if (text)
        {
            g_signal_handlers_block_matched(d->rule[i].text, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                            entry_changed, NULL);
            gtk_entry_set_text(GTK_ENTRY(d->rule[i].text), text);
            gtk_editable_set_position(GTK_EDITABLE(d->rule[i].text), -1);
            g_signal_handlers_unblock_matched(d->rule[i].text, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                              entry_changed, NULL);
...
static void combo_changed(GtkWidget *combo, dt_lib_collect_rule_t *d)
{
    ...
    g_signal_handlers_unblock_matched(d->text, G_SIGNAL_MATCH_FUNC, 0, 0, NULL, entry_changed,
                                      NULL);
                                     ...
                                     GdkEventButton *event, dt_lib_collect_t *d)
{
            ...
            else if (item == DT_COLLECTION_PROP_FOLDERS)
            {
                ...
                force_update_view = TRUE;
                 ...
                 (item == DT_COLLECTION_PROP_DAY || _is_time_property(item) ||
                  item == DT_COLLECTION_PROP_APERTURE || item == DT_COLLECTION_PROP_FOCAL_LENGTH ||
                  item == DT_COLLECTION_PROP_ISO || item == DT_COLLECTION_PROP_EXPOSURE ||
                  item == DT_COLLECTION_PROP_ASPECT_RATIO || item == DT_COLLECTION_PROP_RATING))
        {
    ...
    g_signal_handlers_block_matched(d->rule[active].text, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                    entry_changed, NULL);
    gtk_entry_set_text(GTK_ENTRY(d->rule[active].text), text);
    gtk_editable_set_position(GTK_EDITABLE(d->rule[active].text), -1);
    g_signal_handlers_unblock_matched(d->rule[active].text, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                      entry_changed, NULL);
    g_free(text);
...
static gboolean entry_focus_in_callback(GtkWidget *w, GdkEventFocus *event,
...
static void collection_updated(gpointer instance, const dt_collection_change_t query_change,
                               const dt_collection_properties_t changed_property, gpointer imgs,
                               const int next, gpointer self)
{
...
static void filmrolls_imported(gpointer instance, const int film_id, gpointer self)
{
    dt_lib_module_t *dm = (dt_lib_module_t *)self;
    dt_lib_collect_t *d = dm->data;
...
static void _metadata_changed(gpointer instance, int type, gpointer self)
{
    ...
    if (type == DT_METADATA_SIGNAL_PREF_CHANGED)
    {
        ...
        for (int i = 0; i < MAX_RULES; i++)
        {
            g_signal_handlers_block_matched(d->rule[i].combo, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                            combo_changed, NULL);
            ...
            g_signal_handlers_unblock_matched(d->rule[i].combo, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                              combo_changed, NULL);
        }
    }
    ...
    if (type == DT_METADATA_SIGNAL_PREF_CHANGED || prop >= DT_COLLECTION_PROP_METADATA_OFFSET)
    {
        dt_collection_update_query(darktable.collection, DT_COLLECTION_CHANGE_RELOAD,
                                   DT_COLLECTION_PROP_METADATA, NULL);
    }
}
...
static void menuitem_clear(GtkMenuItem *menuitem, dt_lib_collect_rule_t *d)
{
    ...
    {
        dt_conf_set_int("plugins/lighttable/collect/num_rules", active - 1);
        if (c->active_rule >= active - 1)
            c->active_rule = active - 2;
    }
    else
    {
...
static gboolean popup_button_callback(GtkWidget *widget, GdkEventButton *event,
                                      dt_lib_collect_rule_t *d)
{
    if (event->button != 1)
        return FALSE;
...
static void view_set_click(gpointer instance, dt_lib_module_t *self)
{
    dt_lib_collect_t *d = self->data;
    ...
    for (GList *iter = dt_metadata_get_list(); iter; iter = iter->next)
    {
        ...
        if (!metadata->internal && metadata->visible)
            ...
            dt_bauhaus_combobox_add_full(
                w, metadata->name, DT_BAUHAUS_COMBOBOX_ALIGN_RIGHT,
                GUINT_TO_POINTER(DT_COLLECTION_PROP_METADATA_OFFSET + metadata->key + 1), NULL,
                TRUE);
    }
...
void _menuitem_preferences(GtkMenuItem *menuitem, dt_lib_module_t *self)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("collections settings"), GTK_WINDOW(win), GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"),
...
static gint _sort_model_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
...
static void _history_apply(GtkWidget *widget, dt_lib_module_t *self)
{
    ...
    if (hid < 0 || hid >= dt_conf_get_int("plugins/lighttable/collect/history_max"))
        return;
...
static void _history_show(GtkWidget *widget, dt_lib_module_t *self)
{
...
GtkWidget *gui_tool_box(dt_lib_module_t *self)
{
    ...
    GtkWidget *sortb = dtgtk_togglebutton_new(
        dtgtk_cairo_paint_sortby, sort_descend ? CPF_DIRECTION_DOWN : CPF_DIRECTION_UP, NULL);
    ...
    g_signal_connect(G_OBJECT(sortb), "toggled", G_CALLBACK(_sort_reverse_changed), self);
    return sortb;
}
...
void gui_init(dt_lib_module_t *self)
{
    ...
    for (int i = 0; i < MAX_RULES; i++)
    {
        ...
        g_signal_connect(G_OBJECT(w), "value-changed", G_CALLBACK(combo_changed), d->rule + i);

        d->rule[i].text = w = dt_ui_entry_new(10);
        gtk_widget_add_events(w, GDK_FOCUS_CHANGE_MASK | GDK_KEY_PRESS_MASK);
        g_signal_connect(G_OBJECT(w), "focus-in-event", G_CALLBACK(entry_focus_in_callback),
                         d->rule + i);
        ...
        g_signal_connect(G_OBJECT(w), "button-press-event", G_CALLBACK(popup_button_callback),
                         d->rule + i);

        d->rule[i].hbox =
            dt_gui_hbox(d->rule[i].combo, dt_gui_expand(d->rule[i].text), d->rule[i].button);
    ...
    GtkTreeModel *listmodel = GTK_TREE_MODEL(gtk_list_store_new(
        DT_LIB_COLLECT_NUM_COLS, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_STRING,
        G_TYPE_BOOLEAN, G_TYPE_BOOLEAN, G_TYPE_UINT, G_TYPE_UINT));
    ...
    gtk_tree_model_filter_set_visible_column(GTK_TREE_MODEL_FILTER(d->listfilter),
                                             DT_LIB_COLLECT_COL_VISIBLE);

    GtkTreeModel *treemodel = GTK_TREE_MODEL(gtk_tree_store_new(
        DT_LIB_COLLECT_NUM_COLS, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_STRING,
        G_TYPE_BOOLEAN, G_TYPE_BOOLEAN, G_TYPE_UINT, G_TYPE_UINT));
    ...
    GtkWidget *btn =
        dt_action_button_new(self, N_("history"), G_CALLBACK(_history_show), self,
    ...
    dt_gui_box_add(
        self->widget,
        dt_ui_resize_wrap(GTK_WIDGET(view), 200, "plugins/lighttable/collect/windowheight"),
        d->history_box);
...
void init(struct dt_lib_module_t *self)
{
    ...
    for (GList *iter = dt_metadata_get_list(); iter; iter = iter->next)
    {
        dt_metadata_t *metadata = iter->data;
        if (!metadata->internal && metadata->visible)
            luaA_enum_value(L, dt_collection_properties_t,
                            DT_COLLECTION_PROP_METADATA_OFFSET + metadata->key);
    }
    dt_pthread_mutex_unlock(&darktable.metadata_threadsafe);
