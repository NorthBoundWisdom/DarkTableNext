                                                       ...
                                                       const gchar *propertyname)
{
    ...
    {
        const char *value;
        if ((value = dt_camctl_camera_property_get_first_choice(darktable.camctl, NULL,
                                                                propertyname)) != NULL)
        {
            // We got a value for property, let's construct the GUI for the property and add values
            int i = 0;
            const char *current_value =
                dt_camctl_camera_get_property(darktable.camctl, NULL, propertyname);
            ...
            gtk_widget_set_tooltip_text(GTK_WIDGET(prop->osd),
                                        _("toggle view property in center view"));
            ...
            do
            {
                ...
                i++;
            } while ((value = dt_camctl_camera_property_get_next_choice(darktable.camctl, NULL,
                                                                        propertyname)) != NULL);

            lib->gui.properties = g_list_append(lib->gui.properties, prop);

            // Does dead lock!!!
            g_signal_connect(G_OBJECT(prop->values), "value-changed",
                             G_CALLBACK(property_changed_callback), (gpointer)prop);
            return prop;
        }
    }
    return NULL;
...
static void _camera_property_value_changed(const dt_camera_t *camera, const char *name,
...
static void _camera_error_callback(const dt_camera_t *camera, dt_camera_error_t error,
                                   void *user_data)
{
...
static void _capture_button_clicked(GtkWidget *widget, gpointer user_data)
{
    ...
    const uint32_t delay =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lib->gui.toggle_timer)) == TRUE ?
            (uint32_t)gtk_spin_button_get_value(GTK_SPIN_BUTTON(lib->gui.timer)) :
            0;
    const uint32_t count =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lib->gui.toggle_sequence)) == TRUE ?
            (uint32_t)gtk_spin_button_get_value(GTK_SPIN_BUTTON(lib->gui.count)) :
            1;
    const uint32_t brackets =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lib->gui.toggle_bracket)) == TRUE ?
...
static void _show_property_popupmenu_clicked(GtkWidget *widget, gpointer user_data)
{
    ...
    dt_gui_menu_popup(lib->gui.properties_menu, widget, GDK_GRAVITY_SOUTH_EAST,
...
static void _toggle_capture_mode_clicked(GtkWidget *widget, gpointer user_data)
{
    ...
    {
        gtk_widget_set_sensitive(lib->gui.brackets,
                                 gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)));
        gtk_widget_set_sensitive(lib->gui.steps,
                                 gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)));
    }
                             ...
                             int32_t pointerx, int32_t pointery)
{
    ...
    cairo_move_to(cr, DT_PIXEL_APPLY_DPI(5),
                  DT_PIXEL_APPLY_DPI(1) + BAR_HEIGHT - ink.height / 2 - fontsize);
    pango_cairo_show_layout(cr, layout);

    // Draw right aligned battery value
    const char *battery_value =
        dt_camctl_camera_get_property(darktable.camctl, NULL, "batterylevel");
    char battery[4096] = {0};
    snprintf(battery, sizeof(battery), "%s: %s", _("battery"),
             battery_value ? battery_value : _("n/a"));
    pango_layout_set_text(layout, battery, -1);
    pango_layout_get_pixel_extents(layout, &ink, NULL);
    cairo_move_to(cr, width - ink.width - DT_PIXEL_APPLY_DPI(5),
                  DT_PIXEL_APPLY_DPI(1) + BAR_HEIGHT - ink.height / 2 - fontsize);
    pango_cairo_show_layout(cr, layout);
    ...
    pango_layout_get_pixel_extents(layout, &ink, NULL);
    cairo_move_to(cr, (width / 2) - (ink.width / 2),
                  DT_PIXEL_APPLY_DPI(1) + BAR_HEIGHT - ink.height / 2 - fontsize);
    pango_cairo_show_layout(cr, layout);
...
void gui_post_expose(dt_lib_module_t *self, cairo_t *cr, int32_t width, int32_t height,
                     int32_t pointerx, int32_t pointery)
{
    // Setup cairo font..
...
void gui_init(dt_lib_module_t *self)
{
    ...
    lib->data.listener->camera_property_accessibility_changed =
        _camera_property_accessibility_changed;
    ...
    lib->gui.toggle_timer =
        DTGTK_TOGGLEBUTTON(dtgtk_togglebutton_new(dtgtk_cairo_paint_timer, 0, NULL));
    lib->gui.toggle_sequence =
        DTGTK_TOGGLEBUTTON(dtgtk_togglebutton_new(dtgtk_cairo_paint_filmstrip, 0, NULL));
    lib->gui.toggle_bracket =
        DTGTK_TOGGLEBUTTON(dtgtk_togglebutton_new(dtgtk_cairo_paint_bracket, 0, NULL));
    ...
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(hbox), GTK_WIDGET(modes_label),
                            GTK_POS_RIGHT, 1, 1);
    ...
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(lib->gui.timer),
                            GTK_WIDGET(timer_label), GTK_POS_RIGHT, 1, 1);
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(lib->gui.count),
                            GTK_WIDGET(count_label), GTK_POS_RIGHT, 1, 1);
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(lib->gui.brackets),
                            GTK_WIDGET(brackets_label), GTK_POS_RIGHT, 1, 1);
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(lib->gui.steps),
                            GTK_WIDGET(steps_label), GTK_POS_RIGHT, 1, 1);

    lib->gui.button1 = dt_action_button_new(self, N_("capture image(s)"), _capture_button_clicked,
                                            lib, NULL, 0, 0);
    gtk_grid_attach(GTK_GRID(self->widget), GTK_WIDGET(lib->gui.button1), 0, lib->gui.rows++, 2, 1);

    gtk_widget_set_tooltip_text(GTK_WIDGET(lib->gui.toggle_timer),
                                _("toggle delayed capture mode"));
    gtk_widget_set_tooltip_text(GTK_WIDGET(lib->gui.toggle_sequence),
                                _("toggle sequenced capture mode"));
    gtk_widget_set_tooltip_text(GTK_WIDGET(lib->gui.toggle_bracket),
                                _("toggle bracketed capture mode"));
    gtk_widget_set_tooltip_text(GTK_WIDGET(lib->gui.timer),
                                _("the count of seconds before actually doing a capture"));
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(lib->gui.count),
    ...
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(lib->gui.brackets),
        _("the amount of brackets on each side of centered shoot, amount of images = (brackets*2) + 1"));
    gtk_widget_set_tooltip_text(
        GTK_WIDGET(lib->gui.steps),
        _("the amount of steps per bracket, steps is camera configurable and usually 3 steps per "
          "stop\nwith other words, 3 steps is 1EV exposure step between brackets"));

    g_signal_connect(G_OBJECT(lib->gui.toggle_timer), "clicked",
                     G_CALLBACK(_toggle_capture_mode_clicked), lib);
    g_signal_connect(G_OBJECT(lib->gui.toggle_sequence), "clicked",
                     G_CALLBACK(_toggle_capture_mode_clicked), lib);
    g_signal_connect(G_OBJECT(lib->gui.toggle_bracket), "clicked",
                     G_CALLBACK(_toggle_capture_mode_clicked), lib);
    ...
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(lib->gui.plabel), GTK_WIDGET(label),
                            GTK_POS_RIGHT, 1, 1);
    ...
    g_signal_connect(G_OBJECT(widget), "clicked", G_CALLBACK(_show_property_popupmenu_clicked),
                     lib);
    ...
    gtk_grid_attach(GTK_GRID(self->widget), GTK_WIDGET(label), 0, lib->gui.rows++, 1, 1);
    gtk_grid_attach_next_to(GTK_GRID(self->widget), GTK_WIDGET(hbox), GTK_WIDGET(label),
                            GTK_POS_RIGHT, 1, 1);
                ...
                struct dt_view_t *new_view)
{
    ...
    if ((prop = _lib_property_add_new(lib, _("aperture"), "f-number")) != NULL)
        _lib_property_add_to_gui(prop, lib);
    else if ((prop = _lib_property_add_new(lib, _("aperture"), "aperture")) !=
             NULL) // for Canon cameras
        _lib_property_add_to_gui(prop, lib);
    ...
    if ((prop = _lib_property_add_new(lib, _("shutterspeed2"), "shutterspeed2")) != NULL)
        _lib_property_add_to_gui(prop, lib);
    else if ((prop = _lib_property_add_new(lib, _("shutterspeed"), "shutterspeed")) !=
             NULL) // Canon, again
        _lib_property_add_to_gui(prop, lib);
