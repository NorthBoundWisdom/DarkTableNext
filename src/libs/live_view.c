...
static const cairo_operator_t _overlay_modes[] = {CAIRO_OPERATOR_OVER,
                                                  CAIRO_OPERATOR_XOR,
                                                  CAIRO_OPERATOR_ADD,
...
static void _focus_button_clicked(GtkWidget *widget, gpointer user_data)
{
    ...
    else
    {
        ...
        case GP_WIDGET_RANGE:
        {
            ...
            dt_camctl_camera_set_property_float(darktable.camctl, NULL, "manualfocusdrive",
                                                focus_amount);
            break;
        }
...
#define NEW_BUTTON(type, paint, direction, callback, data, action)                                 \
    ...
    dt_action_define(DT_ACTION(self), NULL, action, button,                                        \
                     *(#type) ? &dt_action_def_toggle : &dt_action_def_button);

    lib->live_view = NEW_BUTTON(toggle, dtgtk_cairo_paint_eye, 0, _toggle_live_view_clicked, lib,
                                N_("toggle live view"));
    dt_shortcut_register(dt_action_section(DT_ACTION(self), N_("toggle live view")), 0, 0,
                         GDK_KEY_v, 0);
    lib->live_view_zoom = NEW_BUTTON(, dtgtk_cairo_paint_zoom, 0, _zoom_live_view_clicked, lib,
                                     N_("zoom live view")); // TODO: see _zoom_live_view_clicked
    dt_shortcut_register(dt_action_section(DT_ACTION(self), N_("zoom live view")), 0, 0, GDK_KEY_w,
                         0);
    lib->rotate_ccw =
        NEW_BUTTON(, dtgtk_cairo_paint_refresh, 0, _rotate_ccw, lib, N_("rotate 90 degrees CCW"));
    lib->rotate_cw = NEW_BUTTON(, dtgtk_cairo_paint_refresh, CPF_DIRECTION_UP, _rotate_cw, lib,
                                N_("rotate 90 degrees CW"));
    lib->flip = NEW_BUTTON(toggle, dtgtk_cairo_paint_flip, CPF_DIRECTION_UP, _toggle_flip_clicked,
                           lib, N_("flip live view horizontally"));
    ...
    lib->focus_in_big =
        NEW_BUTTON(, dtgtk_cairo_paint_solid_triangle, CPF_DIRECTION_LEFT, _focus_button_clicked,
                   GINT_TO_POINTER(DT_FOCUS_NEARER), N_("move focus point in (big steps)"));
    lib->focus_in_small =
        NEW_BUTTON(, dtgtk_cairo_paint_arrow, CPF_DIRECTION_LEFT, _focus_button_clicked,
                   GINT_TO_POINTER(DT_FOCUS_NEAR),
                   N_("move focus point in (small steps)")); // TODO icon not centered
    lib->auto_focus = NEW_BUTTON(, dtgtk_cairo_paint_lock, 0, _auto_focus_button_clicked,
                                 GINT_TO_POINTER(1), N_("run autofocus"));
    lib->focus_out_small = NEW_BUTTON(, dtgtk_cairo_paint_arrow, CPF_DIRECTION_RIGHT,
                                      _focus_button_clicked, GINT_TO_POINTER(DT_FOCUS_FAR),
                                      N_("move focus point out (small steps)")); // TODO same here
    lib->focus_out_big =
        NEW_BUTTON(, dtgtk_cairo_paint_solid_triangle, CPF_DIRECTION_RIGHT, _focus_button_clicked,
                   GINT_TO_POINTER(DT_FOCUS_FARTHER), N_("move focus point out (big steps)"));
#undef NEW_BUTTON
    ...
    g_signal_connect(G_OBJECT(lib->overlay_id), "value-changed", G_CALLBACK(_overlay_id_changed),
                     lib);
    ...
    g_signal_connect(G_OBJECT(lib->overlay_splitline), "value-changed",
                     G_CALLBACK(_overlay_splitline_changed), lib);
            ...
            if (use_splitline)
            {
                ...
                default:
                    dt_print(DT_DEBUG_ALWAYS,
                             "OMFG, the world will collapse, this shouldn't be reachable!");
                    dt_pthread_mutex_unlock(&cam->live_view_buffer_mutex);
                    return;
                }
                   ...
                   const int which, const int type, const uint32_t state)
{
    ...
    if (dt_is_valid_imgid(imgid) && dt_bauhaus_combobox_get(lib->overlay_splitline))
    {
        ...
        const gboolean mouse_over_control =
            (lib->splitline_rotation % 2 == 0) ? (fabs(sl_x - x) < 5) : (fabs(sl_y - y) < 5);
