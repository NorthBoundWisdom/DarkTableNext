...
static gboolean _lib_duplicate_caption_out_callback(GtkWidget *widget, GdkEvent *event,
                                                    dt_lib_module_t *self)
{
    ...
    dt_metadata_set(imgid, "Xmp.darktable.version_name", gtk_entry_get_text(GTK_ENTRY(widget)),
                    FALSE);
...
static void _lib_duplicate_new_clicked_callback(GtkWidget *widget, GdkEventButton *event,
                                                dt_lib_module_t *self)
{
...
static void _lib_duplicate_duplicate_clicked_callback(GtkWidget *widget, GdkEventButton *event,
                                                      dt_lib_module_t *self)
{
    const dt_imgid_t imgid = darktable.develop->image_storage.id;
    const dt_imgid_t newid = dt_image_duplicate(imgid);
    ...
    if (imgid == darktable.develop->image_storage.id)
    {
        ...
        for (GList *l = d->thumbs; l; l = g_list_next(l))
        {
            ...
            if (thumb->imgid == imgid)
            {
                ...
                if (!l2)
                    l2 = g_list_previous(l);
                if (l2)
                {
                    dt_thumbnail_t *th2 = (dt_thumbnail_t *)l2->data;
                    DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_VIEWMANAGER_THUMBTABLE_ACTIVATE, th2->imgid);
                    break;
                }
            }
        }
...
static gboolean _lib_duplicate_thumb_press_callback(GtkWidget *widget, GdkEventButton *event,
                                                    dt_lib_module_t *self)
{
    dt_lib_duplicate_t *d = self->data;
...
static gboolean _lib_duplicate_thumb_release_callback(GtkWidget *widget, GdkEventButton *event,
                                                      dt_lib_module_t *self)
{
    dt_lib_duplicate_t *d = self->data;
                     ...
                     const int32_t pointerx, const int32_t pointery)
{
    ...
    if (!view_ok || d->preview_id != d->imgid)
    {
        ...
        dt_dev_image(d->imgid, width, height, -1, &d->buf, &d->scale, &d->buf_width, &d->buf_height,
                     d->zoom_pos, -1, NULL, DT_DEVICE_NONE, FALSE);

        d->preview_id = d->imgid;
    }

    if (d->buf)
        dt_view_paint_surface(cri, width, height, &darktable.develop->full, DT_WINDOW_MAIN, d->buf,
                              d->scale, d->buf_width, d->buf_height, d->zoom_pos);
}
...
static void _lib_duplicate_init_callback(gpointer instance, dt_lib_module_t *self)
{
    ...
    dt_control_signal_block_by_func(darktable.signals, G_CALLBACK(_lib_duplicate_init_callback),
                                    self);
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        dt_thumbnail_t *thumb = dt_thumbnail_new(
            ...
            DT_THUMBNAIL_CONTAINER_DUPLICATE, TRUE, DT_THUMBNAIL_SELECTION_UNSELECTED);
    ...
    //unblock signals
    dt_control_signal_unblock_by_func(darktable.signals, G_CALLBACK(_lib_duplicate_init_callback),
                                      self);
}
...
static void _lib_duplicate_mipmap_updated_callback(gpointer instance, const dt_imgid_t imgid,
                                                   dt_lib_module_t *self)
{
    dt_lib_duplicate_t *d = self->data;
...
void gui_init(dt_lib_module_t *self)
{
    ...
    self->widget = dt_gui_vbox(
        dt_ui_resize_wrap(d->duplicate_box, 1, "plugins/darkroom/duplicate/windowheight"),
        dt_gui_hbox(
            dt_action_button_new(self, N_("duplicate"), _lib_duplicate_duplicate_clicked_callback,
                                 self, _("create a duplicate of the image with same history stack"),
                                 0, 0),
            dt_action_button_new(
                self, N_("original"), _lib_duplicate_new_clicked_callback, self,
