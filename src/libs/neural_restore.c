...
#define CONF_PREVIEW_TOOLTIP_ZOOM "plugins/lighttable/neural_restore/preview_tooltip_zoom"
...
static int _preview_capture_write_image(dt_imageio_module_data_t *data, const char *filename,
                                        const void *in_void,
                                        dt_colorspaces_color_profile_type_t over_type,
                                        const char *over_filename, void *exif, int exif_len,
                                        dt_imgid_t imgid, const int num, const int total,
...
static int _extract_source_jpeg_preview(const char *src_path, uint8_t **out_jpeg, int *out_jpeg_len,
                                        int *out_w, int *out_h)
{
    *out_jpeg = NULL;
    *out_jpeg_len = 0;
    ...
    *out_h = 0;
    ...
    if (dt_imageio_jpeg_decompress_header(raw_buf, raw_size, &jpg) != 0 || jpg.width <= 0 ||
        jpg.height <= 0)
    {
        free(raw_buf);
        return 1;
...
static void _float_rgb_to_cairo(const float *const restrict src, unsigned char *const restrict dst,
...
static void _nn_upscale(const float *const restrict src, const int src_w, const int src_h,
                        float *const restrict dst, const int dst_w, const int dst_h)
{
...
static int _write_tiff_scanline(TIFF *tif, const float *src, const int width, const int bpp,
                                const int row, void *scratch)
{
    if (bpp == 32)
...
static dt_restore_context_t *_load_for_task(dt_restore_env_t *env, dt_neural_task_t task)
{
    switch (task)
    {
...
static gboolean _task_model_available(dt_restore_env_t *env, const dt_neural_task_t task)
{
    switch (task)
    {
    case NEURAL_TASK_DENOISE:
                                            ...
                                            const gboolean preserve_wide_gamut)
{
    const dt_colorspaces_color_profile_t *lin_cp =
        dt_colorspaces_get_profile(DT_COLORSPACE_LIN_REC2020, "", DT_PROFILE_DIRECTION_OUT);
...
static cmsHTRANSFORM _build_output_color_transform(dt_colorspaces_color_profile_type_t dst_type,
                                                   const char *dst_filename)
{
    const dt_colorspaces_color_profile_t *lin_cp =
        dt_colorspaces_get_profile(DT_COLORSPACE_LIN_REC2020, "", DT_PROFILE_DIRECTION_OUT);
    const dt_colorspaces_color_profile_t *dst_cp =
        dt_colorspaces_get_profile(dst_type, dst_filename, DT_PROFILE_DIRECTION_OUT);
    if (!lin_cp || !dst_cp || lin_cp->profile == dst_cp->profile)
        return NULL;
    return cmsCreateTransform(lin_cp->profile, TYPE_RGB_FLT, dst_cp->profile, TYPE_RGB_FLT,
                              INTENT_PERCEPTUAL, 0);
}
...
static int _buf_row_writer(const float *scanline, const int out_w, const int y, void *user_data)
{
...
static int _tiff_row_writer(const float *scanline, const int out_w, const int y, void *user_data)
{
    ...
    return (_write_tiff_scanline(wd->tif, src, out_w, wd->bpp, y, wd->scratch) < 0) ? 1 : 0;
}

static int _ai_write_image(dt_imageio_module_data_t *data, const char *filename,
                           const void *in_void, dt_colorspaces_color_profile_type_t over_type,
                           ...
                           dt_dev_pixelpipe_t *pipe, const gboolean export_masks)
{
    ...
    const dt_colorspaces_color_profile_t *work_cp = dt_colorspaces_get_work_profile(imgid);
    const dt_colorspaces_color_profile_type_t dst_type =
        (job->icc_type == DT_COLORSPACE_NONE) ?
            ...
            job->icc_type;
    const char *dst_filename = (job->icc_type == DT_COLORSPACE_NONE) ?
    ...
    dt_print(DT_DEBUG_AI, "[neural_restore] processing %dx%d -> %dx%d (scale=%d)", width, height,
             out_w, out_h, S);
    ...
    const int bpp = (job->bpp == NEURAL_BPP_8) ? 8 : (job->bpp == NEURAL_BPP_16) ? 16 : 32;
    const int sample_fmt = (bpp == 32) ? SAMPLEFORMAT_IEEEFP : SAMPLEFORMAT_UINT;
    ...
    if (need_buffer)
    {
        ...
        _buf_writer_data_t bwd = {.out_buf = out_4ch, .out_w = out_w};
        res = dt_restore_process_tiled(job->ctx, in_data, width, height, S, _buf_row_writer, &bwd,
                                       job->control_job);
        ...
        if (res == 0)
        {
            ...
            void *cvt = (bpp < 32) ? g_malloc((size_t)out_w * 3 * sizeof(uint16_t)) : NULL;
            for (int y = 0; y < out_h && res == 0; y++)
            {
                ...
                if (xform)
                    cmsDoTransform(xform, scan, scan, out_w);
    ...
    {
        void *scratch = (bpp < 32) ? g_try_malloc((size_t)out_w * 3 * sizeof(uint16_t)) : NULL;
        float *xform_scratch = xform ? g_try_malloc((size_t)out_w * 3 * sizeof(float)) : NULL;
        ...
        res = dt_restore_process_tiled(job->ctx, in_data, width, height, S, _tiff_row_writer, &twd,
                                       job->control_job);
        g_free(scratch);
...
static void _import_image(const char *filename, const dt_imgid_t source_imgid)
{
    dt_film_t film;
    dt_film_init(&film);
...
static void _job_cleanup(void *param)
{
    ...
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)job->self->data;
...
static int _ensure_raw_ctx(dt_neural_job_t *j, const dt_restore_sensor_class_t cls)
{
...
static int _process_raw_denoise_bayer(dt_neural_job_t *j, const dt_imgid_t imgid,
                                      const char *out_filename, const char *src_path,
                                      const dt_image_t *img_meta)
{
    dt_mipmap_buffer_t mbuf;
    dt_mipmap_cache_get(&mbuf, imgid, DT_MIPMAP_FULL, DT_MIPMAP_BLOCKING, 'r');
    if (!mbuf.buf)
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] failed to load raw mosaic for imgid %d", imgid);
    ...
    int res = dt_restore_raw_bayer(j->ctx, img_meta, cfa_in, width, height, cfa_out,
    ...
    const int exif_len = dt_exif_read_blob(&exif_blob, src_path, imgid, FALSE, width, height, TRUE);
    uint8_t *jpeg_buf = NULL;
    int jpeg_len = 0, jpeg_w = 0, jpeg_h = 0;
    dt_imageio_dng_preview_t preview = {0};
    const int prev_rc =
        _extract_source_jpeg_preview(src_path, &jpeg_buf, &jpeg_len, &jpeg_w, &jpeg_h);
    if (prev_rc == 0)
    {
        ...
        dt_print(DT_DEBUG_AI, "[neural_restore] embedded JPEG preview from source %dx%d (%d bytes)",
    ...
    else
    {
        dt_print(DT_DEBUG_AI,
                 ...
                 "writing DNG without thumbnail",
                 prev_rc);
    }
    res = dt_imageio_dng_write_cfa_bayer(out_filename, cfa_out, width, height, img_meta, exif_blob,
                                         exif_len, jpeg_buf ? &preview : NULL);
    g_free(jpeg_buf);
    g_free(exif_blob);
    g_free(cfa_out);
    if (res != 0)
...
static int _process_raw_denoise_linear(dt_neural_job_t *j, const dt_imgid_t imgid,
                                       const char *out_filename, const char *src_path,
                                       const dt_image_t *img_meta)
{
    float *rgb = NULL;
    int w = 0, h = 0;
    int res = dt_restore_raw_linear(j->ctx, imgid, &rgb, &w, &h, j->raw_strength, j->control_job);
    if (res != 0 || !rgb)
    {
        g_free(rgb);
        return res ? res : 1;
    ...
    uint8_t *exif_blob = NULL;
    const int exif_len = dt_exif_read_blob(&exif_blob, src_path, imgid, FALSE, w, h, TRUE);
    uint8_t *jpeg_buf = NULL;
    int jpeg_len = 0, jpeg_w = 0, jpeg_h = 0;
    dt_imageio_dng_preview_t preview = {0};
    const int prev_rc =
    ...
    res = dt_imageio_dng_write_linear(out_filename, rgb, w, h, img_meta, exif_blob, exif_len,
...
static int _process_raw_denoise_one(dt_neural_job_t *j, const dt_imgid_t imgid,
                                    const char *out_filename, const char *src_path)
{
    ...
    dt_mipmap_cache_get(&warmup, imgid, DT_MIPMAP_FULL, DT_MIPMAP_BLOCKING, 'r');
    const gboolean loaded = (warmup.buf != NULL);
    dt_mipmap_cache_release(&warmup);
    if (!loaded)
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] mipmap warmup failed for imgid %d", imgid);
    ...
    const char *cls_name = (cls == DT_RESTORE_SENSOR_CLASS_BAYER)  ? "bayer" :
    ...
    dt_print(DT_DEBUG_AI,
             ...
             "filters=0x%x (%s)",
             imgid, flags, channels, filters, cls_name);
    ...
    if (_ensure_raw_ctx(j, cls))
        return 1;

    switch (cls)
    {
    case DT_RESTORE_SENSOR_CLASS_BAYER:
        return _process_raw_denoise_bayer(j, imgid, out_filename, src_path, &img_meta);
    case DT_RESTORE_SENSOR_CLASS_XTRANS:
        ...
        return _process_raw_denoise_linear(j, imgid, out_filename, src_path, &img_meta);
    case DT_RESTORE_SENSOR_CLASS_LINEAR:
        return _process_raw_denoise_linear(j, imgid, out_filename, src_path, &img_meta);
    default:
        return 1;
...
static int32_t _process_job_run(dt_job_t *job)
{
    ...
    dt_print(DT_DEBUG_AI, "[neural_restore] job started: task=%s, scale=%d, images=%d", task_name,
             j->scale, g_list_length(j->images));

    dt_imageio_module_format_t fmt = {.mime = _ai_get_mime,
    ...
    for (GList *iter = j->images; iter; iter = g_list_next(iter))
    {
        ...
        vp->filename = srcpath;
        vp->imgid = imgid;
        char *out_dir = dt_variables_expand(vp, (gchar *)dir_pattern, FALSE);
        ...
        {
            dt_print(DT_DEBUG_AI, "[neural_restore] failed to create output directory: %s",
                     out_dir_resolved);
        ...
        const char *ext = (j->task == NEURAL_TASK_RAW_DENOISE) ? "dng" : "tif";
        ...
        dt_print(DT_DEBUG_AI, "[neural_restore] processing imgid %d -> %s", imgid, filename);
        ...
        else
        {
            ...
            step_err = dt_imageio_export_with_flags(
                imgid, filename, &fmt, (dt_imageio_module_data_t *)&fmt_params,
                ...
                FALSE, // copy_metadata
                FALSE, // export_masks
                DT_COLORSPACE_LIN_REC2020, NULL, DT_INTENT_PERCEPTUAL, NULL, NULL, count, total,
                NULL, -1);
            if (step_err)
            {
                dt_print(DT_DEBUG_AI, "[neural_restore] export failed for imgid %d", imgid);
    ...
    if (successes > 0)
        dt_control_log(
            ngettext("neural %s: %d image processed", "neural %s: %d images processed", successes),
            task_name, successes);
...
static gboolean _check_model_available(dt_lib_neural_restore_t *d, const dt_neural_task_t task)
{
...
static void _update_button_sensitivity(dt_lib_neural_restore_t *d)
{
    ...
    const gboolean sensitive = d->model_available && has_images && !_any_selected_processing(d);
    gtk_widget_set_sensitive(d->process_button, sensitive);
    // picker only available after preview has been generated
    gtk_widget_set_sensitive(d->pick_button, d->export_pixels != NULL);
}
...
static void _update_info_label(dt_lib_neural_restore_t *d)
{
    ...
    if (scale > 1 && dt_is_valid_imgid(imgid))
    {
        ...
        if (fw > 0 && fh > 0)
        {
            ...
            snprintf(d->info_text_left, sizeof(d->info_text_left), "%.0fMP", in_mp);
            snprintf(d->info_text_right, sizeof(d->info_text_right), "%.0fMP", out_mp);
        ...
        d->preview_raw_imgid == imgid)
    {
        ...
        snprintf(d->info_text_left, sizeof(d->info_text_left), "%s",
                 is_bayer_out ? _("output: Bayer CFA DNG") : _("output: LinearRaw DNG"));
    }
    ...
    if (d->task != NEURAL_TASK_RAW_DENOISE)
    {
        const dt_colorspaces_color_profile_type_t icc_type =
            dt_conf_key_exists(CONF_ICC_TYPE) ? dt_conf_get_int(CONF_ICC_TYPE) : DT_COLORSPACE_NONE;
        if (dt_image_has_wide_gamut_output_profile(imgid, icc_type))
        {
            const gboolean preserve = dt_conf_key_exists(CONF_PRESERVE_WIDE_GAMUT) ?
                                          ...
                                          TRUE;
            const char *msg = (scale == 1 && preserve) ? _("wide-gamut preserved, not denoised") :
                                                         _("wide-gamut clipped");
            if (d->info_text_right[0])
            {
                const size_t used = strlen(d->info_text_right);
                snprintf(d->info_text_right + used, sizeof(d->info_text_right) - used, "  ·  %s",
                         msg);
            }
            else
            {
...
static void _schedule_preview_failed(dt_lib_module_t *self, dt_nr_preview_err_t err);
...
static void _task_changed(dt_lib_neural_restore_t *d)
{
    ...
    {
        const float saved =
            dt_conf_key_exists(CONF_STRENGTH) ? dt_conf_get_float(CONF_STRENGTH) : 100.0f;
    ...
    gtk_widget_set_visible(d->recovery_slider, d->task != NEURAL_TASK_RAW_DENOISE);
    ...
    const gboolean tiff_knobs_visible = (d->task != NEURAL_TASK_RAW_DENOISE);
    const gboolean wide_gamut_visible = (d->task == NEURAL_TASK_DENOISE);
    if (d->bpp_combo)
...
static void _preview_cache_free_slot(dt_lib_neural_restore_t *d, const dt_neural_task_t task)
{
...
static gboolean _preview_cache_hit(dt_lib_neural_restore_t *d, const dt_neural_task_t task,
                                   const dt_imgid_t imgid)
{
    if (task >= NEURAL_TASK_COUNT)
        return FALSE;
    ...
    return e->valid && e->imgid == imgid && e->patch_center[0] == d->patch_center[0] &&
...
static void _preview_cache_store(dt_lib_neural_restore_t *d, const dt_neural_task_t task,
                                 const dt_imgid_t imgid, const float patch_center[2],
                                 const int crop_w, const int crop_h, const float *before,
                                 const float *after, const float *detail)
{
    // task is an unsigned enum, no need for < 0 check
    if (task >= NEURAL_TASK_COUNT)
        return;
    if (crop_w <= 0 || crop_h <= 0 || !before || !after)
        return;
...
{
    if (!d->preview_after || !d->cairo_after)
        return;
...
static gboolean _preview_result_idle(gpointer data)
{
    ...
    d->preview_detail =
        dt_restore_compute_dwt_detail(res->before, res->after, res->width, res->height);
    ...
    _float_rgb_to_cairo(d->preview_before, d->cairo_before, res->width, res->height, stride);
    ...
    _preview_cache_store(d, res->task, res->imgid, res->patch_center, res->width, res->height,
                         d->preview_before, d->preview_after, d->preview_detail);
...
static gpointer _preview_thread(gpointer data)
{
    ...
    else
    {
        ...
        dt_imageio_export_with_flags(pd->imgid, "unused", &fmt, (dt_imageio_module_data_t *)&cap,
                                     TRUE,  // ignore_exif
                                     FALSE, // display_byteorder
                                     TRUE,  // high_quality
                                     FALSE, // upscale
                                     ...
                                     FALSE, // copy_metadata
                                     FALSE, // export_masks
                                     DT_COLORSPACE_LIN_REC2020, NULL, DT_INTENT_PERCEPTUAL, NULL,
                                     NULL, 1, 1, NULL, -1);
    ...
    dt_print(DT_DEBUG_AI,
             ...
             "export_size=%d",
             owns_pixels ? "exported" : "reusing", pixels_w, pixels_h, pd->scale,
    ...
    int pw = MIN(MIN(pd->preview_w, pixels_w * pd->scale), max_crop * pd->scale);
    int ph = MIN(MIN(pd->preview_h, pixels_h * pd->scale), max_crop * pd->scale);
    ...
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] preview: export too small (%dx%d)", pixels_w,
                 pixels_h);
        if (owns_pixels)
            g_free(cap.pixels);
        goto cleanup;
    ...
    if (!ctx)
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] preview: failed to load model");
        g_free(crop_4ch);
        g_free(crop_rgb);
        if (owns_pixels)
            g_free(cap.pixels);
    ...
    dt_print(DT_DEBUG_AI, "[neural_restore] preview: tiled inference %dx%d", crop_w, crop_h);
    ...
    const int ret = dt_restore_process_tiled(ctx, crop_4ch, crop_w, crop_h, pd->scale,
    ...
    cmsHTRANSFORM xform_disp = _build_output_color_transform(DT_COLORSPACE_LIN_REC709, "");
    if (xform_disp)
    {
...
static void _blend_raw_into_preview(dt_lib_neural_restore_t *d, float strength)
{
    ...
    for (size_t i = 0; i < n3; i++)
        d->preview_after[i] = a * d->preview_raw_denoised_rgb[i] + ia * d->preview_raw_src_rgb[i];
...
static void _install_cache_slot_raw(dt_lib_module_t *self, const dt_neural_task_t task)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    const __typeof__(d->preview_cache[0]) *e = &d->preview_cache[task];
    if (!e->valid)
    ...
    const float strength = dt_bauhaus_slider_get(d->raw_strength_slider) / 100.0f;
    _blend_raw_into_preview(d, strength);
}

static void _install_cache_slot_rgb(dt_lib_module_t *self, const dt_neural_task_t task)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    const __typeof__(d->preview_cache[0]) *e = &d->preview_cache[task];
    if (!e->valid)
    ...
    g_free(d->cairo_before);
    g_free(d->cairo_after);
    const int stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, e->crop_w);
    d->cairo_before = g_malloc(stride * e->crop_h);
    d->cairo_after = g_malloc(stride * e->crop_h);
    d->cairo_stride = stride;
    _float_rgb_to_cairo(d->preview_before, d->cairo_before, e->crop_w, e->crop_h, stride);
    _rebuild_cairo_after(d);
    d->preview_ready = TRUE;
    gtk_widget_queue_draw(d->preview_area);
}

static void _install_cache_slot(dt_lib_module_t *self, const dt_neural_task_t task)
{
...
static void _schedule_raw_strength_reblend(dt_lib_module_t *self)
{
    ...
    d->preview_strength_timer =
        g_timeout_add(RAW_PREVIEW_STRENGTH_DEBOUNCE_MS, _strength_blend_timer_cb, self);
}
...
static void _schedule_preview_failed(dt_lib_module_t *self, const dt_nr_preview_err_t err)
{
...
{
    dt_neural_preview_result_raw_t *res = (dt_neural_preview_result_raw_t *)data;
    // skip if gui_cleanup ran between schedule and dispatch
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)res->self->data;
    if (!d)
    {
    ...
    _preview_cache_store(d, NEURAL_TASK_RAW_DENOISE, res->full_imgid, res->patch_center, res->width,
                         res->height, d->preview_raw_src_rgb, d->preview_raw_denoised_rgb, NULL);
...
static gpointer _preview_thread_raw(gpointer data)
{
    ...
    dt_mipmap_buffer_t warmup;
    dt_mipmap_cache_get(&warmup, pd->imgid, DT_MIPMAP_FULL, DT_MIPMAP_BLOCKING, 'r');
    const gboolean warm_loaded = (warmup.buf != NULL);
    dt_mipmap_cache_release(&warmup);
    if (!warm_loaded)
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] raw preview: mipmap warmup failed for imgid %d",
                 pd->imgid);
        goto cleanup;
    }
    ...
    if (cls != DT_RESTORE_SENSOR_CLASS_BAYER && cls != DT_RESTORE_SENSOR_CLASS_XTRANS)
    {
        dt_print(DT_DEBUG_AI,
    ...
    dt_print(DT_DEBUG_AI,
             ...
             pd->imgid, is_xtrans ? "x-trans" : "bayer", pd->patch_center[0], pd->patch_center[1],
    ...
    dt_pthread_mutex_lock(&d->ctx_lock);
    {
        const gboolean cached_is_raw_correct = d->cached_ctx &&
    ...
    if (!ctx)
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] raw preview: failed to load model");
        goto cleanup;
    }
    if (pd->sequence != g_atomic_int_get(&d->preview_sequence))
    {
    ...
    const gboolean cache_matches =
        d->preview_raw_imgid == pd->imgid && d->preview_raw_sensor_class == cls &&
        ((is_xtrans && d->preview_full_lin) || (!is_xtrans && d->preview_full_cfa));
    ...
    {
        if (dt_restore_raw_linear_prepare(ctx, pd->imgid, &take_full_lin, &full_w, &full_h) != 0 ||
            !take_full_lin)
        {
            dt_restore_unref(ctx);
            goto cleanup;
    ...
    else
    {
        ...
        dt_mipmap_buffer_t mbuf;
        dt_mipmap_cache_get(&mbuf, pd->imgid, DT_MIPMAP_FULL, DT_MIPMAP_BLOCKING, 'r');
        if (!mbuf.buf)
        {
            dt_mipmap_cache_release(&mbuf);
            dt_restore_unref(ctx);
    ...
    if (!cache_matches)
    {
        ...
        const dt_colorspaces_color_profile_type_t cfg_type =
            dt_conf_key_exists(CONF_ICC_TYPE) ? dt_conf_get_int(CONF_ICC_TYPE) : DT_COLORSPACE_NONE;
        gchar *cfg_file =
            (cfg_type == DT_COLORSPACE_FILE) ? dt_conf_get_string(CONF_ICC_FILE) : NULL;
        const dt_colorspaces_color_profile_t *work_cp = dt_colorspaces_get_work_profile(pd->imgid);
        const dt_colorspaces_color_profile_type_t dst_type =
        ...
        dt_imageio_export_with_flags(pd->imgid, "unused", &fmt, (dt_imageio_module_data_t *)&cap,
                                     ...
                                     FALSE, // copy_metadata
                                     FALSE, // export_masks
                                     dst_type, cfg_file, DT_INTENT_PERCEPTUAL, NULL, NULL, 1, 1,
                                     NULL, -1);
        g_free(cfg_file);
    ...
    dt_print(DT_DEBUG_AI,
             ...
             full_w, full_h, (unsigned)ori, pd->patch_center[0], pd->patch_center[1], crop_x,
             crop_y, crop_w, crop_h, is_xtrans ? "linear" : "bayer");
    ...
    int err;
    if (is_xtrans)
        err = dt_restore_raw_linear_preview_piped(ctx, &img_meta, pd->imgid, full_lin_use, full_w,
                                                  full_h, crop_x, crop_y, crop_w, crop_h, &src_rgb,
                                                  &denoised_rgb, &actual_w, &actual_h);
    else
        err = dt_restore_raw_bayer_preview_piped(ctx, &img_meta, pd->imgid, full_cfa_use, full_w,
                                                 full_h, crop_x, crop_y, crop_w, crop_h, &src_rgb,
                                                 &denoised_rgb, &actual_w, &actual_h);
    ...
    dt_print(DT_DEBUG_AI,
             ...
             err, (void *)src_rgb, (void *)denoised_rgb, crop_w, crop_h, actual_w, actual_h);
    ...
    dt_neural_preview_result_raw_t *res = g_new0(dt_neural_preview_result_raw_t, 1);
    res->self = pd->self;
    res->src_rgb = src_rgb;
...
static gpointer _preview_thread_dispatch(gpointer data)
{
    ...
    gpointer res =
        (pd->task == NEURAL_TASK_RAW_DENOISE) ? _preview_thread_raw(data) : _preview_thread(data);
...
static void _schedule_preview_refresh(dt_lib_module_t *self, const guint delay_ms)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    ...
    if (d->preview_trigger_timer)
        g_source_remove(d->preview_trigger_timer);
    d->preview_trigger_timer = g_timeout_add(delay_ms, _trigger_preview_from_timer, self);
}
...
static void _trigger_preview(dt_lib_module_t *self)
{
    ...
    {
        dt_print(DT_DEBUG_AI, "[neural_restore] preview cache hit for %s", _task_log_name(d->task));
    ...
    d->preview_thread = g_thread_new("neural_preview", _preview_thread_dispatch, pd);
}
...
static dt_neural_task_t _task_from_page(dt_lib_neural_restore_t *d, const int page)
{
    switch (page)
    {
    case 0:
        return NEURAL_TASK_RAW_DENOISE;
    case 1:
        return NEURAL_TASK_DENOISE;
    default:
    {
...
static void _raw_strength_slider_changed(GtkWidget *widget, dt_lib_module_t *self)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    dt_conf_set_float(CONF_RAW_STRENGTH, dt_bauhaus_slider_get(d->raw_strength_slider));
    ...
    if (d->task == NEURAL_TASK_RAW_DENOISE && d->preview_raw_src_rgb && d->preview_raw_denoised_rgb)
...
static void _process_clicked(GtkWidget *widget, gpointer user_data)
{
    ...
    job_data->raw_strength = dt_conf_key_exists(CONF_RAW_STRENGTH) ?
                                 ...
                                 1.0f;
    job_data->bpp =
        dt_conf_key_exists(CONF_BIT_DEPTH) ? dt_conf_get_int(CONF_BIT_DEPTH) : NEURAL_BPP_16;
    job_data->add_to_catalog =
        dt_conf_key_exists(CONF_ADD_CATALOG) ? dt_conf_get_bool(CONF_ADD_CATALOG) : TRUE;
    char *out_dir = dt_conf_get_string(CONF_OUTPUT_DIR);
    job_data->output_dir = (out_dir && out_dir[0]) ? out_dir : NULL;
    if (!job_data->output_dir)
        g_free(out_dir);
    job_data->icc_type =
        dt_conf_key_exists(CONF_ICC_TYPE) ? dt_conf_get_int(CONF_ICC_TYPE) : DT_COLORSPACE_NONE;
    job_data->icc_filename =
        (job_data->icc_type == DT_COLORSPACE_FILE) ? dt_conf_get_string(CONF_ICC_FILE) : NULL;
...
static void _picking_geometry(const dt_lib_neural_restore_t *d, const int w, const int h,
                              double *img_w, double *img_h, double *ox, double *oy)
{
...
static void _build_export_cairo(dt_lib_neural_restore_t *d)
{
    ...
    const int stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, ew);
    ...
    for (int y = 0; y < eh; y++)
    {
        ...
        for (int x = 0; x < ew; x++)
        {
            ...
            const uint8_t r = (uint8_t)(_linear_to_srgb(d->export_pixels[si + 0]) * 255.0f + 0.5f);
            const uint8_t g = (uint8_t)(_linear_to_srgb(d->export_pixels[si + 1]) * 255.0f + 0.5f);
            const uint8_t b = (uint8_t)(_linear_to_srgb(d->export_pixels[si + 2]) * 255.0f + 0.5f);
            row[x] = ((uint32_t)r << 16) | ((uint32_t)g << 8) | (uint32_t)b;
        }
    }
}
...
static gboolean _pick_double_click(GtkWidget *widget, GdkEventButton *event, dt_lib_module_t *self)
{
...
static gboolean _zoom_tooltip_draw(GtkWidget *widget, cairo_t *cr, dt_lib_module_t *self)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    if (!d->cairo_before || !d->cairo_after)
        return FALSE;
    ...
    cairo_pattern_set_filter(cairo_get_source(cr),
                             scale >= 1.0 ? CAIRO_FILTER_NEAREST : CAIRO_FILTER_BILINEAR);
    cairo_paint(cr);
    cairo_restore(cr);
...
static gboolean _preview_query_tooltip(GtkWidget *widget, const gint x, const gint y,
                                       ...
                                       dt_lib_module_t *self)
{
    ...
    if (!d->cairo_before || !d->cairo_after)
        return FALSE;
    if (d->preview_generating)
        return FALSE;
    if (d->dragging_split)
        return FALSE;
    if (d->picking_thumbnail)
        return FALSE;
    if (d->preview_w <= 0 || d->preview_h <= 0)
        return FALSE;
    ...
    float zoom = dt_conf_key_exists(CONF_PREVIEW_TOOLTIP_ZOOM) ?
                     (float)dt_conf_get_float(CONF_PREVIEW_TOOLTIP_ZOOM) :
                     2.0f;
    if (zoom <= 0.0f)
        return FALSE;
    ...
    {
        const double s = (double)DT_NR_TOOLTIP_MAX_PX / (double)MAX(tw, th);
        tw = (int)(tw * s);
        th = (int)(th * s);
    }
    ...
    g_signal_connect(area, "draw", G_CALLBACK(_zoom_tooltip_draw), self);
...
static gboolean _preview_draw(GtkWidget *widget, cairo_t *cr, dt_lib_module_t *self)
{
    ...
    if (d->picking_thumbnail && d->export_cairo)
    {
        cairo_surface_t *thumb_surf = cairo_image_surface_create_for_data(
            d->export_cairo, CAIRO_FORMAT_RGB24, d->export_w, d->export_h, d->export_cairo_stride);
        ...
        double rw, rh;
        if (d->task == NEURAL_TASK_RAW_DENOISE && d->preview_raw_crop_w > 0 &&
            d->preview_raw_crop_h > 0 && d->export_w > 0 && d->export_h > 0)
        {
    ...
    if (!d->preview_ready || !d->cairo_before || !d->cairo_after)
    {
        ...
        cairo_text_extents_t ext;
        const char *text = !d->model_available   ? _("model not available") :
                           d->preview_generating ? _("generating preview...") :
                           !d->preview_requested ? _("click to generate preview") :
    ...
    if (d->info_text_left[0])
    {
        ...
        const double total_w =
            with_right ? ext_l.width + gap + arrow_w + gap + ext_r.width : ext_l.width;
...
static gboolean _preview_button_press(GtkWidget *widget, GdkEventButton *event,
                                      dt_lib_module_t *self)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
...
static gboolean _preview_button_release(GtkWidget *widget, GdkEventButton *event,
                                        dt_lib_module_t *self)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    if (d->dragging_split)
...
static gboolean _preview_motion(GtkWidget *widget, GdkEventMotion *event, dt_lib_module_t *self)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    ...
    if (d->dragging_split)
    {
        ...
        const double scale = fmax(1.0, fmin((double)w / pw, (double)ah / ph));
        const double ox = (w - pw * scale) / 2.0;
        const double img_w = pw * scale;
    ...
    if (d->preview_ready && d->preview_w > 0 && d->preview_h > 0)
    {
        const int w = gtk_widget_get_allocated_width(widget);
        const int h = gtk_widget_get_allocated_height(widget);
        const double scale = fmax(1.0, fmin((double)w / d->preview_w, (double)h / d->preview_h));
        const double ox = (w - d->preview_w * scale) / 2.0;
        const double div_x = ox + d->split_pos * d->preview_w * scale;
        ...
        if (win)
        {
            ...
            {
                GdkCursor *cursor =
                    gdk_cursor_new_from_name(gdk_display_get_default(), "col-resize");
...
static void _bpp_combo_changed(GtkWidget *w, dt_lib_module_t *self)
{
...
static void _profile_combo_changed(GtkWidget *w, dt_lib_module_t *self)
{
...
static void _catalog_toggle_changed(GtkWidget *w, dt_lib_module_t *self)
{
    const gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(w));
    dt_conf_set_bool(CONF_ADD_CATALOG, active);
}

static void _preserve_wide_gamut_toggled(GtkWidget *w, dt_lib_module_t *self)
{
    const gboolean active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(w));
...
static void _output_dir_changed(GtkEditable *editable, dt_lib_module_t *self)
{
    dt_conf_set_string(CONF_OUTPUT_DIR, gtk_entry_get_text(GTK_ENTRY(editable)));
}

static void _output_dir_browse(GtkWidget *button, dt_lib_module_t *self)
{
    dt_lib_neural_restore_t *d = (dt_lib_neural_restore_t *)self->data;
    GtkWidget *dialog = gtk_file_chooser_dialog_new(
        _("select output folder"), GTK_WINDOW(dt_ui_main_window(darktable.gui->ui)),
        GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("_cancel"), GTK_RESPONSE_CANCEL, _("_select"),
        GTK_RESPONSE_ACCEPT, NULL);

    const char *current = gtk_entry_get_text(GTK_ENTRY(d->output_dir_entry));
...
void gui_init(dt_lib_module_t *self)
{
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("page"), GTK_WIDGET(d->notebook), &notebook_def);
    ...
    d->raw_denoise_page =
        dt_ui_notebook_page(d->notebook, N_("raw denoise"), _("AI raw denoising"));
    d->denoise_page = dt_ui_notebook_page(d->notebook, N_("denoise"), _("AI denoising"));
    d->upscale_page = dt_ui_notebook_page(d->notebook, N_("upscale"), _("AI upscaling"));

    // raw denoise page: strength slider. 100 = full model output,
    // 0 = unchanged source CFA, linear blend in raw ADC space
    const float saved_raw_strength =
        dt_conf_key_exists(CONF_RAW_STRENGTH) ? dt_conf_get_float(CONF_RAW_STRENGTH) : 100.0f;
    d->raw_strength_slider =
        dt_bauhaus_slider_new_action(DT_ACTION(self), 0.0f, 100.0f, 1.0f, saved_raw_strength, 0);
    dt_bauhaus_widget_set_label(d->raw_strength_slider, NULL, N_("strength"));
    dt_bauhaus_slider_set_format(d->raw_strength_slider, "%");
    gtk_widget_set_tooltip_text(d->raw_strength_slider, _("blend between the source CFA (0%) and "
    ...
    const float saved_strength =
        dt_conf_key_exists(CONF_STRENGTH) ? dt_conf_get_float(CONF_STRENGTH) : 100.0f;
    d->recovery_slider =
        dt_bauhaus_slider_new_action(DT_ACTION(self), 0.0f, 100.0f, 1.0f, saved_strength, 0);
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->scale_combo, self, NULL, N_("scale"), _("upscale factor"), 0,
                                 _scale_combo_changed, self, N_("2x"), N_("4x"));
    ...
    d->patch_center[1] = 0.5f;
    d->picking_thumbnail = FALSE;
    d->pick_button = dtgtk_togglebutton_new(dtgtk_cairo_paint_colorpicker, 0, NULL);
    gtk_widget_set_tooltip_text(d->pick_button,
                                _("click to pick preview area on the image thumbnail\n"
                                  "double-click to reset to center"));
    g_signal_connect(d->pick_button, "toggled", G_CALLBACK(_pick_toggled), self);
    g_signal_connect(d->pick_button, "button-press-event", G_CALLBACK(_pick_double_click), self);

    // preview area (resizable via dt_ui_resize_wrap)
    d->preview_area = GTK_WIDGET(dt_ui_resize_wrap(NULL, 200, CONF_PREVIEW_HEIGHT));
    gtk_widget_add_events(d->preview_area, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                                               GDK_POINTER_MOTION_MASK);
    g_signal_connect(d->preview_area, "draw", G_CALLBACK(_preview_draw), self);
    g_signal_connect(d->preview_area, "button-press-event", G_CALLBACK(_preview_button_press),
                     self);
    g_signal_connect(d->preview_area, "button-release-event", G_CALLBACK(_preview_button_release),
                     self);
    g_signal_connect(d->preview_area, "motion-notify-event", G_CALLBACK(_preview_motion), self);
    ...
    g_signal_connect(d->preview_area, "query-tooltip", G_CALLBACK(_preview_query_tooltip), self);

    // process button
    d->process_button = dt_action_button_new(self, N_("process"), _process_clicked, self,
    ...
    self->widget = dt_gui_vbox(GTK_WIDGET(d->notebook), d->preview_area, action_row);

    // output settings
    dt_gui_new_collapsible_section(&d->cs_output, CONF_EXPAND_OUTPUT, _("output parameters"),
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(d->bpp_combo, self, NULL, N_("bit depth"),
                                 _("output TIFF bit depth"), saved_bpp, _bpp_combo_changed, self,
        ...
        if (saved_type != DT_COLORSPACE_NONE)
        {
            ...
            for (GList *l = darktable.color_profiles->profiles; l; l = g_list_next(l))
            {
                ...
                if (pp->out_pos > -1 && pp->type == saved_type &&
                    (pp->type != DT_COLORSPACE_FILE || g_strcmp0(pp->filename, saved_file) == 0))
                {
    ...
    gtk_widget_set_tooltip_text(d->profile_combo, _("color profile embedded in the output TIFF"));
    g_signal_connect(d->profile_combo, "value-changed", G_CALLBACK(_profile_combo_changed), self);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->preserve_wide_gamut_toggle),
    ...
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(d->catalog_toggle),
        dt_conf_key_exists(CONF_ADD_CATALOG) ? dt_conf_get_bool(CONF_ADD_CATALOG) : TRUE);
    gtk_widget_set_tooltip_text(d->catalog_toggle, _("automatically import the output image into"
                                                     " the current collection"));
    g_signal_connect(d->catalog_toggle, "toggled", G_CALLBACK(_catalog_toggle_changed), self);
    ...
    g_signal_connect(d->output_dir_button, "clicked", G_CALLBACK(_output_dir_browse), self);

    dt_gui_box_add(dir_box, d->output_dir_entry);
    dt_gui_box_add(dir_box, d->output_dir_button);
    dt_gui_box_add(cs_box, dir_box);

    g_signal_connect(d->notebook, "switch-page", G_CALLBACK(_notebook_page_changed), self);
