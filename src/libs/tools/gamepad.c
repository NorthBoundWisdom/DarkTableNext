...
static const char *_button_names[] = {
    ...
    N_("button back"), N_("button guide"),  N_("button start"),   N_("left stick"),
    N_("right stick"), N_("left shoulder"), N_("right shoulder"), N_("dpad up"),
    N_("dpad down"),   N_("dpad left"),     N_("dpad right"),     N_("button misc1"),
    N_("paddle1"),     N_("paddle2"),       N_("paddle3"),        N_("paddle4"),
    N_("touchpad"),    N_("left trigger"),  N_("right trigger"),  NULL};

static gchar *_key_to_string(const guint key, const gboolean display)
{
    const gchar *name =
        key < SDL_CONTROLLER_BUTTON_MAX + 2 ? _button_names[key] : N_("invalid gamepad button");
...
static const char *_move_names[] = {N_("left x"),         N_("left y"),        N_("right x"),
                                    N_("right y"),        N_("left diagonal"), N_("left skew"),
                                    N_("right diagonal"), N_("right skew"),    NULL};

static gchar *_move_to_string(const guint move, const gboolean display)
{
    const gchar *name = move < SDL_CONTROLLER_AXIS_MAX + 4 /* diagonals */ ?
                            _move_names[move] :
                            N_("invalid gamepad axis");
    return g_strdup(display ? _(name) : name);
}
...
static void _process_axis_timestep(dt_gamepad_device_t *gamepad, const Uint32 timestamp)
{
    if (timestamp > gamepad->timestamp)
    {
        ...
        for (SDL_GameControllerAxis axis = SDL_CONTROLLER_AXIS_LEFTX;
             axis <= SDL_CONTROLLER_AXIS_RIGHTY; axis++)
        {
...
static void _process_axis_and_send(dt_gamepad_device_t *gamepad, const Uint32 timestamp)
{
...
static gboolean _poll_devices(gpointer user_data)
{
    ...
    while (SDL_PollEvent(&event) > 0)
    {
        ...
        case SDL_CONTROLLERBUTTONDOWN:
            dt_print(DT_DEBUG_INPUT, "SDL button down event time %d id %d button %hhd state %hhd",
                     event.cbutton.timestamp, event.cbutton.which, event.cbutton.button,
                     event.cbutton.state);
        ...
        case SDL_CONTROLLERAXISMOTION:
            dt_print(DT_DEBUG_INPUT, "SDL axis event type %d time %d id %d axis %hhd value %hd",
                     event.caxis.type, event.caxis.timestamp, event.caxis.which, event.caxis.axis,
                     event.caxis.value);
                ...
                event.caxis.axis == SDL_CONTROLLER_AXIS_TRIGGERRIGHT)
            {
                ...
                {
                    dt_shortcut_key_release(gamepad->id, event.cbutton.timestamp,
                                            SDL_CONTROLLER_BUTTON_MAX + trigger);
                    dt_shortcut_key_press(gamepad->id, event.cbutton.timestamp,
                                          SDL_CONTROLLER_BUTTON_MAX + trigger);
    ...
    for (GSList *gamepads = self->data; gamepads; gamepads = gamepads->next)
        _process_axis_and_send(gamepads->data, SDL_GetTicks());

    if (num_events)
        dt_print(DT_DEBUG_INPUT, "sdl num_events: %d time: %u", num_events, SDL_GetTicks());
    return G_SOURCE_CONTINUE;
}
        ...
        if (SDL_IsGameController(i))
        {
            ...
            if (!controller)
            {
                dt_print(DT_DEBUG_ALWAYS,
                         "[_gamepad_open_devices] ERROR opening game controller '%s'",
