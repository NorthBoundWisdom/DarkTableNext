...
static int _time_compare_at_zoom(dt_datetime_t t1, dt_datetime_t t2, dt_lib_timeline_zooms_t zoom)
{
    ...
    if (zoom >= DT_LIB_TIMELINE_ZOOM_YEAR)
    {
        ...
        if (zoom >= DT_LIB_TIMELINE_ZOOM_4MONTH)
        {
            ...
            {
                if (t1.hour / 2 != t2.hour / 2)
                    return (t1.hour / 2 - t2.hour / 2);
                if (zoom >= DT_LIB_TIMELINE_ZOOM_DAY)
                {
                    ...
                    {
                        if (t1.minute / 3 != t2.minute / 3)
                            return (t1.minute / 3 - t2.minute / 3);
...
{
    if (new_zoom == strip->zoom)
        return strip->time_pos;
    ...
    if (_time_compare(tt, strip->time_mini) < 0)
        tt = strip->time_mini;
    return tt;
}
...
static gchar *_time_format_for_ui(dt_datetime_t t, dt_lib_timeline_zooms_t zoom)
{
    ...
    else if (zoom == DT_LIB_TIMELINE_ZOOM_10DAY)
    {
        ...
        int x2 = x + 9;
        if (x2 == 30)
            x2 = _time_days_in_month(t.year, t.month);
    ...
    {
        return g_strdup_printf("%02d/%02d/%02d (h%02d-%02d)", t.day, t.month, t.year % 100,
                               t.hour / 6 * 6, t.hour / 6 * 6 + 5);
    }
    else if (zoom == DT_LIB_TIMELINE_ZOOM_HOUR)
    {
    ...
    {
        return g_strdup_printf("%02d/%02d/%02d %02dh(%02d-%02d)", t.day, t.month, t.year % 100,
                               t.hour, t.minute / 10 * 10, t.minute / 10 * 10 + 9);
    }
    else if (zoom == DT_LIB_TIMELINE_ZOOM_MINUTE)
    {
        return g_strdup_printf("%02d/%02d/%02d %02d:%02d", t.day, t.month, t.year % 100, t.hour,
                               t.minute);
    }
...
static gchar *_time_format_for_collect(dt_datetime_t t, dt_lib_timeline_zooms_t zoom)
{
    if (zoom == DT_LIB_TIMELINE_ZOOM_YEAR)
        ...
        return g_strdup_printf("%04d:%02d:%02d %02d:%02d", t.year, t.month, t.day, t.hour,
                               t.minute);
...
static gboolean _time_read_bounds_from_db(dt_lib_module_t *self)
{
    ...
    sqlite3_stmt *stmt;
    const char *query =
        "SELECT MIN(datetime_taken) AS dt FROM main.images WHERE datetime_taken > 1";
...
static int _block_get_at_zoom(dt_lib_module_t *self, int width)
{
    ...
    if (_time_compare_at_zoom(strip->start_t, strip->time_pos, strip->zoom) < 0)
        strip->start_x = -2;
    if (_time_compare_at_zoom(strip->stop_t, strip->time_pos, strip->zoom) < 0)
        strip->stop_x = -1;
  ...
  gchar *query = g_strdup_printf("SELECT db.datetime_taken AS dt,"
    ...
    while (TRUE)
    {
        dt_lib_timeline_block_t *bloc =
            (dt_lib_timeline_block_t *)calloc(1, sizeof(dt_lib_timeline_block_t));
        ...
        if (strip->zoom == DT_LIB_TIMELINE_ZOOM_YEAR)
            tt.month = 1;
                 ...
                 strip->zoom == DT_LIB_TIMELINE_ZOOM_MONTH)
            tt.day = 1;
        else if (strip->zoom == DT_LIB_TIMELINE_ZOOM_10DAY ||
                 strip->zoom == DT_LIB_TIMELINE_ZOOM_DAY)
            tt.hour = 0;
        else if (strip->zoom == DT_LIB_TIMELINE_ZOOM_6HOUR ||
                 strip->zoom == DT_LIB_TIMELINE_ZOOM_HOUR)
            tt.minute = 0;
        ...
        if (w > width || stat != SQLITE_ROW)
        {
            ...
            if (_time_compare_at_zoom(strip->start_t, tt, strip->zoom) >= 0)
                strip->start_x = strip->panel_width + 1;
            if (_time_compare_at_zoom(strip->stop_t, tt, strip->zoom) >= 0)
                strip->stop_x = strip->panel_width + 2;
            break;
        }
    }
    sqlite3_finalize(stmt);
...
static gboolean _time_is_visible(dt_datetime_t t, dt_lib_timeline_t *strip)
{
    ...
    if (_time_compare_at_zoom(t, strip->time_pos, strip->zoom) < 0)
        return FALSE;
    ...
    {
        dt_lib_timeline_block_t *blo = bl->data;
        if (_time_compare_at_zoom(t, blo->init, strip->zoom) > 0)
            return FALSE;
    }
    ...
    if (e->button == GDK_BUTTON_PRIMARY)
    {
        if (e->type == GDK_BUTTON_PRESS)
        {
            ...
            else
            {
                ...
                if (_time_compare(tt, _time_init()) == 0)
                    strip->start_t = strip->stop_t =
                        strip->time_maxi; //we are past the end so selection extends until the end
                else
...
static gboolean _lib_timeline_button_release_callback(GtkWidget *w, GdkEventButton *e,
                                                      dt_lib_module_t *self)
{
    dt_lib_timeline_t *strip = self->data;
        ...
        else
        {
            ...
            if (strip->zoom <= DT_LIB_TIMELINE_ZOOM_DAY)
            {
                ...
                if (strip->zoom <= DT_LIB_TIMELINE_ZOOM_MONTH)
                {
                    ...
                    {
                        strip->stop_t.day =
                            _time_days_in_month(strip->stop_t.year, strip->stop_t.month);
                    }
                }
            }
        }
...
static gboolean _lib_timeline_motion_notify_callback(GtkWidget *w, GdkEventMotion *e,
                                                     dt_lib_module_t *self)
{
    dt_lib_timeline_t *strip = self->data;
...
static gboolean _lib_timeline_scroll_callback(GtkWidget *w, GdkEventScroll *e,
                                              dt_lib_module_t *self)
{
    dt_lib_timeline_t *strip = self->data;
    ...
    if (dt_modifier_is(e->state, GDK_CONTROL_MASK))
    {
        ...
        if (dt_gui_get_scroll_unit_delta(e, &delta_y))
        {
            if (delta_y < 0)
            {
                if (z != DT_LIB_TIMELINE_ZOOM_HOUR)
                    z++;
            }
            else if (delta_y > 0)
            {
                if (z != DT_LIB_TIMELINE_ZOOM_YEAR)
                    z--;
            }
        }
...
static gboolean _lib_timeline_mouse_leave_callback(GtkWidget *w, GdkEventCrossing *e,
                                                   dt_lib_module_t *self)
{
    dt_lib_timeline_t *strip = self->data;
...
void gui_init(dt_lib_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(d->timeline), "button-press-event",
                     G_CALLBACK(_lib_timeline_button_press_callback), self);
    g_signal_connect(G_OBJECT(d->timeline), "button-release-event",
                     G_CALLBACK(_lib_timeline_button_release_callback), self);
    g_signal_connect(G_OBJECT(d->timeline), "scroll-event",
                     G_CALLBACK(_lib_timeline_scroll_callback), self);
    g_signal_connect(G_OBJECT(d->timeline), "motion-notify-event",
    ...
    dt_action_register(DT_ACTION(self), N_("start selection"), _selection_start,
                       GDK_KEY_bracketleft, 0);
    dt_action_register(DT_ACTION(self), N_("stop selection"), _selection_stop, GDK_KEY_bracketright,
                       0);
}
