...
static const char *_note_names[] = {"C", "C#", "D", "D#", "E", "F", "F#",
                                    "G", "G#", "A", "A#", "B", NULL};

static gchar *_key_to_string(const guint key, const gboolean display)
{
    // The MIDI note range is from C−1 (note #0) to G9 (note #127).
    return g_strdup_printf(display ? "%s%d (%d)" : "%s%d", _note_names[key % 12], (int)key / 12 - 1,
                           key);
}

static gboolean _string_to_key(const gchar *string, guint *key)
{
    int octave = 0;
    char name[3];
...
static gchar *_move_to_string(const guint move, const gboolean display)
{
    return g_strdup_printf("CC%u", move);
}

static gboolean _string_to_move(const gchar *string, guint *move)
{
    return sscanf(string, "CC%u", move) == 1;
}

static gboolean _key_to_move(dt_lib_module_t *self, const dt_input_device_t id, const guint key,
                             guint *move)
{
...
static void _midi_write(dt_midi_device_t *midi, const gint channel, const gint type, const gint key,
                        const gint velocity)
{
...
static gint _calculate_move(dt_midi_device_t *midi, const gint controller, const gint velocity)
{
    ...
    default: // absolute
    {
        ...
        if (last != -1)
        {
            if (midi->num_identical)
            {
                ...
                {
                    dt_control_log(
                        _("using absolute encoding; reinitialise to switch to relative"));
                    midi->num_identical = 0;
                }
                else if (--midi->num_identical)
                    dt_control_log(
                        _("%d more identical (down) moves before switching to relative encoding"),
                        midi->num_identical);
                else
                {
            ...
            else
            {
                ...
                if (controller == midi->last_controller && diff * midi->last_diff < 0)
                {
...
static void _midi_write_bcontrol(dt_midi_device_t *midi, const gchar seq, gchar *str)
{
    // sysex string contains zeros so can't use standard string handling and Pm_WriteSysEx
    unsigned char sysex[100];
    const int syslen =
        g_snprintf((gchar *)sysex, sizeof(sysex), "\xF0%c\x20\x32\x7F\x7F\x20%c%c%s\xF7%c%c%c", 0,
                   0, seq, str, 0, 0, 0);
    PmEvent buffer[sizeof(sysex) / 4] = {0};
    for (int i = 0; i < syslen / 4; i++)
        buffer[i].message = sysex[i * 4] | (sysex[i * 4 + 1] << 8) | (sysex[i * 4 + 2] << 16) |
                            (sysex[i * 4 + 3] << 24);
...
static void _update_with_move(dt_midi_device_t *midi, const PmTimestamp timestamp,
                              const gint controller, const float move)
{
    float new_position = dt_shortcut_move(midi->id, timestamp, controller, move);

    const int new_pattern = DT_ACTION_IS_INVALID(new_position) ? 1 :
    ...
    if (midi->behringer == 'M')
    {
        if (midi->first_key == 8 ? controller < 9 /* layer A */
                                   :
    ...
    {
        if (midi->first_key == 16 ? (controller >= 10 && controller <= 25) /* layer A */
                                    :
    ...
    else if (new_pattern != midi->rotor_lights[controller])
    {
        ...
        if ((midi->behringer == 'R' || midi->behringer == 'F') && controller < 32 &&
            midi->portmidi_out)
        {
            ...
            _midi_write_bcontrol(midi, 2,
                                 g_strdup_printf("  .easypar CC 1 %d 0 127 absolute", controller));
            _midi_write_bcontrol(midi, 3,
                                 g_strdup_printf("  .mode %s", bcontrol_codes[new_pattern]));
...
static gboolean _poll_devices(gpointer user_data)
{
    ...
    for (GSList *devices = self->data; devices; devices = devices->next)
    {
        ...
        for (int i = 0; i < num_events; i++)
        {
            ...
            {
            case 0x9: // note on
                dt_print(DT_DEBUG_INPUT, "Note On: Channel %d, Data1 %d", midi->channel,
                         event_data1);
                ...
                break;
            case 0x8: // note off
                dt_print(DT_DEBUG_INPUT, "Note Off: Channel %d, Data1 %d", midi->channel,
                         event_data1);
            ...
            if (midi->behringer == 'M' || midi->behringer == 'C')
            {
                ...
                midi->first_key =
                    midi->behringer == 'M' ? (layer_B ? 32 : 8) : (layer_B ? 71 : 16); // 'C'

                if (midi->first_key != old_first)
                    for (int j = 1; j <= midi->num_knobs; j++)
                        midi->last_known[j] = -1;
            }
        }
    }
...
static void _midi_open_devices(dt_lib_module_t *self)
{
    ...
    for (int i = 0; i < Pm_CountDevices(); i++)
    {
        ...
        dt_print(DT_DEBUG_INPUT, "[_midi_open_devices] found midi device '%s' via '%s'", info->name,
                 info->interf);
        ...
        if (info->input && !strstr(info->name, "Midi Through Port"))
        {
            ...
            if (error < 0)
            {
                dt_print(DT_DEBUG_ALWAYS,
                         "[_midi_open_devices] ERROR opening midi device '%s' via '%s'", info->name,
                         info->interf);
                continue;
            }
            else
            {
                dt_print(DT_DEBUG_INPUT,
                         "[_midi_open_devices] opened midi device '%s' via '%s' as midi%d",
            ...
            midi->num_identical =
                midi->behringer || encoding ?
                    0 :
                    5; // countdown "relative down" moves received before switching to relative mode
            ...
            for (int j = 0; j < Pm_CountDevices(); j++)
            {
                ...
                if (!strcmp(info->name, infoOutput->name) && infoOutput->output &&
                    !infoOutput->opened)
                {
...
static gboolean _update_devices(gpointer user_data)
{
    ...
    while (devices)
    {
        ...
        gint global = midi->behringer == 'M' ? 0 : midi->behringer == 'C' ? 1 : midi->channel;
