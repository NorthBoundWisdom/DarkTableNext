...
typedef struct dt_lib_tool_preferences_t
{
    ...
    GtkWidget *over_label, *over_r0, *over_r1, *over_r2, *over_r3, *over_r4, *over_r5, *over_r6,
        *over_timeout, *over_tt;
    GtkWidget *over_culling_label, *over_culling_r0, *over_culling_r3, *over_culling_r4,
        *over_culling_r6, *over_culling_timeout, *over_culling_tt;
...
static gboolean _lib_keymap_button_press_release(GtkWidget *button, GdkEventButton *event,
                                                 gpointer user_data);
...
static void _overlays_toggle_button(GtkWidget *w, dt_lib_module_t *self)
{
    ...
    dt_ui_thumbtable(darktable.gui->ui)->show_tooltips =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(d->over_tt));
    ...
    if (thumbs_state)
    {
        ...
        else
        {
            gtk_widget_set_tooltip_text(d->over_timeout,
                                        _("timeout only available for block overlay"));
        }

        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(d->over_tt),
                                     dt_ui_thumbtable(darktable.gui->ui)->show_tooltips);
...
static void _main_icons_register_size(GtkWidget *widget, GdkRectangle *allocation,
                                      gpointer user_data)
{
    ...
    int width = allocation->width - margin.left - margin.right - border.left - border.right -
...
void gui_init(dt_lib_module_t *self)
{
    ...
    dt_action_define(&darktable.control->actions_global, NULL, N_("grouping"), d->grouping_button,
    ...
    g_signal_connect(G_OBJECT(d->grouping_button), "clicked",
                     G_CALLBACK(_lib_filter_grouping_button_clicked), NULL);

    /* create the "show/hide overlays" button */
    d->overlays_button = dtgtk_button_new(dtgtk_cairo_paint_overlays, 0, NULL);
    dt_action_define(&darktable.control->actions_global, NULL, N_("thumbnail overlays options"),
                     d->overlays_button, &dt_action_def_button);
    gtk_widget_set_tooltip_text(d->overlays_button,
    ...
    g_signal_connect(G_OBJECT(d->overlays_button), "clicked", G_CALLBACK(_overlays_show_popup),
                     self);
    // we register size of overlay icon to keep in sync thumbtable overlays
    g_signal_connect(G_OBJECT(d->overlays_button), "size-allocate",
                     G_CALLBACK(_main_icons_register_size), NULL);
...
#define NEW_RADIO(widget, box, callback, label)                                                    \
    ...
    dt_action_t *ac =
        dt_action_section(&darktable.control->actions_global, N_("thumbnail overlays"));
    GtkWidget *rb = NULL;
    NEW_RADIO(d->over_r0, d->thumbnails_box, _overlays_toggle_button, N_("no overlays"));
    NEW_RADIO(d->over_r1, d->thumbnails_box, _overlays_toggle_button,
              N_("overlays on mouse hover"));
    NEW_RADIO(d->over_r2, d->thumbnails_box, _overlays_toggle_button,
              N_("extended overlays on mouse hover"));
    NEW_RADIO(d->over_r3, d->thumbnails_box, _overlays_toggle_button, N_("permanent overlays"));
    NEW_RADIO(d->over_r4, d->thumbnails_box, _overlays_toggle_button,
              N_("permanent extended overlays"));
    NEW_RADIO(d->over_r5, d->thumbnails_box, _overlays_toggle_button,
              N_("permanent overlays extended on mouse hover"));
    ...
    g_signal_connect(G_OBJECT(d->over_timeout), "value-changed",
                     G_CALLBACK(_overlays_timeout_changed), self);
    ...
    rb = NULL;
    NEW_RADIO(d->over_culling_r0, d->culling_box, _overlays_toggle_culling_button,
              N_("no overlays"));
    NEW_RADIO(d->over_culling_r3, d->culling_box, _overlays_toggle_culling_button,
              N_("permanent overlays"));
    NEW_RADIO(d->over_culling_r4, d->culling_box, _overlays_toggle_culling_button,
              N_("permanent extended overlays"));
    hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    NEW_RADIO(d->over_culling_r6, hbox, _overlays_toggle_culling_button,
              N_("overlays block on mouse hover"));
    gtk_box_pack_start(GTK_BOX(hbox), gtk_label_new(_("during (s)")), FALSE, FALSE, 0);
    d->over_culling_timeout = gtk_spin_button_new_with_range(-1, 99, 1);
    g_signal_connect(G_OBJECT(d->over_culling_timeout), "value-changed",
                     G_CALLBACK(_overlays_timeout_changed), self);
    ...
    g_signal_connect(G_OBJECT(d->over_culling_tt), "toggled",
                     G_CALLBACK(_overlays_toggle_culling_button), self);
...
#undef NEW_RADIO
    ...
    dt_action_define(&darktable.control->actions_global, NULL, N_("help"), d->help_button,
                     &dt_action_def_toggle);
    gtk_widget_set_tooltip_text(
        d->help_button, _("enable this, then click on a control element to see its online help"));
    ...
    dt_action_define(&darktable.control->actions_global, NULL, N_("shortcuts"), d->keymap_button,
                     &dt_action_def_toggle);
    gtk_widget_set_tooltip_text(d->keymap_button,
                                _("define keyboard shortcuts for on-screen controls\n"
    ...
    g_signal_connect(G_OBJECT(d->keymap_button), "clicked", G_CALLBACK(_lib_keymap_button_clicked),
                     d);
    g_signal_connect(G_OBJECT(d->keymap_button), "button-press-event",
                     G_CALLBACK(_lib_keymap_button_press_release), d);
    g_signal_connect(G_OBJECT(d->keymap_button), "button-release-event",
                     G_CALLBACK(_lib_keymap_button_press_release), d);
    ...
    ac = dt_action_define(&darktable.control->actions_global, NULL, N_("preferences"),
                          d->preferences_button, &dt_action_def_button);
    ...
    g_signal_connect(G_OBJECT(d->preferences_button), "clicked",
                     G_CALLBACK(_lib_preferences_button_clicked), NULL);
    ...
    self->widget = dt_gui_hbox(d->grouping_button, d->overlays_button, d->help_button,
                               d->keymap_button, d->preferences_button);
}
...
{
    gint x, y, w, h;
} _shortcuts_dialog_posize = {};

static gboolean _resize_shortcuts_dialog(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
    gtk_window_get_position(GTK_WINDOW(widget), &_shortcuts_dialog_posize.x,
                            &_shortcuts_dialog_posize.y);
    gtk_window_get_size(GTK_WINDOW(widget), &_shortcuts_dialog_posize.w,
                        &_shortcuts_dialog_posize.h);
...
static void _set_mapping_mode_cursor(GtkWidget *widget)
{
    ...
    else if (dt_action_widget(darktable.control->mapping_widget) && darktable.develop)
    {
        guint size = gdk_display_get_default_cursor_size(display);
        cairo_surface_t *surface =
            cairo_image_surface_create(CAIRO_FORMAT_ARGB32, size * 1.5, size);
        cairo_t *cr = cairo_create(surface);

        dtgtk_cairo_paint_flags_t flags =
        ...
        gdk_window_set_cursor(window,
                              NULL); // work around bug where gtk seems to buffer surface cursors
    }
    else
...
{
    GtkWidget *shortcuts_dialog = gtk_dialog_new_with_buttons(
    ...
    {
        gtk_window_move(GTK_WINDOW(shortcuts_dialog), _shortcuts_dialog_posize.x,
                        _shortcuts_dialog_posize.y);
        gtk_window_resize(GTK_WINDOW(shortcuts_dialog), _shortcuts_dialog_posize.w,
                          _shortcuts_dialog_posize.h);
    }
    g_signal_connect(G_OBJECT(shortcuts_dialog), "configure-event",
                     G_CALLBACK(_resize_shortcuts_dialog), NULL);
...
static void _main_do_event_keymap(GdkEvent *event, gpointer data)
{
    ...
    case GDK_LEAVE_NOTIFY:
    case GDK_ENTER_NOTIFY:
        if (darktable.control->mapping_widget && event->crossing.mode == GDK_CROSSING_UNGRAB)
            break;
    ...
    case GDK_FOCUS_CHANGE:
        ...
        break;
    case GDK_BUTTON_PRESS:
        if (gdk_display_device_is_grabbed(gdk_window_get_display(event->button.window),
                                          event->button.device))
            break;
        ...
        else
        {
            ...
            if (GTK_IS_EVENT_BOX(event_widget))
                event_widget = gtk_bin_get_child(GTK_BIN(event_widget));
