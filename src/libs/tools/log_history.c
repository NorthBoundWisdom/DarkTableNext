...
static gboolean _button_press_release(GtkWidget *button, GdkEventButton *event,
                                      dt_lib_module_t *self);
static gboolean _suppress_popup(GtkWidget *widget, gpointer user_data);
static gboolean _label_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data);
...
static void _populate_list_box(dt_lib_module_t *self)
{
    ...
    if (entries)
    {
        ...
        for (GList *elem = g_list_first(entries); elem; elem = g_list_next(elem))
        {
            ...
            g_signal_connect(G_OBJECT(label), "button-press-event", G_CALLBACK(_label_button_press),
                             NULL);
            g_signal_connect(G_OBJECT(label), "popup-menu", G_CALLBACK(_suppress_popup), NULL);
                                      ...
                                      dt_lib_module_t *self)
{
    ...
    if ((event->type == GDK_BUTTON_PRESS &&
         (event->button == GDK_BUTTON_PRIMARY || event->button == GDK_BUTTON_SECONDARY)) ||
...
void gui_init(dt_lib_module_t *self)
{
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(d->scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
    gtk_scrolled_window_set_min_content_height(GTK_SCROLLED_WINDOW(d->scrolled),
                                               DT_PIXEL_APPLY_DPI(200));

    gtk_widget_set_size_request(d->scrolled, DT_PIXEL_APPLY_DPI(700), DT_PIXEL_APPLY_DPI(200));

    gtk_container_add(GTK_CONTAINER(d->scrolled), d->list_box);
    gtk_container_add(GTK_CONTAINER(d->popover), d->scrolled);

    g_signal_connect(G_OBJECT(d->button), "button-press-event", G_CALLBACK(_button_press_release),
                     self);
    g_signal_connect(G_OBJECT(d->button), "button-release-event", G_CALLBACK(_button_press_release),
                     self);

    self->widget = dt_gui_hbox(overlay);

    DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_CONTROL_LOG_REDRAW, G_CALLBACK(_log_redraw_callback), self);
}
