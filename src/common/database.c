...
static gboolean _migrate_schema(dt_database_t *db, const int version)
{
  ...
  sqlite3_prepare_v2(db->handle,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (!last_name || strcmp(last_name, name) || !last_operation ||
            strcmp(last_operation, operation) || last_op_version != op_version)
        {
            g_free(last_name);
  ...
  _SQLITE3_EXEC(
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "[init] failing query: `%s' sql: '%s", failing_query,
...
static int _upgrade_library_schema_step(dt_database_t *db, const int version)
{
    sqlite3_stmt *stmt;
    ...
    else if (version == 0)
    {
        ...
        new_version =
            1; // the version we transformed the db to. this way it might be possible to roll back or
               // add fast paths
    }
    else if (version == 1)
    {
    ...
    else if (version == 6)
    {
        ...
        if (sqlite3_exec(db->handle, "SELECT style_id FROM main.style_items", NULL, NULL, NULL) ==
            SQLITE_OK)
        {
            // clang-format off
...
#define FINALIZE                                                                                   \
    ...
    {                                                                                              \
        ...
        stmt = NULL;
        sqlite3_stmt *insert_stmt = NULL, *delete_stmt = NULL, *select_stmt = NULL,
                     *count_clashes_stmt = NULL, *update_name_stmt = NULL;
        // clang-format off
    ...
    TRY_PREPARE(delete_stmt, "DELETE FROM main.presets WHERE rowid = ?1", "can't prepare deletion statement");
        ...
        while (sqlite3_step(select_stmt) == SQLITE_ROW)
        {
            ...
            } while (sqlite3_step(count_clashes_stmt) == SQLITE_ROW &&
                     sqlite3_column_int(count_clashes_stmt, 0) > 0);
...
#define FINALIZE                                                                                   \
    ...
    {                                                                                              \
        ...
        insert_stmt = NULL;
        delete_stmt = NULL;
        sqlite3_stmt *select_new_stmt = NULL, *copy_style_items_stmt = NULL,
                     *delete_style_items_stmt = NULL;
        // clang-format off
    ...
    TRY_PREPARE(delete_style_items_stmt, "DELETE FROM main.style_items WHERE styleid = ?1",
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            TRY_STEP(copy_style_items_stmt, SQLITE_DONE,
                     "can't copy style items into data database");

            // delete the style items from the old table
            sqlite3_bind_int(delete_style_items_stmt, 1, id);
            TRY_STEP(delete_style_items_stmt, SQLITE_DONE,
                     "can't delete style items from database");
...
#define FINALIZE
    ...
    TRY_PREPARE(sel_stmt, "SELECT DISTINCT operation FROM main.history WHERE iop_order <= 0 OR iop_order IS NULL",
        ...
        while (sqlite3_step(sel_stmt) == SQLITE_ROW)
        {
            ...
            dt_print(DT_DEBUG_ALWAYS,
                     "[init] operation %s with no iop_order while upgrading database", op_name);
        }
        ...
        while (has_row)
        {
            ...
            if (imgid != current_imgid || !has_row)
            {
                ...
                if (item_list != NULL)
                {
                    ...
                    const dt_iop_order_t new_order_version =
                        current_order_version == 2 ? DT_IOP_ORDER_LEGACY : DT_IOP_ORDER_V30;
                    ...
                    while (e)
                    {
                        ...
                        while (s && strcmp(((dt_iop_order_entry_t *)s->data)->operation,
                                           e_entry->operation))
                        {
                            s = g_list_next(s);
                        }
                    ...
                    if (kind == DT_IOP_ORDER_CUSTOM || has_multiple_instances)
                    {
                        ...
                        TRY_STEP(ins_stmt, SQLITE_DONE,
                                 "can't insert into module_order (custom order)");
                    ...
                    else
                    {
                        ...
                        sqlite3_bind_int(ins_stmt, 2, kind);
                        TRY_STEP(ins_stmt, SQLITE_DONE,
                                 "can't insert into module_order (standard order)");
                        sqlite3_finalize(ins_stmt);
                    }
    ...
    char *query = g_strdup_printf(
        ...
        // clang-format on
        TRY_PREPARE(h_stmt, query, "can't prepare selecting history for history_hash migration");
      ...
      sqlite3_prepare_v2(db->handle,
            ...
            // clang-format on
            sqlite3_bind_int(h2_stmt, 1, imgid);
            sqlite3_bind_blob(h2_stmt, 2, altered ? NULL : hash, altered ? 0 : hash_len,
                              SQLITE_TRANSIENT);
    ...
    else if (version == 34)
    {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            {
                GDateTime *gdt = dt_datetime_exif_to_gdatetime(
                    (const char *)sqlite3_column_text(stmt, 1), darktable.utc_tz);
                if (gdt)
                {
        ...
        TRY_EXEC("ALTER TABLE `images_new` RENAME TO `images`",
                 "can't rename images_new table to images");
    ...
    else if (version == 38)
    {
        ...
        // create new indexes
        TRY_EXEC("CREATE INDEX makers_name ON makers (name)", "can't create makers_name");
        TRY_EXEC("CREATE INDEX model_name ON models (name)", "can't create model_name");
        TRY_EXEC("CREATE INDEX lens_name ON lens (name)", "can't create lens_name");
        TRY_EXEC("CREATE INDEX camera_name ON cameras (name)", "can't create camera_name");
    ...
    TRY_EXEC("ALTER TABLE main.images DROP COLUMN lens",
        ...
        TRY_EXEC(
            "CREATE TABLE images_new (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER, "
            "film_id INTEGER, "
        ...
        TRY_EXEC("DROP TABLE images", "can't drop table images_old");

        TRY_EXEC("ALTER TABLE images_new RENAME TO images", "can't rename images");
        ...
        TRY_EXEC("CREATE VIEW v_images AS"
    ...
    else if (version == 39)
    {
        ...
        TRY_EXEC("DROP TABLE cameras", "can't drop cameras table");
        ...
        // NOTE: datetime_taken is in nano-second since "0001-01-01 00:00:00"
        TRY_EXEC("DROP VIEW v_images", "can't drop v_images view");
    ...
    else if (version == 44)
    {
        ...
        TRY_EXEC("DROP TABLE history_hash", "can't drop table history_hash");
        ...
        TRY_EXEC("DROP TABLE tmp_history_hash", "can't drop table tmp_history_hash");
    ...
    {
        TRY_EXEC("DROP TABLE IF EXISTS legacy_presets", "can't drop legacy_presets");
    ...
    else if (version == 48)
    {
        ...
        TRY_EXEC("DROP TABLE selected_images", "can't drop selected_images");
        ...
        TRY_EXEC("DROP TABLE tmp_selected_images", "can't drop tmp_selected_images");
    ...
    else if (version == 49)
    {
        ...
        TRY_EXEC("DROP INDEX selected_images_ni", "can't drop index selected_images_ni");
    ...
    else if (version == 50)
    {
        ...
        TRY_EXEC("DROP TABLE images", "can't drop table images");
    ...
    else if (version == 51 || version == 52)
    {
       ...
       let's repeat the migration steps if we are potentially on that broken interim version 52 */
        ...
        TRY_EXEC(
            "CREATE TABLE selected_images (num INTEGER PRIMARY KEY AUTOINCREMENT, imgid INTEGER)",
    ...
    else if (version == 53)
    {
        ...
        // meta data
        TRY_EXEC("ALTER TABLE `meta_data` RENAME TO `meta_data_old`", "can't rename meta_data");
        TRY_EXEC("CREATE TABLE `meta_data` (id integer, key integer, value varchar, "
                 "FOREIGN KEY(id) REFERENCES images(id) ON DELETE CASCADE ON UPDATE CASCADE)",
                 "can't create new meta_data table");
        ...
        TRY_EXEC("DROP TABLE meta_data_old", "can't drop table meta_data_old");
    ...
    else if (version == 55)
    {
        ...
        TRY_EXEC("DROP TABLE overlay", "can't drop old table 'overlay'");
    ...
    else if (version == 56)
    {
        ...
        new_version =
            version; // should be the fallback so that calling code sees that we are in an infinite loop
...
static int _upgrade_data_schema_step(dt_database_t *db, const int version)
{
    ...
    TRY_PREPARE(sel_stmt, "SELECT DISTINCT operation FROM data.style_items WHERE iop_order <= 0 OR iop_order IS NULL",
        ...
        while (sqlite3_step(sel_stmt) == SQLITE_ROW)
        {
            const char *op_name = (const char *)sqlite3_column_text(sel_stmt, 0);
            dt_print(DT_DEBUG_ALWAYS,
                     "operation %s with no iop_order while upgrading style_items in database",
                     op_name);
        }
        sqlite3_finalize(sel_stmt);
        // clang-format off
    ...
    else if (version == 5)
    {
        ...
        TRY_EXEC("ALTER TABLE data.styles RENAME TO s", "can't rename styles to s");
        TRY_EXEC(
            "CREATE TABLE data.styles (id INTEGER PRIMARY KEY, name VARCHAR, description VARCHAR, iop_list VARCHAR)",
        ...
        TRY_EXEC("DROP TABLE s", "can't drop table s");
    ...
    {
        TRY_EXEC(
            "CREATE TABLE data.meta_data (key INTEGER PRIMARY KEY, tagname VARCHAR, "
        ...
        const char *metadata_fields[][3] = {
            ...
            {"Xmp.dc.description", _("description"),
             "plugins/lighttable/metadata/description_flag"},
            ...
            {"Xmp.darktable.version_name", _("version name"),
             "plugins/lighttable/metadata/version name_flag"},
            {"Xmp.darktable.image_id", _("image id"), NULL},
            {"Xmp.xmpMM.PreservedFileName", _("preserved filename"),
...
static void _ask_for_upgrade(const gchar *dbname, const gboolean has_gui)
{
    ...
    char *label_text =
        g_markup_printf_escaped(_("the database schema has to be upgraded for\n"
                                  "\n"
                                  "<span style='italic'>%s</span>\n"
...
static void _database_delete_mipmaps_files()
{
    ...
    gchar *mipmapfilename = g_build_filename(cachedir, "mipmaps", NULL);
    ...
    if (g_access(mipmapfilename, F_OK) != -1)
    {
        ...
        g_free(mipmapfilename);
        mipmapfilename = g_build_filename(cachedir, "mipmaps.fallback", NULL);
