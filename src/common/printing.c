...
void dt_printing_setup_display(dt_images_box *imgs, const float px, const float py,
                               const float pwidth, const float pheight, const float ax,
                               const float ay, const float awidth, const float aheight,
                               const gboolean borderless)
{
    ...
    dt_print(DT_DEBUG_PRINT, "[printing] screen/page  (%3.1f, %3.1f) -> (%3.1f, %3.1f)", px, py,
             pwidth, pheight);
    dt_print(DT_DEBUG_PRINT, "[printing] screen/parea (%3.1f, %3.1f) -> (%3.1f, %3.1f)", ax, ay,
             awidth, aheight);
...
void dt_printing_setup_box(dt_images_box *imgs, const int idx, const float x, const float y,
                           const float width, const float height)
{
    const float dx = fminf(imgs->screen.print_area.width, fmaxf(100.0f, width));
    const float dy = fminf(imgs->screen.print_area.height, fmaxf(100.0f, height));
...
void dt_printing_setup_page(dt_images_box *imgs, const float page_width, const float page_height,
...
void _align_pos(const dt_image_pos *ref, const dt_alignment_t alignment, const int32_t width,
                const int32_t height, dt_image_pos *pos)
{
    pos->width = width;
    pos->height = height;
...
void dt_printing_get_screen_pos(const dt_images_box *imgs, const dt_image_box *img,
                                dt_image_pos *pos)
{
    _clear_pos(pos);
...
void dt_printing_get_screen_rel_pos(const dt_images_box *imgs, const dt_image_box *img,
                                    dt_image_pos *pos)
{
...
void dt_printing_get_image_pos_mm(const dt_images_box *imgs, const dt_image_box *img,
                                  dt_image_pos *pos)
{
    dt_image_pos rpos;
...
void dt_printing_get_image_pos(const dt_images_box *imgs, const dt_image_box *img,
                               dt_image_pos *pos)
{
    dt_image_pos rpos;
...
void dt_printing_setup_image(dt_images_box *imgs, const int idx, const dt_imgid_t imgid,
