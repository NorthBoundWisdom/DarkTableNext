...
float *dt_read_pfm(const char *filename, int *err, int *wd, int *ht, int *ch, const size_t planes)
{
    ...
    if (!filename || filename[0] == 0)
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[pfm_read] no filename provided for 'dt_read_pfm'");
        return NULL;
    }
    ...
    if (!f)
    {
        ...
        if (err)
            *err = DT_IMAGEIO_FILE_NOT_FOUND;
        dt_print(DT_DEBUG_ALWAYS, "[pfm_read] failed to open file '%s'", filename);
        return NULL;
    }
    g_free(normalized_filename);
    ...
    if (ret != 2 || head[0] != 'P')
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[pfm_read] format of data is not Portable Float Map in '%s'",
                 filename);
        goto error;
    }
    ...
    if (!readbuf || !image)
    {
        ...
        dt_print(DT_DEBUG_ALWAYS, "[pfm_read] failed to allocate memory for PFM file `%s'",
                 filename);
        goto error;
    }
    ...
    if (ret != npixels)
    {
        if (err)
            *err = DT_IMAGEIO_IOERROR;
        dt_print(DT_DEBUG_ALWAYS, "[pfm_read] failed to read all PFM file contents from '%s'",
                 filename);
        goto error;
    }
                  ...
                  const size_t bpp)
{
    if (!filename || filename[0] == 0)
    {
        dt_print(DT_DEBUG_ALWAYS, "[pfm_write] no filename provided for 'dt_write_pfm'");
        return;
    }
    ...
    if (!f)
    {
        dt_print(DT_DEBUG_ALWAYS, "[pfm_write] failed to open file '%s'", filename);
        return;
    }
