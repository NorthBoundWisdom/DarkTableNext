...
const char *iop_order_string[] = {N_("custom"),    N_("legacy"),   N_("v3.0 RAW"),
                                  N_("v3.0 JPEG"), N_("v5.0 RAW"), N_("v5.0 JPEG")};
...
const dt_iop_order_entry_t legacy_order[] = {{{1.0f}, "rawprepare", 0},
                                             {{2.0f}, "invert", 0},
                                             {{3.0f}, "temperature", 0},
                                             {{3.1f}, "rasterfile", 0},
                                             {{4.0f}, "highlights", 0},
...
const dt_iop_order_entry_t *const _iop_order_tables[DT_IOP_ORDER_LAST] = {
    NULL, legacy_order, v30_order, v30_jpg_order, v50_order, v50_jpg_order};
...
static dt_iop_order_t _ioppr_get_default_iop_order_version(const dt_imgid_t imgid)
{
    ...
    const dt_iop_order_t iop_order_version =
        is_display_referred ? DT_IOP_ORDER_LEGACY :
                              (is_ldr ? DT_DEFAULT_IOP_ORDER_JPG : DT_DEFAULT_IOP_ORDER_RAW);
...
GList *dt_ioppr_get_iop_order_list(const dt_imgid_t imgid, const gboolean sorted)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            else if (version >= DT_IOP_ORDER_LEGACY && version < DT_IOP_ORDER_LAST)
            {
