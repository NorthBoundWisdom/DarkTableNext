...
template <int D>
static void _mf_filter(PermutohedralLattice<D, 2> &lattice, const float *const __restrict__ probs,
                       float *const __restrict__ out, const int npixels)
{
    lattice.clearValues();
...
static void _initial_splat(PermutohedralLattice<D, 2> &lattice,
                           ...
                           const float *const __restrict__ probs, const int npixels)
{
    DT_OMP_FOR(shared(lattice))
    for (int i = 0; i < npixels; i++)
    {
...
template <int D>
static void _slice_normalize(PermutohedralLattice<D, 2> &lattice, float *const __restrict__ out,
                             const int npixels)
{
    DT_OMP_FOR(shared(lattice))
    for (int i = 0; i < npixels; i++)
...
extern "C" void dt_dense_crf_binary(float *probabilities, const unsigned char *rgb, const int width,
                                    const int height, const float sigma_spatial,
                                    const float sigma_rgb, const float w_spatial,
                                    const float w_bilateral, const int n_iterations)
{
    if (!probabilities || !rgb || width <= 0 || height <= 0 || n_iterations <= 0 ||
    ...
    float *__restrict__ spatial_features = (float *)dt_alloc_aligned((size_t)n * 2 * sizeof(float));
    float *__restrict__ bilateral_features =
        (float *)dt_alloc_aligned((size_t)n * 5 * sizeof(float));
    float *const __restrict__ unary = (float *)dt_alloc_aligned((size_t)n * sizeof(float));
    float *const __restrict__ msg_spatial = (float *)dt_alloc_aligned((size_t)n * sizeof(float));
    float *const __restrict__ msg_bilateral = (float *)dt_alloc_aligned((size_t)n * sizeof(float));

    if (!spatial_features || !bilateral_features || !unary || !msg_spatial || !msg_bilateral)
    {
        dt_print(DT_DEBUG_ALWAYS, "[densecrf] failed to allocate scratch buffers for %dx%d", width,
                 height);
    ...
    size_t spatial_grid = (size_t)((float)height * inv_sigma_s * (float)width * inv_sigma_s);
    size_t bilateral_grid =
        (size_t)((float)height * inv_sigma_s * (float)width * inv_sigma_s * inv_sigma_c *
                 inv_sigma_c * inv_sigma_c * 256.0f * 256.0f * 256.0f);
    if (spatial_grid > MAX_GRID_HINT)
        spatial_grid = MAX_GRID_HINT;
    if (bilateral_grid > MAX_GRID_HINT)
        bilateral_grid = MAX_GRID_HINT;
    ...
    try
    {
        ...
        for (int iter = 0; iter < n_iterations; iter++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                const float msg = w_spatial * (2.0f * msg_spatial[i] - 1.0f) +
