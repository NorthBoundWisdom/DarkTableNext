...
static char *_find_latest_compatible_release(const char *repository, char **error_msg)
{
    ...
    char *url = g_strdup_printf("https://raw.githubusercontent.com/%s/HEAD/releases-index.json",
                                repository);
    ...
    {
        dt_print(DT_DEBUG_AI, "[ai_models] failed to fetch releases-index.json: curl=%d, http=%ld",
                 res, http_code);
        if (error_msg)
        {
            if (res != CURLE_OK)
                ...
                *error_msg = g_strdup_printf(
                    _("model repository \"%s\" missing releases-index.json"), repository);
            else
                *error_msg =
                    g_strdup_printf(_("could not fetch releases-index.json (HTTP %ld)"), http_code);
        }
    ...
    if (json_object_has_member(root_obj, "schema"))
    {
        ...
        if (schema != 1)
            dt_print(DT_DEBUG_AI,
                     ...
                     "supported by this darktable; expected 1",
                     schema);
    }

    if (json_object_has_member(root_obj, "compatible_releases"))
    {
        JsonObject *map = json_object_get_object_member(root_obj, "compatible_releases");
    ...
    {
        dt_print(DT_DEBUG_AI, "[ai_models] releases-index.json missing 'compatible_releases'");
    }

    g_object_unref(parser);

    if (tag)
        dt_print(DT_DEBUG_AI, "[ai_models] found compatible release: %s (darktable %s)", tag,
                 dt_version);
    else
        dt_print(DT_DEBUG_AI,
...
static char *_fetch_asset_digest(const char *repository, const char *release_tag,
                                 const char *asset_name)
{
    char *model_id = g_strdup(asset_name);
    char *ext = strrchr(model_id, '.');
    if (ext && g_strcmp0(ext, ".dtmodel") == 0)
        *ext = '\0';

    char *url = g_strdup_printf("https://github.com/%s/releases/download/%s/versions.json",
    ...
    if (res != CURLE_OK || http_code != 200)
    {
        dt_print(DT_DEBUG_AI, "[ai_models] failed to fetch versions.json: curl=%d, http=%ld", res,
                 http_code);
        g_string_free(response, TRUE);
        g_free(model_id);
        return NULL;
    ...
    if (!digest)
        dt_print(DT_DEBUG_AI, "[ai_models] no sha256 for %s in versions.json (release %s)",
...
static gboolean _setup_registry(dt_ai_registry_t *registry)
{
    ...
    if (!models)
        models = g_build_filename(g_get_user_data_dir(), "darktable", "models", NULL);
    ...
    dt_print(DT_DEBUG_AI, "[ai_models] initialized: models_dir=%s", registry->models_dir);
    dt_print(DT_DEBUG_AI, "[ai_models] initialized: cache_dir=%s", registry->cache_dir);
    return ok;
}
...
gboolean dt_ai_models_load_registry(void)
{
    ...
    if (json_object_has_member(root_obj, "models"))
    {
        ...
        for (guint i = 0; i < len; i++)
        {
            ...
            if (model)
            {
                ...
                dt_print(DT_DEBUG_AI, "[ai_models] registered model: %s (%s)", model->name,
                         model->id);
            }
        }
    }
    ...
    dt_print(DT_DEBUG_AI, "[ai_models] registry loaded: %d models from %s", model_count,
             registry_path);
...
static dt_ai_model_t *_parse_local_model_config(const char *config_path, const char *dir_name)
{
    JsonParser *parser = json_parser_new();
    GError *error = NULL;
    ...
    {
        dt_print(DT_DEBUG_AI, "[ai_models] failed to parse %s: %s", config_path,
    ...
    const char *id =
        json_object_has_member(obj, "id") ? json_object_get_string_member(obj, "id") : dir_name;
    const char *name =
        json_object_has_member(obj, "name") ? json_object_get_string_member(obj, "name") : dir_name;
    ...
    if (json_object_has_member(obj, "spatial_dims"))
    {
        ...
        if (arr && json_array_get_length(arr) >= 2)
        {
            ...
            if (h && h[0])
                model->spatial_dim_h = g_strdup(h);
            if (w && w[0])
                model->spatial_dim_w = g_strdup(w);
        }
    }
    ...
    for (GList *l2 = registry->models; l2; l2 = g_list_next(l2))
    {
            ...
            g_file_test(config_path, G_FILE_TEST_EXISTS))
        {
            ...
            if (local)
            {
                ...
                if (model->min_version && _version_compare(model->version, model->min_version) < 0)
                {
                    model->status = DT_AI_MODEL_UPDATE_REQUIRED;
                    dt_print(DT_DEBUG_AI,
                             "[ai_models] model %s v%s is older than "
                             "required v%s - please update",
                             model->id, model->version ? model->version : "0.0",
                             model->min_version);
                }
            }
        }
            ...
            while ((entry_name = g_dir_read_name(dir)))
            {
                    ...
                    g_file_test(config_path, G_FILE_TEST_EXISTS))
                {
                    ...
                    if (model)
                    {
                        ...
                        dt_print(DT_DEBUG_AI, "[ai_models] discovered local model: %s (%s)",
                                 model->name, model->id);
                    }
                }
...
void dt_ai_models_check_updates(void)
{
    ...
    char *error_msg = NULL;
    char *release_tag = _find_latest_compatible_release(repository, &error_msg);
    if (!release_tag)
    {
        dt_print(DT_DEBUG_AI, "[ai_models] check_updates: no compatible release found%s%s",
    ...
    if (!json_parser_load_from_data(parser, response->str, response->len, NULL))
    {
        g_object_unref(parser);
        g_string_free(response, TRUE);
        dt_print(DT_DEBUG_AI, "[ai_models] check_updates: failed to parse versions.json");
        return;
    }
    g_string_free(response, TRUE);
    ...
    JsonNode *root = json_parser_get_root(parser);
    JsonObject *root_obj = json_node_get_object(root);
    JsonObject *models_obj = json_object_has_member(root_obj, "models") ?
    ...
    {
        g_object_unref(parser);
        dt_print(DT_DEBUG_AI, "[ai_models] check_updates: no 'models' object in "
                              "versions.json");
        return;
    }
    ...
    for (GList *l = registry->models; l; l = g_list_next(l))
    {
        ...
        const char *remote_version = json_object_has_member(m_obj, "version") ?
                                         json_object_get_string_member(m_obj, "version") :
                                         NULL;
        const char *remote_sha256 = json_object_has_member(m_obj, "sha256") ?
                                        json_object_get_string_member(m_obj, "sha256") :
                                        NULL;
        ...
        if (remote_version && model->status == DT_AI_MODEL_DOWNLOADED &&
            _version_compare(model->version, remote_version) < 0)
        {
            model->status = DT_AI_MODEL_UPDATE_AVAILABLE;
            dt_print(DT_DEBUG_AI, "[ai_models] update available for %s: v%s -> v%s", model->id,
                     model->version ? model->version : "?", remote_version);
        }
    }
    g_mutex_unlock(&registry->lock);
    g_object_unref(parser);
...
static gboolean _verify_checksum(const char *filepath, const char *expected)
{
    ...
    if (!match)
    {
        dt_print(DT_DEBUG_AI, "[ai_models] checksum mismatch: expected %s, got %s", expected_hash,
                 computed);
    }
...
static gboolean _extract_zip_atomic(const char *zippath, const char *dest_root,
                                    const char *model_id)
{
    ...
    gchar *staging = g_strdup_printf("%s%s.staging.XXXXXX", dest_root, G_DIR_SEPARATOR_S);
    if (!g_mkdtemp(staging))
    {
        dt_print(DT_DEBUG_AI, "[ai_models] failed to create staging dir under %s", dest_root);
        g_free(staging);
        return FALSE;
    }
    ...
    {
        dt_print(DT_DEBUG_AI, "[ai_models] archive top-level does not match model id '%s'",
                 model_id);
        _rmdir_recursive(staging);
        g_free(staging);
        g_free(staged_model);
    ...
    if (!ok)
        dt_print(DT_DEBUG_AI, "[ai_models] failed to move staged model into place: %s -> %s",
...
static char *_zip_top_dir(const char *zippath)
{
    ...
    if (archive_read_next_header(a, &entry) == ARCHIVE_OK)
    {
        ...
        if (slash)
            result = g_strndup(path, slash - path);
        else if (path)
...
static void _activate_if_unset(dt_ai_registry_t *registry, const char *model_id)
{
    if (!registry || !model_id)
        return;
    gchar *task = NULL;
...
char *dt_ai_models_install_local(const char *filepath)
{
    ...
    if (!_valid_model_id(installed_id))
    {
        ...
        char *err =
            g_strdup_printf(_("archive top-level directory is not a valid model id: \"%s\""),
...
static const char *_pick_fallback_active_unlocked(dt_ai_registry_t *registry, const char *task)
{
    if (!registry || !task)
        return NULL;
    ...
    for (GList *l = registry->models; l; l = g_list_next(l))
    {
        ...
        if (!m->task || strcmp(m->task, task) != 0)
            continue;
        if (m->status != DT_AI_MODEL_DOWNLOADED)
            continue;
        if (m->is_default)
            return m->id;
        if (!first_installed)
            first_installed = m->id;
    }
...
#define SET_STATUS_AND_RETURN(status_val, err_expr)                                                \
    ...
    if (!repository || !g_regex_match_simple("^[a-zA-Z0-9._-]+/[a-zA-Z0-9._-]+$", repository, 0, 0))
    {
...

    {
        ...
        dt_print(DT_DEBUG_AI, "[ai_models] darktable version: %s (full: %s)", ver ? ver : "unknown",
    ...
    char *url = g_strdup_printf("https://github.com/%s/releases/download/%s/%s", repository,
                                release_tag, asset);
    g_free(release_tag);
    ...
    dt_ai_download_data_t dl = {.registry = registry,
...
char *dt_ai_models_get_active_for_task(const char *task)
{
    ...
    if (darktable.ai_registry)
    {
        ...
        for (GList *l = darktable.ai_registry->models; l; l = g_list_next(l))
        {
            ...
            if (m->task && strcmp(m->task, task) == 0 && m->is_default &&
                m->status == DT_AI_MODEL_DOWNLOADED)
            {
...
dt_ai_model_card_t *dt_ai_models_get_card(const char *model_id)
{
    ...
    if (!model_path)
    {
        dt_print(DT_DEBUG_AI, "[ai_models] model card: %s not on disk", model_id);
        return NULL;
    }
