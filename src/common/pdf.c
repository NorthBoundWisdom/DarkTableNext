...
static void _pdf_set_offset(dt_pdf_t *pdf, int id, const size_t offset)
{
...
dt_pdf_t *dt_pdf_start(const char *filename, const float width, const float height, const float dpi,
...
static size_t _pdf_stream_encoder_ASCIIHex(dt_pdf_t *pdf, const unsigned char *data,
                                           const size_t len)
{
...
static size_t _pdf_stream_encoder_Flate(dt_pdf_t *pdf, const unsigned char *data, const size_t len)
{
    int result;
...
static size_t _pdf_write_stream(dt_pdf_t *pdf, const dt_pdf_stream_encoder_t encoder,
...
int dt_pdf_add_icc_from_data(dt_pdf_t *pdf, const unsigned char *data, const size_t size)
{
...
dt_pdf_image_t *dt_pdf_add_image(dt_pdf_t *pdf, const unsigned char *image, const int width,
                                 const int height, const int bpp, const int icc_id,
                                 const float border)
{
    size_t stream_size = 0;
    size_t bytes_written = 0;
    ...
    bytes_written += fprintf(pdf->fd,
                             ...
                             "/Width %d\n"
                             "/Height %d\n",
                             pdf_image->object_id, pdf_image->name_id,
                             stream_encoder_filters[pdf->default_encoder], width, height);
    ...
    // the stream
    stream_size =
        _pdf_write_stream(pdf, pdf->default_encoder, image, (size_t)3 * (bpp / 8) * width * height);
    if (stream_size == 0)
    {
        free(pdf_image);
        return NULL;
...
dt_pdf_page_t *dt_pdf_add_page(dt_pdf_t *pdf, dt_pdf_image_t **images, const int n_images)
{
...
void dt_pdf_finish(dt_pdf_t *pdf, dt_pdf_page_t **pages, const int n_pages)
{
...
int main(int argc, char *argv[])
{
    ...
    dt_pdf_t *pdf =
        dt_pdf_start(argv[argc - 1], page_width, page_height, 360, DT_PDF_STREAM_ENCODER_FLATE);
