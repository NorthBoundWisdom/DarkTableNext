...
static gchar *_dt_full_locale_name(const char *locale)
{
    ...
    if (!g_spawn_command_line_sync("locale -a", &output, NULL, NULL, &error))
    {
        if (error)
        {
            dt_print(DT_DEBUG_ALWAYS, "[l10n] couldn't check locale: '%s'", error->message);
            g_error_free(error);
        }
    }
    else
...
static void _dt_get_language_names(GList *languages)
{
    ...
    if (!root)
    {
        dt_print(DT_DEBUG_ALWAYS, "[l10n] error: can't get root node of `%s'", filename);
        goto end;
    }
    ...
    for (int i = 0; i < n_elements; i++)
    {
        ...
        if (name && (alpha_2 || alpha_3))
        {
           ...
           * maybe localization failed. Try now in the main dialect. */
                    ...
                    language->name = g_strdup_printf("%s (%s)%s", localized_name, language->code,
                                                     language->is_default ? " *" : "");
                    g_free(localized_name);
            ...
            dt_print(DT_DEBUG_ALWAYS, "[l10n] error: element %d has no name, skipping", i);
...
dt_l10n_t *dt_l10n_init(const gboolean init_list)
{
    ...
    if (init_list)
    {
        ...
        if (dir)
        {
            ...
            {
                gchar *testname =
                    g_build_filename(localedir, locale, "LC_MESSAGES", GETTEXT_PACKAGE ".mo", NULL);
                if (g_file_test(testname, G_FILE_TEST_EXISTS))
                {
                    ...
                    if (delimiter)
                    {
                        ...
                        language->base_code =
                            g_strndup(language->base_code, delimiter - language->base_code);
                        g_free(tmp);
                    }
                    ...
                    language->name =
                        g_strdup_printf("%s%s", locale, language->is_default ? " *" : "");
            ...
            dt_print(DT_DEBUG_ALWAYS, "[l10n] error: can't open directory `%s'", localedir);
        ...
        {
            if (iter->data == sys_default)
                result->sys_default = i;
