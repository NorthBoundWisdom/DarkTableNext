...
static potrace_bitmap_t *_bm_new(const int w, const int h)
{
...
static inline void _scale_point(float p[2], const float xscale, const float yscale, const float cx,
                                const float cy, const float iwidth, const float iheight)
{
    p[0] = ((p[0] * xscale) + cx) / iwidth;
    p[1] = ((p[1] * yscale) + cy) / iheight;
}

static void _add_point(dt_masks_form_t *form, const dt_image_t *const image, const float width,
                       const float height, const float x, const float y, const float ctl1_x,
                       const float ctl1_y, const float ctl2_x, const float ctl2_y)
{
                 ...
                 const double alphamax, GList **out_signs)
{
    ...
    for (const potrace_path_t *p = st->plist; p; p = p->next)
    {
        ...
        // the path is closed, so the last segment wraps back to the start point
        {
            ...
            for (int i = 0; i < n; i++)
            {
                if (cv->tag[i] == POTRACE_CURVETO)
                {
                    ...
                    ps->ctrl2[1] = c0.y;
                    if (image)
                        _scale_point(ps->ctrl2, xsc, ysc, image->crop_x, image->crop_y,
                    ...
                    pe->ctrl1[1] = c1.y;
                    if (image)
                        _scale_point(pe->ctrl1, xsc, ysc, image->crop_x, image->crop_y,
                                     image->width, image->height);
                }
    ...
    if (out_signs)
        *out_signs = g_list_reverse(signs);
    return forms;
}
