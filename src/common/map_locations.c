    ...
    if (name)
    {
        if (g_str_has_prefix(name, location_tag_prefix))
        {
            ...
            DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                        "DELETE FROM data.locations WHERE tagid=?1", -1, &stmt,
                                        NULL);
...
static gboolean _is_point_in_polygon(const dt_geo_map_display_point_t *pt, const gint plg_pts,
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    else
    {
        DT_DEBUG_SQLITE3_BIND_BLOB(stmt, 8, g->polygons->data,
                                   g->plg_pts * (int)sizeof(dt_geo_map_display_point_t),
                                   SQLITE_STATIC);
    }
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (sqlite3_column_int(stmt, 1) == MAP_LOCATION_SHAPE_POLYGONS)
        {
            ...
            {
                const gint plg_pts =
                    sqlite3_column_bytes(stmt2, 0) / sizeof(dt_geo_map_display_point_t);
...
void dt_map_location_update_locations(const dt_imgid_t imgid, const GList *tags)
{
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    for (GList *tag = old_tags; tag; tag = g_list_next(tag))
    {
        ...
        {
            dt_tag_detach(GPOINTER_TO_INT(tag->data), imgid, FALSE, FALSE);
        }
    }
    ...
    for (GList *tag = (GList *)tags; tag; tag = g_list_next(tag))
    {
        ...
        {
            dt_tag_attach(GPOINTER_TO_INT(tag->data), imgid, FALSE, FALSE);
        }
    }
...
gboolean dt_map_location_included(const float lon, const float lat, dt_map_location_data_t *g)
{
    ...
    if ((g->shape == MAP_LOCATION_SHAPE_ELLIPSE &&
        ...
        (g->shape == MAP_LOCATION_SHAPE_RECTANGLE && lon > g->lon - g->delta1 &&
         lon < g->lon + g->delta1 && lat > g->lat - g->delta2 && lat < g->lat + g->delta2))
    {
        included = TRUE;
    }
    return included;
