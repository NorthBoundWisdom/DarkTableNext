    ...
    if (dest)
    {
        ...
        if (ppd)
        {
            ...
            if (attr)
            {
                ...
                sscanf(attr->value, "%lf %lf %lf %lf", &pinfo->hw_margin_left,
                       &pinfo->hw_margin_bottom, &pinfo->hw_margin_right, &pinfo->hw_margin_top);
...
static int _dest_cb(void *user_data, const unsigned flags, cups_dest_t *dest)
{
...
GList *dt_get_papers(const dt_printer_info_t *printer)
{
...

    {
        ...
        if (dest)
        {
            http_t *hcon = cupsConnectDest(dest, 0, 2000, &cancel, resource, sizeof(resource), NULL,
                                           (void *)NULL);
                ...
                for (int k = 0; k < count; k++)
                {
                                                ...
                                                &size))
                    {
                        if (size.width != 0 && size.length != 0 &&
                            !paper_exists(result, size.media))
                        {
                            ...
                            dt_print(DT_DEBUG_PRINT,
                                     "[print] new media paper %4d %6.2f x %6.2f (%s) (%s)", k,
                                     paper->width, paper->height, paper->name, paper->common_name);
                        }
                    }
                }
    ...
    if (ppd)
    {
        ...
        for (int k = 0; k < ppd->num_sizes; k++)
        {
            if (size->width != 0 && size->length != 0 && !paper_exists(result, size->name))
            {
                ...
                dt_print(DT_DEBUG_PRINT, "[print] new ppd paper %4d %6.2f x %6.2f (%s) (%s)", k,
                         paper->width, paper->height, paper->name, paper->common_name);
            }
            size++;
        }
    ...
    if (ppd)
    {
        ...
        if (opt)
        {
            ...
            for (int k = 0; k < opt->num_choices; k++)
            {
                ...
                dt_print(DT_DEBUG_PRINT, "[print] new media %2d (%s) (%s)", k, media->name,
                         media->common_name);
                choice++;
            }
        }
...
void dt_print_file(const dt_imgid_t imgid, const char *filename, const char *job_title,
                   const dt_print_info_t *pinfo)
{
    ...
    {
        dt_control_log(_("file `%s' to print not found for image %d on `%s'"), filename, imgid,
                       pinfo->printer.name);
        return;
    }
    ...
    {
        const char *tp_intent_name[] = {"perception_0", "colorimetric-relative_1", "saturation_1",
    ...
    else
    {
        ...
        for (int j = 0; j < dest->num_options; j++)
            if (cupsGetOption(dest->options[j].name, num_options, options) == NULL)
                num_options = cupsAddOption(dest->options[j].name, dest->options[j].value,
                                            num_options, &options);
        ...
        num_options = cupsAddOption("cm-calibration", *pinfo->printer.profile ? "true" : "false",
                                    num_options, &options);
        ...
        if (*pinfo->printer.profile)
        {
            ...
            num_options = cupsAddOption("AP.ColorMatchingMode", "AP_ApplicationColorMatching",
                                        num_options, &options);
            num_options = cupsAddOption("AP_ColorMatchingMode", "AP_ApplicationColorMatching",
                                        num_options, &options);
        }
#endif
        ...
        if (dt_conf_get_bool("plugins/print/cups/force_landscape"))
            num_options = cupsAddOption("landscape", pinfo->page.landscape ? "true" : "false",
                                        num_options, &options);
    }
    ...
    const int job_id =
        cupsPrintFile(pinfo->printer.name, filename, job_title, num_options, options);
...
void dt_get_print_layout(const dt_print_info_t *prt, const int32_t area_width,
                         const int32_t area_height, float *px, float *py, float *pwidth,
                         float *pheight, float *ax, float *ay, float *awidth, float *aheight,
                         gboolean *borderless)
{
    ...
    *borderless = border_left < np_left || border_right < np_right || border_top < np_top ||
