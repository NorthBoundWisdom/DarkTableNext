...
void dt_grouping_add_to_group(const dt_imgid_t group_id, const dt_imgid_t image_id)
{
    ...
    dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "const char*", "image-group-information-changed", LUA_ASYNC_TYPENAME,
                            "const char*", "add", LUA_ASYNC_TYPENAME, "dt_lua_image_t",
                            GINT_TO_POINTER(image_id), LUA_ASYNC_TYPENAME, "dt_lua_image_t",
                            GINT_TO_POINTER(group_id), LUA_ASYNC_DONE);
#endif
}
...
#ifdef USE_LUA
            dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                                    "const char*", "image-group-information-changed",
                                    LUA_ASYNC_TYPENAME, "const char*", "remove-leader",
                                    LUA_ASYNC_TYPENAME, "dt_lua_image_t", GINT_TO_POINTER(image_id),
                                    LUA_ASYNC_TYPENAME, "dt_lua_image_t",
                                    GINT_TO_POINTER(new_group_id), LUA_ASYNC_DONE);
#endif
        }
        else
        {
    ...
    else
    {
        ...
        if (wimg)
        {
            ...
            dt_lua_async_call_alien(
                ...
                "image-group-information-changed", LUA_ASYNC_TYPENAME, "const char*", "remove",
                LUA_ASYNC_TYPENAME, "dt_lua_image_t", GINT_TO_POINTER(image_id), LUA_ASYNC_TYPENAME,
                "dt_lua_image_t", GINT_TO_POINTER(img_group_id), LUA_ASYNC_DONE);
#endif
        }
    }
    DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_IMAGE_INFO_CHANGED, imgs);
...
dt_imgid_t dt_grouping_change_representative(const dt_imgid_t image_id)
{
    ...
    dt_lua_async_call_alien(dt_lua_event_trigger_wrapper, 0, NULL, NULL, LUA_ASYNC_TYPENAME,
                            "const char*", "image-group-information-changed", LUA_ASYNC_TYPENAME,
                            "const char*", "leader-change", LUA_ASYNC_TYPENAME, "dt_lua_image_t",
                            GINT_TO_POINTER(image_id), LUA_ASYNC_TYPENAME, "dt_lua_image_t",
                            GINT_TO_POINTER(image_id), LUA_ASYNC_DONE);
#endif
