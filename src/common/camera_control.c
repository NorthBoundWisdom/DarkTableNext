...
static void _camera_configuration_single_update(const dt_camctl_t *c, const dt_camera_t *camera,
                                                const char *name);
...
static void _dispatch_camera_error(const dt_camctl_t *c, const dt_camera_t *camera,
...
static gboolean _camera_timeout_job(gpointer data)
{
    ...
    dt_print(DT_DEBUG_CAMCTL, "[camera_control] Calling timeout func for camera %p", cam);
    cam->timeout(cam->gpcam, cam->gpcontext);
    return TRUE;
}

static int _camera_start_timeout_func(Camera *c, const unsigned int timeout, CameraTimeoutFunc func,
                                      void *data)
{
...
static void _camera_stop_timeout_func(Camera *c, const int id, void *data)
{
    dt_camera_t *cam = (dt_camera_t *)data;
    dt_print(DT_DEBUG_CAMCTL, "[camera_control] Removing timeout %d for camera %p", id, cam);
    g_source_remove(id);
    cam->timeout = NULL;
}
...
static void _camera_process_job(const dt_camctl_t *c, const dt_camera_t *camera, gpointer job)
{
    ...
    case _JOB_TYPE_EXECUTE_CAPTURE:
    {
        ...
        if ((res = gp_camera_capture(camera->gpcam, GP_CAPTURE_IMAGE, &fp, c->gpcontext)) == GP_OK)
        {
            ...
            if (handle != -1)
            {
                ...
                if (gp_camera_file_get(camera->gpcam, fp.folder, fp.name, GP_FILE_TYPE_NORMAL,
                                       destination, c->gpcontext) == GP_OK)
                {
                ...
                dt_print(DT_DEBUG_CAMCTL, "[camera_control] failed to download file %s", output);
            g_free(output);
        }
        else
    ...
    case _JOB_TYPE_SET_PROPERTY_INT:
    {
        ...
        if (gp_widget_get_child_by_name(cam->configuration, spj->name, &widget) == GP_OK)
        {
            ...
            if (set_value_succeeds != GP_OK)
            {
                dt_print(DT_DEBUG_CAMCTL, "[camera_control] setting config failed with code %d",
                         set_config_succeeds);
            }
        }
        dt_pthread_mutex_unlock(&cam->config_lock);
    ...
    break;
    default:
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] process of unknown job type 0x%x", j->type);
        break;
    }
...
gboolean dt_camctl_camera_start_live_view(const dt_camctl_t *c)
{
    ...
    dt_pthread_create(&cam->live_view_thread, &_camctl_camera_get_live_view, (void *)camctl);
...
static void _camctl_lock(const dt_camctl_t *c, const dt_camera_t *cam)
{
    ...
    dt_print(DT_DEBUG_CAMCTL, "[camera_control] camera control locked for %s", cam->model);
...
static void _camctl_unlock(const dt_camctl_t *c)
{
    ...
    if (cam)
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] camera control un-locked for %s", cam->model);
    else
...
dt_camctl_t *dt_camctl_new()
{
    ...
    gp_context_set_status_func(camctl->gpcontext, (GPContextStatusFunc)_status_func_dispatch25,
                               camctl);
    gp_context_set_error_func(camctl->gpcontext, (GPContextErrorFunc)_error_func_dispatch25,
                              camctl);
    gp_context_set_message_func(camctl->gpcontext, (GPContextMessageFunc)_message_func_dispatch25,
                                camctl);
...
static void _camctl_camera_destroy(dt_camera_t *cam)
{
    if (!cam)
        return;
    dt_print(DT_DEBUG_CAMCTL, "[camera_control] destroy %s on port %s", cam->model, cam->port);
...
void dt_camctl_register_listener(const dt_camctl_t *c, dt_camctl_listener_t *listener)
{
    ...
    if (g_list_find(camctl->listeners, listener) == NULL)
    {
        ...
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] registering listener %p", listener);
    }
    else
...
static gboolean _camctl_update_cameras(const dt_camctl_t *c)
{
    ...
    {
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] loaded %d port drivers", ports_available);
    ...
    {
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] %d cameras connected", connected_cnt);
    ...
    for (int i = 0; i < gp_list_count(available_cameras); i++)
    {
            ...
            !_have_camera_on_port(testcam, c->unused_cameras))
        {
            ...
            dt_print(DT_DEBUG_CAMCTL, "[camera_control] found new %s on port %s", testcam->model,
                     testcam->port);
        ...
        do
        {
            ...
            else
            {
                if (cam->trymount)
                {
                    ...
                    if (gp_camera_get_summary(camera->gpcam, &camera->summary, c->gpcontext) ==
                        GP_OK)
                    {
        ...
        do
        {
            ...
            if (removed)
            {
                ...
                dt_print(DT_DEBUG_CAMCTL,
                         ...
                         cam->model, cam->port);
                dt_control_log(_("camera `%s' on port `%s' disconnected while mounted"), cam->model,
                               cam->port);
...
static void *_update_cameras_thread(void *ptr)
{
    ...
    while (dt_control_running())
    {
        if (camctl->import_ui == FALSE && dt_view_get_current() == DT_VIEW_LIGHTTABLE)
        {
            ...
            camctl->tickmask =
                3;        // want to be responsive in one sec right after other modes are done
...
static void *_camera_event_thread(void *data)
{
    ...
    dt_print(DT_DEBUG_CAMCTL, "[camera_control] starting camera event thread of context %p", data);
...
static gboolean _camera_initialize(const dt_camctl_t *c, dt_camera_t *cam)
{
    ...
    if (cam->gpcam == NULL)
    {
        ...
        {
            dt_print(DT_DEBUG_CAMCTL,
                     "[camera_control] failed to gp_abilities_list_get_abilities %s", cam->model);
            return FALSE;
        }
        ...
        if (err != GP_OK)
        {
            dt_print(DT_DEBUG_CAMCTL, "[camera_control] failed to gp_camera_set_abilities %s",
                     cam->model);
            return FALSE;
        }
        ...
        if (err != GP_OK)
        {
            dt_print(DT_DEBUG_CAMCTL, "[camera_control] failed to gp_port_info_list_get_info %s",
                     cam->model);
            return FALSE;
        }
        ...
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] %s on port %s initialized", cam->model,
                 cam->port);
    }
    return TRUE;
}
...
void dt_camctl_import(const dt_camctl_t *c, const dt_camera_t *cam, GList *images)
{
    ...
    for (GList *ifile = ifiles; ifile; ifile = g_list_next(ifile))
    {
        ...
        gp_file_new(&camfile);
        if ((res = gp_camera_file_get(cam->gpcam, folder, filename, GP_FILE_TYPE_NORMAL, camfile,
                                      NULL)) < GP_OK)
        {
...
time_t dt_camctl_get_image_file_timestamp(const dt_camctl_t *c, const char *path,
                                          const char *filename)
{
    time_t timestamp = 0;
    if (!path || !filename)
    ...
    CameraFileInfo cfi;
    if (!(gp_camera_file_get_info(c->active_camera->gpcam, path, filename, &cfi, c->gpcontext) ==
          GP_OK))
    {
        dt_print(DT_DEBUG_CAMCTL,
...
static GList *_camctl_recursive_get_list(const dt_camctl_t *c, char *path)
{
    ...
    if (gp_camera_folder_list_files(c->active_camera->gpcam, path, files, c->gpcontext) == GP_OK)
    {
        for (int i = 0; i < gp_list_count(files); i++)
        {
            ...
            // Lets check the type of file...
            CameraFileInfo cfi;
            if (!(gp_camera_file_get_info(c->active_camera->gpcam, path, filename, &cfi,
                                          c->gpcontext) == GP_OK))
            {
                dt_print(DT_DEBUG_CAMCTL,
                         "[camera_control] failed to get file information"
    ...
    CameraList *folders;
    gp_list_new(&folders);
    if (gp_camera_folder_list_folders(c->active_camera->gpcam, path, folders, c->gpcontext) ==
        GP_OK)
    {
...
static GdkPixbuf *_camctl_get_thumbnail(const dt_camctl_t *c, dt_camera_t *cam,
                                        const char *filename)
{
    ...
    // Lets check the type of file...
    CameraFileInfo cfi;
    if (!(gp_camera_file_get_info(c->active_camera->gpcam, folder, fn, &cfi, c->gpcontext) ==
          GP_OK))
    {
        dt_print(DT_DEBUG_CAMCTL,
                 "[camera_control] failed to get file information"
    ...
    if (gotpreview == 0)
    {
        ...
        preview = NULL;
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] failed preview of %s in folder %s", fn, folder);
        return NULL;
    }
...
static void _camera_build_property_menu(CameraWidget *widget, GtkMenu *menu,
    ...
    if ((num_children = gp_widget_count_children(widget)) > 0)
    {
        ...
        for (int i = 0; i < num_children; i++)
        {
            ...
            if (gp_widget_count_children(child) > 0)
            {
                ...
                _camera_build_property_menu(child, GTK_MENU(gtk_menu_item_get_submenu(item)),
                                            item_activate, user_data);
            ...
            else
            {
                ...
                if (type == GP_WIDGET_MENU || type == GP_WIDGET_TEXT || type == GP_WIDGET_RADIO)
                {
                    ...
                    gp_widget_get_name(child, &sk);
                    GtkMenuItem *item = GTK_MENU_ITEM(gtk_menu_item_new_with_label(sk));
                    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(item_activate),
                                     user_data);
                                     ...
                                     const char *property_name)
{
    ...
    if (!cam && (cam = camctl->active_camera) == NULL && (cam = camctl->wanted_camera) == NULL)
    {
        dt_print(DT_DEBUG_CAMCTL, "[camera_control] failed to check if property exists"
                                                      ...
                                                      const char *property_name)
{
    ...
    if (camera->current_choice.widget)
    {
        if (++camera->current_choice.index < gp_widget_count_choices(camera->current_choice.widget))
        {
            ...
            gp_widget_get_choice(camera->current_choice.widget, camera->current_choice.index,
                                 &value);
        }
        else
        {
    ...
    if (gp_camera_wait_for_event(cam->gpcam, 30, &event, &data, c->gpcontext) == GP_OK)
    {
      ...
      camera driver dependent... very ugly! */
            ...
            )
            {
                ...
                falling back to updating by code",
                                 (char *)data);
                    }
                    char code[5];
        ...
        else if (event == GP_EVENT_FILE_ADDED)
        {
            if (cam->is_tethering)
            {
                ...
                {
                    gp_file_new_from_fd(&destination, handle);
                    if (gp_camera_file_get(cam->gpcam, fp->folder, fp->name, GP_FILE_TYPE_NORMAL,
                                           destination, c->gpcontext) == GP_OK)
                    {
                        // Notify listeners of captured image
                                                ...
                                                CameraWidget *new_config, CameraWidget *old_config)
{
    ...
    if (gp_widget_get_name(new_config, &new_config_name) != GP_OK)
        return;
    ...
    else
    {
        ...
        if (gp_widget_get_child_by_name(old_config, new_config_name, &old_config_child) != GP_OK)
            return;
    ...
    */
        ...
        //
        if (new_config_type == GP_WIDGET_MENU || new_config_type == GP_WIDGET_TEXT ||
            new_config_type == GP_WIDGET_RADIO || old_config_type == GP_WIDGET_MENU ||
            old_config_type == GP_WIDGET_TEXT || old_config_type == GP_WIDGET_RADIO)
        {
            ...
            else if (gp_widget_get_value(new_config, &new_config_value) != GP_OK)
                goto end;
            ...
            else if (gp_widget_get_value(old_config_child, &old_config_value) != GP_OK)
                goto end;
