...
static float *_preprocess_image(const uint8_t *rgb_data, const int width, const int height,
                                const gboolean normalize, float *out_scale)
{
    ...
    for (int y = 0; y < new_h; y++)
    {
        ...
        for (int x = 0; x < new_w; x++)
        {
            ...
            for (int c = 0; c < 3; c++)
            {
                ...
                const float pixel =
                    normalize ? (val - IMG_MEAN[c]) / IMG_STD[c] // ImageNet normalization (SAM)
                                :
...
static inline float _bilinear4(const float v00, const float v01, const float v10, const float v11,
                               const float fx, const float fy)
{
    return v00 * (1.0f - fx) * (1.0f - fy) + v01 * fx * (1.0f - fy) + v10 * (1.0f - fx) * fy +
           v11 * fx * fy;
}
...
static void _crop_resize_mask(const float *const restrict src, const int src_w, const int src_h,
                              float *const restrict dst, const int dst_w, const int dst_h,
                              const float scale, const gboolean apply_sigmoid,
                              const uint8_t *const restrict guide_rgb, const int guide_w,
                              const int guide_h, const float sigma_range)
{
    const int valid_w = MIN((int)(dst_w * scale + 0.5f), src_w);
    const int valid_h = MIN((int)(dst_h * scale + 0.5f), src_h);

    const gboolean use_jbu =
        (guide_rgb != NULL && sigma_range > 1e-4f && guide_w == dst_w && guide_h == dst_h);
    ...
    for (int y = 0; y < dst_h; y++)
    {
        ...
        {
            const float sx =
                (dst_w > 1) ? (float)x * (float)(valid_w - 1) / (float)(dst_w - 1) : 0.0f;
            ...
            else
            {
                ...
                const float gx0 =
                    (valid_w > 1) ? (float)x0 * (float)(dst_w - 1) / (float)(valid_w - 1) : 0.0f;
                const float gx1 =
                    (valid_w > 1) ? (float)x1 * (float)(dst_w - 1) / (float)(valid_w - 1) : 0.0f;
                const float gy0 =
                    (valid_h > 1) ? (float)y0 * (float)(dst_h - 1) / (float)(valid_h - 1) : 0.0f;
                const float gy1 =
                    (valid_h > 1) ? (float)y1 * (float)(dst_h - 1) / (float)(valid_h - 1) : 0.0f;
                ...
                val = (wsum > 1e-6f) ? (v00 * w00 + v01 * w01 + v10 * w10 + v11 * w11) / wsum :
...
dt_seg_context_t *dt_seg_load(dt_ai_environment_t *env, const char *model_id)
{
    ...
    dt_ai_context_t *encoder = dt_ai_load_model(env, model_id, "encoder.onnx", enc_provider);
    if (!encoder)
    {
    ...
    dt_ai_context_t *decoder = dt_ai_load_model_ext(
        env, model_id, "decoder.onnx", DT_AI_PROVIDER_CPU, DT_AI_OPT_DISABLED, NULL, 0);
    if (!decoder)
    {
    ...
    {
        dt_print(DT_DEBUG_AI, "[segmentation] unsupported encoder output count %d for %s",
    ...
    {
        ctx->enc_ndims[i] = dt_ai_get_output_shape(encoder, i, ctx->enc_shapes[i], MAX_TENSOR_DIMS);
        if (ctx->enc_ndims[i] <= 0)
        {
            dt_print(DT_DEBUG_AI, "[segmentation] failed to query encoder output %d shape for %s",
                     i, model_id);
            dt_seg_free(ctx);
            return NULL;
        }
    ...
    int off = 0;
    for (int di = 0; di < ctx->n_enc_outputs; di++)
        off += snprintf(map + off, sizeof(map) - off, "%sdec[%d] <- enc[%d]", di ? ", " : "", di,
                        ctx->enc_order[di]);
    dt_print(DT_DEBUG_AI, "[segmentation] tensor routing: %s", map);

    // detect model type from arch field in model registry
    const dt_ai_model_info_t *minfo = dt_ai_get_model_info_by_id(env, model_id);
    ...
    {
        dt_print(DT_DEBUG_AI, "[segmentation] unknown arch '%s' for %s", arch, model_id);
        dt_seg_free(ctx);
        return NULL;
    }
    ...
    if (ctx->model_type == DT_SEG_MODEL_SAM)
    {
        ...
        {
            dt_print(DT_DEBUG_AI, "[segmentation] clamping num_masks from %d to %d", ctx->num_masks,
                     MAX_NUM_MASKS);
        ...
        if (ctx->dec_mask_h <= 0 || ctx->dec_mask_w <= 0)
        {
            ...
            ctx->decoder = dt_ai_load_model_ext(env, model_id, "decoder.onnx", DT_AI_PROVIDER_CPU,
            ...
            const int new_ndim = dt_ai_get_output_shape(decoder, 0, dec_out_shape, MAX_TENSOR_DIMS);
            ...
            dt_print(DT_DEBUG_AI, "[segmentation] after reload: dec_dims=%dx%d, num_masks=%d",
    ...
    else
    {
        ...
        ctx->num_masks = 1;
        ctx->dec_mask_h =
            (dec_out_ndim >= 4 && dec_out_shape[2] > 0) ? (int)dec_out_shape[2] : SAM_INPUT_SIZE;
        ctx->dec_mask_w =
            (dec_out_ndim >= 4 && dec_out_shape[3] > 0) ? (int)dec_out_shape[3] : SAM_INPUT_SIZE;
    ...
    dt_print(DT_DEBUG_AI,
             ...
             model_id, type_name, ctx->n_enc_outputs, ctx->num_masks, ctx->dec_mask_h,
             ctx->dec_mask_w, ctx->prev_mask_dim);
    return ctx;
}
...
void dt_seg_warmup_decoder(dt_seg_context_t *ctx)
{
    ...
    // single dummy decode: one foreground point at the origin, no previous mask
    {
        ...
        for (int i = 0; i < ctx->n_enc_outputs; i++)
        {
            ...
            inputs[ni++] = (dt_ai_tensor_t){.data = use_real ? ctx->enc_data[ei] : dummy_enc[ei],
                                            .type = DT_AI_FLOAT,
                                            .shape = ctx->enc_shapes[ei],
                                            .ndim = ctx->enc_ndims[ei]};
        }

        inputs[ni++] =
            (dt_ai_tensor_t){.data = coords, .type = DT_AI_FLOAT, .shape = coords_shape, .ndim = 3};
        inputs[ni++] =
            (dt_ai_tensor_t){.data = labels, .type = DT_AI_FLOAT, .shape = labels_shape, .ndim = 2};
        ...
        if (wr != 0)
            dt_print(DT_DEBUG_AI, "[segmentation] decoder warmup failed (rc=%d)", wr);
    }
...
cleanup:
    ...
    dt_print(DT_DEBUG_AI, "[segmentation] decoder warmup done in %.3fs%s", dt_get_wtime() - t0,
                             ...
                             const int height)
{
    ...
    if (has_dynamic)
        dt_print(DT_DEBUG_AI, "[segmentation] encoder has dynamic output shapes, "
                              "using ONNX Runtime-allocated outputs");

    dt_ai_tensor_t outputs[MAX_ENCODER_OUTPUTS];
    for (int i = 0; i < ctx->n_enc_outputs; i++)
    {
        outputs[i] = (dt_ai_tensor_t){.data = enc_bufs[i], // NULL for dynamic outputs
                                      .type = DT_AI_FLOAT,
                                      .shape = ctx->enc_shapes[i],
                                      .ndim = ctx->enc_ndims[i]};
    }

    dt_print(DT_DEBUG_AI, "[segmentation] encoding image %dx%d (scale=%.3f)...", width, height,
             scale);
    ...
    for (int i = 0; i < ctx->n_enc_outputs; i++)
    {
        ...
        memcpy(ctx->enc_shapes[i], outputs[i].shape, outputs[i].ndim * sizeof(int64_t));
    }
                           ...
                           int *out_width, int *out_height)
{
    ...
    if (ctx->has_prev_mask)
    {
        ...
        {
            if (ctx->prev_mask[k] < pm_min)
                pm_min = ctx->prev_mask[k];
            if (ctx->prev_mask[k] > pm_max)
                pm_max = ctx->prev_mask[k];
        }
        dt_print(DT_DEBUG_AI,
    ...
    {
        dt_print(DT_DEBUG_AI, "[segmentation] has_prev_mask=0 (no previous mask), n_points=%d",
                 n_points);
    }
    ...
    {
        const int ei = ctx->enc_order[i];
        inputs[ni++] = (dt_ai_tensor_t){.data = ctx->enc_data[ei],
                                        .type = DT_AI_FLOAT,
                                        .shape = ctx->enc_shapes[ei],
                                        .ndim = ctx->enc_ndims[ei]};
    }
    ...
    if (is_sam)
    {
        ...
        dec_outputs[0] =
            (dt_ai_tensor_t){.data = masks, .type = DT_AI_FLOAT, .shape = masks_shape, .ndim = 4};
        dec_outputs[1] =
            (dt_ai_tensor_t){.data = iou_pred, .type = DT_AI_FLOAT, .shape = iou_shape, .ndim = 2};
        n_dec_out = 2;
    ...
    if (is_sam)
    {
        ...
        else
        {
            ...
            memcpy(ctx->prev_mask, masks + (size_t)best * per_mask, per_mask * sizeof(float));
        }
    }
    else
    {
    ...
    _crop_resize_mask(masks + (size_t)best * per_mask, dec_w, dec_h, result, final_w, final_h,
                      mask_scale, is_sam, ctx->encoded_rgb, ctx->encoded_width, ctx->encoded_height,
                      0.1f);
    // SegNext decoder already outputs sigmoid probabilities; SAM outputs logits
    dt_print(DT_DEBUG_AI, "[segmentation] resized mask (%dx%d -> %dx%d, scale=%.4f)", dec_w, dec_h,
             final_w, final_h, mask_scale);
...
void dt_seg_reset_prev_mask(dt_seg_context_t *ctx)
{
    ...
    ctx->has_prev_mask = FALSE;
    if (ctx->prev_mask)
        memset(ctx->prev_mask, 0, (size_t)ctx->prev_mask_dim * ctx->prev_mask_dim * sizeof(float));
}
...
static gboolean _get_cache_dir(char *out, size_t size)
{
    ...
    const gchar *dbpath = darktable.db ? dt_database_get_path(darktable.db) : NULL;
                                ...
                                const int rgb_h)
{
    ...
    if (!fp)
    {
        dt_print(DT_DEBUG_AI, "[segmentation] disk cache: cannot open %s for writing", path);
        return FALSE;
    }
    ...
    const uint32_t mver_len = ctx->model_version ? (uint32_t)strlen(ctx->model_version) : 0;
    ...
    for (int i = 0; i < n_out && ok; i++)
    {
        ...
        ok = ok && fwrite(ctx->enc_shapes[i], sizeof(int64_t), ctx->enc_ndims[i], fp) ==
                       (size_t)ctx->enc_ndims[i];
        const uint64_t data_sz = ctx->enc_sizes[i];
        ok = ok && fwrite(&data_sz, 8, 1, fp) == 1;
        if (data_sz > 0 && ctx->enc_data[i])
            ok = ok && fwrite(ctx->enc_data[i], sizeof(float), data_sz, fp) == data_sz;
    }
    ...
    {
        g_unlink(path);
        dt_print(DT_DEBUG_AI, "[segmentation] disk cache: write error for imgid %d", imgid);
        return FALSE;
    }

    dt_print(DT_DEBUG_AI, "[segmentation] disk cache: saved imgid %d (%dx%d)", imgid,
             ctx->encoded_width, ctx->encoded_height);
                                ...
                                const dt_hash_t distort_hash)
{
    ...
    if (!ok || magic != SEG_CACHE_MAGIC || version != SEG_CACHE_VERSION || file_imgid != imgid ||
        file_distort_hash != distort_hash || !ctx->model_id ||
        strcmp(file_model_id, ctx->model_id) != 0 || strcmp(file_model_ver, cur_ver) != 0)
    {
        fclose(fp);
        return FALSE;
    }
    ...
    if (n_out != ctx->n_enc_outputs || n_out <= 0 || n_out > MAX_ENCODER_OUTPUTS)
    {
        fclose(fp);
        dt_print(DT_DEBUG_AI,
    ...
    for (int i = 0; i < n_out && ok; i++)
    {
        ...
        if (!ok || tmp_ndims[i] <= 0 || tmp_ndims[i] > MAX_TENSOR_DIMS)
        {
            ok = FALSE;
            break;
        }
        ok = ok && fread(tmp_shapes[i], sizeof(int64_t), tmp_ndims[i], fp) == (size_t)tmp_ndims[i];
        ...
        if (ok && data_sz > 0)
        {
            ...
            ok = ok && fread(tmp_data[i], sizeof(float), data_sz, fp) == data_sz;
        }
    }
    ...
    if (!ok)
    {
        for (int i = 0; i < n_out; i++)
            g_free(tmp_data[i]);
        g_free(rgb);
        dt_print(DT_DEBUG_AI, "[segmentation] disk cache: read error for imgid %d", imgid);
        return FALSE;
    }
    ...
    for (int i = 0; i < n_out; i++)
    {
        ...
        memcpy(ctx->enc_shapes[i], tmp_shapes[i], tmp_ndims[i] * sizeof(int64_t));
    }
    ...
    dt_print(DT_DEBUG_AI, "[segmentation] disk cache: loaded imgid %d (%dx%d, rgb=%dx%d)", imgid,
             enc_w, enc_h, rw, rh);
    return TRUE;
}
...
const uint8_t *dt_seg_get_encoded_rgb(const dt_seg_context_t *ctx, int *out_w, int *out_h)
{
    ...
    if (out_w)
        *out_w = rgb ? ctx->encoded_width : 0;
    if (out_h)
        *out_h = rgb ? ctx->encoded_height : 0;
    return rgb;
}
