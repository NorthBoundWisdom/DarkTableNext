...
static void _linear_build_M_boosted(const float input_to_cam[9], float inv_boost,
                                    const float wb_norm[3], float M[9])
{
    for (int k = 0; k < 3; k++)
        for (int i = 0; i < 3; i++)
...
static void _linear_gain_match(const float *tile_in, float *tile_out, size_t per_ch,
                               float *out_gain)
{
...
static void _linear_exposure_boost(const dt_restore_context_t *ctx, float *rgb_planar, size_t plane,
                                   float *out_mean, float *out_boost)
{
...
static void _resolve_linear_wb(const dt_restore_context_t *ctx, const dt_image_t *img,
                               float wb_norm[3])
{
    wb_norm[0] = wb_norm[1] = wb_norm[2] = 1.0f;
    const dt_restore_wb_mode_t mode = ctx ? ctx->wb_mode : DT_RESTORE_WB_AS_SHOT;
...
static gboolean _build_cam_matrices(const dt_restore_context_t *ctx, const dt_image_t *img,
                                    float cam_to_input[9], float input_to_cam[9])
{
    const dt_restore_colorspace_t cs = ctx ? ctx->input_colorspace : DT_RESTORE_CS_LIN_REC2020;
    ...
    const float *xyz_to_input =
        (cs == DT_RESTORE_CS_SRGB_LINEAR) ? xyz_to_srgb_d65 : xyz_to_rec2020_d65;
    const float *input_to_xyz =
        (cs == DT_RESTORE_CS_SRGB_LINEAR) ? srgb_to_xyz_d65 : rec2020_to_xyz_d65;
...
static int _run_demosaic_pipe(const dt_imgid_t imgid, float **out_buf, int *out_w, int *out_h)
{
    dt_develop_t dev;
    ...
    {
        dt_print(DT_DEBUG_AI, "[restore_raw_linear] could not load raw for imgid %d", imgid);
    ...
    {
        dt_print(DT_DEBUG_AI, "[restore_raw_linear] pipe init_export failed (%dx%d)", iw, ih);
        dt_mipmap_cache_release(&mbuf);
        dt_dev_cleanup(&dev);
        return 1;
    }
    ...
    dt_dev_pixelpipe_set_input(&pipe, &dev, (float *)mbuf.buf, iw, ih, mbuf.iscale);
    dt_dev_pixelpipe_create_nodes(&pipe, &dev);
    dt_dev_pixelpipe_synch_all(&pipe, &dev);

    if (!pipe.nodes)
    {
        dt_print(DT_DEBUG_AI, "[restore_raw_linear] pipe has no nodes — aborting");
    ...
    dt_dev_pixelpipe_get_dimensions(&pipe, &dev, iw, ih, &pipe.processed_width,
    ...
    if (!pipe.backbuf || !pipe.backbuf_width || !pipe.backbuf_height)
    {
        ...
        dt_mipmap_cache_release(&mbuf);
        dt_dev_cleanup(&dev);
        dt_print(DT_DEBUG_AI, "[restore_raw_linear] pipe produced no backbuffer");
        return 1;
    }
                          ...
                          int *out_w, int *out_h, float strength, struct _dt_job_t *control_job)
{
    ...
    *out_rgb = NULL;
    ...
    float *rgba = NULL;
    int w = 0, h = 0;
    if (_run_demosaic_pipe(imgid, &rgba, &w, &h))
        return 1;
    ...
    const gboolean matrix_ok = _build_cam_matrices(ctx, &img_meta, cam_to_input, input_to_cam);
    if (!matrix_ok)
    {
        for (int i = 0; i < 9; i++)
            cam_to_input[i] = input_to_cam[i] = (i % 4 == 0) ? 1.0f : 0.0f;
    ...
    dt_print(DT_DEBUG_AI,
             ...
             matrix_ok ? "cam->input from adobe_XYZ_to_CAM" : "identity (no color matrix)");
    ...
    dt_print(DT_DEBUG_AI,
             ...
             "mean=%.4f boost=%.2fx",
             w, h, dbg_min[0], dbg_max[0], dbg_min[1], dbg_max[1], dbg_min[2], dbg_max[2],
    ...
    dt_print(DT_DEBUG_AI, "[restore_raw_linear] tile T=%d step=%d, grid %dx%d (%d tiles)", T, step,
             cols, rows, total_tiles);
    ...
    for (int ty = 0; ty < rows && res == 0; ty++)
    {
        ...
        for (int tx = 0; tx < cols && res == 0; tx++)
        {
            ...
            if (dt_restore_run_patch_3ch_raw(ctx, tile_in, T, T, tile_out) != 0)
            {
                ...
                {
                    dt_print(DT_DEBUG_AI, "[restore_raw_linear] GPU inference failed; "
                                          "retrying on CPU");
                ...
                dt_print(DT_DEBUG_AI, "[restore_raw_linear] inference failed at tile %d,%d (T=%d)",
                         tx, ty, T);
                res = 1;
                break;
            }
            ...
            if (tx == 0 && ty == 0)
                dt_print(DT_DEBUG_AI, "[restore_raw_linear] tile0 match_gain=%.3e", (double)gain);
            ...
            if (has_right)
            {
                ...
                {
                    v_strip_right =
                        g_try_malloc0((size_t)(2 * sensor_O) * v_strip_right_h * 3 * sizeof(float));
                    if (!v_strip_right)
                    {
                        res = 1;
                        break;
            ...
            for (int sr = ext_y0; sr < ext_y1; sr++)
            {
                ...
                const size_t h_strip_row_off = h_strip ? (size_t)(sr - h_strip_sy0) * w : 0;
                ...
                for (int sc = ext_x0; sc < ext_x1; sc++)
                {
                    ...
                    else
                    {
                        for (int k = 0; k < 3; k++)
                        {
                            const float model_v = tile_out[tloc + (size_t)k * per_ch];
                            const float src_v = rgb_src[dst + (size_t)k * plane];
                            rgb_out[dst + (size_t)k * plane] = alpha * model_v + inv_alpha * src_v;
                        }
                    }
                }
            }
...
int dt_restore_raw_linear_preview_piped(dt_restore_context_t *ctx, const dt_image_t *img,
                                        ...
                                        int *out_w, int *out_h)
{
    ...
    *out_denoised_rgb = NULL;
    ...
    while (dt_restore_run_patch_3ch_raw(ctx, tile_in, T, T, tile_out) != 0)
    {
        ...
        {
            dt_print(DT_DEBUG_AI, "[restore_raw_linear] preview GPU inference failed; "
                                  "retrying on CPU");
            dt_control_log(_("AI raw denoise: GPU inference failed, "
                             "falling back to CPU"));
            cpu_fallback_done = TRUE;
        ...
        dt_print(DT_DEBUG_AI, "[restore_raw_linear] preview inference failed at T=%d", T);
        g_free(tile_in);
        g_free(tile_out);
        return 1;
    }
    ...
    for (int py = patch_y0; py < patch_y1; py++)
    {
        ...
        for (int px = patch_x0; px < patch_x1; px++)
        {
            ...
            const float clipped = adc < 0.0f ? 0.0f : (adc > white ? white : adc);
    ...
    int err = dt_restore_run_user_pipe_roi(imgid, patched, raw_w, raw_h, crop_x, crop_y, crop_w,
                                           crop_h, &dw, &dh, out_denoised_rgb);
    g_free(patched);

    if (err == 0)
    {
        err = dt_restore_run_user_pipe_roi(imgid, (void *)mbuf.buf, raw_w, raw_h, crop_x, crop_y,
