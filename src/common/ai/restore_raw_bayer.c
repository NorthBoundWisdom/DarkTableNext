...
static gboolean _bayer_wb_daylight(const dt_image_t *img, float wb[3])
{
    ...
    for (int c = 0; c < 3; c++)
    {
        ...
        mag += fabsf(img->adobe_XYZ_to_CAM[c][0]) + fabsf(img->adobe_XYZ_to_CAM[c][1]) +
...
static int _compute_bayer_prep(const dt_restore_context_t *ctx, const dt_image_t *img,
                               _bayer_prep_t *p)
{
    if (!img || !p)
        return 1;
    ...
    {
        dt_print(DT_DEBUG_AI, "[restore_raw_bayer] unsupported CFA pattern (filters=0x%x)",
                 p->filters);
        return 1;
    }
    ...
    const dt_restore_wb_mode_t mode = ctx ? ctx->wb_mode : DT_RESTORE_WB_DAYLIGHT;
...
static inline float _bayer_remosaic_raw(int r, int c, int ch, float model_val,
...
// diagnostic
static void _bayer_gain_match(const float *tile_in, float *tile_out, int T, double *out_in_mean,
                              double *out_out_mean, float *out_gain)
{
    ...
    const float gain = (fabs(out_mean) > thr) ? (float)(in_mean / out_mean) : 1.0f;
    if (gain != 1.0f)
    {
...
static void _bayer_tile_geometry(const dt_restore_context_t *ctx, const _bayer_prep_t *prep,
                                 int sr0_base, int sc0_base, int width, int height, int vis_y_lo,
                                 int vis_y_hi, int vis_x_lo, int vis_x_hi, int *sr0_origin,
                                 int *sc0_origin, int *mir_y_lo, int *mir_y_hi, int *mir_x_lo,
                                 int *mir_x_hi)
{
    const gboolean force_rggb = !ctx || ctx->bayer_orientation == DT_RESTORE_BAYER_FORCE_RGGB;
    ...
    *sc0_origin = sc0_base + x0;
    ...
    const gboolean cropped_mirror =
        ctx && force_rggb && ctx->edge_pad == DT_RESTORE_EDGE_MIRROR_CROPPED;
    if (cropped_mirror)
    {
        ...
        if ((eff_y_hi - eff_y_lo) & 1)
            eff_y_hi -= 1;
        if ((eff_x_hi - eff_x_lo) & 1)
            eff_x_hi -= 1;
    }
    if (eff_y_lo < 0)
        eff_y_lo = 0;
...
// would see
static void _pack_bayer_tile(const float *cfa, int width, int height, int sr0_origin,
                             int sc0_origin, int mir_y_lo, int mir_y_hi, int mir_x_lo, int mir_x_hi,
                             int T, const _bayer_prep_t *prep, float *tile_in)
{
    ...
    for (int dy = 0; dy < T; dy++)
    {
        ...
        for (int dx = 0; dx < T; dx++)
        {
            ...
            for (int k = 0; k < 4; k++)
            {
                ...
                tile_in[k * tile_in_plane + (size_t)dy * T + dx] = normalized * wb_norm[ch];
            }
        }
    }
}

int dt_restore_raw_bayer(dt_restore_context_t *ctx, const dt_image_t *img, const float *cfa_in,
                         int width, int height, uint16_t *cfa_out, float strength,
                         struct _dt_job_t *control_job)
{
    if (!ctx || !img || !cfa_in || !cfa_out || width <= 0 || height <= 0)
        return 1;

    const float alpha = strength < 0.0f ? 0.0f : (strength > 1.0f ? 1.0f : strength);
    const float inv_alpha = 1.0f - alpha;

    _bayer_prep_t prep;
    if (_compute_bayer_prep(ctx, img, &prep))
        return 1;
    const uint32_t filters = prep.filters;
    ...
    if (Hh <= 0 || Wh <= 0)
        return 0; // too small; output == input
    ...
    dt_print(DT_DEBUG_AI,
             ...
             "%dx%d grid (%d tiles)",
             width, height, img->p_width, img->p_height, img->crop_x, img->crop_y, y0, x0, Wh, Hh,
    ...
    // diagnostic: raw CFA range and preprocessing params
    {
        ...
        dt_print(DT_DEBUG_AI,
                 ...
                 in_min, in_max, black[0], black[1], black[2], black[3], white, img->wb_coeffs[0],
                 img->wb_coeffs[1], img->wb_coeffs[2], img->wb_coeffs[3], wb_norm[0], wb_norm[1],
                 wb_norm[2]);
    }
    ...
    for (int ty = 0; ty < rows && res == 0; ty++)
    {
        ...
        {
            if (control_job && dt_control_job_get_state(control_job) == DT_JOB_STATE_CANCELLED)
            {
                res = 1;
                break;
            }
            ...
            _bayer_tile_geometry(ctx, &prep, 2 * (py_base - O), 2 * (px_base - O), width, height,
                                 vis_y_lo, vis_end_y, vis_x_lo, vis_end_x, &sr0_origin, &sc0_origin,
                                 &mir_y_lo, &mir_y_hi, &mir_x_lo, &mir_x_hi);
            _pack_bayer_tile(cfa_in, width, height, sr0_origin, sc0_origin, mir_y_lo, mir_y_hi,
                             mir_x_lo, mir_x_hi, T, &prep, tile_in);
            ...
            if (dt_restore_run_patch_bayer(ctx, tile_in, T, T, tile_out) != 0)
            {
                ...
                if (tx == 0 && ty == 0 && !cpu_fallback_done && dt_restore_reload_session_cpu(ctx))
                {
                    dt_print(DT_DEBUG_AI, "[restore_raw_bayer] GPU inference failed; "
                                          "retrying on CPU");
                ...
                dt_print(DT_DEBUG_AI, "[restore_raw_bayer] inference failed at tile %d,%d (T=%d)",
                         tx, ty, T);
                res = 1;
                break;
            }
            ...
            if (ctx->output_scale == DT_RESTORE_OUT_MATCH_GAIN)
                _bayer_gain_match(tile_in, tile_out, T, &in_mean, &out_mean, &gain);
            ...
            if (tx == 0 && ty == 0)
            {
                ...
                dt_print(DT_DEBUG_AI,
                         ...
                         mn[0], mx[0], mn[1], mx[1], mn[2], mx[2], in_mean, out_mean, (double)gain);
            }
            ...
            if (has_bot && tx == 0)
                h_strip_bot_sy0 = sensor_py_end - sensor_O;
            ...
            if (has_right)
            {
                ...
                {
                    v_strip_right =
                        g_try_malloc0((size_t)(2 * sensor_O) * v_strip_right_h * sizeof(float));
                    if (!v_strip_right)
                    {
                        res = 1;
                        break;
            ...
            for (int sr = ext_y0; sr < ext_y1; sr++)
            {
                ...
                const float ay =
                    _seam_ay(sr, sensor_py_base, sensor_py_end, sensor_O, has_top, has_bot);
                ...
                const size_t h_strip_row_off = h_strip ? (size_t)(sr - h_strip_sy0) * width : 0;

                for (int sc = ext_x0; sc < ext_x1; sc++)
                {
                    const int mx = 2 * O + (sc - sensor_px_base);
                    const float ax =
                        _seam_ax(sc, sensor_px_base, sensor_px_end, sensor_O, has_left, has_right);
                    const gboolean in_vert_seam = (ax < 1.0f);

                    const int ch = FC(sr, sc, filters); // 0=R, 1=G, 2=B
                    const float model_val = tile_out[(size_t)ch * tile_out_plane + mo_row + mx];
                    const float raw_val = _bayer_remosaic_raw(sr, sc, ch, model_val, &prep);

                    const size_t pidx = (size_t)sr * width + sc;
                    const float blended = alpha * raw_val + inv_alpha * cfa_in[pidx];
                    ...
                    {
                        const float clipped =
                            blended < 0.0f ? 0.0f : (blended > clip_max ? clip_max : blended);
            ...
            if (v_strip_left)
            {
                for (int sr = v_strip_left_sy0; sr < v_strip_left_sy0 + v_strip_left_h; sr++)
                {
                    ...
                    for (int dxs = 0; dxs < 2 * sensor_O; dxs++)
                    {
                        ...
                        const float clipped = v < 0.0f ? 0.0f : (v > clip_max ? clip_max : v);
    ...
    if (res == 0)
    {
        ...
        dt_print(DT_DEBUG_AI,
                 ...
                 (unsigned)omin, (unsigned)omax, n ? (double)osum / n : 0.0, black[0], white);
    }
...
int dt_restore_raw_bayer_preview_piped(dt_restore_context_t *ctx, const dt_image_t *img,
                                       ...
                                       int *out_h)
{
    ...
    *out_denoised_rgb = NULL;
    ...
    if (width <= 0 || height <= 0 || crop_w <= 0 || crop_h <= 0)
        return 1;
    ...
    _bayer_tile_geometry(ctx, &prep, inf_y, inf_x, width, height, pp_vis_y_lo, pp_vis_y_hi,
                         pp_vis_x_lo, pp_vis_x_hi, &pp_sr0, &pp_sc0, &pp_mir_y_lo, &pp_mir_y_hi,
                         &pp_mir_x_lo, &pp_mir_x_hi);
    _pack_bayer_tile(cfa_full, width, height, pp_sr0, pp_sc0, pp_mir_y_lo, pp_mir_y_hi, pp_mir_x_lo,
                     pp_mir_x_hi, T, &prep, tile_in);
    ...
    while (dt_restore_run_patch_bayer(ctx, tile_in, T, T, tile_out) != 0)
    {
        ...
        {
            dt_print(DT_DEBUG_AI, "[restore_raw_bayer] preview GPU inference failed; "
                                  "retrying on CPU");
            dt_control_log(_("AI raw denoise: GPU inference failed, "
                             "falling back to CPU"));
            cpu_fallback_done = TRUE;
        ...
        dt_print(DT_DEBUG_AI, "[restore_raw_bayer] preview inference failed at T=%d", T);
        g_free(tile_in);
        g_free(tile_out);
        return 1;
    }
    ...
    {
        dt_print(DT_DEBUG_AI, "[restore_raw_bayer] preview_piped: unsupported raw datatype %d",
    ...
    for (int sr = patch_y0; sr < patch_y1; sr++)
    {
        ...
        for (int sc = patch_x0; sc < patch_x1; sc++)
        {
            ...
            const float model_val = tile_out[(size_t)ch * tile_out_plane + mo_row + (sc - pp_sc0)];
            const float raw_val = _bayer_remosaic_raw(sr, sc, ch, model_val, &prep);
            const float clipped = raw_val < 0.0f ? 0.0f : (raw_val > clip_max ? clip_max : raw_val);
    ...
    int err = dt_restore_run_user_pipe_roi(imgid, patched, width, height, crop_x, crop_y, crop_w,
                                           crop_h, &dw, &dh, out_denoised_rgb);
    g_free(patched);
    ...
    if (err == 0)
    {
        err = dt_restore_run_user_pipe_roi(imgid, (void *)mbuf.buf, width, height, crop_x, crop_y,
                                           crop_w, crop_h, &bw, &bh, out_before_rgb);
    }
    ...
    if (err || dw != bw || dh != bh)
    {
        ...
        if (dw != bw || dh != bh)
            dt_print(DT_DEBUG_AI,
                     ...
                     bw, bh, dw, dh);
        g_free(*out_before_rgb);
        *out_before_rgb = NULL;
        g_free(*out_denoised_rgb);
        *out_denoised_rgb = NULL;
        return 1;
    }
    if (out_w)
