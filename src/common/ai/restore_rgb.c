...
extern void dwt_denoise(float *buf, int width, int height, int bands, const float *noise);
...
static void _compute_adaptive_noise(const float *const restrict buf, const size_t npix,
...
void dt_restore_set_profile(dt_restore_context_t *ctx, void *profile)
{
    ...
    if (!dt_colorspaces_get_primaries_and_whitepoint_from_profile((cmsHPROFILE)profile, primaries,
                                                                  whitepoint))
    {
        dt_print(DT_DEBUG_AI, "[restore_rgb] could not read primaries from working profile, "
    ...
    dt_make_transposed_matrices_from_primaries_and_whitepoint(primaries, whitepoint, wp_to_xyz_T);
    float wp_to_xyz[9];
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
    ...
    {
        dt_print(DT_DEBUG_AI, "[restore_rgb] singular WP->XYZ matrix, falling back to gamma-only");
        ctx->has_profile = FALSE;
        return;
    }
...
{
    if (ctx)
        ctx->preserve_wide_gamut = preserve;
}
                         ...
                         float *out_patch, int scale)
{
    ...
    const gboolean need_gamut_mask = ctx->has_profile && scale == 1 && ctx->preserve_wide_gamut;
    if (need_gamut_mask)
    {
    ...
    if (ctx->has_profile)
    {
        ...
        for (size_t p = 0; p < plane; p++)
        {
            ...
            if (in_gamut_mask)
            {
                ...
                in_gamut_mask[p] = (sr >= -m && sr <= 1.0f + m && sg >= -m && sg <= 1.0f + m &&
                                    sb >= -m && sb <= 1.0f + m) ?
                                       1 :
                                       0;
            }
    ...
    inputs[0] = (dt_ai_tensor_t){
        .data = (void *)srgb_in, .shape = input_shape, .ndim = 4, .type = DT_AI_FLOAT};
    ...
    if (num_inputs >= 2)
    {
        ...
        inputs[1] = (dt_ai_tensor_t){
            .data = (void *)noise_map, .shape = noise_shape, .ndim = 4, .type = DT_AI_FLOAT};
    }
    ...
    int ret = dt_ai_run(ctx->ai_ctx, inputs, num_inputs, &output, 1);
    g_free(srgb_in);
    g_free(noise_map);
    if (ret != 0)
    {
...
static gboolean _image_has_deep_shadows(const float *in_data, int w, int h)
{
    ...
    for (size_t y = 0; y < (size_t)h; y += stride)
        for (size_t x = 0; x < (size_t)w; x += stride)
        {
            ...
            const float luma =
                0.2126f * in_data[p] + 0.7152f * in_data[p + 1] + 0.0722f * in_data[p + 2];
            if (luma < _SHADOW_BOOST_THRESHOLD)
                dark++;
            total++;
        }
                             ...
                             struct _dt_job_t *control_job)
{
    ...
    if (ctx->shadow_boost_capable)
    {
        ...
        dt_print(DT_DEBUG_AI, "[restore_rgb] shadow boost %s", dark ? "enabled" : "disabled");
    }
    ...
    for (int ty = 0; ty < rows; ty++)
    {
        ...
        for (int tx = 0; tx < cols; tx++)
        {
            ...
            if (dt_restore_run_patch(ctx, tile_in, T, T, tile_out, S) != 0)
            {
                ...
                {
                    dt_print(DT_DEBUG_AI, "[restore_rgb] GPU inference failed; retrying on CPU");
                ...
                dt_print(DT_DEBUG_AI, "[restore_rgb] inference failed at tile %d,%d (T=%d)", tx, ty,
                         T);
                res = 1;
                goto cleanup;
            }
        ...
        for (int dy = 0; dy < valid_h_out; dy++)
        {
            ...
            if (row_writer(src, out_w, y * S + dy, writer_data) != 0)
            {
                res = 1;
                goto cleanup;
            }
...
#pragma omp parallel for simd default(none)                                                        \
    dt_omp_firstprivate(original_4ch, denoised_4ch, lum_residual, npix) schedule(simd : static)    \
    aligned(original_4ch, denoised_4ch, lum_residual : 64)
#endif
    for (size_t i = 0; i < npix; i++)
    {
        const size_t p = i * 4;
        const float lum_orig = 0.2126f * original_4ch[p + 0] + 0.7152f * original_4ch[p + 1] +
                               0.0722f * original_4ch[p + 2];
        const float lum_den = 0.2126f * denoised_4ch[p + 0] + 0.7152f * denoised_4ch[p + 1] +
    ...
    dwt_denoise(lum_residual, width, height, DWT_DETAIL_BANDS, noise);

#ifdef _OPENMP
#pragma omp parallel for simd default(none)                                                        \
    dt_omp_firstprivate(denoised_4ch, lum_residual, npix, alpha) schedule(simd : static)           \
                                     ...
                                     int height)
{
    ...
    {
        const size_t si = i * 3;
        const float lum_orig = 0.2126f * before_3ch[si + 0] + 0.7152f * before_3ch[si + 1] +
                               0.0722f * before_3ch[si + 2];
        const float lum_den =
            0.2126f * after_3ch[si + 0] + 0.7152f * after_3ch[si + 1] + 0.0722f * after_3ch[si + 2];
        lum_residual[i] = lum_orig - lum_den;
    }
