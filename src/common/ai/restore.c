...
static dt_restore_input_kind_t _parse_input_kind(const char *s)
{
    ...
    if (!g_strcmp0(s, "bayer_v1"))
        return DT_RESTORE_INPUT_KIND_BAYER_V1;
    if (!g_strcmp0(s, "xtrans_v1"))
        return DT_RESTORE_INPUT_KIND_XTRANS_V1;
    if (!g_strcmp0(s, "linear_v1"))
        return DT_RESTORE_INPUT_KIND_LINEAR_V1;
    return DT_RESTORE_INPUT_KIND_UNKNOWN;
}
...
static const char *_input_kind_name(dt_restore_input_kind_t k)
{
    switch (k)
    {
    case DT_RESTORE_INPUT_KIND_BAYER_V1:
        return "bayer_v1";
    case DT_RESTORE_INPUT_KIND_XTRANS_V1:
        return "xtrans_v1";
    case DT_RESTORE_INPUT_KIND_LINEAR_V1:
        return "linear_v1";
    default:
        return "unknown";
    }
...
static dt_restore_colorspace_t _parse_colorspace(const char *s, dt_restore_colorspace_t dflt)
{
    if (!s)
        return dflt;
    if (!g_strcmp0(s, "lin_rec2020"))
        return DT_RESTORE_CS_LIN_REC2020;
    if (!g_strcmp0(s, "camRGB"))
        return DT_RESTORE_CS_CAMRGB;
    if (!g_strcmp0(s, "srgb_linear"))
        return DT_RESTORE_CS_SRGB_LINEAR;
    dt_print(DT_DEBUG_AI, "[restore] unknown input_colorspace '%s', using default", s);
    return dflt;
}

static dt_restore_wb_mode_t _parse_wb_mode(const char *s, dt_restore_wb_mode_t dflt)
{
    if (!s)
        return dflt;
    if (!g_strcmp0(s, "daylight"))
        return DT_RESTORE_WB_DAYLIGHT;
    if (!g_strcmp0(s, "as_shot"))
        return DT_RESTORE_WB_AS_SHOT;
    if (!g_strcmp0(s, "none"))
        return DT_RESTORE_WB_NONE;
    dt_print(DT_DEBUG_AI, "[restore] unknown wb_norm '%s', using default", s);
    return dflt;
}

static dt_restore_output_scale_t _parse_output_scale(const char *s, dt_restore_output_scale_t dflt)
{
    if (!s)
        return dflt;
    if (!g_strcmp0(s, "match_gain"))
        return DT_RESTORE_OUT_MATCH_GAIN;
    if (!g_strcmp0(s, "absolute"))
        return DT_RESTORE_OUT_ABSOLUTE;
    dt_print(DT_DEBUG_AI, "[restore] unknown output_scale '%s', using default", s);
    return dflt;
}

static dt_restore_bayer_orientation_t _parse_bayer_orientation(const char *s,
                                                               dt_restore_bayer_orientation_t dflt)
{
    if (!s)
        return dflt;
    if (!g_strcmp0(s, "force_rggb"))
        return DT_RESTORE_BAYER_FORCE_RGGB;
    if (!g_strcmp0(s, "native"))
        return DT_RESTORE_BAYER_NATIVE;
    dt_print(DT_DEBUG_AI, "[restore] unknown bayer_orientation '%s', using default", s);
    return dflt;
}

static dt_restore_edge_pad_t _parse_edge_pad(const char *s, dt_restore_edge_pad_t dflt)
{
    if (!s)
        return dflt;
    if (!g_strcmp0(s, "mirror_cropped"))
        return DT_RESTORE_EDGE_MIRROR_CROPPED;
    if (!g_strcmp0(s, "mirror"))
        return DT_RESTORE_EDGE_MIRROR;
    dt_print(DT_DEBUG_AI, "[restore] unknown edge_pad '%s', using default", s);
    return dflt;
}
...
static float _parse_target_mean(const dt_ai_model_info_t *info, const char *key, float dflt)
{
    ...
    {
        dt_print(DT_DEBUG_AI, "[restore] target_mean '%s' not parseable, using default", s);
        g_free(s);
        return dflt;
    }
    g_free(s);
...
static int _resolve_tile_size(const dt_ai_model_info_t *info, const char *stem)
{
    int n = 0;
    int *sizes = NULL;
    if (stem)
...
static dt_restore_context_t *_load(dt_restore_env_t *env, const char *task, const char *stem,
                                   const char *default_file, dt_restore_input_kind_t expected_kind,
                                   int scale)
{
    if (!env)
        return NULL;
    ...
    const dt_ai_model_info_t *info = dt_ai_get_model_info_by_id(env->ai_env, model_id);

    char *model_file = stem ? g_strdup_printf("%s.onnx", stem) : g_strdup(default_file);
    ...
    if (stem)
    {
        ...
        if (expected_kind != DT_RESTORE_INPUT_KIND_UNKNOWN)
        {
            ...
            const gboolean mismatch = !missing && declared != expected_kind;
            if (mismatch || (!missing && declared == DT_RESTORE_INPUT_KIND_UNKNOWN))
            {
                dt_print(DT_DEBUG_AI,
                         ...
                         model_id, stem, input_kind, _input_kind_name(expected_kind));
                dt_control_log(_("raw denoise model %s: incompatible input_kind"), model_id);
                g_free(model_file);
                g_free(input_kind);
                g_free(cs_str);
                g_free(wb_str);
        ...
        dt_print(DT_DEBUG_AI, "[restore] '%s': file=%s input_kind=%s", stem, model_file,
    ...
    if (tile_size <= 0)
    {
        dt_print(DT_DEBUG_AI,
                 ...
                 model_id, stem ? " stem=" : "", stem ? stem : "");
        dt_control_log(_("AI model %s: missing input_sizes in manifest"), model_id);
        g_free(model_file);
        g_free(input_kind);
        g_free(cs_str);
        g_free(wb_str);
    ...
    dt_ai_context_t *ai_ctx = dt_ai_load_model_ext(
        env->ai_env, model_id, model_file, DT_AI_PROVIDER_CONFIGURED, DT_AI_OPT_ALL, NULL, 0);
    if (!ai_ctx)
    {
        g_free(model_id);
        g_free(model_file);
    ...
    ctx->input_kind_enum = expected_kind;
    {
        ...
        // manifest if so
        const gboolean demosaic_pipeline = (expected_kind == DT_RESTORE_INPUT_KIND_LINEAR_V1) ||
        ...
        char *k_tm = stem ? g_strdup_printf("%s.target_mean", stem) : NULL;
        ctx->target_mean = k_tm ? _parse_target_mean(info, k_tm, default_tm) : default_tm;
        g_free(k_tm);
        ...
        const dt_restore_bayer_orientation_t default_bo =
            (expected_kind == DT_RESTORE_INPUT_KIND_BAYER_V1) ? DT_RESTORE_BAYER_FORCE_RGGB :
    ...
    ctx->shadow_boost_capable = dt_ai_model_attribute_bool(info, "shadow_boost");
    ctx->shadow_boost = ctx->shadow_boost_capable;
    if (ctx->shadow_boost_capable)
        dt_print(DT_DEBUG_AI, "[restore] model %s declares shadow_boost attribute", model_id);
    return ctx;
}

dt_restore_context_t *dt_restore_load_denoise(dt_restore_env_t *env)
{
    return _load(env, TASK_DENOISE, NULL, NULL, DT_RESTORE_INPUT_KIND_UNKNOWN, 1);
}
...
dt_restore_sensor_class_t dt_restore_classify_sensor(const dt_image_t *img)
{
    ...
    if (filters == 9u)
        return DT_RESTORE_SENSOR_CLASS_XTRANS;
    if (filters != 0u)
        return DT_RESTORE_SENSOR_CLASS_BAYER;
    return DT_RESTORE_SENSOR_CLASS_LINEAR;
}
...
dt_restore_context_t *dt_restore_load_rawdenoise_bayer(dt_restore_env_t *env)
{
    ...
    return _load(env, TASK_RAWDENOISE, "model_bayer", NULL, DT_RESTORE_INPUT_KIND_BAYER_V1, 1);
}
...
dt_restore_context_t *dt_restore_load_rawdenoise_linear(dt_restore_env_t *env)
{
    ...
    return _load(env, TASK_RAWDENOISE, "model_linear", NULL, DT_RESTORE_INPUT_KIND_LINEAR_V1, 1);
}
...
dt_restore_context_t *dt_restore_load_rawdenoise_xtrans(dt_restore_env_t *env)
{
    ...
    dt_restore_context_t *ctx =
        _load(env, TASK_RAWDENOISE, "model_xtrans", NULL, DT_RESTORE_INPUT_KIND_XTRANS_V1, 1);
    if (!ctx)
        ctx = _load(env, TASK_RAWDENOISE, "model_linear", NULL, DT_RESTORE_INPUT_KIND_LINEAR_V1, 1);
    return ctx;
}
...
dt_restore_context_t *dt_restore_load_upscale_x2(dt_restore_env_t *env)
{
    ...
    return _load(env, TASK_UPSCALE, "model_x2", NULL, DT_RESTORE_INPUT_KIND_UNKNOWN, 2);
}

dt_restore_context_t *dt_restore_load_upscale_x4(dt_restore_env_t *env)
{
    return _load(env, TASK_UPSCALE, "model_x4", NULL, DT_RESTORE_INPUT_KIND_UNKNOWN, 4);
}
...
static gboolean _model_available(dt_restore_env_t *env, const char *task)
{
    if (!env || !env->ai_env)
        return FALSE;
    char *model_id = dt_ai_models_get_active_for_task(task);
...
gboolean dt_restore_reload_session_cpu(dt_restore_context_t *ctx)
{
    ...
    dt_ai_context_t *new_ctx =
        dt_ai_load_model_ext(ctx->env->ai_env, ctx->model_id, ctx->model_file, DT_AI_PROVIDER_CPU,
                             DT_AI_OPT_ALL, NULL, 0);
    if (!new_ctx)
    {
        dt_print(DT_DEBUG_AI, "[restore] CPU fallback session load failed for %s", ctx->model_id);
        return FALSE;
    }
    ctx->ai_ctx = new_ctx;
    return TRUE;
                                 ...
                                 float **out_rgb)
{
    ...
    if (out_h)
        *out_h = 0;
    if (!input_native || iw <= 0 || ih <= 0 || roi_w <= 0 || roi_h <= 0)
        return 1;
    ...
    dt_dev_pixelpipe_set_icc(&pipe, DT_COLORSPACE_LIN_REC709, NULL, DT_INTENT_PERCEPTUAL);

    dt_ioppr_resync_modules_order(&dev);
    dt_dev_pixelpipe_set_input(&pipe, &dev, (float *)input_native, iw, ih, 1.0f);
    ...
    float corners[8] = {
        (float)roi_x, (float)roi_y,           (float)(roi_x + roi_w), (float)roi_y,
        (float)roi_x, (float)(roi_y + roi_h), (float)(roi_x + roi_w), (float)(roi_y + roi_h),
    };
    dt_dev_distort_transform_plus(&dev, &pipe, 0.0, DT_DEV_TRANSFORM_DIR_ALL_GEOMETRY, corners, 4);
    ...
    if (pipe_roi_x < 0)
    {
        pipe_roi_w += pipe_roi_x;
        pipe_roi_x = 0;
    }
    if (pipe_roi_y < 0)
    {
        pipe_roi_h += pipe_roi_y;
        pipe_roi_y = 0;
    }
    if (pipe_roi_x + pipe_roi_w > pw)
        pipe_roi_w = pw - pipe_roi_x;
    if (pipe_roi_y + pipe_roi_h > ph)
        pipe_roi_h = ph - pipe_roi_y;
    ...
    dt_dev_pixelpipe_process_no_gamma(&pipe, &dev, pipe_roi_x, pipe_roi_y, pipe_roi_w, pipe_roi_h,
                                      1.0f);
    ...
    if (bw != pipe_roi_w || bh != pipe_roi_h)
        dt_print(DT_DEBUG_AI, "[restore] pipe ROI %dx%d -> backbuf %dx%d", pipe_roi_w, pipe_roi_h,
                 bw, bh);
