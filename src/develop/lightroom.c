...
typedef struct dt_iop_vignette_params_t
{
    ...
    float
        falloff_scale; // 0 - 100 Radius for falloff -- outer radius = inner radius + falloff_scale
...
static float get_interpolate(lr2dt_t lr2dt_table[], const float value)
{
    ...
    while (lr2dt_table[k + 1].lr < value)
        k++;
...
static void dt_add_hist(const dt_imgid_t imgid, const char *operation,
                        const dt_iop_params_t *params, const int params_size, char *imported,
                        const size_t imported_len, const int version, int *import_count)
{
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
                                "SELECT COUNT(*) FROM main.history WHERE imgid = ?1", -1, &stmt,
                                NULL);
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    DT_DEBUG_SQLITE3_BIND_BLOB(stmt, 6, &blend_params, sizeof(dt_lr_develop_blend_params_t),
                               SQLITE_TRANSIENT);
  ...
  DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
    ...
    if (imported[0])
        g_strlcat(imported, ", ", imported_len);
                  ...
                  const xmlChar *name, const xmlChar *value, const xmlNodePtr node, lr_data_t *data)
{
    ...
    if (value)
    {
        ...
        else if (!xmlStrcmp(name, (const xmlChar *)"Orientation"))
        {
            ...
            if (dev != NULL && ((dev->image_storage.orientation == ORIENTATION_NONE &&
                                 data->orientation != EXIF_ORIENTATION_NONE) ||
                                (dev->image_storage.orientation == ORIENTATION_ROTATE_CW_90_DEG &&
                                 data->orientation != EXIF_ORIENTATION_ROTATE_CW_90_DEG) ||
                                (dev->image_storage.orientation == ORIENTATION_ROTATE_CCW_90_DEG &&
                                 data->orientation != EXIF_ORIENTATION_ROTATE_CCW_90_DEG)))
                data->has_flip = TRUE;
        }
        ...
        else if (!xmlStrcmp(name, (const xmlChar *)"GrainFrequency"))
        {
            const int v = atoi((char *)value);
            if (v != 0)
                data->pg.scale = lr2dt_grain_frequency((float)v);
        }
                        ...
                        !xmlStrcmp(name, (const xmlChar *)"hierarchicalSubject")))
    {
        ...
        while (tagNode)
        {
            if (!xmlStrcmp(tagNode->name, (const xmlChar *)"li"))
            {
                ...
                guint tagid = 0;
                if (!dt_tag_exists((char *)cvalue, &tagid))
                    dt_tag_new((char *)cvalue, &tagid);
    ...
    else if (dev != NULL && !xmlStrcmp(name, (const xmlChar *)"RetouchInfo"))
    {
        ...
        while (riNode)
        {
            if (!xmlStrcmp(riNode->name, (const xmlChar *)"li"))
            {
                ...
                if (_read_float(&startptr, "centerX", &x) && _skip_comma(&startptr) &&
                    _read_float(&startptr, "centerY", &y) && _skip_comma(&startptr) &&
                    _read_float(&startptr, "radius", &radius) && _skip_comma(&startptr) &&
                    _skip_key_value_pair(&startptr, "sourceState") && _skip_comma(&startptr) &&
                    _read_float(&startptr, "sourceX", &xc) && _skip_comma(&startptr) &&
            ...
            if (data->ps.num_spots == MAX_SPOTS)
                break;
            riNode = riNode->next;
        }
    }
    else if (dev != NULL && !xmlStrcmp(name, (const xmlChar *)"ToneCurvePV2012"))
    {
        ...
        while (tcNode)
        {
            if (!xmlStrcmp(tcNode->name, (const xmlChar *)"li"))
            {
                ...
                if (sscanf((const char *)cvalue, "%d, %d", &(data->curve_pts[data->n_pts][0]),
                           &(data->curve_pts[data->n_pts][1])))
                    data->n_pts++;
                xmlFree(cvalue);
            }
            if (data->n_pts == MAX_PTS)
                break;
            tcNode = tcNode->next;
        }
    }
...
{
    return !strcmp(name, "subject") || !strcmp(name, "hierarchicalSubject") ||
           !strcmp(name, "RetouchInfo") || !strcmp(name, "ToneCurvePV2012") ||
           !strcmp(name, "title") || !strcmp(name, "description") || !strcmp(name, "creator") ||
           !strcmp(name, "publisher") || !strcmp(name, "rights");
};

/* handle a specific xpath */
static void _handle_xpath(const dt_develop_t *dev, xmlDoc *doc, const dt_imgid_t imgid,
                          xmlXPathContext *ctx, const xmlChar *xpath, lr_data_t *data)
{
    ...
    if (xpathObj != NULL)
    {
        ...
        for (int k = 0; k < n; k++)
        {
            ...
            if (_has_list((char *)node->name))
            {
                ...
                if (listnode)
                    listnode = listnode->xmlChildrenNode;
                if (listnode)
                    listnode = listnode->next;
                if (listnode)
...
gboolean dt_lightroom_import(dt_imgid_t imgid, dt_develop_t *dev, gboolean iauto)
{
    ...
    {
        if (!iauto)
            dt_control_log(_("`%s' is not a Lightroom XMP!"), pathname);
        g_free(pathname);
        return FALSE;
    }
    ...
    xmlXPathRegisterNs(xpathCtx, BAD_CAST "stEvt",
                       BAD_CAST "http://ns.adobe.com/xap/1.0/sType/ResourceEvent#");

    xmlXPathObjectPtr xpathObj =
        xmlXPathEvalExpression((const xmlChar *)"//@stEvt:softwareAgent", xpathCtx);
    ...
    xmlXPathRegisterNs(xpathCtx, BAD_CAST "crs",
    ...
    if (dev != NULL && is_lr && dt_image_is_raw(&dev->image_storage))
    {
        ...
        dt_iop_colorin_params_v1_t pci =
            (dt_iop_colorin_params_v1_t){"cmatrix", DT_INTENT_PERCEPTUAL};

        dt_add_hist(imgid, "colorin", (dt_iop_params_t *)&pci, sizeof(dt_iop_colorin_params_v1_t),
                    imported, sizeof(imported), LRDT_COLORIN_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    if (dev != NULL && data.has_crop)
    {
        ...
        data.pc.k_apply = 0;
        data.pc.crop_auto =
            0; // Cannot use crop-auto=1 (the default at clipping GUI), as it does not allow to cover all cropping cases.
        ...
        if (orientation == ORIENTATION_FLIP_HORIZONTALLY ||
            orientation == ORIENTATION_FLIP_VERTICALLY || orientation == ORIENTATION_TRANSPOSE ||
            orientation == ORIENTATION_TRANSVERSE)
            data.pc.angle = -data.pc.angle;

        data.fratio = (data.pc.cw - data.pc.cx) / (data.pc.ch - data.pc.cy);

        dt_add_hist(imgid, "clipping", (dt_iop_params_t *)&data.pc,
                    sizeof(dt_iop_clipping_params_t), imported, sizeof(imported),
                    LRDT_CLIPPING_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    if (dev != NULL && data.has_flip)
    {
        ...
        dt_add_hist(imgid, "flip", (dt_iop_params_t *)&data.pf, sizeof(dt_iop_flip_params_t),
                    imported, sizeof(imported), LRDT_FLIP_VERSION, &n_import);
        refresh_needed = TRUE;
    }

    if (dev != NULL && data.has_exposure)
    {
        dt_add_hist(imgid, "exposure", (dt_iop_params_t *)&data.pe,
                    sizeof(dt_iop_exposure_params_t), imported, sizeof(imported),
                    LRDT_EXPOSURE_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    if (dev != NULL && data.has_vignette)
    {
        ...
        if (data.has_crop)
            data.pv.whratio = data.pv.whratio * data.fratio;
        ...
        {
            float newratio =
                data.pv.whratio - (data.pv.whratio - 1) * (data.crop_roundness / 100.0);
        ...
        dt_add_hist(imgid, "vignette", (dt_iop_params_t *)&data.pv,
                    sizeof(dt_iop_vignette_params_t), imported, sizeof(imported),
                    LRDT_VIGNETTE_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    if (dev != NULL && data.has_spots)
    {
        ...
        dt_add_hist(imgid, "spots", (dt_iop_params_t *)&data.ps, sizeof(dt_iop_spots_params_t),
                    imported, sizeof(imported), LRDT_SPOTS_VERSION, &n_import);
        refresh_needed = TRUE;
    }

    if (dev != NULL && (data.curve_kind != linear || data.ptc_value[0] != 0 ||
                        data.ptc_value[1] != 0 || data.ptc_value[2] != 0 || data.ptc_value[3] != 0))
    {
        ...
        // linear a, b curves
        for (int k = 0; k < 7; k++)
            data.ptc.tonecurve[ch_a][k].x = linear_ab[k];
        for (int k = 0; k < 7; k++)
            data.ptc.tonecurve[ch_a][k].y = linear_ab[k];
        for (int k = 0; k < 7; k++)
            data.ptc.tonecurve[ch_b][k].x = linear_ab[k];
        for (int k = 0; k < 7; k++)
            data.ptc.tonecurve[ch_b][k].y = linear_ab[k];
        ...
        if (data.curve_kind == linear)
        {
            ...
            data.ptc.tonecurve[ch_L][2].x =
                data.ptc_split[1] - (data.ptc_split[1] - data.ptc_split[0]) / 2.0;
            data.ptc.tonecurve[ch_L][2].y =
                data.ptc_split[1] - (data.ptc_split[1] - data.ptc_split[0]) / 2.0;
            data.ptc.tonecurve[ch_L][3].x =
                data.ptc_split[1] + (data.ptc_split[2] - data.ptc_split[1]) / 2.0;
            data.ptc.tonecurve[ch_L][3].y =
                data.ptc_split[1] + (data.ptc_split[2] - data.ptc_split[1]) / 2.0;
        ...
        if (data.curve_kind != custom)
        {
            ...
            data.ptc.tonecurve[ch_L][1].y +=
                data.ptc.tonecurve[ch_L][1].y * ((float)data.ptc_value[0] / 100.0);
            data.ptc.tonecurve[ch_L][2].y +=
                data.ptc.tonecurve[ch_L][2].y * ((float)data.ptc_value[1] / 100.0);
            data.ptc.tonecurve[ch_L][3].y +=
                data.ptc.tonecurve[ch_L][3].y * ((float)data.ptc_value[2] / 100.0);
            data.ptc.tonecurve[ch_L][4].y +=
                data.ptc.tonecurve[ch_L][4].y * ((float)data.ptc_value[3] / 100.0);
        ...
        dt_add_hist(imgid, "tonecurve", (dt_iop_params_t *)&data.ptc,
                    sizeof(dt_iop_tonecurve_params_t), imported, sizeof(imported),
                    LRDT_TONECURVE_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    if (dev != NULL && data.has_colorzones)
    {
        ...
        dt_add_hist(imgid, "colorzones", (dt_iop_params_t *)&data.pcz,
                    sizeof(dt_iop_colorzones_params_t), imported, sizeof(imported),
                    LRDT_COLORZONES_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    if (dev != NULL && data.has_splittoning)
    {
        ...
        dt_add_hist(imgid, "splittoning", (dt_iop_params_t *)&data.pst,
                    sizeof(dt_iop_splittoning_params_t), imported, sizeof(imported),
                    LRDT_SPLITTONING_VERSION, &n_import);
        refresh_needed = TRUE;
    }
    ...
    {
        if (imported[0])
            g_strlcat(imported, ", ", sizeof(imported));
    ...
    {
        dt_control_log(ngettext("%s has been imported", "%s have been imported", n_import),
                       imported);
