...
#define CONF_OBJECT_REFINE_BOUNDARY_ITER_KEY                                                       \
    "plugins/darkroom/masks/object/refine_boundary_iterations"
#define CONF_OBJECT_REFINE_BOUNDARY_SIGMA_COLOR_KEY                                                \
    "plugins/darkroom/masks/object/refine_boundary_sigma_color"
#define CONF_OBJECT_REFINE_BOUNDARY_W_BILATERAL_KEY                                                \
    "plugins/darkroom/masks/object/refine_boundary_weight_bilateral"
...
static dt_hash_t _compute_distort_hash(dt_develop_t *dev)
{
    ...
    for (GList *l = dev->history; l; l = g_list_next(l))
    {
        ...
        if (item->module && item->module->enabled &&
...
static void _on_view_changed(gpointer instance, dt_view_t *old_view, dt_view_t *new_view,
                             gpointer user_data)
{
    (void)instance;
    (void)new_view;
    ...
    if (old_view && old_view->view(old_view) == DT_VIEW_DARKROOM)
    {
        ...
        if (seg->ctx)
        {
            dt_print(DT_DEBUG_AI, "[object mask] freeing persistent model");
            dt_seg_free(seg->ctx);
            seg->ctx = NULL;
        }
        if (seg->env)
...
static gpointer _encode_thread_func(gpointer data)
{
    ...
    dt_print(DT_DEBUG_AI, "[object mask] rendering %dx%d (scale=%.3f) for encoding...", out_w,
             out_h, final_scale);
    ...
    if (ok)
        dt_seg_disk_cache_save(d->seg, imgid, distort_hash, rgb, out_w, out_h);
    g_free(rgb);
...
static void _keep_seed_component(float *mask, const int w, const int h, const float threshold,
                                 const int seed_x, const int seed_y)
{
    const int npix = w * h;
...
static float _mask_iou(const float *const restrict a, const float *const restrict b, const size_t n,
...
static gboolean _find_peak_point(const float *const restrict mask, const size_t w, const size_t h,
                                 const float threshold, const dt_seg_point_t *const exclude,
                                 const int n_exclude, const float min_separation,
                                 dt_seg_point_t *const out)
{
    ...
    const float max_dist =
        dt_image_distance_transform(mask, dist, w, h, threshold, DT_DISTANCE_TRANSFORM_MASK);
    if (max_dist <= min_depth)
    {
        dt_free_align(dist);
        return FALSE;
    }
...
static gboolean _compute_bbox(const float *const restrict mask, const int w, const int h,
                              const float threshold, const float padding, dt_seg_point_t *const tl,
...
static void _run_decoder(dt_masks_form_gui_t *gui)
{
    ...
    const int n_passes = CLAMP(dt_conf_get_int(CONF_OBJECT_REFINE_PASSES_KEY), 1, 3);
    ...
    for (int pass = 0; pass < n_passes; pass++)
    {
        ...
        if (mask && _mask_iou(mask, new_mask, (size_t)mw * mh, threshold) > 0.99f)
        {
            g_free(mask);
            mask = new_mask;
            dt_print(DT_DEBUG_AI, "[object mask] converged at pass %d/%d", pass + 1, n_passes);
            break;
        }
        g_free(mask);
        mask = new_mask;
        ...
        dt_seg_point_t peak;
        if (_find_peak_point(mask, mw, mh, threshold, points, n_points, 8.0f, &peak))
        {
    ...
    if (mask)
    {
        ...
        if (d->preview_refine)
        {
            ...
            if (rgb && rgb_w == mw && rgb_h == mh)
            {
                ...
                const float crf_sigma_color = CLAMP(
                    dt_conf_get_float(CONF_OBJECT_REFINE_BOUNDARY_SIGMA_COLOR_KEY), 1.0f, 50.0f);
                const float crf_w_bilateral = CLAMP(
                    dt_conf_get_float(CONF_OBJECT_REFINE_BOUNDARY_W_BILATERAL_KEY), 0.5f, 30.0f);
                const double t0 = dt_get_wtime();
                dt_dense_crf_binary(mask, rgb, mw, mh, 5.0f, crf_sigma_color, 3.0f, crf_w_bilateral,
                                    crf_iter);
                dt_print(DT_DEBUG_AI, "[object mask] CRF refinement: %dx%d (%.2fs)", mw, mh,
                         dt_get_wtime() - t0);
            }
        }
...
static void _update_preview(_object_data_t *d)
{
    ...
    const float thresh = 1.0f - CLAMP(dt_conf_get_float(CONF_OBJECT_THRESHOLD_KEY), 0.3f, 0.9f);
    d->preview_forms = ras2forms(inv_mask, d->mask_w, d->mask_h, NULL, thresh, d->preview_cleanup,
                                 (double)d->preview_smoothing, &d->preview_signs);
    g_free(inv_mask);
...
static void _save_raster_mask(const float *mask, const int w, const int h, const float threshold)
{
    if (!mask || w <= 0 || h <= 0)
        return;
    ...
    for (int seq = 1; g_file_test(outpath, G_FILE_TEST_EXISTS) && seq < 1000; seq++)
    {
        g_free(outpath);
    ...
    cairo_surface_t *surface =
        cairo_image_surface_create_for_data(buf, CAIRO_FORMAT_RGB24, w, h, stride);
    if (surface)
    {
...
static dt_masks_form_t *_register_vectorized_forms(dt_iop_module_t *module, GList *forms,
                                                   GList *signs, const int mask_w, const int mask_h)
{
    (void)module;
...
static dt_masks_form_t *_finalize_from_preview(dt_iop_module_t *module, dt_masks_form_gui_t *gui)
{
    _object_data_t *d = _get_data(gui);
...
static dt_masks_form_t *_finalize_mask(dt_iop_module_t *module, dt_masks_form_t *form,
                                       dt_masks_form_gui_t *gui)
{
    ...
    GList *signs = NULL;
    GList *forms =
        ras2forms(inv_mask, d->mask_w, d->mask_h, NULL, thresh, cleanup, (double)smoothing, &signs);
    g_free(inv_mask);
...
static int _object_events_mouse_scrolled(dt_iop_module_t *module, const float pzx, const float pzy,
                                         const gboolean up, const uint32_t state,
                                         dt_masks_form_t *form, const dt_imgid_t parentid,
...
static int _object_events_button_pressed(dt_iop_module_t *module, float pzx, float pzy,
                                         const double pressure, const int which, const int type,
                                         const uint32_t state, dt_masks_form_t *form,
                                         const dt_imgid_t parentid, dt_masks_form_gui_t *gui,
                                         const int index)
{
    (void)pressure;
    (void)parentid;
    ...
    if (gui->creation && which == 1 && dt_modifier_is(state, GDK_CONTROL_MASK | GDK_SHIFT_MASK))
    {
    ...
    else if (gui->creation && which == 3)
    {
        ...
        if (d && d->has_selection && d->mask && dt_modifier_is(state, GDK_SHIFT_MASK))
        {
            const float thresh = CLAMP(dt_conf_get_float(CONF_OBJECT_THRESHOLD_KEY), 0.3f, 0.9f);
...
static int _object_events_button_released(dt_iop_module_t *module, const float pzx, const float pzy,
                                          ...
                                          dt_masks_form_gui_t *gui, const int index)
{
    ...
    const float label = (d->has_selection && dt_modifier_is(state, GDK_SHIFT_MASK)) ? 0.0f : 1.0f;
...
static int _object_events_mouse_moved(dt_iop_module_t *module, const float pzx, const float pzy,
...
static void _object_events_post_expose(cairo_t *cr, const float zoom_scale,
                                       dt_masks_form_gui_t *gui, const int index,
                                       const int num_points)
{
    ...
    if (!d)
    {
        ...
        // if the active model changed in preferences, discard the old one
        {
            ...
            if (ps->ctx && active && g_strcmp0(active, persistent_id) != 0)
            {
            ...
            {
                DT_CONTROL_SIGNAL_CONNECT(DT_SIGNAL_VIEWMANAGER_VIEW_CHANGED, _on_view_changed,
                                          NULL);
    ...
    if (d->preview_forms)
    {
        ...
        for (GList *fl = d->preview_forms; fl; fl = g_list_next(fl))
        {
            ...
            cairo_move_to(cr, first_pt->corner[0] * msx, first_pt->corner[1] * msy);
            ...
            {
                dt_masks_point_path_t *pt = p->data;
                cairo_curve_to(cr, prev_pt->ctrl2[0] * msx, prev_pt->ctrl2[1] * msy,
                               pt->ctrl1[0] * msx, pt->ctrl1[1] * msy, pt->corner[0] * msx,
                               pt->corner[1] * msy);
                prev_pt = pt;
            }
            ...
            {
                dt_masks_point_path_t *pt = p->data;
                dt_masks_draw_anchor(cr, FALSE, zoom_scale, pt->corner[0] * msx,
                                     pt->corner[1] * msy);
            }
        }
    }
    ...
    if (pointer && gdk_device_get_window_at_position(pointer, NULL, NULL) != win)
        return;
    const gboolean has_sel = d && d->has_selection;
    const gboolean ctrl_shift_held = has_sel && (mod & (GDK_CONTROL_MASK | GDK_SHIFT_MASK)) ==
                                                    (GDK_CONTROL_MASK | GDK_SHIFT_MASK);
    const gboolean shift_held = has_sel && !ctrl_shift_held && (mod & GDK_SHIFT_MASK) != 0;

    // convert device coordinates to preview pipe pixel space
    {
        float pzx, pzy, zs;
        dt_dev_get_pointer_zoom_pos(&darktable.develop->full, (float)dev_x, (float)dev_y, &pzx,
                                    &pzy, &zs);
        gui->posx = pzx * wd;
        gui->posy = pzy * ht;
    }
...
static GSList *_object_setup_mouse_actions(const struct dt_masks_form_t *const form)
{
    ...
    lm = dt_mouse_action_create_simple(lm, DT_MOUSE_ACTION_LEFT, GDK_CONTROL_MASK | GDK_SHIFT_MASK,
                                       _("[OBJECT] clear selection"));
    lm = dt_mouse_action_create_simple(lm, DT_MOUSE_ACTION_RIGHT, 0, _("[OBJECT] apply mask"));
...
static void _object_modify_property(dt_masks_form_t *const form, const dt_masks_property_t prop,
                                    ...
                                    int *count, float *min, float *max)
{
    ...
    case DT_MASKS_PROPERTY_REFINE:
    {
        ...
        const gboolean enabled = dt_conf_get_bool(CONF_OBJECT_REFINE_BOUNDARY_KEY);
        if (d)
