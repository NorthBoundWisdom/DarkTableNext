...
int dt_iop_legacy_params(dt_iop_module_t *module, const void *const old_params,
                         const int32_t old_params_size, const int old_version, void **new_params,
                         int new_version)
{
    int ret = 0;
    ...
    if (module->legacy_params)
    {
        ...
        {
            params = NULL;
            ret = module->legacy_params(module, oparams, cversion, &params, &params_size, &version);
...
static void _init_presets(dt_iop_module_so_t *module_so)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
      ...
      DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db),
            ...
            DT_DEBUG_SQLITE3_BIND_BLOB(stmt2, 2, auto_init ? NULL : new_params,
                                       auto_init ? 0 : new_params_size, SQLITE_TRANSIENT);
...
static void _init_module_so(void *m)
{
    ...
    if (darktable.gui)
    {
        ...
        if (module->gui_init && !dt_iop_load_module_by_so(module_instance, module, NULL))
        {
            ...
            char *msg = g_strdup_printf(_("%s: %s"), _("loading processing modules"),
