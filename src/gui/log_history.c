...
static void _populate(void)
{
    ...
    dt_gui_container_destroy_children(_list_box);
...
void dt_gui_log_history_show(GtkWindow *parent)
{
    ...
    _window = dt_gui_toplevel_window_new();
    ...
    GtkWidget *scrolled = dt_gui_scrolled_window_new(NULL, NULL);
    ...
    dt_gui_scrolled_window_set_child(GTK_SCROLLED_WINDOW(scrolled), _list_box);
    dt_gui_window_set_child(GTK_WINDOW(_window), scrolled);
