...
static GtkWidget *_build_page_widget(_dt_page_t *pg)
{
    ...
    for (guint qi = 0; qi < pg->questions->len; qi++)
    {
        ...
        else if (q->qtype == DT_QUESTION_DIRCHOOSER)
        {
            ...
            g_signal_connect(G_OBJECT(entry), "changed", G_CALLBACK(_on_dir_entry_changed),
                             q->conf_key);
            ...
            g_signal_connect(G_OBJECT(btn), "clicked", G_CALLBACK(_on_browse_dir_clicked), entry);
        ...
        else if (q->qtype == DT_QUESTION_COMBOBOX)
        {
            ...
            for (int oi = 0; oi < q->n_options; oi++)
            {
                ...
                const char *display = (q->option_labels && q->option_labels[oi]) ?
                                          q->option_labels[oi] :
                                          q->options[oi];
        ...
        else
        {
            ...
            if (q->action.type != DT_WELCOME_ACTION_NONE)
                g_signal_connect(G_OBJECT(cb), "toggled", G_CALLBACK(_on_answer_toggled),
                                 &q->action);
...
static void _page_add_checkbox(dt_welcome_screen_t *ws, int page_idx, const char *label,
                               const char *description, gboolean default_active,
                               _dt_welcome_action_t action)
{
...
static void _page_add_combobox(dt_welcome_screen_t *ws, int page_idx, const char *label,
                               const char *description, const char *conf_key,
                                     ...
                                     const char *description)
{
    ...
    case DT_ENUM:
    {
        ...
        if (n > 0)
            _page_add_combobox(ws, page_idx, label, description, conf_key,
                               (const char *const *)opts, n);
        g_strfreev(opts);
        break;
    }
    case DT_PATH:
    ...
    case DT_STRING:
    {
        ...
        if (n > 0)
            ...
            dt_print(DT_DEBUG_ALWAYS, "[welcome] string conf key '%s' has no welcome options\n",
                     conf_key);
        break;
    }
    default:
...
void dt_welcome_screen_show(dt_welcome_screen_t *ws)
{
    ...
    dt_gui_box_add(GTK_BOX(footer), dt_gui_expand(progress_box), btn_prev, btn_next, btn_close);
    ...
    g_signal_connect_swapped(G_OBJECT(window), "destroy", G_CALLBACK(g_main_loop_quit), loop);
...
static dt_welcome_screen_t *_build_welcome_screen(void)
{
    ...
    dt_welcome_screen_page_add_paragraph(ws, page_idx,
                                         _("let's set up a few options to get you started."), TRUE);
    ...
    for (GList *l = keys; l; l = l->next)
    {
        ...
        if (dt_confgen_get_welcome_dirchooser(key))
            dt_welcome_screen_page_add_dirchooser(ws, page_idx, _(dt_confgen_get_label(key)), desc,
                                                  key);
        else
    ...
    dt_welcome_screen_page_add_paragraph(ws, page_idx, _("<b>You are all set!</b>"), TRUE);
    dt_welcome_screen_page_add_paragraph(
        ws, page_idx,
    ...
    dt_welcome_screen_page_add_paragraph(ws, page_idx, _("Happy editing with darktable!"), TRUE);
