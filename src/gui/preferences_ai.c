...
static gint _model_sort_func(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                             gpointer user_data)
{
    gchar *task_a, *task_b, *default_a, *default_b, *name_a, *name_b;
    gtk_tree_model_get(model, a, COL_TASK, &task_a, COL_DEFAULT, &default_a, COL_NAME, &name_a, -1);
    gtk_tree_model_get(model, b, COL_TASK, &task_b, COL_DEFAULT, &default_b, COL_NAME, &name_b, -1);
...
static void _update_download_selected_sensitivity(dt_prefs_ai_data_t *data)
{
    ...
    {
        gboolean sel = FALSE;
        gtk_tree_model_get(GTK_TREE_MODEL(data->model_store), &iter, COL_SELECTED, &sel, -1);
        if (sel)
        {
            any = TRUE;
            break;
...
static void _refresh_model_list(dt_prefs_ai_data_t *data)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        dt_print(DT_DEBUG_AI, "[preferences_ai] adding model: %s",
        ...
        gtk_list_store_set(
            data->model_store, &iter, COL_SELECTED, FALSE, COL_ENABLED, is_active,
            COL_ENABLED_SENSITIVE, is_downloaded, COL_NAME, model->name ? model->name : model->id,
            COL_TASK, model->task ? model->task : "", COL_STATUS, _status_to_string(model->status),
            COL_DEFAULT, model->is_default ? _("yes") : _("no"), COL_VERSION,
            (model->status == DT_AI_MODEL_DOWNLOADED ||
             model->status == DT_AI_MODEL_UPDATE_AVAILABLE ||
             model->status == DT_AI_MODEL_UPDATE_REQUIRED) ?
                (model->version ? model->version : "0.0") :
                "–",
            COL_ID, model->id, COL_INFO, is_downloaded, -1);
...
static void _update_controls_sensitivity(dt_prefs_ai_data_t *data, gboolean enabled)
{
    ...
    if (data->settings_grid)
    {
        ...
        for (GList *l = children; l; l = g_list_next(l))
        {
            ...
            int child_row = 0;
            gtk_container_child_get(GTK_CONTAINER(data->settings_grid), child, "top-attach",
                                    &child_row, NULL);
...
static void _on_enable_toggled(GtkWidget *widget, gpointer user_data)
{
    ...
    _update_string_indicator(data->enable_indicator, "plugins/ai/enabled");
}

static int _provider_to_combo_idx(const dt_ai_provider_t provider, const guint supported);
static void _on_provider_changed(GtkWidget *widget, gpointer user_data);
static void _update_provider_status(dt_prefs_ai_data_t *data, const dt_ai_provider_t provider);
...
{
    return dt_ai_providers[i].available && (supported & (1u << dt_ai_providers[i].value));
}
...
{
    if (!data->provider_combo)
        return;
    g_signal_handlers_block_by_func(data->provider_combo, _on_provider_changed, data);
    ...
    {
        if (!_provider_visible(i, data->supported_providers))
            continue;
        const char *label = dt_ai_providers[i].value == DT_AI_PROVIDER_AUTO ?
                                _("auto") :
                                dt_ai_providers[i].display_name;
    ...
    else if (prev != DT_AI_PROVIDER_AUTO && prev != DT_AI_PROVIDER_CPU)
    {
        ...
        for (int i = 0; i < DT_AI_PROVIDER_COUNT; i++)
        {
            ...
            if (v == DT_AI_PROVIDER_AUTO || v == DT_AI_PROVIDER_CPU)
                continue;
            selected = v;
            break;
        }
    ...
    dt_bauhaus_combobox_set(data->provider_combo,
                            _provider_to_combo_idx(selected, data->supported_providers));
    ...
    if (selected != prev)
    {
        for (int i = 0; i < DT_AI_PROVIDER_COUNT; i++)
        {
            ...
            {
                dt_conf_set_string(DT_AI_CONF_PROVIDER, dt_ai_providers[i].config_string);
                break;
            }
        }
    }

    g_signal_handlers_unblock_by_func(data->provider_combo, _on_provider_changed, data);
...
static int _combo_idx_to_provider(const int combo_idx, const guint supported)
{
    ...
    for (int i = 0; i < DT_AI_PROVIDER_COUNT; i++)
    {
        if (!_provider_visible(i, supported))
            continue;
...
static void _update_provider_status(dt_prefs_ai_data_t *data, const dt_ai_provider_t provider)
{
    ...
    if ((dt_ai_ort_path_changed_since_load() || dt_ai_provider_changed_since_load() ||
         dt_ai_device_id_changed_since_load(provider)) &&
        provider != DT_AI_PROVIDER_AUTO && provider != DT_AI_PROVIDER_CPU)
    {
        gtk_label_set_markup(GTK_LABEL(data->provider_status), _("<i>restart to apply</i>"));
        return;
    }
...
static gboolean _reset_enable_click(GtkWidget *label, GdkEventButton *event, GtkWidget *widget)
{
...
static gboolean _reset_provider_click(GtkWidget *label, GdkEventButton *event, gpointer user_data)
{
    if (event->type == GDK_2BUTTON_PRESS)
    {
        dt_prefs_ai_data_t *data = (dt_prefs_ai_data_t *)user_data;
        ...
        dt_bauhaus_combobox_set(data->provider_combo,
                                _provider_to_combo_idx(provider, data->supported_providers));
        return TRUE;
    }
    return FALSE;
}

static void _on_model_selection_toggled(GtkCellRendererToggle *cell, gchar *path_string,
                                        gpointer user_data)
{
    dt_prefs_ai_data_t *data = (dt_prefs_ai_data_t *)user_data;
    ...
    gboolean selected;
    gtk_tree_model_get(GTK_TREE_MODEL(data->model_store), &iter, COL_SELECTED, &selected, -1);
...
static void _on_enabled_toggled(GtkCellRendererToggle *cell, gchar *path_string, gpointer user_data)
{
    dt_prefs_ai_data_t *data = (dt_prefs_ai_data_t *)user_data;
    ...
    gchar *model_id = NULL;
    gchar *task = NULL;
    gtk_tree_model_get(GTK_TREE_MODEL(data->model_store), &iter, COL_ENABLED, &currently_enabled,
                       COL_ID, &model_id, COL_TASK, &task, -1);
...
static GList *_get_selected_model_ids(dt_prefs_ai_data_t *data)
{
    ...
    while (valid)
    {
        gboolean selected;
        gchar *model_id;
        gtk_tree_model_get(GTK_TREE_MODEL(data->model_store), &iter, COL_SELECTED, &selected,
                           COL_ID, &model_id, -1);
...
static void _download_progress_callback(const char *model_id, double progress, gpointer user_data)
{
...
static gpointer _download_thread_func(gpointer user_data)
{
    ...
    char *error =
        dt_ai_models_download_sync(dl->model_id, _download_progress_callback, dl, &dl->cancelled);
...
static gboolean _download_model_with_dialog(dt_prefs_ai_data_t *data, const char *model_id)
{
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        _("downloading AI model"), GTK_WINDOW(data->parent_dialog),
    ...
    else if (response != GTK_RESPONSE_CANCEL)
    {
        ...
        GtkWidget *err = gtk_message_dialog_new(GTK_WINDOW(data->parent_dialog), GTK_DIALOG_MODAL,
                                                GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", dl->error);
...
static void _on_install_model(GtkButton *button, gpointer user_data)
{
    ...
    GtkFileChooserNative *filechooser =
        gtk_file_chooser_native_new(_("install AI models"), GTK_WINDOW(data->parent_dialog),
    ...
    if (errors->len)
    {
        GtkWidget *err_dialog =
            gtk_message_dialog_new(GTK_WINDOW(data->parent_dialog), GTK_DIALOG_MODAL,
                                   GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", errors->str);
...
static void _on_delete_selected(GtkButton *button, gpointer user_data)
{
    ...
    const gboolean confirmed = dt_gui_show_yes_no_dialog(
        ...
        ngettext("do you really want to delete %d selected model?",
                 "do you really want to delete %d selected models?", delete_count),
        delete_count);
...
static void _show_model_card(dt_prefs_ai_data_t *data, const char *model_id)
{
    ...
    const char *name = (card && card->name) ? card->name : model_id;
    const char *desc = (card && card->long_description) ? card->long_description : dash;

    GtkWidget *dlg = gtk_message_dialog_new(GTK_WINDOW(data->parent_dialog),
                                            GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
                                            GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE, "%s", desc);
    ...
    GtkWidget *msg_area = gtk_message_dialog_get_message_area(GTK_MESSAGE_DIALOG(dlg));
    gtk_widget_set_margin_top(msg_area, 8);
    gtk_container_add(GTK_CONTAINER(msg_area), grid);

    const char *labels[] = {N_("scope"),   N_("author"),        N_("source"),       N_("paper"),
                            N_("license"), N_("training data"), N_("data license"), N_("notes")};
    const char *values[] = {card ? card->scope : NULL,
    ...
    for (int i = 0; i < n_fields; i++)
    {
        ...
        if (g_str_has_prefix(v, "http://") || g_str_has_prefix(v, "https://"))
        {
            gchar *markup = g_markup_printf_escaped("<a href=\"%s\">%s</a>", v, v);
...
static gboolean _info_active_at_bin(dt_prefs_ai_data_t *data, GtkTreeView *tv, gint bx, gint by)
{
        ...
        column == data->info_col)
    {
        ...
        if (gtk_tree_model_get_iter(GTK_TREE_MODEL(data->model_store), &iter, path))
            gtk_tree_model_get(GTK_TREE_MODEL(data->model_store), &iter, COL_INFO, &active, -1);
    }
    if (path)
        gtk_tree_path_free(path);
    return active;
...
static gboolean _on_query_tooltip(GtkWidget *widget, gint x, gint y, gboolean keyboard_mode,
...
static gboolean _on_tree_motion(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
    GtkTreeView *tv = GTK_TREE_VIEW(widget);
    ...
    {
        gtk_tree_view_convert_widget_to_bin_window_coords(tv, (gint)event->x, (gint)event->y, &bx,
                                                          &by);
    }
    if (_info_active_at_bin(user_data, tv, bx, by))
    {
...
static gboolean _on_info_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
    ...
    if (!gtk_tree_view_get_path_at_pos(tv, (gint)event->x, (gint)event->y, &path, &column, NULL,
                                       NULL))
        return FALSE;
    ...
    if (gtk_tree_model_get_iter(GTK_TREE_MODEL(data->model_store), &iter, path))
    {
        ...
        gtk_tree_model_get(GTK_TREE_MODEL(data->model_store), &iter, COL_ID, &model_id, COL_INFO,
                           &has_info, -1);
...
static void _show_ort_probe_result(GtkWindow *parent, const char *path, const char *version)
{
    ...
    if (version)
        dlg = gtk_message_dialog_new(
            parent, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
            GTK_BUTTONS_OK, _("ONNX Runtime %s detected.\nRestart darktable to apply."), version);
    else
        dlg = gtk_message_dialog_new(parent, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
...
static void _on_detect_system_ort(GtkButton *button, gpointer user_data)
{
    ...
    else if (count == 1)
    {
        ...
        GtkWidget *dlg = gtk_message_dialog_new(
            GTK_WINDOW(data->parent_dialog), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
            GTK_MESSAGE_INFO, GTK_BUTTONS_OK,
            _("ONNX Runtime %s [%s]\n%s\n\nRestart darktable to apply."), f->version, f->eps,
            f->path);
        gtk_dialog_run(GTK_DIALOG(dlg));
        gtk_widget_destroy(dlg);
    }
    else
    {
        GtkWidget *dlg = gtk_dialog_new_with_buttons(
            _("select ONNX Runtime library"), GTK_WINDOW(data->parent_dialog),
            GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, _("_cancel"), GTK_RESPONSE_CANCEL,
...
void init_tab_ai(GtkWidget *dialog, GtkWidget *stack)
{
    ...
    g_signal_connect(data->enable_toggle, "toggled", G_CALLBACK(_on_enable_toggled), data);
    g_signal_connect(enable_labelev, "button-press-event", G_CALLBACK(_reset_enable_click),
                     data->enable_toggle);
    ...
    g_signal_connect(data->provider_combo, "value-changed", G_CALLBACK(_on_provider_changed), data);
    g_signal_connect(provider_labelev, "button-press-event", G_CALLBACK(_reset_provider_click),
                     data);
    g_signal_connect(data->gpu_combo, "value-changed", G_CALLBACK(_on_gpu_changed), data);
    ...
    GtkWidget *provider_hbox =
        dt_gui_hbox(data->provider_combo, data->gpu_combo, data->provider_status);
...
#if !defined(__APPLE__)
    {
...
#if defined(_WIN32)
        gtk_entry_set_placeholder_text(GTK_ENTRY(data->ort_path_entry), _("bundled (DirectML)"));
#else
        gtk_entry_set_placeholder_text(GTK_ENTRY(data->ort_path_entry), _("bundled (CPU only)"));
#endif
        ...
        gtk_widget_set_tooltip_text(browse_btn, _("select a custom ONNX Runtime shared library"));
        ...
        g_signal_connect(path_labelev, "button-press-event", G_CALLBACK(_reset_ort_path_click),
                         data);
    }
#endif // !__APPLE__
    ...
    gtk_tree_sortable_set_default_sort_func(GTK_TREE_SORTABLE(data->model_store), _model_sort_func,
                                            NULL, NULL);
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(data->model_store),
    ...
    g_signal_connect(toggle_renderer, "toggled", G_CALLBACK(_on_model_selection_toggled), data);
    GtkTreeViewColumn *select_col =
        gtk_tree_view_column_new_with_attributes("", toggle_renderer, "active", COL_SELECTED, NULL);
    ...
    g_signal_connect(data->select_all_toggle, "toggled", G_CALLBACK(_on_select_all_toggled), data);
    ...
    g_signal_connect(select_col_button, "clicked", G_CALLBACK(_on_select_all_header_clicked), data);

    // name column
    GtkCellRenderer *text_renderer = gtk_cell_renderer_text_new();
    GtkTreeViewColumn *name_col =
        gtk_tree_view_column_new_with_attributes(_("name"), text_renderer, "text", COL_NAME, NULL);
    ...
    GtkCellRenderer *info_renderer = dtgtk_paint_cell_new(dtgtk_cairo_paint_info, 0, NULL);
    data->info_col =
        gtk_tree_view_column_new_with_attributes("", info_renderer, "visible", COL_INFO, NULL);
    gtk_tree_view_column_set_clickable(data->info_col, FALSE);
    gtk_tree_view_append_column(GTK_TREE_VIEW(data->model_list), data->info_col);

    // version column
    GtkTreeViewColumn *version_col = gtk_tree_view_column_new_with_attributes(
        _("version"), text_renderer, "text", COL_VERSION, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(data->model_list), version_col);

    // task column
    GtkTreeViewColumn *task_col =
        gtk_tree_view_column_new_with_attributes(_("task"), text_renderer, "text", COL_TASK, NULL);
    ...
    g_signal_connect(enabled_renderer, "toggled", G_CALLBACK(_on_enabled_toggled), data);
    GtkTreeViewColumn *enabled_col = gtk_tree_view_column_new_with_attributes(
        _("enabled"), enabled_renderer, "active", COL_ENABLED, "sensitive", COL_ENABLED_SENSITIVE,
        "activatable", COL_ENABLED_SENSITIVE, NULL);
    ...
    GtkTreeViewColumn *default_col = gtk_tree_view_column_new_with_attributes(
        _("default"), text_renderer, "text", COL_DEFAULT, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(data->model_list), default_col);

    gtk_widget_set_has_tooltip(data->model_list, TRUE);
    g_signal_connect(data->model_list, "query-tooltip", G_CALLBACK(_on_query_tooltip), data);
    gtk_widget_add_events(data->model_list, GDK_POINTER_MOTION_MASK);
    g_signal_connect(data->model_list, "motion-notify-event", G_CALLBACK(_on_tree_motion), data);
    g_signal_connect(data->model_list, "button-press-event", G_CALLBACK(_on_info_button_press),
                     data);
    ...
    data->download_default_btn = gtk_button_new_with_label(_("download / update default"));
    gtk_widget_set_tooltip_text(data->download_default_btn,
                                _("download or update all default models"));
    g_signal_connect(data->download_default_btn, "clicked", G_CALLBACK(_on_download_default), data);
    dt_gui_box_add(button_box, data->download_default_btn);

    // download / update selected button
    data->download_selected_btn = gtk_button_new_with_label(_("download / update selected"));
    gtk_widget_set_tooltip_text(data->download_selected_btn,
                                _("download or update the selected models"));
    g_signal_connect(data->download_selected_btn, "clicked", G_CALLBACK(_on_download_selected),
                     data);
    ...
    gtk_widget_set_tooltip_text(data->install_btn, _("install a model from a local .dtmodel file"));
    ...
    g_signal_connect(data->delete_selected_btn, "clicked", G_CALLBACK(_on_delete_selected), data);
