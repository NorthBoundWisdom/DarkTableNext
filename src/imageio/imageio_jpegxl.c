...
dt_imageio_retval_t dt_imageio_open_jpegxl(dt_image_t *img, const char *filename,
                                           dt_mipmap_buffer_t *mbuf)
{
    ...
    if (!inputfile)
    {
        dt_print(DT_DEBUG_ALWAYS, "[jpegxl_open] cannot open file for read: %s", filename);
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "[jpegxl_open] failed to read entire file (%zu bytes) from '%s'",
    ...
    if (!runner)
    {
        dt_print(DT_DEBUG_ALWAYS, "[jpegxl_open] JxlResizableParallelRunnerCreate failed");
    ...
    if (JxlDecoderSubscribeEvents(decoder, JXL_DEC_BASIC_INFO | JXL_DEC_COLOR_ENCODING |
                                               JXL_DEC_BOX | JXL_DEC_FULL_IMAGE) != JXL_DEC_SUCCESS)
    {
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "[jpegxl_open] JxlDecoderSetParallelRunner failed");
        JxlResizableParallelRunnerDestroy(runner);
        JxlDecoderDestroy(decoder);
        free(read_buffer);
        return DT_IMAGEIO_LOAD_FAILED;
    ...
    while (1)
    {
        ...
        if (status == JXL_DEC_BASIC_INFO)
        {
            ...
            {
                dt_print(DT_DEBUG_ALWAYS, "[jpegxl_open] JXL image declares zero dimensions");
                JxlResizableParallelRunnerDestroy(runner);
                JxlDecoderDestroy(decoder);
                free(read_buffer);
                return DT_IMAGEIO_FILE_CORRUPTED;
        ...
        if (status == JXL_DEC_BOX)
        {
            ...
            if ((status != JXL_DEC_SUCCESS) || (exif_size <= 8))
                continue;
                ...
                if (icc_size)
                {
                    ...
                    if (img->profile)
                    {
                        JxlDecoderGetColorAsICCProfile(decoder,
                                                       ...
                                                       &pixel_format,
#endif
                                                       JXL_COLOR_PROFILE_TARGET_DATA, img->profile,
                                                       icc_size);
        ...
        if (status == JXL_DEC_NEED_IMAGE_OUT_BUFFER)
        {
            ...
            JxlDecoderSetImageOutBuffer(decoder, &pixel_format, mipbuf,
    ...
    if (!img->exif_inited && exif_data)
    {
        ...
        const uint32_t exif_offset =
            exif_data[0] << 24 | exif_data[1] << 16 | exif_data[2] << 8 | exif_data[3];
