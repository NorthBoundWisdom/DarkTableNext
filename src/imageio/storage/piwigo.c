...
static void _piwigo_api_post(_piwigo_api_context_t *ctx, GList *args, char *filename,
                             const gboolean isauth);

static size_t curl_write_data_cb(void *ptr, const size_t size, const size_t nmemb, void *data)
{
...
static _piwigo_account_t *_piwigo_get_account(dt_storage_piwigo_gui_data_t *ui, const gchar *server)
{
    ...
    for (const GList *a = ui->accounts; a; a = g_list_next(a))
    {
        ...
        ;
        if (account->server && !strcmp(server, account->server))
            return account;
    }
...
static int _piwigo_api_post_internal(_piwigo_api_context_t *ctx, GList *args, char *filename,
                                     const gboolean isauth)
{
    curl_mime *form = NULL;
    ...
    if (res == CURLE_OK)
    {
        GError *error = NULL;
        gboolean ret =
            json_parser_load_from_data(ctx->json_parser, response->str, response->len, &error);
        if (!ret)
            goto cleanup;
...
/** Refresh albums */
static gboolean _piwigo_refresh_albums(dt_storage_piwigo_gui_data_t *ui, const gchar *select_album)
{
    gtk_widget_set_sensitive(GTK_WIDGET(ui->album_list), FALSE);
    ...
    if (ui->api == NULL || ui->api->authenticated == FALSE)
    {
        ...
        if (ui->api == NULL || !ui->api->authenticated)
            return FALSE;
    }
    ...
    if (ui->api->response && !ui->api->error_occured)
    {
        ...
        for (int i = 0; i < album_len; i++)
        {
            ...
            snprintf(data, sizeof(data), "%*c%s (%" PRId64 ")", indent * 3, ' ', new_album->name,
                     new_album->size);
...
static gboolean _piwigo_api_create_new_album(dt_storage_piwigo_params_t *p)
{
    ...
    args = _piwigo_query_add_arguments(args, "status",
                                       p->preset_data.privacy == 0 ? "public" : "private");
...
static int _piwigo_api_get_image_id(dt_storage_piwigo_params_t *p, dt_image_t *img,
                                    dt_imageio_module_format_t *format,
                                    dt_imageio_module_data_t *fdata, const int page)
{
    GList *args = NULL;
    char album_id[10];
    char page_string[10];
                ...
                if (paging_node != NULL && json_node_get_node_type(paging_node) == JSON_NODE_OBJECT)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        for (int i = 0; i < json_array_get_length(existing_images); i++)
                        {
                            ...
                            {
                                if (strcmp(filename, json_object_get_string_member(existing_image,
                                                                                   "file")) == 0)
                                {
                                    g_free(filename);
...
static gboolean _piwigo_api_set_info(dt_storage_piwigo_params_t *p, gchar *author, gchar *caption,
                                     gchar *description, const int pwg_image_id)
{
    GList *args = NULL;
...
static gboolean _piwigo_api_upload_photo(dt_storage_piwigo_params_t *p, gchar *fname, gchar *author,
                                         gchar *caption, gchar *description, const int pwg_image_id)
{
    GList *args = NULL;
    char cat[10];
    char privacy[10];
...
{
    dt_conf_set_string("plugins/imageio/storage/export/piwigo/filename_pattern",
...
void gui_init(dt_imageio_module_storage_t *self)
{
    ...
    for (const GList *a = ui->accounts; a; a = g_list_next(a))
    {
        ...
        if (!strcmp(account->server, server))
            account_index = index;
        index++;
    }
    ...
    // server
    ui->server_entry = GTK_ENTRY(dt_action_entry_new(
        DT_ACTION(self), N_("server"), G_CALLBACK(_piwigo_server_entry_changed), ui,
        _("the server name\n"
    ...
    // login
    ui->user_entry = GTK_ENTRY(dt_action_entry_new(DT_ACTION(self), N_("user"),
    ...
    // password
    ui->pwd_entry = GTK_ENTRY(dt_action_entry_new(DT_ACTION(self), N_("password"),
                                                  G_CALLBACK(_piwigo_entry_changed), ui, NULL,
    ...
    g_signal_connect(G_OBJECT(login_button), "clicked", G_CALLBACK(_piwigo_login_clicked), self);
    ...
    // permissions list
    DT_BAUHAUS_COMBOBOX_NEW_FULL(ui->permission_list, self, NULL, N_("visible to"), NULL, 0, NULL,
                                 NULL, N_("everyone"), N_("contacts"), N_("friends"), N_("family"),
                                 N_("you"));
    ...
    g_signal_connect(G_OBJECT(ui->album_list), "value-changed", G_CALLBACK(_piwigo_album_changed),
                     (gpointer)ui);
    ...
    g_signal_connect(G_OBJECT(refresh_button), "clicked", G_CALLBACK(_piwigo_refresh_clicked),
                     (gpointer)ui);
    ...
    ui->create_box = GTK_BOX(dt_gui_vbox(
        dt_gui_hbox(dt_ui_label_new(_("title")), ui->new_album_entry), ui->parent_album_list));
    gtk_widget_set_no_show_all(GTK_WIDGET(ui->create_box), TRUE);

    _piwigo_set_status(ui, _("click login button to start"), "#ffffff");

    // filename pattern
    ui->filename_pattern_entry = GTK_ENTRY(dt_action_entry_new(
        DT_ACTION(self), N_("filename pattern"),
        G_CALLBACK(_filname_pattern_entry_changed_callback), self,
    ...
    self->widget = dt_gui_vbox(
        ui->account_list, dt_gui_hbox(dt_ui_label_new(_("server")), ui->server_entry),
        dt_gui_hbox(dt_ui_label_new(_("user")), ui->user_entry),
        dt_gui_hbox(dt_ui_label_new(_("password")), ui->pwd_entry), login_button, ui->status_label,
        ui->permission_list, dt_gui_hbox(ui->album_list, refresh_button), ui->create_box,
...
int store(dt_imageio_module_storage_t *self, dt_imageio_module_data_t *sdata,
          ...
          const dt_iop_color_intent_t icc_intent, dt_export_metadata_t *metadata)
{
    dt_storage_piwigo_gui_data_t *ui = self->gui_data;
    ...
    if (p->api == NULL)
    {
        dt_print(DT_DEBUG_ALWAYS, "[imageio_storage_piwigo] not logged in to Piwigo server!");
    ...
    if (dt_imageio_export(imgid, fname, format, fdata, high_quality, upscale, is_scaling,
    ...
    dt_pthread_mutex_lock(&darktable.plugin_threadsafe);
    {
        ...
        if (p->new_album)
        {
            ...
            if (!status)
                dt_control_log(_("cannot create a new Piwigo album!"));
        }
        ...
        if (status)
        {
            ...
            {
                status =
                    _piwigo_api_upload_photo(p, fname, author, caption, description, pwg_image_id);
                if (!status)
                {
                    dt_print(DT_DEBUG_ALWAYS,
...
cleanup:
    ...
    else if (!result && !skipped)
    {
        ...
        dt_control_log(
            ngettext("%d/%d exported to Piwigo webalbum", "%d/%d exported to Piwigo webalbum", num),
            num, total);
    }
    return result;
}
...
void *get_params(dt_imageio_module_storage_t *self)
{
    ...
    if (ui->api && ui->api->authenticated == TRUE)
    {
        ...
        if (index >= 0)
        {
            ...
            {
            case 0: // Create album
                p->parent_album_id = _piwigo_album_id(
                    dt_bauhaus_combobox_get_text(ui->parent_album_list), ui->albums);
