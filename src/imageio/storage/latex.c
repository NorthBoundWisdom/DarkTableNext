...
typedef struct dt_imageio_latex_t
{
    ...
    char title[1024];
    char cached_dirname
        [DT_MAX_PATH_FOR_PARAMS]; // expanded during first img store, not stored in param struct.
                    ...
                    size_t *new_size)
{
    ...
    if (old_version == 1)
    {
        ...
        *new_version = 2;
        *new_size = sizeof(dt_imageio_latex_v2_t) - 2 * sizeof(void *) - DT_MAX_PATH_FOR_PARAMS;
        return n;
    }
...
static void button_clicked(GtkWidget *widget, dt_imageio_module_storage_t *self)
{
    ...
    if (gtk_native_dialog_run(GTK_NATIVE_DIALOG(filechooser)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        gtk_entry_set_text(GTK_ENTRY(d->entry),
                           escaped); // the signal handler will write this to conf
        g_free(dir);
        g_free(composed);
        g_free(escaped);
    }
...
void gui_init(dt_imageio_module_storage_t *self)
{
    ...
    d->title_entry = GTK_ENTRY(
        dt_action_entry_new(DT_ACTION(self), N_("path"), G_CALLBACK(title_changed_callback), self,
                            _("enter the title of the book"),
                            dt_conf_get_string_const("plugins/imageio/storage/latex/title")));

    self->widget = dt_gui_vbox(dt_gui_hbox(d->entry, widget),
...
void gui_reset(dt_imageio_module_storage_t *self)
{
    ...
    gtk_entry_set_text(d->entry,
                       dt_confgen_get("plugins/imageio/storage/latex/file_directory", DT_DEFAULT));
    gtk_entry_set_text(d->title_entry,
                       dt_confgen_get("plugins/imageio/storage/latex/title", DT_DEFAULT));
    dt_conf_set_string("plugins/imageio/storage/latex/file_directory",
          ...
          dt_iop_color_intent_t icc_intent, dt_export_metadata_t *metadata)
{
    ...
    dt_pthread_mutex_lock(&darktable.plugin_threadsafe);
    {
        ...
        if (g_file_test(d->filename, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR) ||
            ((d->filename + strlen(d->filename))[0] == '/' ||
             (d->filename + strlen(d->filename))[0] == '\\'))
            snprintf(d->filename + strlen(d->filename), sizeof(d->filename) - strlen(d->filename),
                     "$(FILE_NAME)");
    ...
    dt_imageio_export(imgid, filename, format, fdata, high_quality, upscale, is_scaling,
                      scale_factor, TRUE, export_masks, icc_type, icc_filename, icc_intent, self,
                      sdata, num, total, metadata);
...
void init(dt_imageio_module_storage_t *self)
{
    ...
    dt_lua_register_module_member(darktable.lua_state.state, self, dt_imageio_latex_t, filename,
                                  char_path_length);
    dt_lua_register_module_member(darktable.lua_state.state, self, dt_imageio_latex_t, title,
                                  char_1024);
#endif
}
