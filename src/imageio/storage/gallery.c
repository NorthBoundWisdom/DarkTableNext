                    ...
                    size_t *new_size)
{
    ...
    if (old_version == 1)
    {
        ...
        *new_version = 2;
        *new_size = sizeof(dt_imageio_gallery_v2_t) - 2 * sizeof(void *) - DT_MAX_PATH_FOR_PARAMS;
        return n;
    }
...
void gui_reset(dt_imageio_module_storage_t *self)
{
    gallery_t *d = (gallery_t *)self->gui_data;
    gtk_entry_set_text(
        d->entry, dt_confgen_get("plugins/imageio/storage/gallery/file_directory", DT_DEFAULT));

    gtk_entry_set_text(d->title_entry,
                       dt_confgen_get("plugins/imageio/storage/gallery/title", DT_DEFAULT));
          ...
          dt_iop_color_intent_t icc_intent, dt_export_metadata_t *metadata)
{
    ...
    snprintf(pair->line, sizeof(pair->line),
             ...
             "      <h1>%s</h1>\n"
             "      %s</div>\n",
             esc_relthumbfilename, num, num - 1, title ? title : "&nbsp;",
    ...
    sprintf(c, "-thumb.%s", ext);
    if (dt_imageio_export(imgid, filename, format, fdata, FALSE, TRUE, FALSE, is_scaling,
                          scale_factor, export_masks, icc_type, icc_filename, icc_intent, self,
...
void init(dt_imageio_module_storage_t *self)
{
    ...
    dt_lua_register_module_member(darktable.lua_state.state, self, dt_imageio_gallery_t, filename,
                                  char_path_length);
    dt_lua_register_module_member(darktable.lua_state.state, self, dt_imageio_gallery_t, title,
                                  char_1024);
#endif
}
...
gboolean supported(dt_imageio_module_storage_t *storage, dt_imageio_module_format_t *format)
{
    ...
    if (strcmp(mime, "image/webp") == 0)
        return TRUE;
    if (strcmp(mime, "image/avif") == 0)
        return TRUE;
    if (strcmp(mime, "image/jxl") == 0)
        return TRUE;
