...
dt_imageio_retval_t dt_imageio_open_im(dt_image_t *img, const char *filename,
                                       dt_mipmap_buffer_t *mbuf)
{
    ...
    if (mipbuf == NULL)
    {
        dt_print(DT_DEBUG_ALWAYS, "[ImageMagick_open] could not alloc full buffer for image '%s'",
                 img->filename);
    ...
    ret =
        MagickExportImagePixels(image, 0, 0, img->width, img->height, colormap, FloatPixel, mipbuf);

    if (ret != MagickTrue)
    {
        dt_print(DT_DEBUG_ALWAYS, "[ImageMagick_open] error reading image '%s'", img->filename);
        goto error;
    }
