...
void init(dt_imageio_module_format_t *self)
{
    ...
    dt_lua_register_module_member(darktable.lua_state.state, self, dt_imageio_webp_t, comp_type,
                                  comp_type_t);
...
void gui_init(dt_imageio_module_format_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->compression, self, NULL, N_("compression"), NULL, comp_type,
                                 compression_changed, self, N_("lossy"), N_("lossless"));

    gui->quality = dt_bauhaus_slider_new_with_range(
        (dt_iop_module_t *)self, dt_confgen_get_int("plugins/imageio/format/webp/quality", DT_MIN),
        dt_confgen_get_int("plugins/imageio/format/webp/quality", DT_MAX), 1,
        dt_confgen_get_int("plugins/imageio/format/webp/quality", DT_DEFAULT), 0);
    dt_bauhaus_widget_set_label(gui->quality, NULL, N_("quality"));
    gtk_widget_set_tooltip_text(gui->quality,
                                _("for lossy, 0 gives the smallest size and 100 the best quality.\n"
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->hint, self, NULL, N_("image hint"),
                                 ...
                                 hint, hint_combobox_changed, self, N_("default"), N_("picture"),
                                 N_("photo"), N_("graphic"));
