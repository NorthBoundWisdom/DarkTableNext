...
int write_image(dt_imageio_module_data_t *data, const char *filename,
                ...
                dt_colorspaces_color_profile_type_t icc_type, const char *icc_filename, void *exif,
                int exif_len, dt_imgid_t imgid, int num, int total, struct dt_dev_pixelpipe_t *pipe,
                const gboolean export_masks)
{
    ...
    {
    case 8:
        err = heif_image_create(width, height, heif_colorspace_RGB, heif_chroma_interleaved_RGB,
                                &image);
        break;
    case 10:
    case 12:
        err = heif_image_create(width, height, heif_colorspace_RGB,
                                heif_chroma_interleaved_RRGGBB_LE, &image);
        break;
    default:
        dt_print(DT_DEBUG_ALWAYS, "[heif export] Unsupported bit depth: %d", bit_depth);
        return 1; // failure
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] Failed in heif_image_create for %s", filename);
        return 1; // failure
    }
    ...
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] Failed in heif_image_add_plane for %s", filename);
    ...
    case 12:
        ...
        for (size_t row = 0; row < height; row++)
        {
            for (size_t x = 0; x < width; x++)
            {
                const float *in_pixel = &in_data[(size_t)4 * ((row * width) + x)];
                uint16_t *out_pixel =
                    (uint16_t *)&out[(row * rowbytes) + (3 * sizeof(uint16_t) * x)];
    ...
    dt_print(DT_DEBUG_IMAGEIO, "[heif colorprofile profile: %s]",
    ...
    if (need_to_embed_icc)
    {
        ...
        if (icc_profile_len > 0)
        {
            ...
            {
                dt_print(DT_DEBUG_ALWAYS, "[heif export] failed to allocate ICC profile");
                heif_image_release(image);
    ...
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] error getting HEVC encoder: %s (%d)", err.message,
                 err.code);
    ...
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] failed to set chrome subsampling %s: %s",
    ...
    if (!options)
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] Failed in heif_encoding_options_alloc for %s",
                 filename);
        heif_encoder_release(encoder);
        heif_context_free(context);
        heif_image_release(image);
    ...
    err = heif_context_encode_image(context, image, encoder,
                                    options, // options
                                    &handle);

    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] error encoding image: %s (%d)", err.message,
                 err.code);
        heif_encoder_release(encoder);
        heif_context_free(context);
        heif_image_release(image);
    ...
    if (exif && exif_len > 0)
    {
        ...
        if (err.code != heif_error_Ok)
        {
            dt_print(DT_DEBUG_ALWAYS, "[heif export] failed to save EXIF metadata: %s (%d)",
    ...
    if (exif && exif_len > 0)
    {
        ...
        if (xmp_string && (xmp_len = strlen(xmp_string)) > 0)
        {
            ...
            if (err.code != heif_error_Ok)
            {
                dt_print(DT_DEBUG_ALWAYS, "[heif export] failed to save XMP metadata: %s (%d)",
                         err.message, err.code);
                heif_image_handle_release(handle);
                heif_context_free(context);
                heif_image_release(image);
    ...
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "[heif export] error writing image to file: %s (%d)", err.message,
                 err.code);
        heif_encoder_release(encoder);
        heif_context_free(context);
        heif_image_release(image);
...
void gui_init(dt_imageio_module_format_t *self)
{
    ...
    const int bit_depth = dt_conf_get_int("plugins/imageio/format/heif/bitdepth");
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->bit_depth, self, NULL, N_("bit depth"), NULL, bit_depth_pos,
                                 bit_depth_changed, self, N_("8 bit"), N_("10 bit"), N_("12 bit"));

    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->compression_type, self, NULL, N_("compression"), NULL,
                                 compression_type, compression_type_changed, self, N_("lossless"),
                                 N_("lossy"));
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(
        ...
        subsample, subsample_combobox_changed, self, N_("auto"), N_("4:4:4"), N_("4:2:2"),
        N_("4:2:0"));
    ...
    self->widget = dt_gui_vbox(gui->bit_depth, gui->compression_type, gui->quality, gui->subsample);
}
...
void gui_reset(dt_imageio_module_format_t *self)
{
    ...
    const int bit_depth = dt_confgen_get_int("plugins/imageio/format/heif/bitdepth", DT_DEFAULT);
    const int quality = dt_confgen_get_int("plugins/imageio/format/heif/quality", DT_DEFAULT);
