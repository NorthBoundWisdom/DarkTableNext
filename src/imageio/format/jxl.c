    ...
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            dt_print(DT_DEBUG_IMAGEIO,                                                             \
                     "[jxl] libjxl call failed with err %d (src/imageio/format/jxl.c#L%d)", err,   \
                     __LINE__);                                                                    \
            goto end;                                                                              \
        }                                                                                          \
    }
...
#define JXL_FAIL(msg, ...)                                                                         \
    ...
    if (!runner)
        JXL_FAIL("could not create resizable parallel runner");
    ...
    LIBJXL_ASSERT(JxlEncoderFrameSettingsSetOption(frame_settings, JXL_ENC_FRAME_SETTING_EFFORT,
                                                   params->effort));

    LIBJXL_ASSERT(JxlEncoderFrameSettingsSetOption(
        frame_settings, JXL_ENC_FRAME_SETTING_DECODING_SPEED, params->tier));
    ...
    if (pipe->icc_intent >= DT_INTENT_PERCEPTUAL && pipe->icc_intent < DT_INTENT_LAST)
        ...
        color_encoding.rendering_intent =
            (JxlRenderingIntent)cmsGetHeaderRenderingIntent(out_profile);
   ...
   * closing file instead */
    ...
    if (exif && exif_len > 0)
    {
        ...
        if (xmp_string && (xmp_len = strlen(xmp_string)) > 0)
        {
            // Exiv2 < 0.28 doesn't support Brotli compressed boxes
            LIBJXL_ASSERT(
                JxlEncoderAddBox(encoder, "xml ", (const uint8_t *)xmp_string, xmp_len, JXL_FALSE));
        }
    }
    ...
    if (!out_buf)
        JXL_FAIL("could not allocate codestream buffer of size %zu", out_len);
...
void gui_init(dt_imageio_module_format_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->bpp, self, NULL, N_("bit depth"), NULL, bpp_enum, bpp_changed,
                                 gui, N_("8 bit"), N_("10 bit"), N_("12 bit"), N_("16 bit"),
                                 N_("32 bit (float)"));

    // pixel type combobox
    const int pixel_type = dt_conf_get_bool("plugins/imageio/format/jxl/pixel_type") & 1;

    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->pixel_type, self, NULL, N_("pixel type"), NULL, pixel_type,
                                 pixel_type_changed, gui, N_("unsigned integer"),
                                 N_("floating point"));
    ...
    gtk_widget_set_tooltip_text(
        gui->quality, _("the quality of the output image\n0-29 = very lossy\n30-99 = JPEG "
                        "quality comparable\n100 = lossless"));
...
void gui_reset(dt_imageio_module_format_t *self)
{
    ...
    dt_bauhaus_combobox_set(
        gui->bpp, _bpp_to_enum(dt_confgen_get_int("plugins/imageio/format/jxl/bpp", DT_DEFAULT)));
    dt_bauhaus_combobox_set(
        gui->pixel_type,
        dt_confgen_get_bool("plugins/imageio/format/jxl/pixel_type", DT_DEFAULT) & 1);
    dt_bauhaus_slider_set(gui->quality,
                          dt_confgen_get_int("plugins/imageio/format/jxl/quality", DT_DEFAULT));
    /* see above
  dt_bauhaus_combobox_set
    (gui->original,
     dt_confgen_get_bool("plugins/imageio/format/jxl/original", DT_DEFAULT) & 1);
*/
    dt_bauhaus_slider_set(gui->effort,
                          dt_confgen_get_int("plugins/imageio/format/jxl/effort", DT_DEFAULT));
    dt_bauhaus_slider_set(gui->tier,
                          dt_confgen_get_int("plugins/imageio/format/jxl/tier", DT_DEFAULT));
}
