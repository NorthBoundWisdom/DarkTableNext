                ...
                struct dt_dev_pixelpipe_t *pipe, const gboolean export_masks)
{
    ...
    if (profile_len > 0)
    {
        ...
        {
            const cmsToneCurve *red_curve =
                (cmsToneCurve *)cmsReadTag(out_profile, cmsSigRedTRCTag);
            const cmsToneCurve *green_curve =
                (cmsToneCurve *)cmsReadTag(out_profile, cmsSigGreenTRCTag);
            const cmsToneCurve *blue_curve =
                (cmsToneCurve *)cmsReadTag(out_profile, cmsSigBlueTRCTag);
    ...
    if (d->bpp == 8)
        ...
        xcf_set(xcf, XCF_PRECISION,
                profile_is_linear ? XCF_PRECISION_I_16_L : XCF_PRECISION_I_16_G);
    else if (d->bpp == 32)
        xcf_set(xcf, XCF_PRECISION,
                profile_is_linear ? XCF_PRECISION_F_32_L : XCF_PRECISION_F_32_G);
    else
    {
    ...
    if (profile)
    {
        xcf_set(xcf, XCF_PROP, XCF_PROP_PARASITES, "icc-profile",
                XCF_PARASITE_PERSISTENT | XCF_PARASITE_UNDOABLE, profile_len, profile);
    }
    ...
    xcf_set(xcf, XCF_PROP, XCF_PROP_PARASITES, "gimp-comment", XCF_PARASITE_PERSISTENT,
    ...
    if (exif && exif_len > 0)
    {
        ...
        xcf_set(xcf, XCF_PROP, XCF_PROP_PARASITES, "exif-data", XCF_PARASITE_PERSISTENT,
                exif_len + 6, exif_buf);
        g_free(exif_buf);
    }
...
void gui_init(dt_imageio_module_format_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->bpp, self, NULL, N_("bit depth"), NULL,
                                 ...
                                 bpp == 32 ? 2 :
                                             0,
                                 bpp_combobox_changed, NULL, N_("8 bit"), N_("16 bit"),
                                 N_("32 bit (float)"));
