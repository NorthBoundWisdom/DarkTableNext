...
void init(dt_imageio_module_format_t *self)
{
    ...
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, NO_COMPRESSION,
                         "off");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, RLE_COMPRESSION,
                         "rle");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, ZIPS_COMPRESSION,
                         "zips");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, ZIP_COMPRESSION,
                         "zip");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, PIZ_COMPRESSION,
                         "piz");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, PXR24_COMPRESSION,
                         "pxr24");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, B44_COMPRESSION,
                         "b44");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, B44A_COMPRESSION,
                         "b44a");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, DWAA_COMPRESSION,
                         "dwaa");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t, DWAB_COMPRESSION,
                         "dwab");
#ifdef IMF_HTJ2K32_COMPRESSION
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t,
                         HTJ2K256_COMPRESSION, "htj2k256");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_compression_t,
                         HTJ2K32_COMPRESSION, "htj2k32");
#endif
    ...
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_pixeltype_t, EXR_PT_HALF,
                         "half");
    luaA_enum_value_name(darktable.lua_state.state, dt_imageio_exr_pixeltype_t, EXR_PT_FLOAT,
                         "float");
                ...
                struct dt_dev_pixelpipe_t *pipe, const gboolean export_masks)
{
    ...
    if (!cmsIsToneCurveLinear(red_curve) || !cmsIsToneCurveLinear(green_curve) ||
...
icc_error:
    dt_control_log(
        _("the selected output profile isn't linear, expect incorrect display by image viewers"));
...
icc_end:
    ...
    if (pixel_type == Imf::PixelType::FLOAT)
    {
        ...
        data.insert("R",
                    Imf::Slice(pixel_type, (char *)(in + 0), stride, stride * exr->global.width));

        data.insert("G",
                    Imf::Slice(pixel_type, (char *)(in + 1), stride, stride * exr->global.width));

        data.insert("B",
                    Imf::Slice(pixel_type, (char *)(in + 2), stride, stride * exr->global.width));
    }
    else
    {
        ...
        data.insert("R",
                    Imf::Slice(pixel_type, (char *)out_image, stride, stride * exr->global.width));
    ...
    if (export_masks && pipe)
    {
        for (GList *iter = pipe->nodes; iter; iter = g_list_next(iter))
        {
            ...
            {
                const char *pagename =
                    (char *)g_hash_table_lookup(piece->module->raster_mask.source.masks, key);
                ...
                gboolean free_mask;
                float *raster_mask = dt_dev_get_raster_mask(piece, piece->module,
                                                            GPOINTER_TO_INT(key), NULL, &free_mask);
                ...
                if (pixel_type == Imf::PixelType::FLOAT)
                {
                    ...
                    data.insert(layername, Imf::Slice(pixel_type, (char *)raster_mask, stride,
                                                      stride * exr->global.width));
                ...
                else
                {
                    ...
                    {
                        dt_print(DT_DEBUG_ALWAYS,
                                 "[exr export] error allocating mask conversion buffer");
                        return 1;
                    }
                    ...
                    for (size_t y = 0; y < height; y++)
                    {
                        for (size_t x = 0; x < width; x++)
                        {
                            ...
                            unsigned short *out_pixel =
                                (unsigned short *)out_mask + ((y * width) + x);

                            out_pixel[0] = half(in_pixel[0]).bits();
                        }
                    }

                    data.insert(layername, Imf::Slice(pixel_type, (char *)out_mask, stride,
                                                      stride * exr->global.width));
...
void *get_params(dt_imageio_module_format_t *self)
{
    ...
    d->compression =
        (dt_imageio_exr_compression_t)dt_conf_get_int("plugins/imageio/format/exr/compression");
...
{
    if (size != (int)self->params_size(self))
        return 1;
...
void gui_init(dt_imageio_module_format_t *self)
{
    ...
    DT_BAUHAUS_COMBOBOX_NEW_FULL(gui->compression, self, NULL, N_("compression"), NULL,
                                 ...
                                 N_("uncompressed"), N_("RLE"), N_("ZIPS"), N_("ZIP"), N_("PIZ"),
                                 N_("PXR24"), N_("B44"), N_("B44A"), N_("DWAA"), N_("DWAB")
#ifdef IMF_HTJ2K32_COMPRESSION
                                                                                     ,
                                 N_("HTJ2K256"), N_("HTJ2K32")
#endif
    );
    dt_bauhaus_combobox_set_default(
        gui->compression, dt_confgen_get_int("plugins/imageio/format/exr/compression", DT_DEFAULT));
