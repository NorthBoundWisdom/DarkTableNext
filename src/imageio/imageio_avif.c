...
dt_imageio_retval_t dt_imageio_open_avif(dt_image_t *img, const char *filename,
                                         dt_mipmap_buffer_t *mbuf)
{
    ...
    {
        dt_print(DT_DEBUG_IMAGEIO, "[avif_open] failed to parse '%s': %s", filename,
    ...
    if (!img->exif_inited)
    {
        ...
        if (exif && exif->size > 0)
        {
            ...
            if (result != AVIF_RESULT_OK)
            {
                dt_print(DT_DEBUG_IMAGEIO, "[avif_open] failed to read tiff header from '%s': %s",
    ...
    if (result != AVIF_RESULT_OK)
    {
        dt_print(DT_DEBUG_IMAGEIO, "[avif_open] failed to allocate pixels for '%s' : %s", filename,
                 avifResultToString(result));
        ret = DT_IMAGEIO_LOAD_FAILED;
        goto out;
    }
    ...
    *out = NULL;
    ...
    if (decoder == NULL || avif_image == NULL)
    {
        dt_print(DT_DEBUG_IMAGEIO,
                 "[avif read profile] failed to create decoder or image struct for '%s'", filename);
        goto out;
    }
    ...
    if (result != AVIF_RESULT_OK)
    {
        dt_print(DT_DEBUG_IMAGEIO, "[avif read profile] failed to parse '%s': %s", filename,
                 avifResultToString(result));
        goto out;
    }
        ...
        if (avif_image->colorPrimaries == AVIF_COLOR_PRIMARIES_BT709)
        {
            ...
            if (over)
            {
                dt_print(DT_DEBUG_IMAGEIO,
                         ...
                         filename, avif_image->transferCharacteristics,
                         avif_image->matrixCoefficients, cicp->transfer_characteristics,
