...
dt_imageio_retval_t dt_imageio_open_gm(dt_image_t *img, const char *filename,
                                       dt_mipmap_buffer_t *mbuf)
{
    ...
    if (!image)
    {
        dt_print(DT_DEBUG_ALWAYS, "[GraphicsMagick_open] ReadImage failed for '%s'", img->filename);
        err = DT_IMAGEIO_FILE_NOT_FOUND;
        goto error;
    }

    dt_print(DT_DEBUG_IMAGEIO, "[GraphicsMagick_open] loading image '%s'", img->filename);
    ...
    if (!mipbuf)
    {
        dt_print(DT_DEBUG_ALWAYS,
                 "[GraphicsMagick_open] could not alloc full buffer for image '%s'", img->filename);
    ...
    int ret = DispatchImage(image, 0, 0, img->width, img->height, colormap, FloatPixel, mipbuf,
                            &exception);
    ...
    if (ret != MagickPass)
    {
        dt_print(DT_DEBUG_ALWAYS, "[GraphicsMagick_open] error reading image pixels for '%s'",
                 img->filename);
    ...
    profile_data = (const uint8_t *)GetImageProfile(image, "ICM", &profile_length);
