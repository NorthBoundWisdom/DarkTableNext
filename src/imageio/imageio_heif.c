...
dt_imageio_retval_t dt_imageio_open_heif(dt_image_t *img, const char *filename,
                                         dt_mipmap_buffer_t *mbuf)
{
    ...
    if (!ctx)
    {
        dt_print(DT_DEBUG_ALWAYS, "Unable to allocate HEIF context");
    ...
    if (err.code != heif_error_Ok)
    {
        ...
        else if (err.code != heif_error_Unsupported_filetype &&
                 err.subcode != heif_suberror_No_ftyp_box)
        {
    ...
    if (num_images == 0)
    {
        dt_print(DT_DEBUG_ALWAYS, "No images found in HEIF file [%s]", filename);
    ...
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "Failed to read primary image from HEIF file [%s]", filename);
        ret = DT_IMAGEIO_UNSUPPORTED_FEATURE;
        goto out;
    }
        ...
        if (count == 1)
        {
            ...
            if (exif_size > 4)
            {
                ...
                if (exif_data)
                {
                    ...
                    if (err.code == heif_error_Ok)
                    {
                        const uint32_t exif_offset = exif_data[0] << 24 | exif_data[1] << 16 |
                                                     exif_data[2] << 8 | exif_data[3];
                        if (exif_size > 4 + exif_offset)
                            dt_exif_read_from_blob(img, exif_data + 4 + exif_offset,
    ...
    err = heif_decode_image(handle, &heif_img, heif_colorspace_RGB,
                            heif_chroma_interleaved_RRGGBB_LE, decode_options);
    heif_decoding_options_free(decode_options);
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "Failed to decode HEIF file [%s]", filename);
        ret = DT_IMAGEIO_FILE_CORRUPTED;
        goto out;
    }
    ...
    int rowbytes = 0;
    const uint8_t *data =
        heif_image_get_plane_readonly(heif_img, heif_channel_interleaved, &rowbytes);
    /*
  ...
  */
    ...
    if (mipbuf == NULL)
    {
        dt_print(DT_DEBUG_ALWAYS, "Failed to allocate mipmap buffer for HEIF image [%s]", filename);
    ...
    const int decoded_values_bit_depth =
        heif_image_get_bits_per_pixel_range(heif_img, heif_channel_interleaved);
    ...
    dt_print(DT_DEBUG_IMAGEIO, "Bit depth: '%d' for HEIF image [%s]", original_values_bit_depth,
             filename);
...
int dt_imageio_heif_read_profile(const char *filename, uint8_t **out, dt_colorspaces_cicp_t *cicp)
{
    ...
    *out = NULL;
    ...
    if (err.code != heif_error_Ok)
    {
        dt_print(DT_DEBUG_ALWAYS, "Failed to read HEIF file [%s]", filename);
        goto out;
    }
    ...
    case heif_color_profile_type_nclx:
        dt_print(DT_DEBUG_IMAGEIO, "Found NCLX color profile for HEIF file [%s]", filename);
        err = heif_image_handle_get_nclx_color_profile(handle, &profile_info_nclx);
        if (err.code != heif_error_Ok)
        {
            dt_print(DT_DEBUG_IMAGEIO, "Failed to get NCLX color profile data from HEIF file [%s]",
                     filename);
            goto out;
        }
        ...
        if (profile_info_nclx->color_primaries == heif_color_primaries_ITU_R_BT_709_5)
        {
            ...
            if (profile_info_nclx->transfer_characteristics ==
                ...
                profile_info_nclx->matrix_coefficients == heif_matrix_coefficients_ITU_R_BT_709_5)
            {
                ...
                cicp->transfer_characteristics =
                    (uint16_t)heif_transfer_characteristic_ITU_R_BT_709_5;
                over = TRUE;
            }
            ...
            if (over)
            {
                dt_print(DT_DEBUG_IMAGEIO,
                         "Overriding nclx color profile for HEIF file `%s': 1/%d/%d to 1/%d/%d",
                         filename, profile_info_nclx->transfer_characteristics,
                         profile_info_nclx->matrix_coefficients, cicp->transfer_characteristics,
    ...
    case heif_color_profile_type_prof:
        ...
        if (err.code != heif_error_Ok)
        {
            dt_print(DT_DEBUG_IMAGEIO, "Failed to read embedded ICC profile from HEIF image [%s]",
                     filename);
            g_free(icc_data);
            goto out;
        }
    ...
    case heif_color_profile_type_not_present:
        dt_print(DT_DEBUG_IMAGEIO, "No color profile for HEIF file [%s]", filename);
        break; /* heif_color_profile_type_not_present */

    default:
        dt_print(DT_DEBUG_IMAGEIO, "Unknown color profile data from HEIF file [%s]", filename);
        break;
    }
