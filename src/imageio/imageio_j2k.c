...
dt_imageio_retval_t dt_imageio_open_j2k(dt_image_t *img, const char *filename,
                                        dt_mipmap_buffer_t *mbuf)
{
    ...
    if (!fsrc)
    {
        dt_print(DT_DEBUG_ALWAYS, "[j2k_open] Error: failed to open '%s' for reading", filename);
    ...
    {
        dt_print(DT_DEBUG_ALWAYS, "[j2k_open] Error: '%s' has unsupported file format", filename);
        return DT_IMAGEIO_UNSUPPORTED_FORMAT;
    }
    ...
    if (!image)
    {
        dt_print(DT_DEBUG_ALWAYS, "[j2k_open] Error: failed to decode image '%s'", filename);
...
-----------------------------------------------------------*/
static void sycc_to_rgb(const int offset, const int upb, const int y, int cb, int cr, int *out_r,
                        int *out_g, int *out_b)
{
    cb -= offset;
    cr -= offset;
...
static void sycc422_to_rgb(opj_image_t *img)
{
    ...
    for (size_t i = 0; i < maxh; ++i)
    {
        ...
        for (size_t j = 0; j < maxw; j += 2)
        {
            ...
            sycc_to_rgb(offset, upb, y[rowstart + j], curr_cb, curr_cr, r + rowstart + j,
                        g + rowstart + j, b + rowstart + j);
            sycc_to_rgb(offset, upb, y[rowstart + j + 1], curr_cb, curr_cr, r + rowstart + j + 1,
                        g + rowstart + j + 1, b + rowstart + j + 1);
        }
    }
    free(img->comps[0].data);
    img->comps[0].data = r;
...
static void sycc420_to_rgb(opj_image_t *img)
{
    ...
    for (size_t i = 0; i < maxh; i += 2)
    {
        ...
        for (size_t j = 0; j < maxw; j += 2)
        {
            ...
            sycc_to_rgb(offset, upb, y[rowstart + j + 1], curr_cb, curr_cr, r + rowstart + j + 1,
                        g + rowstart + j + 1, b + rowstart + j + 1);
            sycc_to_rgb(offset, upb, y[nextrow + j], curr_cb, curr_cr, r + nextrow + j,
                        g + nextrow + j, b + nextrow + j);
            sycc_to_rgb(offset, upb, y[nextrow + j + 1], curr_cb, *cr, r + nextrow + j + 1,
                        g + nextrow + j + 1, b + nextrow + j + 1);
        }
    }
    free(img->comps[0].data);
    img->comps[0].data = r;
...
static void color_sycc_to_rgb(opj_image_t *img)
{
    ...
    if ((img->comps[0].dx == 1) && (img->comps[1].dx == 2) && (img->comps[2].dx == 2) &&
        (img->comps[0].dy == 1) && (img->comps[1].dy == 2) &&
