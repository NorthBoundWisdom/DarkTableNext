                                        ...
                                        dt_mipmap_buffer_t *mbuf)
{
    ...
    if (!img->exif_inited)
    {
        ...
        if (exif)
        {
            ...
            if (exif_size > 0)
                dt_exif_read_from_blob(img, exif_blob, exif_size);
        }
        else
        {
            if (Imf::hasOwner(header))
            {
                dt_metadata_set_import_lock(img->id, "Xmp.dc.rights", Imf::owner(header).c_str());
            }
...
// the OPENEXR_VERSION_HEX macro is broken for 3.2.0 and earlier, must compute directly
#if (((OPENEXR_VERSION_MAJOR << 24) | (OPENEXR_VERSION_MINOR << 16) |                              \
      (OPENEXR_VERSION_PATCH << 8)) >= 0x03020000)
            if (Imf::hasCameraMake(header))
            {
                g_strlcpy(img->exif_maker, Imf::cameraMake(header).c_str(),
                          sizeof(img->exif_maker));
            }
            if (Imf::hasCameraModel(header))
            {
                g_strlcpy(img->exif_model, Imf::cameraModel(header).c_str(),
                          sizeof(img->exif_model));
            }
    ...
    frameBuffer.insert("R", Imf::Slice(Imf::FLOAT, (char *)(buf - dx * 4 - dy * img->width * 4 + 0),
                                       xstride, ystride, 1, 1, 0.0));
    frameBuffer.insert("G", Imf::Slice(Imf::FLOAT, (char *)(buf - dx * 4 - dy * img->width * 4 + 1),
                                       xstride, ystride, 1, 1, 0.0));
    frameBuffer.insert("B", Imf::Slice(Imf::FLOAT, (char *)(buf - dx * 4 - dy * img->width * 4 + 2),
                                       xstride, ystride, 1, 1, 0.0));
    frameBuffer.insert("A", Imf::Slice(Imf::FLOAT, (char *)(buf - dx * 4 - dy * img->width * 4 + 3),
                                       xstride, ystride, 1, 1, 0.0));
