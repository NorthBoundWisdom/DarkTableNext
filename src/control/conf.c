...
{
    const dt_confgen_value_t *ea = g_hash_table_lookup(darktable.conf->x_confgen, (const char *)a);
    const dt_confgen_value_t *eb = g_hash_table_lookup(darktable.conf->x_confgen, (const char *)b);
    if (!ea || !eb)
        return 0;
    ...
    for (GList *l = result; l; l = l->next)
    {
        ...
        if (e->welcome_pagenum != cur_page)
        {
            if (e->welcome_pagenum != expected_page)
                fprintf(stderr,
                        "[confgen] welcome screen: expected page %d but got page %d (key: %s)\n",
        ...
        if (e->welcome_questionnum != expected_question)
            fprintf(
                stderr,
                "[confgen] welcome screen: on page %d expected question %d but got %d (key: %s)\n",
