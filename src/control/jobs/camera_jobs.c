...
static int32_t dt_camera_capture_job_run(dt_job_t *job)
{
    ...
    dt_control_job_set_progress_message(
        job, ngettext("capturing %d image", "capturing %d images", total), total);
    ...
    if (!expprogram)
        expprogram =
            (char *)dt_camctl_camera_get_property(darktable.camctl, NULL, "autoexposuremode");
    ...
    const char *value =
        dt_camctl_camera_property_get_first_choice(darktable.camctl, NULL, "shutterspeed");
    ...
    for (uint32_t i = 0; i < params->count; i++)
    {
        ...
        for (uint32_t b = 0; b < (params->brackets * 2) + 1; b++)
        {
            ...
            if (params->brackets && current_value)
                dt_camctl_camera_set_property_string(darktable.camctl, NULL, "shutterspeed",
                                                     current_value->data);
...
dt_job_t *dt_camera_capture_job_create(const char *jobcode, const uint32_t delay,
                                       const uint32_t count, const uint32_t brackets,
...
static int32_t dt_camera_import_job_run(dt_job_t *job)
{
    ...
    dt_control_job_set_progress_message(
        job, ngettext("importing %d image from camera", "importing %d images from camera", total),
        total);
...
{
    dt_job_t *job =
        dt_control_job_create(&dt_camera_import_job_run, "import selected images from camera");
    if (!job)
        return NULL;
