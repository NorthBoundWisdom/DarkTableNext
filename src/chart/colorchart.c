...
chart_t *parse_cht(const char *filename)
{
    ...
    while (fgets(line, MAX_LINE_LENGTH, fp))
    {
        ...
        if (!g_strcmp0(keyword, "BOXES") && last_block < BLOCK_BOXES)
        {
            ...
            while (fgets(line, MAX_LINE_LENGTH, fp))
            {
                ...
                else if ((*c == 'D') || (*c == 'X') || (*c == 'Y'))
                {
                    ...
                    *c++ = '\0';
                    ...
                    size_t lxs_len = strlen(lxs), lxe_len = strlen(lxe), lys_len = strlen(lys),
                           lye_len = strlen(lye);
                    if (lxs_len > lxe_len || lys_len > lye_len)
                        ERROR;
                    ...
                    y_max = MAX(y_max, y + h);
                    if (kl == 'X' || kl == 'Y')
                        g_hash_table_insert(result->patch_sets,
                                            g_strdup_printf("%s .. %s", first_label, last_label),
                                            labels);
            ...
            if (n_boxes !=
                g_hash_table_size(result->d_table) + g_hash_table_size(result->box_table))
                ERROR;
...
int parse_it8(const char *filename, chart_t *chart)
{
    ...
    dt_colorspaces_color_profile_type_t color_space = DT_COLORSPACE_NONE;
    int column_SAMPLE_ID = -1, column_X = -1, column_Y = -1, column_Z = -1, column_L = -1,
    ...
    while (g_hash_table_iter_next(&table_iter, &key, &value))
    {
        ...
        checker_set_color(box, color_space, cmsIT8GetDataDbl(hIT8, key, columns[0]),
