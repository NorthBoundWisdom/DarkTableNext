...
{
    const double r = sqrt((x[0] - y[0]) * (x[0] - y[0]) + (x[1] - y[1]) * (x[1] - y[1]) +
                          (x[2] - y[2]) * (x[2] - y[2]));
...
static inline double compute_error(const tonecurve_t *c, const double **target,
                                   const double *residual_L, const double *residual_a,
                                   const double *residual_b, const int wd, double *maxerr)
{
    // compute error:
    double err = 0.0;
    double merr = 0.0;
    for (int i = 0; i < wd; i++)
    {
        ...
        err += localerr;
#else
        const double localerr = sqrt(residual_L[i] * residual_L[i] + residual_a[i] * residual_a[i] +
...
static inline int solve(double *As, double *w, double *v, const double *b, double *coeff,
                        const int wd, const int s, const int S)
{
    ...
    double *maxerr)   // max error
{
    ...
    for (int i = 0; i < N; i++)
        A[i * wd + N + 0] = A[(N + 0) * wd + i] = 1.0f;
    for (int i = 0; i < N; i++)
        A[i * wd + N + 1] = A[(N + 1) * wd + i] = point[3 * i + 0];
    for (int i = 0; i < N; i++)
        A[i * wd + N + 2] = A[(N + 2) * wd + i] = point[3 * i + 1];
    for (int i = 0; i < N; i++)
        A[i * wd + N + 3] = A[(N + 3) * wd + i] = point[3 * i + 2];
    ...
    for (int k = 0; k < dim; k++)
        b[k] = target[k];
    for (int k = 0; k < dim; k++)
        memcpy(r[k], b[k], sizeof(double) * wd);
    ...
    for (; s < wd; s++)
    {
        ...
        { // already have chosen S-4 patches as columns, now do the replacement:
            ...
            if (minerr < 1. / maxdot)
            {
                ...
                norm[mincol] = 0.0;
                for (int j = 0; j < wd; j++)
                    norm[mincol] += A[j * wd + mincol] * A[j * wd + mincol];
