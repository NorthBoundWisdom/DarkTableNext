...
static const point_t bb_ref[] = {{.x = .0, .y = .0},
                                 {.x = 1., .y = 0.},
                                 {.x = 1., .y = 1.},
                                 {.x = 0., .y = 1.}};
...
{
    // gtk gui
    GtkWidget *window, *image_button, *cht_button, *it8_button, *reference_image_button,
        *reference_it8_box, *reference_image_box, *process_button, *export_button,
        *export_raw_button, *reference_mode, *number_patches, *source_shrink, *reference_shrink,
        *result_label;
    GtkWidget *treeview;
    GtkTreeModel *model;
...
static void get_xyz_sample_from_image(const image_t *const image, float shrink, box_t *box,
                                      float *xyz);
...
static gboolean handle_motion(GtkWidget *widget, GdkEventMotion *event, dt_lut_t *self,
                              image_t *image);
static int find_closest_corner(point_t *bb, float x, float y);
static void map_mouse_to_0_1(GtkWidget *widget, GdkEventMotion *event, image_t *image, float *x,
                             float *y);
...
{
    for (int i = 0; i < 4; i++)
        bb[i] = map_point_to_view(image, image->bb[i]);
}
...
static gboolean motion_notify_callback_source(GtkWidget *widget, GdkEventMotion *event,
                                              gpointer user_data)
{
...
static gboolean motion_notify_callback_reference(GtkWidget *widget, GdkEventMotion *event,
                                                 gpointer user_data)
{
    dt_lut_t *self = (dt_lut_t *)user_data;
...
static gboolean handle_motion(GtkWidget *widget, GdkEventMotion *event, dt_lut_t *self,
                              image_t *image)
{
    if (!(event->state & GDK_BUTTON1_MASK) || !image->image)
        return FALSE;
...
static gboolean open_source_image(dt_lut_t *self, const char *filename)
{
    ...
    if (!res)
        gtk_file_chooser_unselect_all(GTK_FILE_CHOOSER(self->image_button));
...
static gboolean open_cht(dt_lut_t *self, const char *filename)
{
    ...
    if (!res)
        gtk_file_chooser_unselect_all(GTK_FILE_CHOOSER(self->cht_button));
...
static char *get_export_filename(dt_lut_t *self, const char *extension, char **name,
                                 char **description, gboolean *basecurve, gboolean *colorchecker,
                                 gboolean *colorin, gboolean *tonecurve)
{
    GtkWidget *name_entry = NULL, *description_entry = NULL;
    GtkWidget *dialog = gtk_file_chooser_dialog_new(
        "save file", GTK_WINDOW(self->window), GTK_FILE_CHOOSER_ACTION_SAVE, _("_cancel"),
    ...
    *description = NULL;
    ...
    if (basecurve)
    {
        ...
        gtk_grid_attach_next_to(GTK_GRID(grid), cb_colorchecker, cb_basecurve, GTK_POS_BOTTOM, 1,
                                1);
...
static void print_patches(dt_lut_t *self, FILE *fd, GList *patch_names)
{
    for (GList *iter = patch_names; iter; iter = g_list_next(iter))
    {
        ...
        fprintf(fd, "%s", key);
        for (int i = 0; i < 3; i++)
            fprintf(fd, ";%s", g_ascii_dtostr(s, sizeof(s), source_Lab[i]));
        for (int i = 0; i < 3; i++)
            fprintf(fd, ";%s", g_ascii_dtostr(s, sizeof(s), reference_Lab[i]));
        fprintf(fd, "\n");
    }
}

static void print_xml_plugin(FILE *fd, int num, int op_version, const char *operation,
                             const char *op_params, gboolean enabled)
{
    fprintf(fd, "  <plugin>\n");
...
static void export_style(dt_lut_t *self, const char *filename, const char *name,
                         const char *description, gboolean include_basecurve,
                         gboolean include_colorchecker, gboolean include_colorin,
                         gboolean include_tonecurve)
{
    ...
    if (include_basecurve)
    {
        print_xml_plugin(
            fd, num++, 2, "basecurve",
            "gz09eJxjYIAAM6vnNnqyn22E9n235b6aa3cy6rVdRaK9/Y970fYf95bbMzA0QPEoGEqADYnNhMQGAO0WEJo=",
            FALSE);
    }
    ...
    if (include_colorin)
    {
        ...
        print_xml_plugin(fd, num++, 4, "colorin",
                         "gz09eJzjZqAfYIHSAAWQABA=", TRUE); // gamut clipping to rec2020
    }
    // 2: add tonecurve
...
static void export_raw_button_clicked_callback(GtkButton *button, gpointer user_data)
{
    ...
    if (filename)
        export_raw(self, filename, name, description);
    g_free(name);
    g_free(description);
    g_free(filename);
}
...
{
    dt_lut_t *self = (dt_lut_t *)user_data;
    if (!self->tonecurve_encoded || !self->colorchecker_encoded)
        return;

    char *name = NULL, *description = NULL;
    gboolean include_basecurve, include_colorchecker, include_colorin, include_tonecurve;
    char *filename =
        get_export_filename(self, ".dtstyle", &name, &description, &include_basecurve,
                            &include_colorchecker, &include_colorin, &include_tonecurve);
    if (filename)
        export_style(self, filename, name, description, include_basecurve, include_colorchecker,
                     include_colorin, include_tonecurve);
    g_free(name);
    g_free(description);
    g_free(filename);
}

static void add_patches_to_array(dt_lut_t *self, GList *patch_names, int *N, int *i,
                                 double *target_L, double *target_a, double *target_b,
                                 double *colorchecker_Lab)
{
    for (GList *iter = patch_names; iter; iter = g_list_next(iter))
    {
        ...
        for_three_channels(j) colorchecker_Lab[3 * (*i) + j] = source_Lab[j];
        ...
        if (deltaE > thrs)
        {
            fprintf(stderr, "warning: ignoring patch %s with large difference deltaE %g!\n", key,
                    deltaE);
            fprintf(stderr, "      %g %g %g -- %g %g %g\n", source_Lab[0], source_Lab[1],
                    source_Lab[2], reference_Lab[0], reference_Lab[1], reference_Lab[2]);
            (*N)--; // ignore this patch.
            (*i)--;
        }
        (*i)++;
                            ...
                            double **colorchecker_Lab)
{
    ...
    if (n_extra_patches > 0)
    {
        ...
        *colorchecker_Lab = realloc(*colorchecker_Lab, sizeof(double) * 3 * (*N + n_extra_patches));
        ...
        memmove(&(*colorchecker_Lab)[3 * n_extra_patches], *colorchecker_Lab,
                sizeof(double) * 3 * *N);
...
static char *encode_colorchecker(int num, const double *point, const double **target,
                                 int *permutation)
{
    ...
    for (int k = 0; k < num - 1; k++)
        for (int j = 0; j < num - k - 1; j++)
        {
            if (thinplate_color_pos(params.source_L[j], params.source_a[j], params.source_b[j]) <
                thinplate_color_pos(params.source_L[j + 1], params.source_a[j + 1],
                                    params.source_b[j + 1]))
            {
                         ...
                         double *colorchecker_Lab, int N, int sparsity)
{
    ...
    int num_tonecurve = 0;
    {
        ...
        for (int i = 0; i < N; i++)
        {
            const double sat_in = colorchecker_Lab[3 * i + 1] * colorchecker_Lab[3 * i + 1] +
                                  colorchecker_Lab[3 * i + 2] * colorchecker_Lab[3 * i + 2];
            const double sat_out = target_a[i] * target_a[i] + target_b[i] * target_b[i];
  ...
  for(int k = 0; k < N; k++) target_L[k] = tonecurve_unapply(&tonecurve, target_L[k]);
    ...
    if (self->result_label != NULL)
    {
        ...
        char *result_string =
            g_strdup_printf(_("average dE: %.02f\nmax dE: %.02f"), avgerr, maxerr);
...
static void process_button_clicked_callback(GtkButton *button, gpointer user_data)
{
    ...
    while (g_hash_table_iter_next(&table_iter, &set_key, &value))
    {
        ...
        add_patches_to_array(self, patch_names, &N, &i, target_L, target_a, target_b,
                             colorchecker_Lab);
    }
...
static GtkWidget *create_notebook_page_source(dt_lut_t *self)
{
    ...
    g_signal_connect(source_shrink, "value-changed", G_CALLBACK(shrink_changed_callback),
                     &self->source);
...
static GtkWidget *create_notebook_page_reference(dt_lut_t *self)
{
    ...
    GtkWidget *it8_button = gtk_file_chooser_button_new("reference data of a color chart",
                                                        GTK_FILE_CHOOSER_ACTION_OPEN);
    g_signal_connect(it8_button, "file-set", G_CALLBACK(it8_changed_callback), self);

    GtkWidget *reference_image_button =
        gtk_file_chooser_button_new("image of a color chart", GTK_FILE_CHOOSER_ACTION_OPEN);
    g_signal_connect(reference_image_button, "file-set", G_CALLBACK(ref_image_changed_callback),
                     self);

    GtkWidget *reference_shrink =
        gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 0.5, 2.0, 0.01);
    gtk_scale_set_value_pos(GTK_SCALE(reference_shrink), GTK_POS_RIGHT);
    g_signal_connect(reference_shrink, "value-changed", G_CALLBACK(shrink_changed_callback),
                     &self->reference);
    ...
    gtk_box_pack_start(GTK_BOX(reference_image_box), gtk_label_new("reference image:"), FALSE, TRUE,
                       0);
...
static GtkWidget *create_notebook_page_process(dt_lut_t *self)
{
    ...
    g_signal_connect(export_raw_button, "clicked", G_CALLBACK(export_raw_button_clicked_callback),
                     self);
...
static GtkWidget *create_table(dt_lut_t *self)
{
    ...
    add_column(GTK_TREE_VIEW(self->treeview), "deltaE (1976)", COLUMN_DE_1976,
               COLUMN_DE_1976_FLOAT);
    add_column(GTK_TREE_VIEW(self->treeview), "deltaE (2000)", COLUMN_DE_2000,
               COLUMN_DE_2000_FLOAT);
        ...
        if (box)
        {
            ...
            if (patch)
            {
                ...
                s_RGB_in = g_strdup_printf("%d; %d; %d", (int)(patch->rgb[0] * 255 + 0.5),
                                           (int)(patch->rgb[1] * 255 + 0.5),
                                           (int)(patch->rgb[2] * 255 + 0.5));
            ...
            gtk_list_store_set(GTK_LIST_STORE(self->model), &iter, COLUMN_RGB_IN, s_RGB_in,
                               COLUMN_LAB_IN, s_Lab_in, COLUMN_LAB_REF, s_Lab_ref, COLUMN_DE_1976,
                               s_deltaE_1976, COLUMN_DE_1976_FLOAT, deltaE_1976, COLUMN_DE_2000,
                               s_deltaE_2000, COLUMN_DE_2000_FLOAT, deltaE_2000, -1);
            g_free(s_RGB_in);
            g_free(s_Lab_in);
            g_free(s_Lab_ref);
...
static void get_Lab_from_box(box_t *box, float *Lab)
{
    ...
    case DT_COLORSPACE_XYZ:
    {
        ...
        for_each_channel(i) XYZ[i] = box->color[i] * 0.01;
...
static void init_table(dt_lut_t *self)
{
    ...
    {
        gtk_list_store_insert_with_values(GTK_LIST_STORE(self->model), NULL, -1, COLUMN_NAME,
                                          (char *)name->data, -1);
    }
...
{
    if (self->chart)
        g_hash_table_foreach(self->chart->box_table, collect_source_patches_foreach, self);
}

static void collect_reference_patches(dt_lut_t *self)
{
    if (self->chart)
        g_hash_table_foreach(self->chart->box_table, collect_reference_patches_foreach, self);
}
    ...
    for (int y = y_start; y < y_end; y++)
        for (int x = x_start; x < x_end; x++)
        {
            if ((x - corners[TOP_LEFT].x) / delta_x_top * delta_y_top + corners[TOP_LEFT].y < y &&
                (x - corners[BOTTOM_LEFT].x) / delta_x_bottom * delta_y_bottom +
                        corners[BOTTOM_LEFT].y >
                    y &&
                (y - corners[TOP_LEFT].y) / delta_y_left * delta_x_left + corners[TOP_LEFT].x < x &&
                (y - corners[TOP_RIGHT].y) / delta_y_right * delta_x_right + corners[TOP_RIGHT].x >
                    x)
            {
...
static box_t get_sample_box(chart_t *chart, box_t *outer_box, float shrink)
{
    ...
    float x_shrink = shrink * chart->box_shrink / chart->bb_w,
          y_shrink = shrink * chart->box_shrink / chart->bb_h;
...
static void get_corners(const float *homography, box_t *box, point_t *corners)
{
    ...
    for (int i = 0; i < 4; i++)
        corners[i] = apply_homography(corners[i], homography);
}
...
static void init_image(dt_lut_t *self, image_t *image, GCallback motion_cb)
{
    ...
    gtk_widget_add_events(image->drawing_area, GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
                                                   GDK_BUTTON_RELEASE_MASK);
    g_signal_connect(image->drawing_area, "size-allocate", G_CALLBACK(size_allocate_callback),
                     image);
...
static void free_image(image_t *image)
{
    ...
    reset_bb(image);
    if (image->image)
        cairo_pattern_destroy(image->image);
    if (image->surface)
...
static void gui_command_line(GApplication *app, GApplicationCommandLine *cmdline,
                             gpointer user_data)
{
    int argc;
    ...
    if (source_filename && open_source_image(self, source_filename))
    {
        ...
        if (cht_filename && open_cht(self, cht_filename))
        {
            ...
            {
                gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(self->reference_image_button),
                                              reference_filename);
...
static int main_gui(dt_lut_t *self, int argc, char *argv[])
{
    GtkApplication *app = gtk_application_new(
        "org.darktable.Chart", G_APPLICATION_HANDLES_COMMAND_LINE | G_APPLICATION_NON_UNIQUE);
...
static int parse_csv(dt_lut_t *self, const char *filename, double **target_L_ptr,
                     double **target_a_ptr, double **target_b_ptr, double **source_Lab_ptr,
                     int *num_gray, char **name, char **description)
{
    *target_L_ptr = NULL;
    *target_a_ptr = NULL;
    *target_b_ptr = NULL;
    ...
    *source_Lab_ptr = source_Lab;
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (fgets(line, sizeof(line) / sizeof(*line), f) == 0)
            break;
        while (*iter != ';')
            iter++;
        *iter++ = '\0';
        ...
        if (sqrt(d[0] * d[0] + d[1] * d[1] + d[2] * d[2]) > thrs)
        {
            ...
            fprintf(stderr, "      %g %g %g -- %g %g %g\n", source_Lab[3 * i + 0],
                    source_Lab[3 * i + 1], source_Lab[3 * i + 2], target_L[i], target_a[i],
                    target_b[i]);
            N--; // ignore this patch.
            i--;
        }
...
static int main_csv(dt_lut_t *self, int argc, char *argv[])
{
    ...
    int N = parse_csv(self, filename_csv, &target_L, &target_a, &target_b, &colorchecker_Lab,
                      &num_tonecurve, &name, &description);
