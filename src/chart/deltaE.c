...
float dt_colorspaces_deltaE_2000(dt_aligned_pixel_t Lab0, dt_aligned_pixel_t Lab1)
{
    ...
    const float T = 1.f - 0.17f * cosf(deg2radf(H_ip - 30.f)) + 0.24f * cosf(deg2radf(2.f * H_ip)) +
                    0.32 * cosf(deg2radf(3.f * H_ip + 6.f)) -
                    0.2f * cosf(deg2radf(4.f * H_ip - 63.f));
    ...
    const float SL = 1.0 + ((0.015 * (L_ip - 50.0) * (L_ip - 50.0)) /
                            sqrtf(20.0 + (L_ip - 50.0) * (L_ip - 50.0)));
    ...
    const float dE = sqrtf(
        (dL_p / (KL * SL)) * (dL_p / (KL * SL)) + (dC_p / (KC * SC)) * (dC_p / (KC * SC)) +
