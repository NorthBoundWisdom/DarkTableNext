...
static gint sort_monitor_list(gconstpointer a, gconstpointer b)
{
    ...
    if (monitor_a->is_primary)
        return -1;
    if (monitor_b->is_primary)
        return 1;
...
int main(int argc __attribute__((unused)), char *arg[] __attribute__((unused)))
{
    ...
    {
        int atom_id =
            0; // the id of the x atom. might be changed when sorting in the primary later!
    ...
    for (GList *iter = monitor_list; iter; iter = g_list_next(iter))
    {
        ...
        int res = XGetWindowProperty(display, monitor->root, atom, 0, G_MAXLONG, FALSE, XA_CARDINAL,
    ...
    if (!client || !cd_client_connect_sync(client, NULL, NULL))
    {
        ...
        for (GList *iter = monitor_list; iter; iter = g_list_next(iter))
        {
            ...
            CdDevice *device = cd_client_find_device_by_property_sync(
                client, CD_DEVICE_METADATA_XRANDR_NAME, monitor->name, NULL, NULL);
    ...
    for (GList *iter = monitor_list; iter; iter = g_list_next(iter))
    {
        ...
        if (message)
            printf("\t%s", message);
        printf("\n\tX atom:\t%s (%zu bytes)\n\t\tdescription: %s\n", x_atom_name,
               monitor->x_atom_length, x_atom_description);
#ifdef HAVE_COLORD
