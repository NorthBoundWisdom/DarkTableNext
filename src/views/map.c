...
static void _view_map_center_on_location(const dt_view_t *view, const gdouble lon,
                                         const gdouble lat, const gdouble zoom);
/* proxy function to center map view on a bounding box */
static void _view_map_center_on_bbox(const dt_view_t *view, const gdouble lon1, const gdouble lat1,
...
static GObject *_view_map_add_marker(const dt_view_t *view, const dt_geo_map_display_t type,
                                     GList *points);
/* proxy function to remove a marker from the map */
static gboolean _view_map_remove_marker(const dt_view_t *view, const dt_geo_map_display_t type,
                                        GObject *marker);
/* proxy function to add a location to the map */
static void _view_map_add_location(const dt_view_t *view, dt_map_location_data_t *g,
                                   const guint locid);
...
static void _view_map_geotag_changed(gpointer instance, GList *imgs, const int locid,
                                     gpointer user_data);
...
static void _view_map_filmstrip_activate_callback(gpointer instance, const dt_imgid_t imgid,
                                                  gpointer user_data);
/* callback when an image is dropped from filmstrip */
static void _drag_and_drop_received(GtkWidget *widget, GdkDragContext *context, const gint x,
                                    const gint y, GtkSelectionData *selection_data,
                                    const guint target_type, const guint time, gpointer data);
...
static void _view_map_dnd_get_callback(GtkWidget *widget, GdkDragContext *context,
                                       ...
                                       const guint time, dt_view_t *self);
...
static gboolean _view_map_drag_motion_callback(GtkWidget *widget, GdkDragContext *dc, const gint x,
                                               const gint y, const guint time, dt_view_t *self);
...
static void _view_map_dnd_remove_callback(GtkWidget *widget, GdkDragContext *context, const gint x,
                                          const gint y, GtkSelectionData *selection_data,
                                          const guint target_type, const guint time, gpointer data);
// find the images clusters on the map
static int _dbscan(const dt_map_t *lib, dt_geo_position_t *points, const unsigned int num_points,
                   const double epsilon, const unsigned int minpts);
...
static OsmGpsMapImage *_view_map_draw_location(dt_map_t *lib, dt_location_draw_t *ld,
                                               const gboolean main);
static void _view_map_remove_location(const dt_map_t *lib, dt_location_draw_t *ld);
...
static int latlon2zoom(const int pix_height, const int pix_width, const float lat1,
                       const float lat2, const float lon1, const float lon2)
{
...
static void osm_gps_map_zoom_fit_bbox(OsmGpsMap *map, const float latitude1, const float latitude2,
                                      const float longitude1, const float longitude2)
{
    GtkAllocation allocation;
    gtk_widget_get_allocation(GTK_WIDGET(map), &allocation);
    const int zoom = latlon2zoom(allocation.height, allocation.width, deg2radf(latitude1),
                                 deg2radf(latitude2), deg2radf(longitude1), deg2radf(longitude2));
...
static void _rgba_from_color(const uint32_t color, float *r, float *g, float *b, float *a)
{
...
static GdkPixbuf *_draw_ellipse(const float dlongitude, const float dlatitude, const gboolean main)
{
    const int dlon = dlongitude > max_size   ? max_size :
                     dlongitude < cross_size ? cross_size :
                                               dlongitude;
    const int dlat = dlatitude > max_size   ? max_size :
                     dlatitude < cross_size ? cross_size :
                                              dlatitude;
    ...
    const int color_hi =
        dlon == max_size || dlon == cross_size ?
            main ? DT_GUI_COLOR_MAP_LOC_SHAPE_DEF : DT_GUI_COLOR_MAP_LOC_SHAPE_HIGH :
                                  ...
                                  const gboolean main)
{
    ...
    dt_gui_gtk_set_source_rgb(
        cr, (dlon == max_size || dlon == cross_size || dlat == max_size || dlat == cross_size) ?
                main ? DT_GUI_COLOR_MAP_LOC_SHAPE_DEF : DT_GUI_COLOR_MAP_LOC_SHAPE_HIGH :
                DT_GUI_COLOR_MAP_LOC_SHAPE_HIGH);
...
void init(dt_view_t *self)
{
    ...
    if (darktable.gui)
    {
        ...
        lib->map = g_object_new(OSM_TYPE_GPS_MAP, "map-source", OSM_GPS_MAP_SOURCE_NULL,
                                "proxy-uri", g_getenv("http_proxy"), NULL);

        g_object_ref(lib->map); // we want to keep map alive until explicit destroy

        lib->osd = g_object_new(OSM_TYPE_GPS_MAP_OSD, "show-scale", TRUE, "show-coordinates", TRUE,
                                "show-dpad", TRUE, "show-zoom", TRUE,
        ...
        gtk_drag_dest_set(GTK_WIDGET(lib->map), GTK_DEST_DEFAULT_ALL, target_list_internal,
                          n_targets_internal, GDK_ACTION_MOVE);
        g_signal_connect(GTK_WIDGET(lib->map), "scroll-event", G_CALLBACK(_view_map_scroll_event),
                         self);
        ...
        g_signal_connect(GTK_WIDGET(lib->map), "changed", G_CALLBACK(_view_map_changed_callback),
                         self);
        ...
        g_signal_connect(G_OBJECT(lib->map), "motion-notify-event",
                         G_CALLBACK(_view_map_motion_notify_callback), self);
        g_signal_connect(G_OBJECT(lib->map), "drag-motion",
                         G_CALLBACK(_view_map_drag_motion_callback), self);
    }
...
void cleanup(dt_view_t *self)
{
    ...
    if (lib->main_query)
        sqlite3_finalize(lib->main_query);
    free(self->data);
}

void configure(dt_view_t *self, const int wd, const int ht)
{
...
static void _view_map_signal_change_raise(const gpointer user_data)
{
    ...
    dt_control_signal_block_by_func(darktable.signals, G_CALLBACK(_view_map_collection_changed),
                                    self);
    DT_CONTROL_SIGNAL_RAISE(DT_SIGNAL_GEOTAG_CHANGED, (GList *)NULL, 0);
    dt_control_signal_unblock_by_func(darktable.signals, G_CALLBACK(_view_map_collection_changed),
                                      self);
    dt_control_signal_unblock_by_func(darktable.signals, G_CALLBACK(_view_map_geotag_changed),
                                      self);
}
...
static void _view_map_angles(const dt_map_t *lib, const gint pixels, const float lat,
                             const float lon, float *dlat_min, float *dlon_min)
{
    ...
    osm_gps_map_convert_screen_to_geographic(lib->map, pixel_x + pixels, pixel_y + pixels, pt1);
...
static void _view_map_thumb_angles(dt_map_t *lib, const float lat, const float lon, float *dlat_min,
                                   float *dlon_min)
{
...
static float _view_map_angles_to_pixels(const dt_map_t *lib, const float lat0, const float lon0,
                                        const float angle)
{
...
static GdkPixbuf *_draw_location(const dt_map_t *lib, int *width, int *height,
                                 const dt_map_location_data_t *data, const gboolean main)
{
...
dt_location_draw_t *_others_location_draw(const dt_map_t *lib, const int locid)
{
    for (GList *other = lib->loc.others; other; other = g_list_next(other))
    {
        dt_location_draw_t *d = other->data;
                                               ...
                                               const gboolean main)
{
    if (ld->data.shape != MAP_LOCATION_SHAPE_POLYGONS)
    {
        ...
        if (draw)
        {
            location = osm_gps_map_image_add_with_alignment(lib->map, ld->data.lat, ld->data.lon,
                                                            draw, 0.5, 0.5);
            g_object_unref(draw);
        }
        return location;
...
static void _view_map_draw_main_location(dt_map_t *lib, const dt_location_draw_t *ld)
{
...
static void _view_map_draw_other_locations(dt_map_t *lib, const dt_map_box_t *bbox)
{
...
static GdkPixbuf *_draw_image(const dt_imgid_t imgid, int *width, int *height,
                              ...
                              const uint32_t frame, const gboolean blocking, const int thumbnail,
                              const dt_view_t *self)
{
    ...
    if (thumbnail == DT_MAP_THUMB_THUMB)
    {
        ...
        dt_mipmap_cache_get(&buf, imgid, mip, blocking ? DT_MIPMAP_BLOCKING : DT_MIPMAP_BEST_EFFORT,
                            'r');

        if (buf.buf && buf.width > 0)
        {
            for (int i = 3; i < (size_t)4 * buf.width * buf.height; i += 4)
                buf.buf[i] = UINT8_MAX;
            ...
            source = gdk_pixbuf_new_from_data(buf.buf, GDK_COLORSPACE_RGB, TRUE, 8, buf.width,
            ...
            gdk_pixbuf_scale(source, thumb, _thumb_border, _thumb_border, w, h, _thumb_border,
                             _thumb_border, (1.0 * w) / buf.width, (1.0 * h) / buf.height,
                             GDK_INTERP_HYPER);
            // add the pin
            gdk_pixbuf_copy_area(lib->image_pin, 0, 0, w + 2 * _thumb_border, _pin_size, thumb, 0,
                                 h + 2 * _thumb_border);
            // add the count
            if (group_count)
            {
                double count_height, count_width;
                count = _view_map_images_count(group_count, group_same_loc, &count_width,
                                               &count_height);
    ...
    else if (thumbnail == DT_MAP_THUMB_COUNT)
    {
        ...
        gdk_pixbuf_copy_area(count, 0, 0, count_width, count_height, thumb, _thumb_border,
                             _thumb_border);
        gdk_pixbuf_copy_area(lib->image_pin, 0, 0, w, _pin_size, thumb, 0,
                             count_height + 2 * _thumb_border);
        if (width)
            *width = count_width;
        if (height)
            *height = count_height;
...
static gboolean _view_map_draw_image(dt_map_image_t *entry, const gboolean blocking,
                                     const int thumbnail, const dt_view_t *self)
{
    const dt_map_t *lib = self->data;
    ...
    if (!entry->image)
    {
        GdkPixbuf *thumb = _draw_image(
            entry->imgid, &entry->width, &entry->height, entry->group_count, entry->group_same_loc,
            entry->selected_in_group ? thumb_frame_sel_color : thumb_frame_color, blocking,
            thumbnail, self);
        if (thumb)
        {
...
static gboolean _view_map_draw_images(const gpointer user_data)
{
    ...
    {
        needs_redraw =
            _view_map_draw_image((dt_map_image_t *)iter->data, FALSE, lib->thumbnail, self);
        img_drawn++;
...
static void _view_map_get_bounding_box(const dt_map_t *lib, dt_map_box_t *bbox)
{
    OsmGpsMapPoint bb[2];
...
static void _view_map_changed_callback_delayed(const gpointer user_data)
{
    ...
    if (!lib->timeout_event_source && lib->thumbnail != DT_MAP_THUMB_NONE)
    {
        ...
        g_object_get(G_OBJECT(lib->map), "zoom", &zoom, "latitude", &center_lat, "longitude",
                     &center_lon, NULL);
            ...
            for (i = 0; i < img_count; i++)
            {
                if (p[i].cluster_id == NOISE)
                {
                    ...
                    if (entry)
                    {
                        ...
                        if (sel_imgs)
                            entry->selected_in_group =
                                g_list_find((GList *)sel_imgs, GINT_TO_POINTER(entry->imgid)) ?
                                    TRUE :
                                    FALSE;
                ...
                else if (!processed[p[i].cluster_id])
                {
                    ...
                    if (entry)
                    {
                        ...
                        entry->selected_in_group =
                            (sel_imgs &&
                             g_list_find((GList *)sel_imgs, GINT_TO_POINTER(p[i].imgid))) ?
                                TRUE :
                                FALSE;
...
static dt_map_image_t *_view_map_get_entry_at_pos(const dt_view_t *self, const double x,
                                                  const double y)
{
    const dt_map_t *lib = self->data;
...
static GList *_view_map_get_imgs_at_pos(const dt_view_t *self, const float x, const float y,
...
static gboolean _display_next_image(const dt_view_t *self, dt_map_image_t *entry,
                                    const gboolean next)
{
    const dt_map_t *lib = self->data;
    if (!entry)
                                    ...
                                    const dt_imgid_t imgid, const int count)
{
    const dt_map_t *lib = self->data;
    int height;
    GdkPixbuf *thumb = _draw_image(imgid, NULL, &height, count, TRUE, thumb_frame_sel_color, TRUE,
...
static gboolean _view_map_motion_notify_callback(GtkWidget *widget, GdkEventMotion *e,
                                                 dt_view_t *self)
{
    dt_map_t *lib = self->data;
         ...
         abs(lib->start_drag_y - (int)ceil(e->y_root))) > DT_PIXEL_APPLY_DPI(8))
    {
        ...
        GdkDragContext *context = gtk_drag_begin_with_coordinates(
            GTK_WIDGET(lib->map), targets, GDK_ACTION_MOVE, 1, (GdkEvent *)e, -1, -1);
        ...
        if (location)
        {
            ...
            gtk_widget_show(image);
            gtk_drag_set_icon_widget(context, image, DT_PIXEL_APPLY_DPI(width),
...
static gboolean _zoom_and_center(const gint x, const gint y, const int direction,
                                 const dt_view_t *self)
{
...
static gboolean _view_map_button_press_callback(GtkWidget *w, GdkEventButton *e, dt_view_t *self)
{
    ...
    if (lib->osd)
    {
        ...
        if (e->x >= osd_x && e->x <= osd_x + 2 * dpad_radius && e->y >= osd_y &&
            e->y <= osd_y + 2 * dpad_radius + zoom_height)
        {
            return FALSE;
        }
    }
    ...
    if (e->button == GDK_BUTTON_PRIMARY)
    {
            ...
            for (GList *other = lib->loc.others; other; other = g_list_next(other))
            {
                ...
                if (dt_map_location_included(lon, lat, &d->data))
                {
                    ...
                    dt_control_signal_unblock_by_func(
                        darktable.signals, G_CALLBACK(_view_map_collection_changed), self);
        ...
        // check if the click was on image(s) or just some random position
        lib->selected_images = _view_map_get_imgs_at_pos(
            self, e->x, e->y, &lib->start_drag_offset_x, &lib->start_drag_offset_y,
...
{
    dt_action_register(DT_ACTION(self), N_("undo"), _view_map_undo_callback, GDK_KEY_z,
                       GDK_CONTROL_MASK);
    dt_action_register(DT_ACTION(self), N_("redo"), _view_map_redo_callback, GDK_KEY_y,
                       GDK_CONTROL_MASK);

    dt_action_register(DT_ACTION(self), N_("scroll right"), _view_map_scroll_right_callback,
                       GDK_KEY_Right, 0);
    ...
    dt_action_register(DT_ACTION(self), N_("scroll left"), _view_map_scroll_left_callback,
                       GDK_KEY_Left, 0);
    dt_action_register(DT_ACTION(self), N_("scroll left wide"), _view_map_scroll_left_wide_callback,
                       GDK_KEY_Left, GDK_CONTROL_MASK);
    dt_action_register(DT_ACTION(self), N_("scroll up"), _view_map_scroll_up_callback, GDK_KEY_Up,
                       0);
    dt_action_register(DT_ACTION(self), N_("scroll up wide"), _view_map_scroll_up_wide_callback,
                       GDK_KEY_Up, GDK_CONTROL_MASK);
    dt_action_register(DT_ACTION(self), N_("scroll down"), _view_map_scroll_down_callback,
                       GDK_KEY_Down, 0);
    dt_action_register(DT_ACTION(self), N_("scroll down wide"), _view_map_scroll_down_wide_callback,
...
static void _view_map_set_map_source(const dt_view_t *view, const OsmGpsMapSource_t map_source)
{
...
static OsmGpsMapImage *_view_map_add_pin(const dt_view_t *view, const GList *points)
{
    const dt_map_t *lib = view->data;
...
static void _track_add_point(OsmGpsMapTrack *track, OsmGpsMapPoint *point,
                             OsmGpsMapPoint *prev_point)
{
        ...
        fabs(lon - prev_lon) > DT_MINIMUM_ANGULAR_DELTA_FOR_GEODESIC)
    {
        ...
        dt_gpx_geodesic_distance(lat, lon, prev_lat, prev_lon, &d, &delta);
    }
    ...
    else
    {
        ...
        for (int i = 1; i <= n_segments; i++)
        {
            ...
            dt_gpx_geodesic_intermediate_point(prev_lat, prev_lon, lat, lon, delta, first_time, f,
                                               &ith_lat, &ith_lon);
...
static OsmGpsMapPolygon *_view_map_add_polygon_location(dt_map_t *lib, dt_location_draw_t *ld)
{
    ...
    {
        const dt_geo_map_display_point_t *p = iter->data;
        if (p->lat <= bbox.lat1 && p->lat >= bbox.lat2 && p->lon >= bbox.lon1 &&
            p->lon <= bbox.lon2)
        {
            if ((fabs(p->lat - prev_lat) > dlat) || (fabs(p->lon - prev_lon) > dlon))
            {
...
static OsmGpsMapImage *_view_map_draw_single_image(const dt_view_t *view, const GList *points)
{
    const dt_map_t *lib = view->data;
    struct
    {
    ...
    p = points->data;
    GdkPixbuf *thumb = _draw_image(p->imgid, NULL, NULL, p->count, TRUE, thumb_frame_gpx_color,
                                   TRUE, DT_MAP_THUMB_THUMB, (dt_view_t *)view);
    OsmGpsMapImage *image = NULL;
    if (thumb)
    {
        image =
            osm_gps_map_image_add_with_alignment(lib->map, p->latitude, p->longitude, thumb, 0, 1);
        g_object_unref(thumb);
    }
    return image;
}
                                     ...
                                     GList *points)
{
    ...
    case MAP_DISPLAY_THUMB:
        return G_OBJECT(_view_map_draw_single_image(view, points));
    default:
        return NULL;
    }
}
    ...
    if (g)
    {
        if (g->delta1 != 0.0 && g->delta2 != 0.0)
        {
            ...
            if (max_lon > min_lon && max_lat > min_lat)
            {
                ...
                if (g->lon < lib->bbox.lon1 || g->lon > lib->bbox.lon2 || g->lat > lib->bbox.lat1 ||
                    g->lat < lib->bbox.lat2)
...
static void _view_map_check_preference_changed(gpointer instance, const gpointer user_data)
{
...
static void _view_map_collection_changed(gpointer instance,
                                         ...
                                         gpointer imgs, int next, const gpointer user_data)
{
    dt_view_t *self = (dt_view_t *)user_data;
    const dt_map_t *lib = self->data;
...
static void _view_map_selection_changed(gpointer instance, const gpointer user_data)
{
    const dt_view_t *self = (dt_view_t *)user_data;
    const dt_map_t *lib = self->data;
    ...
    if (selection_data != NULL && target_type == DND_TARGET_IMGID)
    {
        ...
        if (imgs_nb)
        {
            ...
            if (imgs_nb == 1 && imgt[0] == -1)
            {
                ...
                lib->loc.main.data.ratio =
                    _view_map_get_angles_ratio(lib, lib->loc.main.data.lat, lib->loc.main.data.lon);
            ...
            if (imgs_nb)
            {
                ...
                if (imgs)
                {
                    ...
                    gtk_selection_data_set(selection_data,
                                           gtk_selection_data_get_target(selection_data), _DWORD,
                                           (guchar *)imgs, imgs_nb * sizeof(uint32_t));
                    free(imgs);
                }
            }
        }
...
static gboolean _view_map_prefs_changed(dt_map_t *lib)
{
    ...
    lib->thumbnail = !g_strcmp0(thumbnail, "thumbnail") ? DT_MAP_THUMB_THUMB :
                     !g_strcmp0(thumbnail, "count")     ? DT_MAP_THUMB_COUNT :
                                                          DT_MAP_THUMB_NONE;
...
static void _view_map_build_main_query(dt_map_t *lib)
{
  ...
  geo_query =
    ...
    DT_DEBUG_SQLITE3_PREPARE_V2(dt_database_get(darktable.db), geo_query, -1, &lib->main_query,
                                NULL);
...
static double round_down(const double x, const double binsize)
{
    const double scaled = floor(x / binsize);
    return scaled * binsize;
}

static void _bin_points(const dt_map_t *lib, const double epsilon)
{
...
static gboolean _not_clustered(const unsigned int lon, const unsigned int lat)
{
...
static void _add_expand_cluster(const unsigned int lon, const unsigned int lat, const int id,
                                int iter)
{
    // add the current bin to the cluster
    if (iter >= 0)
    {
        for (unsigned int pt = db.geo_bins[lon][lat].points; pt != NO_NEXT_POINT;
...
static gboolean _identical_positions(const unsigned int lon, const unsigned int lat)
{
...
static gboolean _can_form_cluster(const unsigned int lon, const unsigned int lat)
{
        ...
        for (unsigned int lon = 0; lon < db.num_lon_bins; lon++)
        {
            ...
            for (unsigned int lat = 0; lat < db.num_lat_bins; lat++)
            {
                ...
                if (db.geo_bins[lon][lat].num_points > 0 &&
                    (id = db.points[db.geo_bins[lon][lat].points].cluster_id) >=
                        0) // already in cluster?
                {
