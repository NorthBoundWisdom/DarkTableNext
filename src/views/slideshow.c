...
static void _shift_right(dt_slideshow_t *d)
{
    ...
    d->buf[S_LEFT].imgid =
        d->buf[S_LEFT].rank >= 0 ? _get_image_at_rank(d->buf[S_LEFT].rank) : NO_IMGID;
    d->id_displayed = -1;
    d->id_preview_displayed = -1;
    dt_free_align(tmp_buf);
}
...
static int _process_image(dt_slideshow_t *d, dt_slideshow_slot_t slot)
{
    ...
    dt_dev_image(imgid, d->width / darktable.gui->ppd, d->height / darktable.gui->ppd, -1, &buf,
                 NULL, &width, &height, NULL, -1, NULL, DT_DEVICE_NONE, FALSE);
...
{
    return d->buf[slot].invalidated && d->buf[slot].buf == NULL &&
           dt_is_valid_imgid(d->buf[slot].imgid) && d->buf[slot].rank >= 0;
}
...
static void _step_state(dt_slideshow_t *d, const dt_slideshow_event_t event)
{
    ...
    if (refresh_display)
        dt_control_queue_redraw_center();
    if (d->auto_advance)
        g_timeout_add_seconds(d->delay, _auto_advance, d);
}
...
static void _slow_down_callback(dt_action_t *action)
{
    ...
    _set_delay(d, 1);
    dt_control_log(
        ngettext("slideshow delay set to %d second", "slideshow delay set to %d seconds", d->delay),
        d->delay);
}
...
{
    dt_action_register(DT_ACTION(self), N_("start and stop"), _start_stop_callback, GDK_KEY_space,
                       0);
    ...
    dt_action_register(DT_ACTION(self), N_("step forward"), _step_forward_callback, GDK_KEY_Right,
                       0);
