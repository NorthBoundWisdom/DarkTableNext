...
static void _print_mipmaps_updated_signal_callback(gpointer instance, dt_imgid_t imgid,
                                                   gpointer user_data)
{
                                    ...
                                    gpointer user_data)
{
    ...
    if (bidx != -1)
    {
        ...
        if (imgs_nb)
        {
            ...
            dt_printing_setup_image(prt->imgs, bidx, imgid, 100, 100, ALIGNMENT_CENTER);
            success = TRUE;
        }
    }
...
static gboolean _drag_motion_received(GtkWidget *widget, GdkDragContext *dc, const gint x,
                                      const gint y, const guint time, gpointer data)
{
...
static void _expose_print_page(dt_view_t *self, cairo_t *cr, const int32_t width,
                               const int32_t height, const int32_t pointerx, const int32_t pointery)
{
    ...
    dt_get_print_layout(prt->pinfo, width, height, &px, &py, &pwidth, &pheight, &ax, &ay, &awidth,
                        &aheight, &borderless);
    ...
    dt_printing_setup_display(prt->imgs, px, py, pwidth, pheight, ax, ay, awidth, aheight,
                              borderless);
...
void enter(dt_view_t *self)
{
    ...
    {
        // change active image
        dt_thumbtable_set_offset_image(dt_ui_thumbtable(darktable.gui->ui), prt->imgs->box[0].imgid,
                                       TRUE);
    ...
    gtk_drag_dest_set(widget, GTK_DEST_DEFAULT_ALL, target_list_all, n_targets_all,
                      GDK_ACTION_MOVE);
