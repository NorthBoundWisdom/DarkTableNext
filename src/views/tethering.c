...
static void _view_capture_filmstrip_activate_callback(gpointer instance, const dt_imgid_t imgid,
                                                      gpointer user_data);
...
static int _tethering_write_image(dt_imageio_module_data_t *data, const char *filename,
...
static void _expose_tethered_mode(dt_view_t *self, cairo_t *cr, const int32_t width,
                                  const int32_t height, const int32_t pointerx,
                                  const int32_t pointery)
{
    ...
    if (cam->is_live_viewing == TRUE) // display the preview
    {
        ...
        if (cam->live_view_buffer)
        {
            ...
            if (tmp_i)
            {
                ...
                cmsDoTransformLineStride(darktable.color_profiles->transform_srgb_to_display, p_buf,
                                         tmp_i, pw, ph, pw * 4, stride, 0, 0);
                pthread_rwlock_unlock(&darktable.color_profiles->xprofile_lock);

                cairo_surface_t *source = dt_cairo_image_surface_create_for_data(
                    tmp_i, CAIRO_FORMAT_RGB24, pw, ph, stride);
                if (cairo_surface_status(source) == CAIRO_STATUS_SUCCESS)
                {
                    ...
                    cairo_translate(cr, width * 0.5,
                                    (height + BAR_HEIGHT) * 0.5); // origin to middle of canvas
            ...
            if (tmp_f)
            {
                ...
                {
                    uint32_t DT_ALIGNED_ARRAY state[4] = {splitmix32(p + 1),
                                                          splitmix32((p + 1) * (p + 3)),
                    ...
                    for (int k = 0; k < 3; k++)
                        tmp_f[p + k] =
                            dt_noise_generator(DT_NOISE_UNIFORM, p_buf[p + k], 0.5f, 0, state) /
                            255.0f;
                }
                ...
                if (darktable.color_profiles->histogram_type == DT_COLORSPACE_WORK)
                {
                    ...
                    profile_to = dt_ioppr_add_profile_info_to_list(
                ...
                // FIXME: if liveview image is tagged and we can read its colorspace, use that
                darktable.lib->proxy.histogram.process(darktable.lib->proxy.histogram.module, tmp_f,
                                                       pw, ph, srgb_profile, profile_to);
    ...
    else if (dt_is_valid_imgid(lib->image_id)) // First of all draw image if available
    {
        ...
        const dt_view_surface_value_t res = dt_view_image_get_surface(
            lib->image_id, width - (MARGIN * 2.0f), height - (MARGIN * 2.0f), &surf, FALSE);
        ...
        if (!dt_imageio_export_with_flags(
                lib->image_id, "unused", &format, (dt_imageio_module_data_t *)&dat, TRUE, FALSE,
                FALSE, FALSE, FALSE, 1.0, FALSE, NULL, FALSE, FALSE, histogram_type,
                histogram_filename, DT_INTENT_PERCEPTUAL, NULL, NULL, 1, 1, NULL, -1))
        {
            const dt_iop_order_iccprofile_info_t *const histogram_profile =
                dt_ioppr_add_profile_info_to_list(darktable.develop, histogram_type,
                                                  histogram_filename,
                                                  DT_INTENT_RELATIVE_COLORIMETRIC);
            darktable.lib->proxy.histogram.process(darktable.lib->proxy.histogram.module, dat.buf,
...
static void _capture_mipmaps_updated_signal_callback(gpointer instance, const dt_imgid_t imgid,
                                                     gpointer user_data)
{
    dt_view_t *self = (dt_view_t *)user_data;
                                                  ...
                                                  void *data)
{
    ...
    /* update import session with original filename so that $(FILE.EXTENSION)
...
static void _camera_capture_image_downloaded(const dt_camera_t *camera, const char *in_folder,
                                             const char *in_filename, const char *filename,
                                             void *data)
{
...
int button_pressed(dt_view_t *self, const double x, const double y, const double pressure,
                   const int which, const int type, const uint32_t state)
{
    ...
    else if ((which == GDK_BUTTON_MIDDLE || which == GDK_BUTTON_SECONDARY) &&
