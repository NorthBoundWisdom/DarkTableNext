...
{
    return dt_iop_set_description(self, _("apply usual image adjustments"), _("creative"),
                                  _("linear, RGB, scene-referred"), _("non-linear, RGB"),
...
static void _signal_profile_user_changed(gpointer instance, uint8_t profile_type,
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    p->middle_grey = (work_profile) ? (dt_ioppr_get_rgb_matrix_luminance(
                                           self->picked_color, work_profile->matrix_in,
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (process_gamma || plain_contrast)
    {
        for (unsigned int i = 0; i < 0x10000; i++)
        {
            ...
            if (process_gamma)
                d->lut_gamma[i] = get_gamma(percentage, gamma);
            if (plain_contrast)
                d->lut_contrast[i] =
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->sl_black_point,
    ...
    g_signal_connect(G_OBJECT(g->sl_middle_grey), "quad-pressed",
    ...
    g->bt_auto_levels =
        dt_action_button_new(NULL, N_("auto"), _auto_levels_callback, self,
                             _("apply auto exposure based on the entire image"), 0, 0);
    ...
    dt_gui_box_add(self->widget, dt_gui_expand(g->bt_auto_levels),
                   dt_gui_expand(g->bt_select_region));

    g->sl_clip = dt_bauhaus_slider_from_params(self, N_("clip"));
    dt_bauhaus_slider_set_digits(g->sl_clip, 3);
    gtk_widget_set_tooltip_text(g->sl_clip,
                                _("adjusts clipping value for auto exposure calculation"));
...
static void _get_auto_exp_histogram(const float *const img, const int width, const int height,
                                    int *box_area, uint32_t **_histogram, unsigned int *_hist_size,
                                    int *_histcompr)
{
    const int ch = 4;
...
static void _get_sum_and_average(const uint32_t *const histogram, const int hist_size, float *_sum,
                                 float *_avg)
{
    float sum = 0.f;
    float avg = 0.f;
...
static void _get_auto_exp(const uint32_t *const histogram, const unsigned int hist_size,
                          const int histcompr, const float defgain, const float clip,
                          const float midgray, float *_expcomp, float *_bright, float *_contr,
                          float *_black, float *_hlcompr, float *_hlcomprthresh)
{
    ...
    {
        expcomp2 =
            0.5f * ((15.5f - histcompr - (2.f * oct7 - oct6)) + logf(scale / rawmax) / M_LN2f);
    }
    else
    {
        expcomp2 = 0.5f * ((15.5f - histcompr - (2.f * octile[7] - octile[6])) +
                           logf(scale / rawmax) / M_LN2f);
    }

    if (fabsf(expcomp1) - fabsf(expcomp2) > 1.f) // for great expcomp
    {
        expcomp = (expcomp1 * fabsf(expcomp2) + expcomp2 * fabsf(expcomp1)) /
                  (fabsf(expcomp1) + fabsf(expcomp2));
    }
    else
    {
    ...
    const float comp = (gain * ((float)whiteclip) / scale - 1.f) *
                       2.3f; // 2.3 instead of 2 to increase slightly comp
    ...
    whiteclipg = igamma2(
        whiteclipg); // need to inverse gamma transform to get correct exposure compensation parameter
...
static void _auto_exposure(const float *const img, const int width, const int height, int *box_area,
                           const float clip, const float midgray, float *_expcomp, float *_bright,
                           float *_contr, float *_black, float *_hlcompr, float *_hlcomprthresh)
{
    uint32_t *histogram = NULL;
    ...
    _get_auto_exp(histogram, hist_size, histcompr, defGain, clip, midgray, _expcomp, _bright,
                  _contr, _black, _hlcompr, _hlcomprthresh);
...
static void _get_selected_area(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                               dt_iop_basicadj_gui_data_t *g, const dt_iop_roi_t *const roi_in,
                               int *box_out)
{
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (g && dt_pipe_is_preview(piece->pipe))
    {
        ...
        if (g->call_auto_exposure == 1 && !DT_IN_GUI_UPDATE())
        {
            ...
            if (src_buffer == NULL)
            {
                dt_print(
                    DT_DEBUG_ALWAYS,
                    "[basicadj process_cl] error allocating memory for color transformation 1");
            ...
            if (err != CL_SUCCESS)
            {
                dt_print(
                    DT_DEBUG_ALWAYS,
                    "[basicadj process_cl] error allocating memory for color transformation 2");
                goto cleanup;
            }
            ...
            _auto_exposure(src_buffer, roi_in->width, roi_in->height, box, g->params.clip,
                           g->params.middle_grey / 100.f, &g->params.exposure,
                           &g->params.brightness, &g->params.contrast, &g->params.black_point,
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_basicadj, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(dev_gamma), CLARG(dev_contrast), CLARG(black_point), CLARG(scale),
        CLARG(process_gamma), CLARG(gamma), CLARG(plain_contrast), CLARG(preserve_colors),
        CLARG(contrast), CLARG(process_saturation_vibrance), CLARG(saturation), CLARG(vibrance),
        CLARG(process_hlcompr), CLARG(hlcomp), CLARG(hlrange), CLARG(middle_grey),
        CLARG(inv_middle_grey), CLARG(dev_profile_info), CLARG(dev_profile_lut),
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (g && dt_pipe_is_preview(piece->pipe))
    {
        ...
        if (g->call_auto_exposure == 1 && !DT_IN_GUI_UPDATE())
        {
            ...
            int box[4] = {0};
            _get_selected_area(self, piece, g, roi_in, box);
            _auto_exposure((const float *const)ivoid, roi_in->width, roi_in->height, box,
                           g->params.clip, g->params.middle_grey / 100.f, &g->params.exposure,
                           &g->params.brightness, &g->params.contrast, &g->params.black_point,
                           &g->params.hlcompr, &g->params.hlcomprthresh);
    ...
    for (size_t k = 0; k < stride; k += ch)
    {
        ...
        if (process_hlcompr)
        {
            const float lum = (work_profile) ?
                                  dt_ioppr_get_rgb_matrix_luminance(
                                      out + k, work_profile->matrix_in, work_profile->lut_in,
        ...
        {
            // gamma
            if (process_gamma && out[k + c] > 0.f)
                out[k + c] = get_lut_gamma(out[k + c], gamma, d->lut_gamma);

            // contrast
            if (plain_contrast && out[k + c] > 0.f)
                out[k + c] = get_lut_contrast(out[k + c], contrast, middle_grey, inv_middle_grey,
                                              d->lut_contrast);
        }
        ...
        if (process_saturation_vibrance)
        {
            ...
            const float delta = sqrtf((average - out[k]) * (average - out[k]) +
                                      (average - out[k + 1]) * (average - out[k + 1]) +
                                      (average - out[k + 2]) * (average - out[k + 2]));
