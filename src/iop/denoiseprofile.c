...
typedef struct dt_iop_denoiseprofile_params_t
{
    ...
    gboolean
        fix_anscombe_and_nlmeans_norm; // $DEFAULT: TRUE $DESCRIPTION: "fix various bugs in algorithm" backward compatibility options
    gboolean
        use_new_vst; // $DEFAULT: TRUE $DESCRIPTION: "upgrade profiled transform" backward compatibility options
...
*/
int legacy_params_to11(dt_iop_module_t *self, const void *const old_params, const int old_version,
                       void *new_params, const int new_version)
{
    ...
    typedef struct dt_iop_denoiseprofile_params_v4_t
    {
        ...
        float y[DT_DENOISE_PROFILE_NONE_V9]
               [DT_IOP_DENOISE_PROFILE_V8_BANDS]; // values to change wavelet force by frequency
  ...
  typedef struct dt_iop_denoiseprofile_params_v10_t
  {
    ...
    if ((old_version == 1 || old_version == 2 || old_version == 3) && new_version == 4)
    {
        ...
        dt_noiseprofile_t interpolated =
            dt_iop_denoiseprofile_get_auto_profile(self, NULL, NULL, 0, NULL, FALSE);
    ...
    else if (new_version == 6)
    {
        ...
        {
            // first update to v5
            if (legacy_params_to11(self, old_params, old_version, &v5, 5))
                return 1;
        }
        else
    ...
    else if (new_version == 7)
    {
        ...
        {
            // first update to v6
            if (legacy_params_to11(self, old_params, old_version, &v6, 6))
                return 1;
        }
        else
    ...
    else if (new_version == 8)
    {
        ...
        {
            // first update to v7
            if (legacy_params_to11(self, old_params, old_version, &v7, 7))
                return 1;
        }
        else
    ...
    else if (new_version == 9)
    {
        ...
        {
            // first update to v8
            if (legacy_params_to11(self, old_params, old_version, &v8, 8))
                return 1;
        }
        else
    ...
    else if (new_version == 10)
    {
        ...
        {
            // first update to v9
            if (legacy_params_to11(self, old_params, old_version, &v9, 9))
                return 1;
        }
        else
    ...
    else if (new_version == 11)
    {
        ...
        if (old_version < 10)
        {
            if (legacy_params_to11(self, old_params, old_version, v11, 10))
                return 1;
        }
        else
            memcpy(v11, old_params, sizeof(*v11)); // was v10 already

        if ((v11->mode == MODE_WAVELETS || v11->mode == MODE_WAVELETS_AUTO) &&
                  ...
                  void **new_params, int32_t *new_params_size, int *new_version)
{
    ...
    if (old_version < 11)
    {
        *new_params = (dt_iop_denoiseprofile_params_v11_t *)malloc(
            sizeof(dt_iop_denoiseprofile_params_v11_t));

        const int ret = legacy_params_to11(self, old_params, old_version, *new_params, 11);
    ...
    if (old_version == 11)
    {
        const dt_iop_denoiseprofile_params_v11_t *o =
            (dt_iop_denoiseprofile_params_v11_t *)old_params;
...
{
    const dt_aligned_pixel_t sigma2_plus_3_8 = {(b[0] / a[0]) * (b[0] / a[0]) + 3.f / 8.f,
                                                (b[1] / a[1]) * (b[1] / a[1]) + 3.f / 8.f,
                                                (b[2] / a[2]) * (b[2] / a[2]) + 3.f / 8.f, 0.0f};
...
static inline void backtransform(float *const buf, const int wd, const int ht,
                                 const dt_aligned_pixel_t a, const dt_aligned_pixel_t b)
{
    const dt_aligned_pixel_t sigma2_plus_1_8 = {(b[0] / a[0]) * (b[0] / a[0]) + 1.f / 8.f,
                                                (b[1] / a[1]) * (b[1] / a[1]) + 1.f / 8.f,
                                                (b[2] / a[2]) * (b[2] / a[2]) + 1.f / 8.f, 0.0f};
    const size_t npixels = (size_t)wd * ht;
                               ...
                               const dt_aligned_pixel_t weights)
{
    ...
    {
        for_each_channel(i) wb[i] = weights[i] * piece->pipe->dsc.processed_maximum[i];
    }
    return;
}
                                  ...
                                  dt_aligned_pixel_t bb, dt_aligned_pixel_t p)
{
    ...
    {
        precondition_v2((float *)ivoid, in, roi_in->width, roi_in->height, d->a[1] * compensate_p,
                        p, d->b[1], wb);
    }
    return compensate_p;
}
                              ...
                              const dt_iop_roi_t *const roi_out)
{
    ...
    if (!d->use_new_vst)
        err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_denoiseprofile_precondition, width,
                                               height, CLARG(dev_in), CLARG(dev_tmp), CLARG(width),
                                               CLARG(height), CLARG(aa), CLARG(sigma2));
    else
...
error:
    ...
    if (err == CL_SUCCESS)
    {
        if (!d->use_new_vst)
        {
            err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_denoiseprofile_finish, width,
                                                   height, CLARG(dev_in), CLARG(dev_U2),
        ...
        else
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_args(
                devid, gd->kernel_denoiseprofile_finish_v2, width, height, CLARG(dev_in),
                CLARG(dev_U2), CLARG(dev_out), CLARG(width), CLARG(height), CLARG(aa), CLARG(p),
                               ...
                               const dt_iop_roi_t *const roi_out)
{
    ...
    {
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_denoiseprofile_backtransform, width, height, CLARG(dev_tmp),
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->fix_anscombe_and_nlmeans_norm =
        dt_bauhaus_toggle_from_params(self, "fix_anscombe_and_nlmeans_norm");
