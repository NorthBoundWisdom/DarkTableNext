...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("fill-light 0.25EV with 4 zones"), self->op, self->version(),
                               ...
                               sizeof(dt_iop_relight_params_t), TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    return dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_relight, width, height, CLARG(dev_in),
                                            CLARG(dev_out), CLARG(width), CLARG(height),
                                            CLARG(center), CLARG(wings), CLARG(ev));
}
#endif
...
#define NEUTRAL_GRAY 0.5
    ...
    g->center = DTGTK_GRADIENT_SLIDER(dtgtk_gradient_slider_new_with_color_and_name(
        _gradient_L[0], _gradient_L[1], "gslider-relight"));
    gtk_widget_set_tooltip_text(GTK_WIDGET(g->center),
    ...
    gtk_widget_set_tooltip_text(GTK_WIDGET(g->colorpicker),
                                _("toggle tool for picking median lightness in image"));
