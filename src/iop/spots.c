...
{
    return dt_iop_set_description(self, _("remove sensor dust spots"), _("corrective"),
                                  _("linear, RGB, scene-referred"), _("geometric, raw"),
                  ...
                  void **new_params, int32_t *new_params_size, int *new_version)
{
    ...
    if (old_version == 1)
    {
        ...
        for (GList *l = self->dev->history; l; l = g_list_next(l))
        {
            ...
            count++;
            if (!strcmp(item->op_name, "spots"))
                last_spot_num = item->num;
        }
...
static gboolean _add_shape(GtkWidget *widget, const int creation_continuous, dt_iop_module_t *self)
{
    ...
    if (!_reset_form_creation(widget, self))
        return TRUE;
...
static gboolean _edit_masks(GtkWidget *widget, GdkEventButton *e, dt_iop_module_t *self)
{
    ...
    dt_masks_set_edit_mode(self, self->dev->form_gui->edit_mode == DT_MASKS_EDIT_FULL ?
                                     DT_MASKS_EDIT_OFF :
                                     DT_MASKS_EDIT_FULL);
...
static int masks_point_calc_delta(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
                                  const dt_iop_roi_t *roi, const float *target, const float *source,
                                  int *dx, int *dy)
{
...
static int masks_get_delta(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece,
...
void _process(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const float *const in,
              ...
              const int ch)
{
    ...
    if (grp && (grp->type & DT_MASKS_GROUP))
    {
        for (const GList *forms = grp->points; (pos < 64) && forms;
             pos++, forms = g_list_next(forms))
        {
            ...
            if (d->clone_algo[pos] == 1 && (form->type & DT_MASKS_CIRCLE))
            {
                ...
                for (int yy = posy; yy < posy + fh; yy++)
                {
                    ...
                    if (yy < roi_out->y || yy >= roi_out->y + roi_out->height)
                        continue;
                    // we test if the source point is inside roi_in
                    if (yy - dy < roi_in->y || yy - dy >= roi_in->y + roi_in->height)
                        continue;
                    for (int xx = posx; xx < posx + fw; xx++)
                    {
                        // we test if we are inside roi_out
                        if (xx < roi_out->x || xx >= roi_out->x + roi_out->width)
                            continue;
                        // we test if the source point is inside roi_in
                        if (xx - dx < roi_in->x || xx - dx >= roi_in->x + roi_in->width)
                            continue;

                        const float f = filter[xx - posx + 1] * filter[yy - posy + 1];
                        for (int c = 0; c < ch; c++)
                            out[ch *
                                    ((size_t)roi_out->width * (yy - roi_out->y) + xx - roi_out->x) +
                                c] = out[ch * ((size_t)roi_out->width * (yy - roi_out->y) + xx -
                                               roi_out->x) +
                                         c] *
                                         (1.0f - f) +
                                     in[ch * ((size_t)roi_in->width *
                                                  (yy - posy + posy_source - roi_in->y) +
                                              xx - posx + posx_source - roi_in->x) +
                                        c] *
                                         f;
                    }
                }
                ...
                if (dx != 0 || dy != 0)
                {
                    ...
                    for (int yy = fts + 1; yy < fts + fhs - 1; yy++)
                    {
                        ...
                        for (int xx = fls + 1; xx < fls + fws - 1; xx++)
                        {
                            ...
                            for (int c = 0; c < ch; c++)
                                out[ch * ((size_t)roi_out->width * (yy - roi_out->y) + xx -
                                          roi_out->x) +
                                    c] = out[ch * ((size_t)roi_out->width * (yy - roi_out->y) + xx -
                                                   roi_out->x) +
                                             c] *
                                             (1.0f - f) +
                                         in[ch * ((size_t)roi_in->width * (yy - dy - roi_in->y) +
                                                  xx - dx - roi_in->x) +
                                            c] *
                                             f;
                        }
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->bt_path = dt_iop_togglebutton_new(self, N_("shapes"), N_("add path"),
                                         N_("add multiple paths"), G_CALLBACK(_add_shape_callback),
                                         TRUE, 0, 0, dtgtk_cairo_paint_masks_path, self->widget);

    g->bt_ellipse = dt_iop_togglebutton_new(
        self, N_("shapes"), N_("add ellipse"), N_("add multiple ellipses"),
        G_CALLBACK(_add_shape_callback), TRUE, 0, 0, dtgtk_cairo_paint_masks_ellipse, self->widget);

    g->bt_circle = dt_iop_togglebutton_new(
        self, N_("shapes"), N_("add circle"), N_("add multiple circles"),
        G_CALLBACK(_add_shape_callback), TRUE, 0, 0, dtgtk_cairo_paint_masks_circle, self->widget);
}
