...
{
    return dt_iop_set_description(self, _("adjust the look of the image"), _("creative"),
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_colisa, width, height, CLARG(dev_in),
                                           CLARG(dev_out), CLARG(width), CLARG(height),
                                           CLARG(saturation), CLARG(dev_cm), CLARG(dev_ccoeffs),
                                           CLARG(dev_lm), CLARG(dev_lcoeffs));
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (size_t k = 0; k < (size_t)width * height; k++)
    {
        ...
        out[k * ch + 0] = (L < 100.0f) ?
                              data->ltable[CLAMP((int)(L / 100.0f * 0x10000ul), 0, 0xffff)] :
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    d->contrast = p->contrast +
                  1.0f; // rescale from [-1;+1] to [0;+2] (zero meaning no contrast -> gray plane)
    d->brightness = p->brightness * 2.0f; // rescale from [-1;+1] to [-2;+2]
    d->saturation =
        p->saturation + 1.0f; // rescale from [-1;+1] to [0;+2] (zero meaning no saturation -> b&w)
