...
extern "C"
{
    ...
    gboolean lut3d_read_gmz(int *const nb_keypoints, unsigned char *const keypoints,
                            const char *const filename, int *const nb_lut, void *widget,
                           ...
                           const char *const filename)
{
    ...
    const size_t img_size = image_list[0]._width * image_list[0]._height * image_list[0]._depth *
                            image_list[0]._spectrum;
    std::memcpy(output_clut_data, image_list[0]._data, img_size * sizeof(float));
    image_list.assign(0);
}

unsigned int lut3d_get_cached_clut(float *const output_clut_data,
                                   const unsigned int output_resolution, const char *const filename)
{
    gmic_list<float> image_list;
    gmic_list<char> image_names;
    char gmic_cmd[512];
    ...
    size_t img_size = image_list[0]._width * image_list[0]._height * image_list[0]._depth *
                      image_list[0]._spectrum;
    if (output_size < img_size) // downsize the cached lut
    {
        std::snprintf(gmic_cmd, sizeof(gmic_cmd), "-r %u,%u,%u,3,3", output_resolution,
        ...
        img_size = image_list[0]._width * image_list[0]._height * image_list[0]._depth *
                   image_list[0]._spectrum;
    }
