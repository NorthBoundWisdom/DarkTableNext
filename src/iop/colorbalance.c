...
static void add_preset(dt_iop_module_so_t *self, const char *name, const char *pi,
                       const int version, const char *bpi, const int blendop_version)
{
    int len, blen;
...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    add_preset(
        self, N_("split-toning teal-orange (2nd instance)"),
        "gz02eJxjZGBg8HhYZX99cYN9kkCDfdCOOnsGhgZ7ruvN9m8CK+yXFNTaz5w50z5PqBku9u9/PVjNv//9jqfP+NgDAHs0HIc=",
        3,
        "gz05eJxjZWBgYGUAgRNODFDAzszAxMBQ5cwI4Tow4AUNdkBsD8E3gGwue9x8uB6q8s+c8bEF8Z9Y9Nnt2f3bbluCN03tg/EBIBckVg==",
        8);
    add_preset(
        self, N_("split-toning teal-orange (1st instance)"),
        "gz02eJxjZACBBvugHXX2E3fU219f3GAP4n/TqLFvfd1oL8HZaH/2jI/9prn1cLHUtDSwGgaGCY7//tfbAwBRixpm",
        3,
        "gz04eJxjZWBgYGUAgRNODFDApgwiq5wZIVyHD4E7bBnwggZ7CIYBRiBbBA8fXT1l/P5DX21i+pnA/Pfv8uw6OzzIMq9I5rgtSH//4wii1AMASbIlcw==",
        8);

    add_preset(
        self, N_("generic film"),
        "gz02eJxjZACBBntN5gb7op/19u5AGsSX3dFgr+jYaL+vttb+0NcM+1Pnq+3XyFTZr/rYBJZPS0sD0hMcQDQA29kXSQ==",
        3, "gz11eJxjYGBgkGAAgRNODGiAEV0AJ2iwh+CRxQcA5qIZBA==", 8);

    add_preset(self, N_("similar to Kodak Portra"),
               "gz02eJxjZACBBnsQfh3YYK8VU28P43s8rLKP6W+yP/Q1w36deyMYLymoBcsZGxcDaQGHs2d87AGnphWu",
               3, "gz11eJxjYGBgkGAAgRNODGiAEV0AJ2iwh+CRxQcA5qIZBA==", 8);

    add_preset(
        self, N_("similar to Kodak Ektar"),
...
static void _process_legacy(const dt_aligned_pixel_t in, dt_aligned_pixel_t out,
                            const size_t npixels, const dt_aligned_pixel_t lift,
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_aligned_pixel_t gamma_inv_legacy = {(gamma[0] != 0.0f) ? 1.0f / gamma[0] : 1000000.0f,
    ...
    for (size_t chunkstart = 0; chunkstart < npixels; chunkstart += chunksize)
    {
        ...
        {
        case LEGACY:
            _process_legacy(in + 4 * chunkstart, out + 4 * chunkstart, end - chunkstart, lift,
                            gamma_inv_legacy, gain);
            break;
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    case LEGACY:
    {
        ...
        err = dt_opencl_enqueue_kernel_2d_args(
            devid, gd->kernel_colorbalance, width, height, CLARG(dev_in), CLARG(dev_out),
            CLARG(width), CLARG(height), CLARG(lift), CLARG(gain), CLARG(gamma_inv),
            CLARG(saturation), CLARG(contrast), CLARG(grey));
        if (err != CL_SUCCESS)
            goto error;
        return CL_SUCCESS;
