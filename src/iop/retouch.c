                  ...
                  void **new_params, int32_t *new_params_size, int *new_version)
{
    typedef struct dt_iop_retouch_params_v3_t
    {
        ...
        dt_iop_retouch_algo_type_t
            algorithm; // $DEFAULT: DT_IOP_RETOUCH_HEAL clone, heal, blur, fill
    ...
    if (old_version == 1)
    {
        typedef struct dt_iop_retouch_form_data_v1_t
        {
            ...
            dt_iop_retouch_fill_modes_t
                fill_mode;         // mode for fill algorithm, erase or fill with color
            float fill_color[3];   // color for fill algorithm
            float fill_brightness; // value to be added to the color
        ...
        typedef struct dt_iop_retouch_params_v1_t
        {
            ...
            dt_iop_retouch_blur_types_t
                blur_type; // $DEFAULT: DT_IOP_RETOUCH_BLUR_GAUSSIAN $DESCRIPTION: "blur type"
                           // gaussian, bilateral
            float
                blur_radius; // $MIN: 0.1 $MAX: 200.0 $DEFAULT: 10.0 $DESCRIPTION: "blur radius" radius for blur
                             // algorithm

            dt_iop_retouch_fill_modes_t
                fill_mode; // $DEFAULT: DT_IOP_RETOUCH_FILL_ERASE $DESCRIPTION: "fill mode" mode
