...
static void invert_histogram(const int *hist, float *inv_hist)
{
    ...
    for (int i = 0; i <= last; i++)
        inv_hist[i] = 100.0 * i / (float)HISTN;
...
static void get_clusters(const float *col, const int n, float2 *mean, float *weight)
{
    ...
    if (Mdist - mdist > 0)
        for (int k = 0; k < n; k++)
            weight[k] = (weight[k] - mdist) / (Mdist - mdist);
    float sum = 0.0f;
    for (int k = 0; k < n; k++)
        sum += weight[k];
    if (sum > 0)
    ...
    for (int it = 0; it < nit; it++)
    {
        ...
        for (int s = 0; s < samples; s++)
        {
            ...
            for (int k = 0; k < n; k++)
            {
                ...
                const dt_aligned_pixel_t Lab = {L, col[3 * (roi->width * j + i) + 1],
                                                col[3 * (roi->width * j + i) + 2]};
    ...
    else if (data->flag == APPLY)
    {
        ...
        for (int k = 0; k < roi_out->height; k++)
        {
            ...
            for (int i = 0; i < roi_out->width; i++)
            {
                ...
                {
                    out[j + 1] +=
                        weight[c] * ((Lab[1] - mean[c][0]) * data->var[mapio[c]][0] / var[c][0] +
                                     data->mean[mapio[c]][0]);
                    out[j + 2] +=
                        weight[c] * ((Lab[2] - mean[c][1]) * data->var[mapio[c]][1] / var[c][1] +
                                     data->mean[mapio[c]][1]);
                }
                out[j + 3] = in[j + 3];
                j += ch;
