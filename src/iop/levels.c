...
static gboolean dt_iop_levels_motion_notify(GtkWidget *widget, GdkEventMotion *event,
                                            dt_iop_module_t *self);
static gboolean dt_iop_levels_button_press(GtkWidget *widget, GdkEventButton *event,
                                           dt_iop_module_t *self);
static gboolean dt_iop_levels_button_release(GtkWidget *widget, GdkEventButton *event,
                                             dt_iop_module_t *self);
static gboolean dt_iop_levels_leave_notify(GtkWidget *widget, GdkEventCrossing *event,
                                           dt_iop_module_t *self);
static gboolean dt_iop_levels_scroll(GtkWidget *widget, GdkEventScroll *event,
                                     dt_iop_module_t *self);
...
{
    return dt_iop_set_description(self, _("adjust black, white and mid-gray points"), _("creative"),
...
static void commit_params_late(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece)
{
    ...
    if (d->mode == LEVELS_MODE_AUTOMATIC)
    {
        ...
        if (dt_pipe_is_preview(piece->pipe) || d->levels[0] == DT_LEVELS_UNINIT ||
            d->levels[1] == DT_LEVELS_UNINIT || d->levels[2] == DT_LEVELS_UNINIT)
        {
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int i = 0; i < 4 * npixels; i += 4)
    {
        ...
        else
        {
            ...
            L_out = percentage < 1.0f ? lut[(int)(percentage * 0x10000ul)] :
                                        100.0f * powf(percentage, inv_gamma);
        }
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gd->kernel_levels, width, height, CLARG(dev_in),
                                           CLARG(dev_out), CLARG(width), CLARG(height),
                                           CLARG(dev_lut), CLARG(d->levels[0]), CLARG(d->levels[2]),
                                           CLARG(d->in_inv_gamma));
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->area = GTK_DRAWING_AREA(dt_ui_resize_wrap(NULL, 0, "plugins/darkroom/levels/graphheight"));
    ...
    GtkWidget *vbox_manual =
        dt_gui_vbox(g->area, dt_gui_hbox(dt_gui_expand(autobutton), dt_gui_expand(g->blackpick),
                                         dt_gui_expand(g->greypick), dt_gui_expand(g->whitepick)));
 ...
 */
static void dt_iop_levels_move_handle(dt_iop_module_t *self, int handle_move, float new_pos,
                                      float *levels, float drag_start_percentage)
{
    dt_iop_levels_gui_data_t *g = self->gui_data;
    float min_x = 0;
    float max_x = 1;
    ...
    if (g->activeToggleButton != NULL)
        gtk_toggle_button_set_active(g->activeToggleButton, FALSE);
    g->last_picked_color = -1;
}
                                            ...
                                            dt_iop_module_t *self)
{
    ...
    if (g->dragging)
    {
        if (g->handle_move >= 0 && g->handle_move < 3)
        {
            ...
            dt_iop_levels_move_handle(self, g->handle_move, mx, p->levels,
                                     ...
                                     dt_iop_module_t *self)
{
    ...
    const float interval =
        0.002 *
        dt_accel_get_speed_multiplier(widget, event->state); // Distance moved for each scroll event
    int delta_y;
    if (dt_gui_get_scroll_unit_delta(event, &delta_y))
    {
        float new_position = p->levels[g->handle_move] - interval * delta_y;
        dt_iop_levels_move_handle(self, g->handle_move, new_position, p->levels,
                                  g->drag_start_percentage);
        dt_dev_add_history_item(darktable.develop, self, TRUE);
        return TRUE;
    }
