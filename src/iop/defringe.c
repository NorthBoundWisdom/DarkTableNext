...
#ifdef HAVE_OPENCL
    tiling->factor_cl =
        2.0f + (float)dt_gaussian_memory_use_cl(width, height, channels) / basebuffer;
#endif
    tiling->maxbuf =
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->thresh_scale,
                                _("threshold for defringe, higher values mean less defringing"));
}
