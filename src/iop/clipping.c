...
static void keystone_get_matrix(const dt_boundingbox_t k_space, float kxa, const float kxb,
                                const float kxc, const float kxd, float kya, const float kyb,
                                const float kyc, const float kyd, float *a, float *b, float *d,
                                float *e, float *g, float *h)
{
    *a = -((kxb * (kyd * kyd - kyc * kyd) - kxc * kyd * kyd + kyb * (kxc * kyd - kxd * kyd) +
            kxd * kyc * kyd) *
           k_space[2]) /
         (kxb * (kxc * kyd * kyd - kxd * kyc * kyd) + kyb * (kxd * kxd * kyc - kxc * kxd * kyd));
    *b = ((kxb * (kxd * kyd - kxd * kyc) - kxc * kxd * kyd + kxd * kxd * kyc +
           (kxc * kxd - kxd * kxd) * kyb) *
          k_space[2]) /
         (kxb * (kxc * kyd * kyd - kxd * kyc * kyd) + kyb * (kxd * kxd * kyc - kxc * kxd * kyd));
    *d = (kyb * (kxb * (kyd * k_space[3] - kyc * k_space[3]) - kxc * kyd * k_space[3] +
                 kxd * kyc * k_space[3]) +
...
DT_OMP_DECLARE_SIMD()
static inline void keystone_backtransform(float *i, const dt_boundingbox_t k_space, const float a,
                                          const float b, const float d, const float e,
                                          const float g, const float h, const float kxa,
                                          const float kya)
{
...
DT_OMP_DECLARE_SIMD()
static inline void keystone_transform(float *i, const dt_boundingbox_t k_space, const float a,
                                      const float b, const float d, const float e, const float g,
                                      const float h, const float kxa, const float kya)
{
...
DT_OMP_DECLARE_SIMD()
static inline void backtransform(float *x, float *o, const float *m, const float t_h,
                                 const float t_v)
{
    x[1] /= (1.0f + x[0] * t_h);
    x[0] /= (1.0f + x[1] * t_v);
...
DT_OMP_DECLARE_SIMD()
static inline void transform(const float *x, float *o, const float *m, const float t_h,
                             const float t_v)
{
    mul_mat_vec_2(m, x, o);
    o[1] *= (1.0f + o[0] * t_h);
                           ...
                           float *const restrict points, size_t points_count)
{
    ...
    const dt_boundingbox_t k_space = {d->k_space[0] * rx, d->k_space[1] * ry, d->k_space[2] * rx,
                                      d->k_space[3] * ry};
    ...
    if (d->k_apply == 1)
        keystone_get_matrix(k_space, kxa, kxb, kxc, kxd, kya, kyb, kyc, kyd, &ma, &mb, &md, &me,
                            &mg, &mh);
    ...
    for (size_t i = 0; i < points_count * 2; i += 2)
    {
        ...
        if (d->k_apply == 1)
            keystone_transform(pi, k_space, ma, mb, md, me, mg, mh, kxa, kya);
                               ...
                               float *const restrict points, size_t points_count)
{
    ...
    for (size_t i = 0; i < points_count * 2; i += 2)
    {
        ...
        po[1] += d->ty / factor;
        if (d->k_apply == 1)
            keystone_backtransform(po, k_space, ma, mb, md, me, mg, mh, kxa, kya);
                  ...
                  const dt_iop_roi_t *const roi_out)
{
    ...
    else
    {
        ...
        const dt_boundingbox_t k_space = {d->k_space[0] * rx, d->k_space[1] * ry,
                                          d->k_space[2] * rx, d->k_space[3] * ry};
        const float kxa = d->kxa * rx, kxb = d->kxb * rx, kxc = d->kxc * rx, kxd = d->kxd * rx;
        const float kya = d->kya * ry, kyb = d->kyb * ry, kyc = d->kyc * ry, kyd = d->kyd * ry;
        float ma, mb, md, me, mg, mh;
        if (d->k_apply == 1)
        ...
        for (int j = 0; j < roi_out->height; j++)
        {
            ...
            for (int i = 0; i < roi_out->width; i++)
            {
                ...
                pi[0] =
                    roi_out->x - roi_out->scale * d->enlarge_x + roi_out->scale * d->cix + i + 0.5f;
                pi[1] =
                    roi_out->y - roi_out->scale * d->enlarge_y + roi_out->scale * d->ciy + j + 0.5f;
                ...
                _out[i] = CLIP(dt_interpolation_compute_sample(interpolation, in, po[0], po[1],
                    ...
                    const dt_iop_roi_t *roi_in_orig)
{
    ...
    if (d->k_apply == 0 && d->crop_auto == 1) // this is the old solution.
    {
        ...
        *roi_out = *roi_in;
        ...
        const float oaabb[4] = {-.5f * roi_in->width, -.5f * roi_in->height, .5f * roi_in->width,
                                .5f * roi_in->height};
        for (int flip = 0; flip < 2; flip++)
        {
            ...
            float p[2], o[2];
            const float aabb[4] = {-.5f * roi_in_width, -.5f * roi_in_height, .5f * roi_in_width,
                                   .5f * roi_in_height};
            for (int c = 0; c < 4; c++)
            {
                ...
                for (int k = 0; k < 2; k++)
                    if (fabsf(o[k]) > 0.001f)
                        newcropscale = fminf(newcropscale, aabb[(o[k] > 0 ? 2 : 0) + k] / o[k]);
            }
        ...
        *roi_out = *roi_in;
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            if (d->k_apply == 1)
            {
                ...
                keystone_transform(o, d->k_space, d->a, d->b, d->d, d->e, d->g, d->h, d->kxa,
                                   d->kya);
                o[0] *= roi_in->width;
                o[1] *= roi_in->height;
            }
        ...
        if (new_x + roi_in->width < 0)
            new_x = -roi_in->width;
        float new_y = min4f(corn_out_y);
        if (new_y + roi_in->height < 0)
            new_y = -roi_in->height;

        float new_sc_x = max4f(corn_out_x);
        if (new_sc_x > 2.0f * roi_in->width)
            new_sc_x = 2.0f * roi_in->width;
        float new_sc_y = max4f(corn_out_y);
        if (new_sc_y > 2.0f * roi_in->height)
            new_sc_y = 2.0f * roi_in->height;

        // be careful, we don't want too small area here !
        if (new_sc_x - new_x < roi_in->width / 8.0f)
        {
            float f = (new_sc_x + new_x) / 2.0f;
            if (f < roi_in->width / 16.0f)
                f = roi_in->width / 16.0f;
            if (f >= roi_in->width * 15.0f / 16.0f)
                f = roi_in->width * 15.0f / 16.0f - 1.0f;
            new_x = f - roi_in->width / 16.0f, new_sc_x = f + roi_in->width / 16.0f;
        }
        if (new_sc_y - new_y < roi_in->height / 8.0f)
        {
            float f = (new_sc_y + new_y) / 2.0f;
            if (f < roi_in->height / 16.0f)
                f = roi_in->height / 16.0f;
            if (f >= roi_in->height * 15.0f / 16.0f)
                f = roi_in->height * 15.0f / 16.0f - 1.0f;
    ...
    {
        dt_print_pipe(DT_DEBUG_PIPE, "safety check", piece->pipe, self, DT_DEVICE_NONE, roi_in,
                      roi_out);
                   ...
                   const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
    ...
    *roi_in = *roi_out;
    ...
    const float roi_out_x = roi_out->x - d->enlarge_x * so,
                roi_out_y = roi_out->y - d->enlarge_y * so;
    float p[2], o[2];
    const dt_boundingbox_t aabb = {roi_out_x + d->cix * so, roi_out_y + d->ciy * so,
                                   roi_out_x + d->cix * so + roi_out->width,
    ...
    for (int c = 0; c < 4; c++)
    {
        ...
        o[1] /= kh;
        if (d->k_apply == 1)
            keystone_backtransform(o, d->k_space, d->a, d->b, d->d, d->e, d->g, d->h, d->kxa,
                                   d->kya);
        o[0] *= kw;
        o[1] *= kh;
...
{
    if (!dt_iop_have_required_input_format(4 /*need full-color pixels*/, self, piece->colors, ivoid,
    ...
    else
    {
        ...
        for (int j = 0; j < roi_out->height; j++)
        {
            ...
            for (int i = 0; i < roi_out->width; i++)
            {
                ...
                dt_interpolation_compute_pixel4c(interpolation, (float *)ivoid, out + ch * i, po[0],
                                                 po[1], roi_in->width, roi_in->height, ch_width);
            }
        }
    }
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    else
    {
        ...
        keystone_get_matrix(k_space, d->kxa * k_sizes[0], d->kxb * k_sizes[0], d->kxc * k_sizes[0],
                            d->kxd * k_sizes[0], d->kya * k_sizes[1], d->kyb * k_sizes[1],
                            d->kyc * k_sizes[1], d->kyd * k_sizes[1], &ma, &mb, &md, &me, &mg, &mh);
        const float ka[2] = {d->kxa * k_sizes[0], d->kya * k_sizes[1]};
        const float maa[4] = {ma, mb, md, me};
        const float mbb[2] = {mg, mh};

        err = dt_opencl_enqueue_kernel_2d_args(
            devid, crkernel, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
            CLARG(height), CLARG(roi_in->width), CLARG(roi_in->height), CLARG(roi), CLARG(roo),
            CLARG(roi_in->scale), CLARG(roi_out->scale), CLARG(d->flip), CLARG(t), CLARG(k),
            CLARG(m), CLARG(k_space), CLARG(ka), CLARG(maa), CLARG(mbb));
    }
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    else if (p->k_type >= 0 && p->k_apply == 1)
    {
        ...
        d->kyc = d->kyc - d->kya;
        d->kyd = d->kyd - d->kya;
        keystone_get_matrix(d->k_space, d->kxa, d->kxb, d->kxc, d->kxd, d->kya, d->kyb, d->kyc,
                            d->kyd, &d->a, &d->b, &d->d, &d->e, &d->g, &d->h);
    ...
    if (p->ratio_d == -2 && p->ratio_n == -2)
    {
        ...
        else
        {
            const dt_interpolation_t *interpolation =
                dt_interpolation_new(DT_INTERPOLATION_USERPREF_WARP);
            const float whratio =
                ((float)(iwd - 2 * interpolation->width) * (fabsf(p->cw) - p->cx)) /
...
static void keystone_type_populate(dt_iop_module_t *self, const gboolean with_applied,
                                   const int select)
{
    const dt_iop_clipping_gui_data_t *g = self->gui_data;
    const dt_iop_clipping_params_t *p = self->params;
    ...
    if (p->k_h != 0 || p->k_v != 0)
        dt_bauhaus_combobox_add(g->keystone_type, _("old system"));
    if (with_applied)
        dt_bauhaus_combobox_add(g->keystone_type, _("correction applied"));
...
void gui_update(dt_iop_module_t *self)
{
    ...
    if (p->ratio_d == -2 && p->ratio_n == -2)
        _ratio_get_aspect(self, g->aspect_presets);
    ...
    // keystone :
    if (p->k_apply == 1)
        g->k_show = 2; // needed to initialise correctly the combobox
    else
        g->k_show = -1;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(
        g->angle, _("right-click and drag a line on the image to drag a straight line"));
    ...
    g_signal_connect(G_OBJECT(g->keystone_type), "value-changed", G_CALLBACK(keystone_type_changed),
                     self);
    ...
    const dt_iop_clipping_aspect_t aspects[] = {
        {_("freehand"), 0, 0},
...
static _grab_region_t get_grab(const float pzx, const float pzy,
                               const dt_iop_clipping_gui_data_t *g, const float border,
...
// draw symmetry signs
static void gui_draw_sym(cairo_t *cr, const float x, const float y, const float scale,
                         const gboolean active)
{
    ...
    dt_gui_draw_rounded_rectangle(cr, ink.width + DT_PIXEL_APPLY_DPI(4) * scale,
                                  ink.height + DT_PIXEL_APPLY_DPI(8) * scale,
                                  x - ink.width / 2.0f - DT_PIXEL_APPLY_DPI(2) * scale,
                                  y - ink.height / 2.0f - DT_PIXEL_APPLY_DPI(4) * scale);
    cairo_move_to(cr, x - ink.width / 2.0f,
                  y - 3.0 * ink.height / 4.0f - DT_PIXEL_APPLY_DPI(4) * scale);
    if (active)
                     ...
                     const float pzx, const float pzy, const float zoom_scale)
{
        ...
        g->k_show != 1)
    {
        ...
        snprintf(dimensions, sizeof(dimensions), "%i x %i", (int)(procw * g->clip_w),
                 (int)(proch * g->clip_h));
    ...
    else if (g->k_show != 1)
    {
        const _grab_region_t grab =
            g->cropping ? g->cropping : get_grab(pzx, pzy, g, border, wd, ht);
        if (grab == GRAB_LEFT)
        ...
        if (grab == GRAB_BOTTOM_LEFT)
            cairo_rectangle(cr, g->clip_x * wd, (g->clip_y + g->clip_h) * ht - border, border,
                            border);
        cairo_stroke(cr);
    }
...
static float dist_seg(const float xa, const float ya, const float xb, const float yb,
                ...
                const int which, const float zoom_scale)
{
    ...
    else if (darktable.control->button_down && darktable.control->button_down_which == 1)
    {
        ...
        if (g->k_drag == TRUE && g->k_selected >= 0)
        {
            ...
            const dt_dev_pixelpipe_iop_t *piece =
                dt_dev_distort_get_iop_pipe(self->dev, self->dev->preview_pipe, self);
            const float xx = pts[0] / (float)piece->buf_out.width,
                        yy = pts[1] / (float)piece->buf_out.height;
            if (g->k_selected == 0)
            {
                if (p->k_sym == 1 || p->k_sym == 3)
                    p->kxa = fminf(xx, (p->kxc + p->kxd - 0.01f) / 2.0f),
                    p->kxb = p->kxc - p->kxa + p->kxd;
                else
                    p->kxa = fminf(xx, p->kxb - 0.01f);
                if (p->k_sym > 1)
                    p->kya = fminf(yy, (p->kyc + p->kyb - 0.01f) / 2.0f),
                    p->kyd = p->kyc - p->kya + p->kyb;
                else
            ...
            {
                if (p->k_sym == 1 || p->k_sym == 3)
                    p->kxb = fmaxf(xx, (p->kxc + p->kxd + 0.01f) / 2.0f),
                    p->kxa = p->kxc - p->kxb + p->kxd;
                else
                    p->kxb = fmaxf(xx, p->kxa + 0.01f);
                if (p->k_sym > 1)
                    p->kyb = fminf(yy, (p->kya + p->kyd - 0.01f) / 2.0f),
                    p->kyc = p->kya - p->kyb + p->kyd;
                else
            ...
            {
                if (p->k_sym == 1 || p->k_sym == 3)
                    p->kxc = fmaxf(xx, (p->kxa + p->kxb + 0.01f) / 2.0f),
                    p->kxd = p->kxa - p->kxc + p->kxb;
                else
                    p->kxc = fmaxf(xx, p->kxd + 0.01f);
                if (p->k_sym > 1)
                    p->kyc = fmaxf(yy, (p->kya + p->kyd + 0.01f) / 2.0f),
                    p->kyb = p->kya - p->kyc + p->kyd;
                else
            ...
            {
                if (p->k_sym == 1 || p->k_sym == 3)
                    p->kxd = fminf(xx, (p->kxa + p->kxb - 0.01f) / 2.0f),
                    p->kxc = p->kxa - p->kxd + p->kxb;
                else
                    p->kxd = fminf(xx, p->kxc - 0.01f);
                if (p->k_sym > 1)
                    p->kyd = fmaxf(yy, (p->kyc + p->kyb + 0.01f) / 2.0f),
                    p->kya = p->kyc - p->kyd + p->kyb;
                else
        ...
        if (g->cropping == GRAB_CENTER && !g->straightening && g->k_show != 1)
        {
            ...
            if (!grab && darktable.control->button_down_which == 3)
                g->straightening = 1;
        }
        if (!g->straightening && darktable.control->button_down_which == GDK_BUTTON_PRIMARY &&
            g->k_show != 1)
        {
            grab = g->cropping;
            ...
            if (grab == GRAB_ALL)
            {
                ...
                if (!g->shift_hold)
                    g->clip_x = fminf(g->clip_max_w + g->clip_max_x - g->clip_w,
                                      fmaxf(g->clip_max_x, g->handle_x + pzx - bzx));

                if (!g->ctrl_hold)
                    g->clip_y = fminf(g->clip_max_h + g->clip_max_y - g->clip_h,
            ...
            float points[4] = {g->clip_x * wd, g->clip_y * ht, (g->clip_x + g->clip_w) * wd,
                               (g->clip_y + g->clip_h) * ht};

            if (dt_dev_distort_backtransform_plus(self->dev, self->dev->preview_pipe,
                                                  self->iop_order, DT_DEV_TRANSFORM_DIR_FORW_EXCL,
                                                  points, 2))
            {
                const dt_dev_pixelpipe_iop_t *piece =
                    dt_dev_distort_get_iop_pipe(self->dev, self->dev->preview_pipe, self);
    ...
    else if (grab && g->k_show != 1)
    {
        ...
        else if (grab == GRAB_NONE)
        {
            dt_control_hinter_message(
                _("<b>commit</b>: double-click, <b>straighten</b>: right-drag"));
    ...
    else
    {
        ...
        if (g->k_show == 1 && g->k_drag == FALSE)
        {
            ...
            if (xx < p->kxa + ext && xx > p->kxa - ext && yy < p->kya + ext && yy > p->kya - ext)
                g->k_selected = 0;
            if (xx < p->kxb + ext && xx > p->kxb - ext && yy < p->kyb + ext && yy > p->kyb - ext)
                g->k_selected = 1;
            if (xx < p->kxc + ext && xx > p->kxc - ext && yy < p->kyc + ext && yy > p->kyc - ext)
                g->k_selected = 2;
            if (xx < p->kxd + ext && xx > p->kxd - ext && yy < p->kyd + ext && yy > p->kyd - ext)
                g->k_selected = 3;
            ...
            {
                dt_control_hinter_message(
                    _("<b>move line</b>: drag, <b>toggle symmetry</b>: click ꝏ"));
                dt_control_change_cursor("crosshair");
            }
            else
            {
                dt_control_hinter_message(
                    _("<b>apply</b>: click <tt>ok</tt>, <b>toggle symmetry</b>: click ꝏ\n"
                       ...
                       dt_iop_clipping_params_t *p)
{
    ...
    g->applied = 1;
    const gboolean changed = fabs(p->cx - old[0]) > eps || fabs(p->cy - old[1]) > eps ||
                             fabs(p->cw - old[2]) > eps || fabs(p->ch - old[3]) > eps;
    ...
    if (which == GDK_BUTTON_SECONDARY || which == GDK_BUTTON_PRIMARY)
    {
        ...
        if (g->k_show == 1)
        {
            ...
            else                  // if we click to the apply button
            {
                ...
                float pts[8] = {p->kxa * wp, p->kya * hp, p->kxb * wp, p->kyb * hp,
                                p->kxc * wp, p->kyc * hp, p->kxd * wp, p->kyd * hp};
                dt_dev_distort_transform_plus(self->dev, self->dev->preview_pipe, self->iop_order,
                                              DT_DEV_TRANSFORM_DIR_FORW_EXCL, pts, 4);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (xx > c[0] - ext && xx < c[0] + ext && yy > c[1] - ext &&
                            yy < c[1] + ext && (p->k_type == 1 || p->k_type == 3))
                        {
                            if (p->k_sym == 0)
                                p->k_sym = 1;
                            else if (p->k_sym == 1)
                        ...
                        else
                        {
                            ...
                            if (xx > c[0] - ext && xx < c[0] + ext && yy > c[1] - ext &&
                                yy < c[1] + ext && (p->k_type == 2 || p->k_type == 3))
                            {
                                if (p->k_sym == 0)
                                    p->k_sym = 2;
                                else if (p->k_sym == 1)
