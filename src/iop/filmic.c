...
void init_presets(dt_iop_module_so_t *self)
{
    ...
    dt_gui_presets_add_generic(_("09 EV (low-key)"), self->op, self->version(), &p, sizeof(p), TRUE,
    ...
    dt_gui_presets_add_generic(_("10 EV (indoors)"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("11 EV (dim outdoors)"), self->op, self->version(), &p, sizeof(p),
    ...
    dt_gui_presets_add_generic(_("12 EV (outdoors)"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("13 EV (bright outdoors)"), self->op, self->version(), &p,
    ...
    dt_gui_presets_add_generic(_("14 EV (backlighting)"), self->op, self->version(), &p, sizeof(p),
                               TRUE, DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("15 EV (sunset)"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("16 EV (HDR)"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
    ...
    dt_gui_presets_add_generic(_("18 EV (HDR++)"), self->op, self->version(), &p, sizeof(p), TRUE,
                               DEVELOP_BLEND_CS_RGB_DISPLAY);
}

// Keep the per-pixel work in an inlineable helper so the surrounding loop
// remains easy for the optimizer to vectorize.
static void _process_pixel(const dt_aligned_pixel_t in, dt_aligned_pixel_t out,
                           const float grey_source, const float black_source,
                           const float inv_dynamic_range, const dt_aligned_pixel_t output_power,
                           const float saturation, const float EPS, const int desaturate,
                           const int preserve_color, const dt_iop_filmic_data_t *const data)
{
    ...
    if (preserve_color)
    {
        ...
        // Save the ratios
        for_each_channel(c) ratios[c] = input_rgb[c] / max;
        ...
        // Re-apply ratios
        for_each_channel(c) rgb[c] = ratios[c] * max;
    ...
    else
    {
        ...
        for_each_channel(c) input_rgb[c] /= grey_source;
        ...
        // Filmic S curve
        for_each_channel(c) rgb[c] = data->table[index[c]];
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_filmic, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(dynamic_range), CLARG(shadows_range), CLARG(grey), CLARG(dev_table),
        CLARG(diff_table), CLARG(contrast), CLARG(power), CLARG(preserve_color), CLARG(saturation));
...
static void saturation_callback(GtkWidget *slider, dt_iop_module_t *self)
{
    ...
    dt_iop_filmic_params_t *p = self->params;
    p->saturation =
        logf(9.0f * dt_bauhaus_slider_get(slider) / 100.0 + 1.0f) / logf(10.0f) * 100.0f;
    dt_iop_color_picker_reset(self, TRUE);
    dt_dev_add_history_item(darktable.develop, self, TRUE);
}
                       ...
                       dt_iop_filmic_data_t *d, dt_iop_filmic_nodes_t *nodes_data)
{
    ...
    const float black_display =
        CLAMP(p->black_point_target, 0.0f, p->grey_point_target) / 100.0f; // in %
    const float grey_display =
        powf(CLAMP(p->grey_point_target, p->black_point_target, p->white_point_target) / 100.0f,
             1.0f / (p->output_power));
    const float white_display =
        CLAMP(p->white_point_target, p->grey_point_target, 100.0f) / 100.0f; // in %
  ...
  **/
    ...
    if ((toe_log == grey_log && toe_display == grey_display) ||
        (toe_log == 0.0f && toe_display == black_display))
    {
        TOE_LOST = TRUE;
    }
    if ((shoulder_log == grey_log && shoulder_display == grey_display) ||
        (shoulder_log == 1.0f && shoulder_display == white_display))
    {
        SHOULDER_LOST = TRUE;
    }
    ...
    if (p->interpolator != 3)
    {
        ...
        if (p->interpolator > CUBIC_SPLINE && p->interpolator <= MONOTONE_HERMITE)
            interpolator = p->interpolator;

        curve = dt_draw_curve_new(0.0, 1.0, interpolator);
        for (int k = 0; k < nodes_data->nodes; k++)
            (void)dt_draw_curve_add_point(curve, nodes_data->x[k], nodes_data->y[k]);
...
void gui_update(dt_iop_module_t *self)
{
    ...
    dt_bauhaus_slider_set(g->saturation,
                          (powf(10.0f, p->saturation / 100.0f) - 1.0f) / 9.0f * 100.0f);
...
void init(dt_iop_module_t *self)
{
    ...
    *(dt_iop_filmic_params_t *)self->default_params = (dt_iop_filmic_params_t){
...
static gboolean dt_iop_tonecurve_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
     ...
     */
        const float x =
            (rescale) ? powf(2.0f, (double)a * nodes_data->x[k] + b) + d : nodes_data->x[k];
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g->grey_point_source =
        dt_bauhaus_slider_new_with_range(self, 0.0, 100., 0, p->grey_point_source, 2);
    ...
    g_signal_connect(G_OBJECT(g->grey_point_source), "value-changed",
                     G_CALLBACK(grey_point_source_callback), self);
    dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE, g->grey_point_source);

    // White slider
    g->white_point_source =
        dt_bauhaus_slider_new_with_range(self, 0.0, 16.0, 0, p->white_point_source, 2);
    ...
    g_signal_connect(G_OBJECT(g->white_point_source), "value-changed",
                     G_CALLBACK(white_point_source_callback), self);
    dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE,
                        g->white_point_source);

    // Black slider
    g->black_point_source =
        dt_bauhaus_slider_new_with_range(self, -16.0, -0.1, 0, p->black_point_source, 2);
    ...
    g_signal_connect(G_OBJECT(g->black_point_source), "value-changed",
                     G_CALLBACK(black_point_source_callback), self);
    dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE,
                        g->black_point_source);

    // Security factor
    g->security_factor =
        dt_bauhaus_slider_new_with_range(self, -50., 50., 0, p->security_factor, 2);
    ...
    g_signal_connect(G_OBJECT(g->security_factor), "value-changed",
                     G_CALLBACK(security_threshold_callback), self);

    // Auto tune slider
    g->auto_button = dt_bauhaus_combobox_new(self);
    dt_bauhaus_widget_set_label(g->auto_button, NULL, N_("auto tune levels"));
    dt_color_picker_new(self, DT_COLOR_PICKER_AREA | DT_COLOR_PICKER_DENOISE, g->auto_button);
    ...
    g_signal_connect(G_OBJECT(g->latitude_stops), "value-changed",
                     G_CALLBACK(latitude_stops_callback), self);
    ...
    // saturation slider
    g->global_saturation =
        dt_bauhaus_slider_new_with_range(self, 0., 1000., 0, p->global_saturation, 2);
    ...
    g_signal_connect(G_OBJECT(g->global_saturation), "value-changed",
                     G_CALLBACK(global_saturation_callback), self);

    // saturation slider
    g->saturation = dt_bauhaus_slider_new_with_range(
        self, 0., 1000., 0, (powf(10.0f, p->saturation / 100.0f) - 1.0f) / 9.0f * 100.0f, 2);
    ...
    g_signal_connect(G_OBJECT(g->saturation), "value-changed", G_CALLBACK(saturation_callback),
                     self);
  ...
  */
    ...
    gtk_widget_set_tooltip_text(
        g->interpolator, _("change this method if you see reversed contrast or faded blacks"));
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->preserve_color), p->preserve_color);
    gtk_widget_set_tooltip_text(
        g->preserve_color, _("ensure the original color are preserved.\n"
                             "may reinforce chromatic aberrations.\n"
                             "you need to manually tune the saturation when using this mode."));
    g_signal_connect(G_OBJECT(g->preserve_color), "toggled", G_CALLBACK(preserve_color_callback),
                     self);

    // Black slider
    g->black_point_target =
        dt_bauhaus_slider_new_with_range(self, 0.0, 100.0, 0, p->black_point_target, 2);
    ...
    g_signal_connect(G_OBJECT(g->black_point_target), "value-changed",
                     G_CALLBACK(black_point_target_callback), self);

    // grey_point_source slider
    g->grey_point_target =
        dt_bauhaus_slider_new_with_range(self, 0.1, 50., 0, p->grey_point_target, 2);
    ...
    g_signal_connect(G_OBJECT(g->grey_point_target), "value-changed",
                     G_CALLBACK(grey_point_target_callback), self);

    // White slider
    g->white_point_target =
        dt_bauhaus_slider_new_with_range(self, 0.0, 100.0, 0, p->white_point_target, 2);
    ...
    gtk_widget_set_tooltip_text(g->white_point_target,
                                _("luminance of output pure white, this should be 100%\n"
                                  "except if you want a faded look"));
    g_signal_connect(G_OBJECT(g->white_point_target), "value-changed",
                     G_CALLBACK(white_point_target_callback), self);
    ...
    gtk_widget_set_tooltip_text(
        g->output_power, _("power or gamma of the transfer function\n"
                           "of the display or color space.\n"
                           "you should never touch that unless you know what you are doing."));
    g_signal_connect(G_OBJECT(g->output_power), "value-changed", G_CALLBACK(output_power_callback),
                     self);

    // add collapsible section for those extra options that are generally not to be used
    g->extra_toggle =
        dtgtk_togglebutton_new(dtgtk_cairo_paint_solid_arrow, CPF_DIRECTION_LEFT, NULL);
    g_signal_connect(G_OBJECT(g->extra_toggle), "toggled",
                     G_CALLBACK(_extra_options_button_changed), (gpointer)self);
    g->extra_expander = dtgtk_expander_new(
        dt_gui_hbox(dt_ui_section_label_new(C_("section", "destination/display")), g->extra_toggle),
        dt_gui_vbox(g->black_point_target, g->grey_point_target, g->white_point_target,
                    g->output_power));
    dtgtk_expander_set_expanded(DTGTK_EXPANDER(g->extra_expander), TRUE);

    self->widget = dt_gui_vbox(
        g->area, dt_ui_section_label_new(C_("section", "logarithmic shaper")), g->grey_point_source,
        g->white_point_source, g->black_point_source, g->security_factor, g->auto_button,
        dt_ui_section_label_new(C_("section", "filmic S curve")), g->contrast, g->latitude_stops,
        g->balance, g->global_saturation, g->saturation, g->interpolator, g->preserve_color,
        g->extra_expander);
