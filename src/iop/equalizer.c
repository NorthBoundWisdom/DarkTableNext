             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    float lm = 0;
    for (int k = MIN(width, height) * piece->iscale / scale; k; k >>= 1)
        lm++; // coarsest level
    lm = MIN(DT_IOP_EQUALIZER_MAX_LEVEL, l1 + lm);
    // level 1 => full resolution
    int numl = 0;
    for (int k = MIN(width, height); k; k >>= 1)
        numl++;
    ...
    for (int level = 1; level < numl_cap; level++)
        dt_iop_equalizer_wtf(ovoid, tmp, level, width, height);

    for (int l = 1; l < numl_cap; l++)
    {
        float *out = (float *)ovoid;
        const float lv =
            (lm - l1) * (l - 1) / (float)(numl_cap - 1) + l1; // appr level in real image.
        ...
        for (int ch = 0; ch < 3; ch++)
        {
            ...
            for (int j = 0; j < height; j += step)
                for (int i = step / 2; i < width; i += step)
                    out[(size_t)chs * width * j + (size_t)chs * i + ch] *= coeff;
    ...
    for (int level = numl_cap - 1; level > 0; level--)
        dt_iop_equalizer_iwtf(ovoid, tmp, level, width, height);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    self->widget =
        dt_ui_label_new(_("this module will be removed in the future\nand is only here so you can "
