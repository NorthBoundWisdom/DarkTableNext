    ...
    if (old_version == 1)
    {
        ...
        if (self->dev && self->dev->image_storage.flags & DT_IMAGE_4BAYER)
        {
            ...
            if (!dt_colorspaces_conversion_matrices_rgb(
                    self->dev->image_storage.adobe_XYZ_to_CAM, RGB_to_CAM, NULL,
...
{
    return dt_iop_set_description(self, _("invert film negatives"), _("corrective"),
                                  _("linear, raw, display-referred"), _("linear, raw"),
...
void color_picker_apply(dt_iop_module_t *self, GtkWidget *picker, dt_dev_pixelpipe_t *pipe)
{
    ...
    if (grayrgb[0] == old[0] && grayrgb[1] == old[1] && grayrgb[2] == old[2] &&
        grayrgb[3] == old[3])
        return;
             ...
             void *const ovoid, const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    const dt_aligned_pixel_t film_rgb_f = {d->color[0] * m[0], d->color[1] * m[1],
                                           d->color[2] * m[2], d->color[3] * m[3]};
    ...
    { // xtrans float mosaiced
        ...
        for (size_t j = 0; j < height; j++)
        {
            ...
            for (; i < alignment && i < width; i++)
                out[p + i] =
                    CLAMP(film_rgb_f[FCxtrans(j, i, roi_out, xtrans)] - in[p + i], 0.0f, 1.0f);

            // set up the filter colors for the current row of the image
            const dt_aligned_pixel_t film[3] = {{film_rgb_f[FCxtrans(j, i + 0, roi_out, xtrans)],
            ...
            for (size_t f = 0; i < width - (4 - 1); f = (f + 1) % 3, i += 4)
            {
                ...
                for_each_channel(c) v[c] = CLAMP(film[f][c] - in[p + i + c], 0.0f, 1.0f);
    ...
    { // bayer float mosaiced
        ...
        for (int j = 0; j < height; j++)
        {
            ...
            for (; i < width - (4 - 1); i += 4)
            {
                ...
                for_four_channels(c, aligned(in)) inv[c] =
                    CLAMP(film[c] - in[p + i + c], 0.0f, 1.0f);
    ...
    { // non-mosaiced
        ...
        {
            dt_aligned_pixel_t inv;
            for_each_channel(c) inv[c] = color[c] - in[4 * k + c];
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    err = dt_opencl_enqueue_kernel_2d_args(
        ...
        CLARG(dev_color), CLARG(filters), CLARG(roi_out->x), CLARG(roi_out->y));
    ...
    if (g)
    {
        ...
        else
        {
            ...
            if (self->dev->image_storage.flags & DT_IMAGE_4BAYER)
            {
                ...
                if (!dt_colorspaces_conversion_matrices_rgb(
                        self->dev->image_storage.adobe_XYZ_to_CAM, g->RGB_to_CAM, g->CAM_to_RGB,
                        self->dev->image_storage.d65_color_matrix, NULL))
                {
                    const char *camera = self->dev->image_storage.camera_makermodel;
...
void gui_init(dt_iop_module_t *self)
{
    ...
    dt_action_define(DT_ACTION(self), NULL, N_("pick color of film material from image"),
                     g->colorpicker, &dt_action_def_button);
