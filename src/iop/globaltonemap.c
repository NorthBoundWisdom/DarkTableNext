                  ...
                  void **new_params, int32_t *new_params_size, int *new_version)
{
    ...
    if (old_version < 3)
    {
        ...
        dt_iop_global_tonemap_params_v3_t *n = malloc(sizeof(dt_iop_global_tonemap_params_v3_t));
                                 ...
                                 dt_iop_global_tonemap_data_t *data)
{
    ...
    for (size_t k = 0; k < (size_t)roi_out->width * roi_out->height; k++)
    {
        ...
        outp[0] = 100.0f * (ldc * logf(fmaxf(eps, lw + 1.0f)) /
                            logf(fmaxf(eps, 2.0f + (powf(lw / lwmax, bl)) * 8.0f)));
        outp[1] = inp[1];
        outp[2] = inp[2];
    }
}
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    if (d->operator== OPERATOR_DRAGO)
    {
        ...
        if (tmp_lwmax == -FLT_MAX)
        {
            ...
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_pixelmax_first, sizes, local, CLARG(dev_in), CLARG(width),
                CLARG(height), CLARG(dev_m),
                CLLOCAL(sizeof(float) * flocopt.sizex * flocopt.sizey));
            if (err != CL_SUCCESS)
                goto finally;
            ...
            local[1] = 1;
            err = dt_opencl_enqueue_kernel_2d_local_args(
                devid, gd->kernel_pixelmax_second, sizes, local, CLARG(dev_m), CLARG(dev_r),
                CLARG(bufsize), CLLOCAL(sizeof(float) * slocopt.sizex));
            if (err != CL_SUCCESS)
                goto finally;
    ...
    err = dt_opencl_enqueue_kernel_2d_args(devid, gtkernel, width, height, CLARG(dev_in),
                     ...
                     dt_develop_tiling_t *tiling)
{
    ...
    tiling->factor =
        2.0f + (detail ?
                    (float)dt_bilateral_memory_use2(width, height, sigma_s, sigma_r) / basebuffer :
                    0.0f);
    tiling->maxbuf =
        (detail ?
             MAX(1.0f, (float)dt_bilateral_singlebuffer_size2(width, height, sigma_s, sigma_r) /
                           basebuffer) :
             1.0f);
    tiling->overhead = 0;
...
void init_global(dt_iop_module_so_t *self)
{
    ...
    gd->kernel_global_tonemap_reinhard =
        dt_opencl_create_kernel(program, "global_tonemap_reinhard");
...
void gui_init(dt_iop_module_t *self)
{
    ...
    gtk_widget_set_tooltip_text(g->drago.max_light,
                                _("the target light for tonemapper specified as cd/m2"));
