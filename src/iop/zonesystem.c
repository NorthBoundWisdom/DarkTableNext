...
static inline int _iop_zonesystem_zone_index_from_lightness(float lightness, float *zonemap,
                                                            int size)
{
                                   ...
                                   const dt_iop_roi_t *const roi_out)
{
    ...
    if (self->dev->gui_attached && dt_pipe_is_preview(piece->pipe) && g && g->in_preview_buffer &&
        g->out_preview_buffer)
    {
        ...
        dt_gaussian_t *gauss =
            dt_gaussian_init(width, height, 1, Lmax, Lmin, sigma, DT_IOP_GAUSSIAN_ZERO);
               ...
               const dt_iop_roi_t *const roi_in, const dt_iop_roi_t *const roi_out)
{
    ...
    for (int k = 0; k < size - 1; k++)
        zonemap_scale[k] = (zonemap[k + 1] - zonemap[k]) * (size - 1);
    for (int k = 0; k < size - 1; k++)
        zonemap_offset[k] = 100.0f * ((k + 1) * zonemap[k] - k * zonemap[k + 1]);

    dev_zmo = dt_opencl_copy_host_to_device_constant(
        devid, sizeof(float) * ROUNDUP(MAX_ZONE_SYSTEM_SIZE, 16), zonemap_offset);
    if (dev_zmo == NULL)
        goto error;
    dev_zms = dt_opencl_copy_host_to_device_constant(
        devid, sizeof(float) * ROUNDUP(MAX_ZONE_SYSTEM_SIZE, 16), zonemap_scale);
    if (dev_zms == NULL)
        goto error;

    err = dt_opencl_enqueue_kernel_2d_args(
        devid, gd->kernel_zonesystem, width, height, CLARG(dev_in), CLARG(dev_out), CLARG(width),
        CLARG(height), CLARG(size), CLARG(dev_zmo), CLARG(dev_zms));
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
    ...
    // precompute scale and offset
    for (int k = 0; k < size - 1; k++)
        d->zonemap_scale[k] = (zonemap[k + 1] - zonemap[k]) * (size - 1);
    for (int k = 0; k < size - 1; k++)
...
static void _iop_zonesystem_redraw_preview_callback(gpointer instance, dt_iop_module_t *self);
...
static void size_allocate_callback(GtkWidget *widget, GtkAllocation *allocation,
                                   dt_iop_module_t *self)
{
    dt_iop_zonesystem_gui_data_t *g = self->gui_data;

    if (g->image)
        cairo_surface_destroy(g->image);
    free(g->image_buffer);
...
void gui_init(dt_iop_module_t *self)
{
    ...
    g_signal_connect(G_OBJECT(g->preview), "size-allocate", G_CALLBACK(size_allocate_callback),
                     self);
    g_signal_connect(G_OBJECT(g->preview), "draw", G_CALLBACK(dt_iop_zonesystem_preview_draw),
                     self);
    gtk_widget_add_events(GTK_WIDGET(g->preview), GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
    ...
    gtk_widget_set_tooltip_text(
        g->zones, _("lightness zones\nuse mouse scrollwheel to change the number of zones\n"
                    "left-click on a border to create a marker\n"
                    "right-click on a marker to delete it"));
    g_signal_connect(G_OBJECT(g->zones), "draw", G_CALLBACK(dt_iop_zonesystem_bar_draw), self);
    g_signal_connect(G_OBJECT(g->zones), "motion-notify-event",
                     G_CALLBACK(dt_iop_zonesystem_bar_motion_notify), self);
    g_signal_connect(G_OBJECT(g->zones), "leave-notify-event",
                     G_CALLBACK(dt_iop_zonesystem_bar_leave_notify), self);
    g_signal_connect(G_OBJECT(g->zones), "button-press-event",
                     G_CALLBACK(dt_iop_zonesystem_bar_button_press), self);
    g_signal_connect(G_OBJECT(g->zones), "button-release-event",
                     G_CALLBACK(dt_iop_zonesystem_bar_button_release), self);
    g_signal_connect(G_OBJECT(g->zones), "scroll-event", G_CALLBACK(dt_iop_zonesystem_bar_scrolled),
                     self);
    gtk_widget_add_events(GTK_WIDGET(g->zones), GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK |
                                                    GDK_BUTTON_RELEASE_MASK |
                                                    GDK_LEAVE_NOTIFY_MASK |
                                               ...
                                               dt_iop_module_t *self)
{
    ...
    if (g->in_preview_buffer && g->out_preview_buffer && self->enabled)
    {
        ...
        guchar *image =
            g_malloc_n((size_t)4 * g->preview_width * g->preview_height, sizeof(guchar));
        ...
        cairo_surface_t *surface =
            cairo_image_surface_create_for_data(image, CAIRO_FORMAT_RGB24, wd, ht, stride);
        cairo_translate(cr, width / 2.0, height / 2.0f);
        cairo_scale(cr, scale, scale);
        cairo_translate(cr, -.5f * wd, -.5f * ht);

        cairo_rectangle(cr, DT_PIXEL_APPLY_DPI(1), DT_PIXEL_APPLY_DPI(1),
                        wd - DT_PIXEL_APPLY_DPI(2), ht - DT_PIXEL_APPLY_DPI(2));
    ...
    else
    {
        ...
        {
            GdkRGBA *color;
            gtk_style_context_get(context, gtk_widget_get_state_flags(self->expander),
                                  "background-color", &color, NULL);

            cairo_set_source_surface(cr, g->image, (width - g->image_width) * 0.5,
                                     (height - g->image_height) * 0.5);
