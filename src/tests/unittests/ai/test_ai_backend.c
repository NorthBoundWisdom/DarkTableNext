...
{
    const dt_ai_model_info_t *info = dt_ai_get_model_info_by_id(env, "test-multiply");
    assert_non_null(info);
    assert_string_equal(info->id, "test-multiply");

    // non-existent model
    const dt_ai_model_info_t *none = dt_ai_get_model_info_by_id(env, "does-not-exist");
    assert_null(none);
}
...
{
    dt_ai_context_t *ctx = dt_ai_load_model(env, "test-multiply", NULL, DT_AI_PROVIDER_CPU);
    assert_non_null(ctx);
...
static void test_inference(void **state)
{
    ...
    dt_ai_tensor_t input = {.data = input_data, .type = DT_AI_FLOAT, .shape = in_shape, .ndim = 4};
    ...
    dt_ai_tensor_t output = {
        .data = output_data, .type = DT_AI_FLOAT, .shape = out_shape, .ndim = 4};
...
static void test_error_bad_model_id(void **state)
{
    ...
    dt_ai_context_t *ctx = dt_ai_load_model(env, "no-such-model", NULL, DT_AI_PROVIDER_CPU);
    assert_null(ctx);
}
...
{
    // DT_AI_OPT_BASIC
    dt_ai_context_t *ctx_basic = dt_ai_load_model_ext(env, "test-multiply", NULL,
    ...
    // DT_AI_OPT_DISABLED
    dt_ai_context_t *ctx_none = dt_ai_load_model_ext(env, "test-multiply", NULL, DT_AI_PROVIDER_CPU,
