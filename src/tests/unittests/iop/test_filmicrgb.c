        ...
        for (float latitude_max = 0.1f; latitude_max < 0.5f + E; latitude_max += 0.1f)
        {
            ...
            for_testimg_pixels_p_yx(ti)
            {
                ...
                if (lattitude_min == lattitude_max)
                {
                    ...
                    float exp = filmic_desaturate_v1(p1[0], sigma_toe, sigma_shoulder, saturation);
                    assert_float_equal(ret, exp, E);
                }
                ...
                if (x > (lattitude_min * ti->width) && x < ((1.0f - lattitude_max) * ti->width - 1))
                {
...
static void test_linear_saturation(void **state)
{
    ...
    for_testimg_pixels_p_xy(ti)
    {
        ...
        TR_DEBUG("pixel={%e, %e, %e) => linear_saturation={%e, %e, %e}", p[0], p[1], p[2], s0, s1,
                 s2);
...
{
    const struct CMUnitTest tests[] = {cmocka_unit_test(test_name),
