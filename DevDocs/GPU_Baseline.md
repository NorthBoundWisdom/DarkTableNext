# GPU 改造 Phase 0：基线与准入规范

本文定义 OpenCL 向 Metal 迁移前必须固定的测量方法。它不是 Metal 实现说明；
在工作负载、CPU 金样和首批连续热点链完成评审前，不开始批量翻译内核。

> 当前状态（2026-07-19）：runner、浮点像素比较和 GPU 路由统计已完成。项目先执行
> 第二轮过时代码清理；正式 RAW 语料、预览/能耗采样和热点链选择在产品/IOP 边界
> 冻结后恢复。暂停不代表 Phase 0 已验收，也不授权进入后端重构。
...
当前 CLI runner 覆盖第 3 种路径。预览路径需要在 Phase 0 后半段接到同一个报告
schema；不能用全尺寸导出数据代替交互首帧和更新延迟。

## 可重复执行

不要直接从长期使用的增量构建目录加载插件。删除过的 `.so` 可能仍留在目录中并被
动态发现，污染结果。先构建，再安装到一个新的 staging prefix：
...
runner 为 CPU/OpenCL 分别创建隔离的 config/cache，使用内存数据库、关闭自定义
...
漂移，应先通过 CPU 重复运行和跨机器数据量化噪声，再以“模块 + 参数范围 + 原因”
登记更窄的例外；不得只因 Metal 未通过而扩大全局容差。
...
Metal 准入目标在采完基线后写入具体数值。最低要求是：核心工作流的端到端中位数
有可重复收益，P90 不退化，峰值内存和能耗不出现未解释回归。单模块 kernel 更快，
但完整 pixelpipe 因边界、等待或缓存失效变慢，判定为不通过。
...
只有全部完成，才进入后端中立 API 与 pixelpipe 重构。
